...
**이유:**
- Vercel 배포 1회당 **10분 이상** 소요
- 100개 파일 작업 시 부분 빌드하면 총 **수십 시간** 낭비
- 전체 완료 후 1회 빌드 = **10분**만 사용

**올바른 워크플로우:**
```bash
# ✅ 올바른 방법
1. 모든 파일 작성/수정 완료
2. 로컬 검증 (validate-charts, lint 등)
3. git add . && git commit && git push  # 1회만!
4. Vercel 배포 대기 (10분)
```
...
2. git push  # 첫 배포 10분
3. 파일 10개 더 작성
4. git push  # 두 번째 배포 10분
5. 반복... # 총 수십 시간!
```

**Orchestra 시스템 규칙:**
- Leader는 전체 작업 완료 전까지 빌드/푸시 금지
- Worker가 모든 파일 생성 완료 후에만 `/wiki-orchestra 푸시` 실행
- 부분 작업 시 반드시 사용자 확인 후 진행
...
1. **🔴 전체 작업 완료 후 1회만 빌드! (10분 절약)**
2. **npm run build 하면 자동으로 검증됨**
3. **문제 있으면 npm run fix-charts**
4. **유효한 차트는 2개만: ComparisonBarChart, PensionBarChart**
