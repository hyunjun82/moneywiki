...
## ★ title 비주얼 다이어그램 (최우선! 모든 것의 시작)

**title을 먼저 쓰고, 나머지(keywords, H2, description, hero.h1)를 title에서 파생하라.**

```
┌──────────────────────────────────┬──────────────────────────────┐
│  메인 롱테일 (자연어, 쉼표 OK)    │  보조 롱테일 / 질문형          │
│  ← 검색량 높은 핵심 키워드 배치 → │  ← 추가 검색 의도·관련 질문 → │
└──────────────────────────────────┴──────────────────────────────┘
                      ↑ 파이프 | 구분자 ↑
                       총 60자 이내

[왼쪽] 메인 롱테일 키워드를 자연어로 배치. 쉼표(,) 사용 가능.
       단어 나열 금지 → 사람이 읽었을 때 자연스러운 문장/구(句).
[오른쪽] 보조 의도를 자연어로. 질문형(~은?/~인가요?) 가능.
         단어 나열 금지 → 독립적으로 검색 가능한 롱테일.
```
...
| 단계 | 할 일 | 산출물 |
|------|-------|--------|
| 1단계 | title 먼저 작성 (위 다이어그램 형식, 60자 이내) | `meta.title` |
| 2단계 | title에서 파생 | `keywords` 4개, `H2` 4개(질문형), `description`, `hero.h1`, `ogTitle` |

**GOOD vs BAD:**
```
GOOD: '청년미래적금 대상, 가입 조건 | 청년도약계좌와의 차이점은?'
  → 왼쪽: 자연어 구(句)  /  오른쪽: 질문형 롱테일

GOOD: '2026 기초생활수급자 1인가구 생계급여 조건 | 소득인정액 계산 방법'
  → 왼쪽: 핵심 키워드 자연 배치  /  오른쪽: 독립 검색 가능 롱테일

BAD:  '문화누리카드 신청 방법 자격 조건 | 자동재충전 발급 기간 2026'
  → 양쪽 다 단어 나열. 사람이 읽으면 부자연스러움. 검색엔진도 싫어함.

BAD:  '문화누리카드 총정리 | 2026 완벽 가이드'
  → 금지어(총정리, 완벽 가이드). 내용 예측 불가.
```
...
```tsx
{
  ...
  content: (
    <>
      ...
      {/* ★ SpokeLinks 필수! 본문 4개 섹션 중 최소 2개에 배치 */}
      <SpokeLinks
        title="[주제] 더 알아보기"
        items={[
          { num: '01', heading: '관련 글 제목', desc: '한줄 설명', href: '/w/실존-슬러그' },
          { num: '02', heading: '관련 글 제목', desc: '한줄 설명', href: '/w/실존-슬러그' },
        ]}
      />
    </>
  ),
...
**SpokeLinks 필수 규칙 (내부링크 순환 = SEO 핵심!):**
```
배치: 본문 섹션 4개 중 최소 2개에 SpokeLinks 배치 (검증 훅: SPOKELINKS-001)
위치: 섹션 content 안에서 시각 컴포넌트 뒤, pasBridge 앞
제목: "[주제 키워드] 더 알아보기"
항목: 1~3개 (같은 허브 내 형제 스포크 또는 부모 허브)
href: 반드시 /w/실존-슬러그 (존재하지 않는 슬러그 금지!)
역할: 나무위키식 내부링크 순환 → 체류시간 증가 + SEO 점수 상승
```
