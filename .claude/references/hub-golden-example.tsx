/**
 * ═══════════════════════════════════════════════════
 * HUB GOLDEN EXAMPLE — 실제 배포 중인 허브 코드 (2026-02-12)
 * ═══════════════════════════════════════════════════
 *
 * 원본: src/data/hub/기초생활수급자-조건-총정리.tsx
 * 라이브: https://jjyu.co.kr/w/기초생활수급자-조건-총정리
 *
 * 이 파일은 에이전트가 새 허브를 작성할 때 참고할 **완성된 실제 글**입니다.
 * hub-template.tsx(빈 뼈대)와 구분하세요.
 *
 * ⚠️ 핵심 규칙 (위반 시 빌드 에러):
 * 1. import: HubTable, HubTipBox, HubFormula 등 Hub* 전용 컴포넌트 사용 (Spoke용 X), 체커는 자체 포함 래퍼 (src/components/checkers/)
 * 2. 타입: HubData from './types' (NOT '@/types/hub')
 * 3. sections 필드: { id, tag, heading, subtitle, content, sectionSpoke?, bridgeCTA? }
 * 4. spokeGroups: { title, spokes: [{ slug, title, desc, badge }] }
 * 5. sources: { name, url, org }
 * 6. faq: { question, answer } (NOT { q, a })
 *
 * ⚠️ Tailwind JIT 함정:
 * - src/data/ 파일에서 임의 색상 클래스(bg-[#FFF8E1] 등)는 CSS 미생성됨
 * - 비표준 색상은 반드시 inline style={{ }} 사용
 * - 표준 Tailwind(bg-white, border-neutral-200 등)는 정상 작동
 *
 * ⚠️ CSS 특이성 주의:
 * - HubPageContent.tsx의 content wrapper에 [&_a]:text-[#1E3A5F] 등 셀렉터 있음
 * - 하위 요소에서 이를 덮어쓰려면 !important(!no-underline 등) 또는 inline style 사용
 */

// ═══ 실제 import 패턴 (src/data/hub/ 기준) ═══
...
// ═══ 체커: 자체 포함 'use client' 래퍼 컴포넌트 사용 (RSC 직렬화 안전) ═══
...
const data: HubData = {
  ...
  // ═══ sections: tag 형식은 "01 개요", "02 기준" 등 ═══
  sections: [
    {
      id: 'sec-what',
      tag: '01 개요',
...

    {
      ...
      content: (
        <>
          ...
          {/* ═══ HubTable: highlight는 { text, highlight: true } 객체로 ═══ */}
          <HubTable
            id="tbl-standard"
...

    {
      ...
      content: (
        <>
          {/* ═══ H3: scroll-mt-20 필수 (스티키 헤더 아래 가림 방지) ═══ */}
      ...
      // ═══ sectionSpoke: 본문 아래 관련 스포크 링크 카드 ═══
      sectionSpoke: [
...

    {
      ...
      content: (
        <>
          {/* ═══ HubFormula: { text, sub? } — 스포크 FormulaBox와 API 다름 ═══ */}
          <HubFormula
          ...
          {/* ═══ CalcLink: 외부 계산기 링크 (spoke 컴포넌트 재사용 가능) ═══ */}
          <CalcLink
    ...
    // ⚠️ <GenericChecker config={checkerConfig} /> 직접 사용 금지!
    //    evaluate 함수가 서버→클라이언트 경계를 넘어 500 에러 발생
    //    반드시 src/components/checkers/ 에 래퍼 컴포넌트 생성 후 사용
    {
      id: 'checker',
      tag: 'CHECK',
...

    {
      ...
      content: (
        <>
          {/* ═══ 인라인 Steps: 허브에서는 HubStepCards 대신 직접 마크업도 가능 ═══ */}
          ...
          {/* ═══ 비표준 색상: 반드시 inline style 사용 (Tailwind JIT 미생성) ═══ */}
          <div
          ...
          {/* ═══ 인라인 CTA 카드: !important로 wrapper 셀렉터 덮어쓰기 ═══ */}
          <a
            href="/w/차상위계층-조건-혜택"
          ...
          >
            ...
            <span className="inline-flex items-center gap-1 text-[13px] font-bold py-[10px] px-5 rounded-lg" style={{ backgroundColor: '#1E3A5F', color: '#fff' }}>
              차상위계층 조건 확인 →
            </span>
          </a>
        </>
  ...
  // ═══ faq: { question, answer } 형식 (NOT { q, a }) ═══
  faq: [
    { question: '기초생활수급자가 되면 어떤 혜택을 받나요?', answer: '<strong>생계급여(현금), 의료급여(병원비), 주거급여(월세), 교육급여(학비)</strong> 4가지를 받아요. 추가로 TV수신료 면제, 통신비 감면, 전기·가스 요금 할인, 문화누리카드(연 13만원) 등 혜택도 있어요.' },
    { question: '기초수급자와 차상위계층은 뭐가 다른가요?', answer: '소득인정액이 중위소득 <strong>32~50% 이하</strong>면 기초수급자(급여 종류에 따라 다름), <strong>50% 이하</strong>이면서 수급자가 아니면 차상위계층이에요. 차상위는 생계급여는 받을 수 없지만 의료비 감면, 통신비 할인 등 혜택이 있어요.' },
    { question: '일을 하면서도 수급자가 될 수 있나요?', answer: '<strong>네.</strong> 근로소득의 30%를 공제하기 때문에 가능해요. 월 100만원 벌면 소득평가액은 70만원이에요. 1인가구 생계급여 기준(82만원)보다 낮으니 수급 가능성이 있어요.' },
    { question: '부모님이 돈을 많이 벌어도 수급자가 될 수 있나요?', answer: '<strong>생계·주거·교육급여는 가능해요.</strong> 부양의무자 기준이 완전 폐지됐어요. 의료급여만 부양의무자 기준(연소득 1.3억 또는 재산 12억 초과)이 남아 있어요.' },
    { question: '자동차가 있으면 수급자가 안 되나요?', answer: '차량가액 <strong>200만원 이하</strong>이거나 배기량 <strong>1,600cc 이하</strong>이면서 10년 이상 된 차량은 일반 재산으로 환산돼요(월 4.17%). 그 외 차량은 월 100%로 환산되어 수급이 어려울 수 있어요.' },
    { question: '수급자가 된 후에 소득이 늘면 자동으로 탈락하나요?', answer: '<strong>매년 소득·재산을 재조사</strong>해요. 소득인정액이 선정기준을 초과하면 해당 급여가 중지돼요. 다만 생계급여 기준만 넘고 의료·주거급여 기준 이하라면, 생계급여만 중지되고 나머지는 계속 받아요.' },
  ],
  ...
  // ═══ sources: { name, url, org } (NOT { label, url }) ═══
  sources: [
