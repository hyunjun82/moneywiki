...
```
1단계: meta.title 작성 — 3대 원칙 필수!

   형식: "[메인 롱테일 키워드] | [연관 롱테일 키워드]"

   원칙 1) 롱테일 | 연관 롱테일 — 일관성 유지
     - 양쪽 다 키워드. 한쪽이 문장이면 FAIL
   원칙 2) 타이틀이 매끄러워야 함
     - 단어 나열 금지 → 사람이 검색창에 치는 자연어 구(句)
   원칙 3) 포털 사이트 실제 검색어
     - 구글/네이버/다음 PAA, 연관검색어에서 추출한 키워드만 사용

   - 60자 이내, | 필수
   - 금지어: 총정리/완벽정리/가이드/완벽 가이드
   - 상세 규칙: .claude/commands/keywords.md "타이틀 황금 규칙" 참조

   GOOD: '2026 기초생활수급자 1인가구 생계급여 조건 | 소득인정액 계산 방법'
         [──────── 롱테일 키워드 1 ────────]   [── 연관 롱테일 2 ──]
   GOOD: '퇴직금 계산 방법 통상임금 기준 | 평균임금 차이 계산기'
   GOOD: '양도세 중과 뜻 기본세율 비교 | 2026 중과세율 유예 종료 시점'

   BAD:  '양도세 중과 유예 연혁 종료일 | 2026년 5월 이후 달라지는 점은?'  ← 오른쪽 질문형!
   BAD:  '양도세 잔금일 기준 계약일 차이 | 양도 시기 판단이 세율을 결정해요'  ← 오른쪽 설명문!
   BAD:  '청년미래적금 대상 가입 조건 | 청년도약계좌와의 차이점은?'  ← 오른쪽 질문형!
   BAD:  '다주택 양도세 중과유예 세율과 절세 전략 총정리'  ← | 없음 + 총정리!
...
```
반드시 읽기 (2개만!):
1. src/data/spoke/types.ts                 — SpokeData 타입 정의
2. .claude/references/checker-patterns.md  — 체커 5유형 (해당 시)

※ title/description/컴포넌트/문체/금지 규칙은 이 파일(writer.md)에 전부 있음.
...
const data: SpokeData = {
  ...
  meta: {
    ...
    // GOOD: '2026 기초생활수급자 1인가구 생계급여 조건 | 소득인정액 계산 방법'
    // GOOD: '퇴직금 계산 방법 통상임금 기준 | 평균임금 차이 계산기'
    // BAD:  '양도세 중과 유예 연혁 종료일 | 2026년 5월 이후 달라지는 점은?'  ← 질문!
    // BAD:  '양도세 잔금일 기준 계약일 차이 | 양도 시기 판단이 세율을 결정해요'  ← 설명문!
    title: '메인 롱테일 키워드 | 연관 롱테일 키워드',  // 60자 이내, | 필수
