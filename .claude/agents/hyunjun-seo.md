...
□ ★★★ | 오른쪽이 롱테일 키워드인가? (질문형/설명문이면 즉시 FAIL!) ★★★
```

**| 오른쪽 검증 방법:**
```
1. ?로 끝나면 → FAIL (질문형)
2. ~해요/~이에요/~돼요/~있어요로 끝나면 → FAIL (설명문)
3. "달라지는 점은", "차이는", "어떤", "어떻게" 등 → FAIL (질문형 변형)
4. "결정해요", "중요해요" 등 동사+요 → FAIL (설명문)
5. 양쪽 다 구글에 실제로 치는 검색어여야 PASS

GOOD: '양도세 중과 뜻 기본세율 비교 | 2026 중과세율 유예 종료 시점'
BAD:  '양도세 중과 유예 연혁 종료일 | 2026년 5월 이후 달라지는 점은?'
BAD:  '양도세 잔금일 기준 계약일 차이 | 양도 시기 판단이 세율을 결정해요'
```
