...
## Step 8: 에이전트 팀 연결 (JSON 저장 후 자동 실행)

JSON 저장 완료 후 사용자에게 질문:

```
"에이전트 팀으로 작성 시작할까요? (Y/N)"
```

**Y 선택 시 자동 실행 순서:**

```
1. TeamCreate ("keywords-[시드키워드]")
2. JSON 읽기 → 허브/스포크별 TaskCreate
   - 허브 먼저 → 스포크는 허브 완료 후 (blockedBy)
3. 팀원 스폰 (Task tool + team_name):
   - hyunjun-hub-writer → 허브 태스크 배분
   - hyunjun-writer → 스포크 태스크 배분 (최대 3명 병렬)
4. 작성 완료 → hyunjun-cctv 스폰 → 최종 검증
5. 검증 통과 → registry 등록 + git commit + push
```

**리라이트인 경우:**
```
- JSON의 "상태"가 "리라이트"인 항목 → 기존 파일 읽고 덮어쓰기
- slug/URL 변경 금지, 내용만 업그레이드
```

**N 선택 시:**
```
- JSON만 저장하고 종료
- 나중에 "양도세 팀 돌려"로 수동 시작 가능
```
