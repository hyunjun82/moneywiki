 ...
 *
 *   ★★★ pasBridge 실전 예시 (question + answer + button 전체 구조 필독!) ★★★
 *
 *   [1] 막막함 → 즉시 해결 → 신청 방법
 ...
 *   A: "MRI·입원비·수술비 포함 본인부담 0원에 가깝고, 건강보험료도 0원이에요. 내가 의료급여 대상인지 아래에서 먼저 확인하세요."
 *   B: "의료급여 1종 2종 차이 보기 →"
 *
 *   [4] 놀라움 → 구체 숫자 → 계산
 *   Q: "전세금 1억, 자동차 있어도 수급자 될 수 있다는 거 아셨나요?"
 *   A: "서울 거주자라면 전세 9,900만원까지 재산 0원 처리돼요. 1,600cc 미만 차량도 조건부 인정. 내 재산이 실제로 얼마로 환산되는지 확인해 보세요."
 *   B: "재산 소득환산율 계산 →"
 *
 *   [5] 구체 상황 → 공제 계산 → 상세
 *   Q: "월 100만원 버는데 수급자가 될 수 있을까요?"
 *   A: "근로소득공제 30%를 적용하면 소득평가액이 70만원이에요. 1인가구 생계급여 기준(82만원)보다 낮아서 수급 가능성이 있어요."
 *   B: "근로소득공제 상세 계산 →"
 *
 *   [6] 체커 유도 (→ 대신 ↓)
 ...
 *   B: "간편 자격 체크 ↓"
 *
 *   핵심: Q(독자의 구체적 상황/고민) → A(숫자·사실로 해결 힌트) → B(도착 페이지 주제, 짧고 명확)
 *   BAD button: "자세히 보기 →", "전체 가이드 →", "바로가기 →", 키워드 나열형
 *
 *   href 대상:
 ...
 *   구조:
 *   hubCTA: {
 *     badge: '도착 허브 핵심 주제 6~8자',  // → 버튼 "{badge} →"
 *     desc: '허브 내용 구체 요약 1줄',     // "확인하세요", "총정리" 금지!
 *   }
 *   badge = 버튼 텍스트. pasBridge buttonText와 동일 패턴 (6~8자)!
 *   - GOOD: "수급자 선정 기준 보기", "국채 투자 조건 비교", "실업급여 수급 조건 보기"
 *   - BAD: "전체 가이드", "올인원", "안내" (추상적), 키워드 5개 나열 (너무 김)
 ...
 * | pasBridge.buttonText "자세히 보기" | → 도착 페이지 주제 + 상세/확인/계산 + → |
 * | hubCTA.badge "전체 가이드" | → 도착 허브 핵심 주제 + 상세/확인 |
 * | hubCTA.desc "확인하세요" | → "정리돼요" / "볼 수 있어요" |
 * |  |  |
 * | **── pasBridge / hubCTA 버튼 실전 예시 (필독!) ──** |  |
 * | question | buttonText |
 * | "전세금 1억, 자동차 있어도 수급자가 될 수 있을까요?" | "재산 소득환산 상세 기준 →" |
 * | "월 100만원 버는데 수급자가 될 수 있을까요?" | "근로소득공제 상세 계산 →" |
 * | "갑자기 직장을 잃었는데, 당장 생활비가 막막하시죠?" | "긴급복지지원 신청 방법 →" |
 * | "내가 수급자가 될 수 있을까? 조건이 헷갈리시나요?" | "간편 자격 체크 ↓" |
 * | 허브 복귀 (hubCTA badge) | "차상위계층 조건 확인" / "퇴직연금 국채 투자 상세 조건" |
...
const data: SpokeData = {
  ...
  hero: {
    ...
    hubCTA: {
      badge: '', // TODO: 도착 허브 주제 (예: '차상위계층 조건 확인', '퇴직연금 국채 투자 상세 조건')
  ...
  sections: [
    ...
    // ═══════════════════════════════════════
    {
      ...
      pasBridge: {
        ...
        buttonText: '', // TODO: '긴급복지지원 신청 방법 →' 패턴: '재산 소득환산 상세 기준 →' 패턴 (도착 페이지 주제 + 상세/확인/계산 + →)
      },
    },
    ...
    // ═══════════════════════════════════════
    {
      ...
      pasBridge: {
        ...
        answer: <>{/* TODO */}</>,
        buttonText: '', // TODO: '긴급복지지원 신청 방법 →' 패턴: '근로소득공제 상세 계산 →' 패턴
      },
    },
    ...
    // ═══════════════════════════════════════
    {
      ...
      pasBridge: {
        ...
        answer: <>{/* TODO */}</>,
        buttonText: '', // TODO: '긴급복지지원 신청 방법 →' 패턴
      },
    },
