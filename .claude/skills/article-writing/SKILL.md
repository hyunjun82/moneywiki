...
```
타이틀: "퇴직금에 세금이 얼마나 붙나요?" → 얼마형
  H2-1: [Calculator] 퇴직소득세, 내 금액으로 바로 계산
  H2-2: [GreenBox] 세금 구조 — 왜 이렇게 나오는지
  H2-3: [Steps] 세금 확인·처리 절차
  H2-4: [Checklist] 절세 체크리스트
  H2-5: [FAQ] 자주 묻는 것들

타이틀: "IRP 퇴직금, 지금 꺼내도 되나요?" → 가능형
  H2-1: [GreenBox] 결론 — 꺼낼 수 있는 3가지 경우
  H2-2: [EligibilityChecker] 내 상황에서 인출 가능 여부
  H2-3: [Calculator] 꺼내면 세금이 얼마나 나오나
  H2-4: [Steps] 인출 신청 절차
  H2-5: [FAQ] 자주 묻는 것들

타이틀: "개인형 IRP로 퇴직금 수령하는 방법은?" → 방법형
  H2-1: [Steps] 수령 절차 4단계
  H2-2: [DocTable] 필요 서류
  H2-3: [Calculator] 연금 수령액 시뮬레이션
  H2-4: [Checklist] 수령 전 체크리스트
  H2-5: [FAQ] 자주 묻는 것들
```

### 컴포넌트 배치 규칙
- FAQ(5개+)는 모든 글에 고정
- 주제에 맞는 컴포넌트 최소 3종 + FAQ (주제에 안 맞는 컴포넌트 억지 삽입 금지)
- DocTable: 서류/준비물이 있는 주제에만
- Checklist: 점검 항목이 있는 주제에만
- EligibilityChecker: 법정 자격 조건이 있을 때만 (관심 확인용 사용 금지)
- **텍스트만 있는 섹션 금지** — 컴포넌트 매칭 안 되면 GreenBox 또는 BorderBox

### EligibilityChecker 사용 기준
- 사용 O: 법정 조건 주제 (실업급여 자격, IRP 중도인출 법정 사유, 기초수급 자격)
- 사용 X: 누구나 해당하는 주제 (퇴직소득세 계산, IRP 수령 방법, 세금 환급)
- 항목 규칙: 모든 항목 = 사실 확인 ("~했어요", "~이에요")
- 금지: 관심/의향 ("~하고 싶어요", "~받고 싶어요", "~할 예정이에요")
- 금지: 글 추천용 ("이 글이 맞는지") — 자격 확인용으로만 사용
...
### Step 1: 키워드 분석 + 검색 의도 분류
1. 키워드를 받으면 **신규/리라이트** 먼저 확인
2. 타이틀 생성 후 `node scripts/suggest-structure.js "타이틀"` 실행
3. 검색 의도 파악: "이 타이틀을 검색해서 들어온 독자가 진짜 알고 싶은 게 뭔가?"
4. 의도 분류 (얼마형/가능형/방법형/비교형/개념형)에 따라 H2 순서 결정
5. 소제목 4~6개 설계 — **H2-1이 타이틀 질문에 바로 답하는 구조**

### Step 2: 컴포넌트 매핑 (의도 기반)
1. H2-1 → 검색 의도에 맞는 핵심 컴포넌트 배치
2. 나머지 H2 → 트리거 키워드 매칭 → 컴포넌트 선택
3. 주제에 안 맞는 컴포넌트 억지 삽입 금지
4. 매칭 안 되면 GreenBox 또는 BorderBox
5. 한 소제목에 1개만, 중복 금지 확인
...
완성 글 기준: `src/app/w/퇴직금-소득세/page.tsx` (얼마형 — H2-1이 Calculator로 답 먼저)

**글 작성 전 이 파일을 읽고, 의도에 맞게 구조를 바꿔서 작성할 것.**
골격(서론→H2 섹션들→FAQ→출처→면책)은 동일, 시각화는 `## 2` 매핑 테이블 기준으로 선택.
