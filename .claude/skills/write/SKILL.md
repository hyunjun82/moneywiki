...
npm run verify <slug>        # 숫자 → 화면 → 뜻 → 누락. dev 서버는 알아서 뜬다
```
...
## 4. 검사한다 — 통과할 때까지
`npm run verify <slug>` — 숫자·화면·뜻·누락이 순서대로 돈다. dev 서버는 알아서 띄우고 끈다.
- 숫자 근거 ✗ → 그 숫자를 증거에서 찾거나 문장을 뺀다
- 화면 ✗ → 검사기가 말한 블록·색·한 줄 답·CTA 를 고친다
- 뜻 ✗ → 훅↔버튼·소제목↔본문·근거 초과 단언·의도 누락을 고친다
- 누락 ✗ → 인용한 조문의 그 항·호를 글에 넣는다. 다른 글 주제라면 그 글로 보내는 문장을 넣는다
검사기를 고치지 않는다. 글을 고친다.
