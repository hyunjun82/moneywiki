"use client";

import { useState, useEffect, useCallback } from "react";

function formatNumber(num: number): string {
  return Math.round(num).toLocaleString("ko-KR");
}

interface InputFieldProps {
  label: string;
  value: number;
  onChange?: (value: number) => void;
  readOnly?: boolean;
  description?: string;
  highlight?: boolean;
  unit?: string;
}

function InputField({ label, value, onChange, readOnly, description, highlight, unit = "명" }: InputFieldProps) {
  return (
    <div className={`flex flex-col sm:flex-row sm:items-center gap-2 py-3 px-4 border-b border-neutral-100 ${highlight ? "bg-[#F5F8FB]" : ""}`}>
      <div className="sm:w-48 flex-shrink-0">
        <span className="text-sm font-medium text-neutral-700">{label}</span>
      </div>
      <div className="flex-1 flex items-center gap-2">
        <input
          type="number"
          min="0"
          max="10"
          value={value}
          onChange={(e) => onChange && onChange(Math.max(0, Math.min(10, parseInt(e.target.value) || 0)))}
          readOnly={readOnly}
          className={`w-full max-w-xs px-3 py-2 text-right border rounded-lg ${
            readOnly
              ? "bg-neutral-100 text-neutral-600 border-neutral-200"
              : "bg-white border-neutral-300 focus:border-[#2B5280] focus:ring-1 focus:ring-[#2B5280]"
          }`}
        />
        <span className="text-sm text-neutral-500">{unit}</span>
      </div>
      {description && (
        <div className="sm:w-64 text-xs text-neutral-500">{description}</div>
      )}
    </div>
  );
}

// 자녀세액공제 계산 (소득세법 기준)
function calcChildCredit(count: number): number {
  if (count <= 0) return 0;
  if (count === 1) return 150000;
  if (count === 2) return 350000;
  return 350000 + 300000 * (count - 2);
}

// 출생·입양 세액공제
function calcBirthCredit(order: number): number {
  if (order === 1) return 300000;
  if (order === 2) return 500000;
  return 700000; // 셋째 이상
}

export default function ChildTaxCreditCalculator() {
  const [childCount, setChildCount] = useState(2);
  const [hasBirth, setHasBirth] = useState(false);
  const [birthOrder, setBirthOrder] = useState(1);

  const [childCredit, setChildCredit] = useState(0);
  const [birthCredit, setBirthCredit] = useState(0);
  const [totalCredit, setTotalCredit] = useState(0);

  const calculate = useCallback(() => {
    const cc = calcChildCredit(childCount);
    const bc = hasBirth ? calcBirthCredit(birthOrder) : 0;
    setChildCredit(cc);
    setBirthCredit(bc);
    setTotalCredit(cc + bc);
  }, [childCount, hasBirth, birthOrder]);

  useEffect(() => {
    calculate();
  }, [calculate]);

  return (
    <div className="w-full max-w-4xl mx-auto">
      {/* 헤더 */}
      <div className="bg-[#1E3A5F] text-white px-6 py-8 rounded-t-2xl">
        <h1 className="text-2xl font-bold mb-2">2026 자녀세액공제 계산기</h1>
        <p className="text-[#B8D4F1] text-sm">
          자녀 수와 출생 여부를 입력하면 연말정산 자녀세액공제 금액을 계산해요
        </p>
      </div>

      {/* 본문 */}
      <div className="bg-white shadow-lg rounded-b-2xl">
        {/* 1. 자녀 정보 */}
        <div className="p-6 border-b-2 border-neutral-200">
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[#1E3A5F] text-white text-sm font-bold">1</span>
            <h2 className="text-lg font-bold text-neutral-800">자녀 정보</h2>
          </div>
          <div className="bg-white border border-neutral-200 rounded-lg overflow-hidden">
            <InputField
              label="자녀 수 (만 7~18세)"
              value={childCount}
              onChange={setChildCount}
              description="만 7세 이상 만 18세 이하"
            />
          </div>
        </div>

        {/* 2. 출생·입양 */}
        <div className="p-6 border-b-2 border-neutral-200">
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[#1E3A5F] text-white text-sm font-bold">2</span>
            <h2 className="text-lg font-bold text-neutral-800">출생·입양 여부</h2>
          </div>
          <div className="space-y-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={hasBirth}
                onChange={(e) => setHasBirth(e.target.checked)}
                className="w-4 h-4"
              />
              <span className="text-sm">2026년에 출생·입양한 자녀가 있어요</span>
            </label>
            {hasBirth && (
              <div className="ml-6">
                <label className="block text-sm font-medium text-neutral-700 mb-2">출생·입양 순서</label>
                <select
                  value={birthOrder}
                  onChange={(e) => setBirthOrder(parseInt(e.target.value))}
                  className="px-3 py-2 border border-neutral-300 rounded-lg bg-white focus:border-[#2B5280] focus:ring-1 focus:ring-[#2B5280]"
                >
                  <option value={1}>첫째 (30만원)</option>
                  <option value={2}>둘째 (50만원)</option>
                  <option value={3}>셋째 이상 (70만원)</option>
                </select>
              </div>
            )}
          </div>
        </div>

        {/* 3. 계산 결과 */}
        <div className="p-6">
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[#1E3A5F] text-white text-sm font-bold">3</span>
            <h2 className="text-lg font-bold text-neutral-800">계산 결과</h2>
          </div>

          <div className="bg-gradient-to-br from-[#1E3A5F] to-[#2B5280] text-white p-6 rounded-xl mb-4">
            <div className="text-sm mb-2 opacity-90">총 자녀세액공제</div>
            <div className="text-3xl font-bold mb-4">{formatNumber(totalCredit)}원</div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="opacity-75 mb-1">자녀공제 ({childCount}명)</div>
                <div className="text-lg font-semibold">{formatNumber(childCredit)}원</div>
              </div>
              <div>
                <div className="opacity-75 mb-1">출생·입양 공제</div>
                <div className="text-lg font-semibold">{formatNumber(birthCredit)}원</div>
              </div>
            </div>
          </div>

          <div className="space-y-3 text-sm">
            <div className="flex justify-between p-3 bg-neutral-50 rounded-lg">
              <span className="text-neutral-600">자녀세액공제 (기본)</span>
              <span className="font-semibold text-neutral-800">{formatNumber(childCredit)}원</span>
            </div>
            {hasBirth && (
              <div className="flex justify-between p-3 bg-neutral-50 rounded-lg">
                <span className="text-neutral-600">출생·입양 추가공제</span>
                <span className="font-semibold text-neutral-800">{formatNumber(birthCredit)}원</span>
              </div>
            )}
          </div>
        </div>

        {/* 안내 사항 */}
        <div className="px-6 pb-6">
          <div className="bg-[#F0F7FF] border border-[#B8D4F1] rounded-lg p-4">
            <h3 className="text-sm font-semibold text-neutral-800 mb-2">💡 안내 사항</h3>
            <ul className="text-xs text-neutral-600 space-y-1.5 list-disc list-inside">
              <li>만 7세 이상 만 18세 이하 자녀만 해당돼요 (12월 31일 기준)</li>
              <li>맞벌이 부부도 한 사람만 공제받을 수 있어요</li>
              <li>출생·입양 공제는 해당 연도 1회만 적용돼요</li>
              <li>세액공제라서 세금에서 직접 차감돼요</li>
            </ul>
          </div>
        </div>

        {/* 기준표 */}
        <div className="px-6 pb-6">
          <h3 className="text-sm font-semibold text-neutral-800 mb-3">📊 자녀 수별 공제액 (2026년)</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm border border-neutral-200 rounded-lg overflow-hidden">
              <thead className="bg-neutral-100">
                <tr>
                  <th className="px-4 py-2 text-left">자녀 수</th>
                  <th className="px-4 py-2 text-right">공제액</th>
                  <th className="px-4 py-2 text-left">계산식</th>
                </tr>
              </thead>
              <tbody>
                {[1, 2, 3, 4, 5].map((n) => (
                  <tr key={n} className="border-t border-neutral-200">
                    <td className="px-4 py-2">{n}명</td>
                    <td className="px-4 py-2 text-right font-semibold">{formatNumber(calcChildCredit(n))}원</td>
                    <td className="px-4 py-2 text-neutral-500">
                      {n === 1 ? "15만원" : n === 2 ? "35만원" : `35만원 + 30만원 × ${n - 2}`}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  );
}
