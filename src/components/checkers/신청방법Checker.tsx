  ...
  evaluate: (sel): CheckerResult => {
    ...
    if (isInvoluntary && hasPeriod && inDeadline) {
      return {
        ...
        links: [
          { icon: '📋', title: '실업인정 구직활동 방법', desc: '회차별 구직활동 요건과 실업인정 신청 안내', href: '/w/실업급여-실업인정-구직활동-방법' },
          { icon: '💰', title: '내 기초일액 계산하기', desc: '1일 지급액 상한·하한 기준 계산', href: '/w/실업급여-기초일액' },
        ],
      }
    }
    ...
    return {
      ...
      links: [
        { icon: '📌', title: '자발적 퇴사 예외 사유', desc: '부당한 근로조건 등 예외 인정 기준', href: '/w/자진퇴사-자발적-퇴사-실업급여-예외-사유' },
        { icon: '📅', title: '피보험기간 180일 계산', desc: '이직 전 18개월 내 합산 방법 안내', href: '/w/실업급여-피보험기간-180일-계산-합산-방법' },
      ],
    }
  },
}
