export const runtime = 'edge';
export const dynamic = 'force-dynamic';
import { Metadata } from "next";
import Link from "next/link";
import { notFound } from "next/navigation";


import FormPreview, {
  표준근로계약서_DATA,
  단시간근로계약서_DATA,
  기간제근로계약서_DATA,
  임대차계약서_DATA,
  사직서_DATA,
  위임장_DATA,
  내용증명_DATA,
  견적서_DATA,
  각서_DATA,
  경위서_DATA,
  고소장_DATA,
  고발장_DATA,
  합의서_DATA,
  차용증_DATA,
  영수증_DATA,
  진술서_DATA,
  탄원서_DATA,
  진정서_DATA,
  거래명세서_DATA,
  세금계산서_DATA,
  매도청구서_DATA,
  산재보험청구서_DATA,
  금전소비대차계약서_DATA,
  동의서_DATA,
  답변서민사_DATA,
  대리점계약서_DATA,
  도급계약서_DATA,
  매매계약서_DATA,
  물품매매계약서_DATA,
  복직신청서_DATA,
  배우자출산휴가신청서_DATA,
  병가신청서_DATA,
  가족돌봄휴가신청서_DATA,
  경력증명서_DATA,
  경력증명서_국가기술자격_DATA,
  권고사직서_DATA,
  재직증명서_DATA,
  퇴직증명서_DATA,
  납세증명서신청서_DATA,
  연봉근로계약서_DATA,
  연차휴가신청서_DATA,
  출장신청서_DATA,
  휴직신청서_DATA,
  출산휴가신청서_DATA,
  육아휴직신청서_DATA,
  이력서_DATA,
  자기소개서_DATA,
  사대보험가입신청서_DATA,
  사대보험자격상실신고서_DATA,
  가족관계증명서신청서_DATA,
  개명신청서_DATA,
  건축물대장신청서_DATA,
  경력기술서_DATA,
  경업금지계약서_DATA,
  교육훈련신청서_DATA,
  근로계약해지통보서_DATA,
  급여명세서_DATA,
  납품서_DATA,
  등기부등본신청서_DATA,
  발주서_DATA,
  법인설립등기신청서_DATA,
  병적증명서신청서_DATA,
  본인서명사실확인서_DATA,
  불합격통지서_DATA,
  비밀유지계약서_DATA,
  사망신고서_DATA,
  사업자등록신청서_DATA,
  상속포기서_DATA,
  상여금지급통지서_DATA,
  성과급지급내역서_DATA,
  소장민사_DATA,
  시말서_DATA,
  실업급여신청서_DATA,
  업무제휴계약서_DATA,
  여권발급신청서_DATA,
  연봉협상합의서_DATA,
  외주계약서_DATA,
  용역계약서_DATA,
  운전경력증명서신청서_DATA,
  유언장_DATA,
  이사회의사록_DATA,
  이의신청서_DATA,
  이혼신고서_DATA,
  인감증명위임장_DATA,
  인사발령통지서_DATA,
  입금확인서_DATA,
  재택근무신청서_DATA,
  전대차계약서_DATA,
  전입신고서_DATA,
  주민등록등본신청서_DATA,
  주주총회의사록_DATA,
  증여계약서_DATA,
  지급명령신청서_DATA,
  지방세완납증명서신청서_DATA,
  징계위원회소집통지서_DATA,
  채권양도통지서_DATA,
  채용합격통지서_DATA,
  출생신고서_DATA,
  출입국사실증명신청서_DATA,
  컨설팅계약서_DATA,
  토지대장신청서_DATA,
  폐업신고서_DATA,
  혼인신고서_DATA,
  // 고용복지 양식 DATA
  고용장려금해지확인서_DATA,
  고용촉진장려금신청서_DATA,
  고용허가준수사항확인서_DATA,
  구직급여지급정지사전고지서_DATA,
  국민연금고용보험료지원신청서_DATA,
  국외취업자모집신고확인증_DATA,
  상병급여청구서_DATA,
  생활안정자금융자추천신청서_DATA,
  실업급여결정통지서_DATA,
  워라밸일자리장려금_실근로시간단축_DATA,
  워라밸일자리장려금신청서_DATA,
  이직확인서_공통_DATA,
  이직확인서_간병퇴사_DATA,
  일가정양립환경개선_유연근무장려금_DATA,
  일가정양립환경개선변경신청서_DATA,
  일가정양립환경개선신청서_DATA,
  정규직전환지원신청서_DATA,
  조기재취업수당청구서_DATA,
  직업능력개발훈련비용지원신청서_DATA,
  청년일자리도약장려금신청서_DATA,
  청년일자리도약장려금점검표_DATA,
  청년일자리도약장려금확인서_DATA,
  출산육아기고용안정장려금_단축업무분담지원금_DATA,
  출산육아기고용안정장려금_대체인력지원금_DATA,
  출산육아기고용안정장려금결과통지서_DATA,
  출산육아기고용안정장려금신청서_DATA,
  // 청구서 양식들
  청구서_DATA,
  공사대금청구서_DATA,
  물품청구서_DATA,
  개명허가청구_DATA,
  건물매수청구_DATA,
  건물부당사용중지청구_DATA,
  건물수리청구_DATA,
  과세전적부심사청구서_DATA,
  구조금지급청구서_DATA,
  국공유지소유권이전등기청구서_DATA,
  국선변호인선정청구서_DATA,
  근로조건위반손해배상청구신청서_DATA,
  금치산선고심판청구_DATA,
  금치산선고취소심판청구_DATA,
  기성고청구내역서_DATA,
  기성고청구서_DATA,
  납품청구서_DATA,
  농산물매매계약불이행손해배상청구_DATA,
  농산물매수청구_DATA,
  매매대금감액청구서_DATA,
  물품구입제작청구서_DATA,
  사망조위금청구서_DATA,
  사용중지청구서_DATA,
  상속승인기간연장허가청구서_DATA,
  상속인수색의공고청구서_DATA,
  심판청구취하서_DATA,
  약식명령에대한정식재판청구서_DATA,
  양육자지정및양육비심판청구서_DATA,
  열람등사청구서_DATA,
  유지보수비용청구서_DATA,
  임대료증액청구서_DATA,
  임대료청구및계약해지통지_DATA,
  임치물반환청구서_DATA,
  재심사청구서_DATA,
  재심청구서_DATA,
  재심청구서02_DATA,
  재항고기각이유고지청구서_DATA,
  재해부조금청구서_DATA,
  특별대리인선임청구서_DATA,
  한정치산선고심판청구_DATA,
  형사보상금지급청구서_DATA,
  형사보상금청구서_DATA,
  후견인선임청구서_DATA,
  확인서_DATA,
  // 누락 매핑 추가
  가족수당신청서_DATA,
  계좌변경서약서_DATA,
  내용증명_계약금반환청구_DATA,
  내용증명_전세금반환청구_DATA,
  사무용품청구의뢰서_DATA,
  이직확인서_52시간초과_DATA,
  이직확인서_근로조건변동_DATA,
  이직확인서_배우자동거_DATA,
  이직확인서_부양가족동거_DATA,
  이직확인서_사업장이전_DATA,
  이직확인서_임신퇴사_DATA,
  이직확인서_질병부상_DATA,
  집합건물매매계약서_DATA,
  출산전후휴가급여결정통지서_DATA,
  통상임금산정지침_DATA,
  평균임금산정특례고시_DATA,
  평균임금상여금취급요령_DATA,
  휴업폐업신고서_DATA,
  헌법소원심판청구_DATA,
  헌법소원심판청구_법령_DATA,
  헌법소원심판청구_불기소처분_DATA,
  헌법소원심판청구_행정부작위_DATA,
  // 노동OK 신규 양식 DATA
  급여대장_DATA,
  급여지급명세서_DATA,
  연차휴가사용계획서_DATA,
  연차휴가사용지정통보서_DATA,
  연차휴가사용시기지정요청서_DATA,
  업무보고서_DATA,
  업무인수인계서_DATA,
  업무일지_DATA,
  여비교통비지출품의서_DATA,
  인사기록카드_DATA,
  임금계산서_DATA,
  임금대장_DATA,
  조퇴결근신청서_DATA,
  직원근무평가서_DATA,
  차량운행일지_DATA,
  채용면접기록표_DATA,
  퇴직휴직신고서_DATA,
  퇴직금지급품의서_DATA,
  퇴직급여충당금조정명세서_DATA,
  피보험자신규자격취득신고서_DATA,
  피복지급대장_DATA,
  학자금지급신청서_DATA,
  해외훈련이수자관리기록카드_DATA,
  현장실습일지_DATA,
  회람_DATA,
  공가신청서_DATA,
  휴직자퇴직자관리명세서_DATA,
  휴가원_DATA,
  시간외근무신청서_DATA,
  휴직원_DATA,
  거래명세표_DATA,
  // 배치2: 노동OK 신규 양식
  개인면담표_DATA,
  개인별시간외근무기록표_DATA,
  개인별연간결근휴가기록표_DATA,
  개인별휴가관리카드_DATA,
  개인별인사기록카드_DATA,
  결근계_DATA,
  경고장_DATA,
  근무성적평정표_DATA,
  근무일지_DATA,
  급여수령증_DATA,
  기안서_DATA,
  노무수령거부통지서_DATA,
  대근신청원_DATA,
  민간인신원진술서_DATA,
  부문별직종별인원요구표_DATA,
  부양가족신고서_DATA,
  설비투자계획서_DATA,
  신상명세서_DATA,
  인사고과표_DATA,
  인사기록부_DATA,
  인적사항_DATA,
  입사지원서_DATA,
  직원채용품의서_DATA,
  징계경고처분요구서_DATA,
  추천서_DATA,
  출장업무보고서_DATA,
  출장여비선급신청서_DATA,
  취학자녀신고서_DATA,
  통지서_DATA,
  해외위탁교육지원자신상조사서_DATA,
  해외출장신청서_DATA,
  학자금지급사항_DATA,
  학자금지급카드_DATA,
  회사행사예정표_DATA,
  회의의사록_DATA,
} from "@/components/forms/FormPreview";
import FormPageClient from "@/components/forms/FormPageClient";
import FormSidebar from "@/components/forms/FormSidebar";
import AdSense, { AD_SLOTS } from "@/components/AdSense";
import ShareButtons from "@/components/ShareButtons";
import { getAllFormSlugs, getFormData, FormData } from "@/lib/forms-loader";

// 프리뷰 데이터 매핑 (previewDataKey -> 실제 데이터)
const PREVIEW_DATA_MAP: Record<string, typeof 표준근로계약서_DATA> = {
  "표준근로계약서_DATA": 표준근로계약서_DATA,
  "단시간근로계약서_DATA": 단시간근로계약서_DATA,
  "기간제근로계약서_DATA": 기간제근로계약서_DATA,
  "임대차계약서_DATA": 임대차계약서_DATA,
  "사직서_DATA": 사직서_DATA,
  "위임장_DATA": 위임장_DATA,
  "내용증명_DATA": 내용증명_DATA,
  "견적서_DATA": 견적서_DATA,
  "각서_DATA": 각서_DATA,
  "경위서_DATA": 경위서_DATA,
  "고소장_DATA": 고소장_DATA,
  "고발장_DATA": 고발장_DATA,
  "합의서_DATA": 합의서_DATA,
  "차용증_DATA": 차용증_DATA,
  "영수증_DATA": 영수증_DATA,
  "진술서_DATA": 진술서_DATA,
  "탄원서_DATA": 탄원서_DATA,
  "진정서_DATA": 진정서_DATA,
  "거래명세서_DATA": 거래명세서_DATA,
  "세금계산서_DATA": 세금계산서_DATA,
  "매도청구서_DATA": 매도청구서_DATA,
  "산재보험청구서_DATA": 산재보험청구서_DATA,
  "금전소비대차계약서_DATA": 금전소비대차계약서_DATA,
  "동의서_DATA": 동의서_DATA,
  "답변서민사_DATA": 답변서민사_DATA,
  "대리점계약서_DATA": 대리점계약서_DATA,
  "도급계약서_DATA": 도급계약서_DATA,
  "매매계약서_DATA": 매매계약서_DATA,
  "물품매매계약서_DATA": 물품매매계약서_DATA,
  "복직신청서_DATA": 복직신청서_DATA,
  "배우자출산휴가신청서_DATA": 배우자출산휴가신청서_DATA,
  "병가신청서_DATA": 병가신청서_DATA,
  "가족돌봄휴가신청서_DATA": 가족돌봄휴가신청서_DATA,
  "경력증명서_DATA": 경력증명서_DATA,
  "경력증명서_국가기술자격_DATA": 경력증명서_국가기술자격_DATA,
  "권고사직서_DATA": 권고사직서_DATA,
  "재직증명서_DATA": 재직증명서_DATA,
  "퇴직증명서_DATA": 퇴직증명서_DATA,
  "납세증명서신청서_DATA": 납세증명서신청서_DATA,
  "연봉근로계약서_DATA": 연봉근로계약서_DATA,
  "연차휴가신청서_DATA": 연차휴가신청서_DATA,
  "출장신청서_DATA": 출장신청서_DATA,
  "휴직신청서_DATA": 휴직신청서_DATA,
  "출산휴가신청서_DATA": 출산휴가신청서_DATA,
  "육아휴직신청서_DATA": 육아휴직신청서_DATA,
  "이력서_DATA": 이력서_DATA,
  "자기소개서_DATA": 자기소개서_DATA,
  "사대보험가입신청서_DATA": 사대보험가입신청서_DATA,
  "사대보험자격상실신고서_DATA": 사대보험자격상실신고서_DATA,
  "가족관계증명서신청서_DATA": 가족관계증명서신청서_DATA,
  "개명신청서_DATA": 개명신청서_DATA,
  "건축물대장신청서_DATA": 건축물대장신청서_DATA,
  "경력기술서_DATA": 경력기술서_DATA,
  "경업금지계약서_DATA": 경업금지계약서_DATA,
  "교육훈련신청서_DATA": 교육훈련신청서_DATA,
  "근로계약해지통보서_DATA": 근로계약해지통보서_DATA,
  "급여명세서_DATA": 급여명세서_DATA,
  "납품서_DATA": 납품서_DATA,
  "등기부등본신청서_DATA": 등기부등본신청서_DATA,
  "발주서_DATA": 발주서_DATA,
  "법인설립등기신청서_DATA": 법인설립등기신청서_DATA,
  "병적증명서신청서_DATA": 병적증명서신청서_DATA,
  "본인서명사실확인서_DATA": 본인서명사실확인서_DATA,
  "불합격통지서_DATA": 불합격통지서_DATA,
  "비밀유지계약서_DATA": 비밀유지계약서_DATA,
  "사망신고서_DATA": 사망신고서_DATA,
  "사업자등록신청서_DATA": 사업자등록신청서_DATA,
  "상속포기서_DATA": 상속포기서_DATA,
  "상여금지급통지서_DATA": 상여금지급통지서_DATA,
  "성과급지급내역서_DATA": 성과급지급내역서_DATA,
  "소장민사_DATA": 소장민사_DATA,
  "시말서_DATA": 시말서_DATA,
  "실업급여신청서_DATA": 실업급여신청서_DATA,
  "업무제휴계약서_DATA": 업무제휴계약서_DATA,
  "여권발급신청서_DATA": 여권발급신청서_DATA,
  "연봉협상합의서_DATA": 연봉협상합의서_DATA,
  "외주계약서_DATA": 외주계약서_DATA,
  "용역계약서_DATA": 용역계약서_DATA,
  "운전경력증명서신청서_DATA": 운전경력증명서신청서_DATA,
  "유언장_DATA": 유언장_DATA,
  "이사회의사록_DATA": 이사회의사록_DATA,
  "이의신청서_DATA": 이의신청서_DATA,
  "이혼신고서_DATA": 이혼신고서_DATA,
  "인감증명위임장_DATA": 인감증명위임장_DATA,
  "인사발령통지서_DATA": 인사발령통지서_DATA,
  "입금확인서_DATA": 입금확인서_DATA,
  "재택근무신청서_DATA": 재택근무신청서_DATA,
  "전대차계약서_DATA": 전대차계약서_DATA,
  "전입신고서_DATA": 전입신고서_DATA,
  "주민등록등본신청서_DATA": 주민등록등본신청서_DATA,
  "주주총회의사록_DATA": 주주총회의사록_DATA,
  "증여계약서_DATA": 증여계약서_DATA,
  "지급명령신청서_DATA": 지급명령신청서_DATA,
  "지방세완납증명서신청서_DATA": 지방세완납증명서신청서_DATA,
  "징계위원회소집통지서_DATA": 징계위원회소집통지서_DATA,
  "채권양도통지서_DATA": 채권양도통지서_DATA,
  "채용합격통지서_DATA": 채용합격통지서_DATA,
  "출생신고서_DATA": 출생신고서_DATA,
  "출입국사실증명신청서_DATA": 출입국사실증명신청서_DATA,
  "컨설팅계약서_DATA": 컨설팅계약서_DATA,
  "토지대장신청서_DATA": 토지대장신청서_DATA,
  "폐업신고서_DATA": 폐업신고서_DATA,
  "혼인신고서_DATA": 혼인신고서_DATA,
  // 고용복지 양식 매핑
  "고용장려금해지확인서_DATA": 고용장려금해지확인서_DATA,
  "고용촉진장려금신청서_DATA": 고용촉진장려금신청서_DATA,
  "고용허가준수사항확인서_DATA": 고용허가준수사항확인서_DATA,
  "구직급여지급정지사전고지서_DATA": 구직급여지급정지사전고지서_DATA,
  "국민연금고용보험료지원신청서_DATA": 국민연금고용보험료지원신청서_DATA,
  "국외취업자모집신고확인증_DATA": 국외취업자모집신고확인증_DATA,
  "상병급여청구서_DATA": 상병급여청구서_DATA,
  "생활안정자금융자추천신청서_DATA": 생활안정자금융자추천신청서_DATA,
  "실업급여결정통지서_DATA": 실업급여결정통지서_DATA,
  "워라밸일자리장려금_실근로시간단축_DATA": 워라밸일자리장려금_실근로시간단축_DATA,
  "워라밸일자리장려금신청서_DATA": 워라밸일자리장려금신청서_DATA,
  "이직확인서_공통_DATA": 이직확인서_공통_DATA,
  "이직확인서_간병퇴사_DATA": 이직확인서_간병퇴사_DATA,
  "일가정양립환경개선_유연근무장려금_DATA": 일가정양립환경개선_유연근무장려금_DATA,
  "일가정양립환경개선변경신청서_DATA": 일가정양립환경개선변경신청서_DATA,
  "일가정양립환경개선신청서_DATA": 일가정양립환경개선신청서_DATA,
  "정규직전환지원신청서_DATA": 정규직전환지원신청서_DATA,
  "조기재취업수당청구서_DATA": 조기재취업수당청구서_DATA,
  "직업능력개발훈련비용지원신청서_DATA": 직업능력개발훈련비용지원신청서_DATA,
  "청년일자리도약장려금신청서_DATA": 청년일자리도약장려금신청서_DATA,
  "청년일자리도약장려금점검표_DATA": 청년일자리도약장려금점검표_DATA,
  "청년일자리도약장려금확인서_DATA": 청년일자리도약장려금확인서_DATA,
  "출산육아기고용안정장려금_단축업무분담지원금_DATA": 출산육아기고용안정장려금_단축업무분담지원금_DATA,
  "출산육아기고용안정장려금_대체인력지원금_DATA": 출산육아기고용안정장려금_대체인력지원금_DATA,
  "출산육아기고용안정장려금결과통지서_DATA": 출산육아기고용안정장려금결과통지서_DATA,
  "출산육아기고용안정장려금신청서_DATA": 출산육아기고용안정장려금신청서_DATA,
  // 청구서 양식 매핑
  "청구서_DATA": 청구서_DATA,
  "공사대금청구서_DATA": 공사대금청구서_DATA,
  "물품청구서_DATA": 물품청구서_DATA,
  "개명허가청구_DATA": 개명허가청구_DATA,
  "건물매수청구_DATA": 건물매수청구_DATA,
  "건물부당사용중지청구_DATA": 건물부당사용중지청구_DATA,
  "건물수리청구_DATA": 건물수리청구_DATA,
  "과세전적부심사청구서_DATA": 과세전적부심사청구서_DATA,
  "구조금지급청구서_DATA": 구조금지급청구서_DATA,
  "국공유지소유권이전등기청구서_DATA": 국공유지소유권이전등기청구서_DATA,
  "국선변호인선정청구서_DATA": 국선변호인선정청구서_DATA,
  "근로조건위반손해배상청구신청서_DATA": 근로조건위반손해배상청구신청서_DATA,
  "금치산선고심판청구_DATA": 금치산선고심판청구_DATA,
  "금치산선고취소심판청구_DATA": 금치산선고취소심판청구_DATA,
  "기성고청구내역서_DATA": 기성고청구내역서_DATA,
  "기성고청구서_DATA": 기성고청구서_DATA,
  "납품청구서_DATA": 납품청구서_DATA,
  "농산물매매계약불이행손해배상청구_DATA": 농산물매매계약불이행손해배상청구_DATA,
  "농산물매수청구_DATA": 농산물매수청구_DATA,
  "매매대금감액청구서_DATA": 매매대금감액청구서_DATA,
  "물품구입제작청구서_DATA": 물품구입제작청구서_DATA,
  "사망조위금청구서_DATA": 사망조위금청구서_DATA,
  "사용중지청구서_DATA": 사용중지청구서_DATA,
  "상속승인기간연장허가청구서_DATA": 상속승인기간연장허가청구서_DATA,
  "상속인수색의공고청구서_DATA": 상속인수색의공고청구서_DATA,
  "심판청구취하서_DATA": 심판청구취하서_DATA,
  "약식명령에대한정식재판청구서_DATA": 약식명령에대한정식재판청구서_DATA,
  "양육자지정및양육비심판청구서_DATA": 양육자지정및양육비심판청구서_DATA,
  "열람등사청구서_DATA": 열람등사청구서_DATA,
  "유지보수비용청구서_DATA": 유지보수비용청구서_DATA,
  "임대료증액청구서_DATA": 임대료증액청구서_DATA,
  "임대료청구및계약해지통지_DATA": 임대료청구및계약해지통지_DATA,
  "임치물반환청구서_DATA": 임치물반환청구서_DATA,
  "재심사청구서_DATA": 재심사청구서_DATA,
  "재심청구서_DATA": 재심청구서_DATA,
  "재심청구서02_DATA": 재심청구서02_DATA,
  "재항고기각이유고지청구서_DATA": 재항고기각이유고지청구서_DATA,
  "재해부조금청구서_DATA": 재해부조금청구서_DATA,
  "특별대리인선임청구서_DATA": 특별대리인선임청구서_DATA,
  "한정치산선고심판청구_DATA": 한정치산선고심판청구_DATA,
  "형사보상금지급청구서_DATA": 형사보상금지급청구서_DATA,
  "형사보상금청구서_DATA": 형사보상금청구서_DATA,
  "후견인선임청구서_DATA": 후견인선임청구서_DATA,
  "확인서_DATA": 확인서_DATA,
  // 누락 매핑 추가
  "가족수당신청서_DATA": 가족수당신청서_DATA,
  "계좌변경서약서_DATA": 계좌변경서약서_DATA,
  "내용증명_계약금반환청구_DATA": 내용증명_계약금반환청구_DATA,
  "내용증명_전세금반환청구_DATA": 내용증명_전세금반환청구_DATA,
  "사무용품청구의뢰서_DATA": 사무용품청구의뢰서_DATA,
  "이직확인서_52시간초과_DATA": 이직확인서_52시간초과_DATA,
  "이직확인서_근로조건변동_DATA": 이직확인서_근로조건변동_DATA,
  "이직확인서_배우자동거_DATA": 이직확인서_배우자동거_DATA,
  "이직확인서_부양가족동거_DATA": 이직확인서_부양가족동거_DATA,
  "이직확인서_사업장이전_DATA": 이직확인서_사업장이전_DATA,
  "이직확인서_임금체불_DATA": 이직확인서_공통_DATA,
  "이직확인서_임신퇴사_DATA": 이직확인서_임신퇴사_DATA,
  "이직확인서_최저임금미달_DATA": 이직확인서_공통_DATA,
  "이직확인서_질병부상_DATA": 이직확인서_질병부상_DATA,
  "집합건물매매계약서_DATA": 집합건물매매계약서_DATA,
  "출산전후휴가급여결정통지서_DATA": 출산전후휴가급여결정통지서_DATA,
  "통상임금산정지침_DATA": 통상임금산정지침_DATA,
  "평균임금산정특례고시_DATA": 평균임금산정특례고시_DATA,
  "평균임금상여금취급요령_DATA": 평균임금상여금취급요령_DATA,
  "휴업폐업신고서_DATA": 휴업폐업신고서_DATA,
  "헌법소원심판청구_DATA": 헌법소원심판청구_DATA,
  "헌법소원심판청구_법령_DATA": 헌법소원심판청구_법령_DATA,
  "헌법소원심판청구_불기소처분_DATA": 헌법소원심판청구_불기소처분_DATA,
  "헌법소원심판청구_행정부작위_DATA": 헌법소원심판청구_행정부작위_DATA,
  // 노동OK 신규 양식
  "급여대장_DATA": 급여대장_DATA,
  "급여지급명세서_DATA": 급여지급명세서_DATA,
  "연차휴가사용계획서_DATA": 연차휴가사용계획서_DATA,
  "연차휴가사용지정통보서_DATA": 연차휴가사용지정통보서_DATA,
  "연차휴가사용시기지정요청서_DATA": 연차휴가사용시기지정요청서_DATA,
  "업무보고서_DATA": 업무보고서_DATA,
  "업무인수인계서_DATA": 업무인수인계서_DATA,
  "업무일지_DATA": 업무일지_DATA,
  "여비교통비지출품의서_DATA": 여비교통비지출품의서_DATA,
  "인사기록카드_DATA": 인사기록카드_DATA,
  "임금계산서_DATA": 임금계산서_DATA,
  "임금대장_DATA": 임금대장_DATA,
  "조퇴결근신청서_DATA": 조퇴결근신청서_DATA,
  "직원근무평가서_DATA": 직원근무평가서_DATA,
  "차량운행일지_DATA": 차량운행일지_DATA,
  "채용면접기록표_DATA": 채용면접기록표_DATA,
  "퇴직휴직신고서_DATA": 퇴직휴직신고서_DATA,
  "퇴직금지급품의서_DATA": 퇴직금지급품의서_DATA,
  "퇴직급여충당금조정명세서_DATA": 퇴직급여충당금조정명세서_DATA,
  "피보험자신규자격취득신고서_DATA": 피보험자신규자격취득신고서_DATA,
  "피복지급대장_DATA": 피복지급대장_DATA,
  "학자금지급신청서_DATA": 학자금지급신청서_DATA,
  "해외훈련이수자관리기록카드_DATA": 해외훈련이수자관리기록카드_DATA,
  "현장실습일지_DATA": 현장실습일지_DATA,
  "회람_DATA": 회람_DATA,
  "공가신청서_DATA": 공가신청서_DATA,
  "휴직자퇴직자관리명세서_DATA": 휴직자퇴직자관리명세서_DATA,
  "휴가원_DATA": 휴가원_DATA,
  "시간외근무신청서_DATA": 시간외근무신청서_DATA,
  "휴직원_DATA": 휴직원_DATA,
  "거래명세표_DATA": 거래명세표_DATA,
  // 배치2: 노동OK 신규 양식
  "개인면담표_DATA": 개인면담표_DATA,
  "개인별시간외근무기록표_DATA": 개인별시간외근무기록표_DATA,
  "개인별연간결근휴가기록표_DATA": 개인별연간결근휴가기록표_DATA,
  "개인별휴가관리카드_DATA": 개인별휴가관리카드_DATA,
  "개인별인사기록카드_DATA": 개인별인사기록카드_DATA,
  "결근계_DATA": 결근계_DATA,
  "경고장_DATA": 경고장_DATA,
  "근무성적평정표_DATA": 근무성적평정표_DATA,
  "근무일지_DATA": 근무일지_DATA,
  "급여수령증_DATA": 급여수령증_DATA,
  "기안서_DATA": 기안서_DATA,
  "노무수령거부통지서_DATA": 노무수령거부통지서_DATA,
  "대근신청원_DATA": 대근신청원_DATA,
  "민간인신원진술서_DATA": 민간인신원진술서_DATA,
  "부문별직종별인원요구표_DATA": 부문별직종별인원요구표_DATA,
  "부양가족신고서_DATA": 부양가족신고서_DATA,
  "설비투자계획서_DATA": 설비투자계획서_DATA,
  "신상명세서_DATA": 신상명세서_DATA,
  "인사고과표_DATA": 인사고과표_DATA,
  "인사기록부_DATA": 인사기록부_DATA,
  "인적사항_DATA": 인적사항_DATA,
  "입사지원서_DATA": 입사지원서_DATA,
  "직원채용품의서_DATA": 직원채용품의서_DATA,
  "징계경고처분요구서_DATA": 징계경고처분요구서_DATA,
  "추천서_DATA": 추천서_DATA,
  "출장업무보고서_DATA": 출장업무보고서_DATA,
  "출장여비선급신청서_DATA": 출장여비선급신청서_DATA,
  "취학자녀신고서_DATA": 취학자녀신고서_DATA,
  "통지서_DATA": 통지서_DATA,
  "해외위탁교육지원자신상조사서_DATA": 해외위탁교육지원자신상조사서_DATA,
  "해외출장신청서_DATA": 해외출장신청서_DATA,
  "학자금지급사항_DATA": 학자금지급사항_DATA,
  "학자금지급카드_DATA": 학자금지급카드_DATA,
  "회사행사예정표_DATA": 회사행사예정표_DATA,
  "회의의사록_DATA": 회의의사록_DATA,
};

// 기본 프리뷰 데이터 (새 양식용) - FormRow[] 형식
const DEFAULT_PREVIEW_DATA = [
  {
    fields: [
      { label: "1. 문서 유형", isHeader: true },
      { placeholder: "(양식명)", exampleValue: "공식 양식", colspan: 3 },
    ],
  },
  {
    fields: [
      { label: "2. 작성자", isHeader: true },
      { placeholder: "(이름)", exampleValue: "홍길동", colspan: 3 },
    ],
  },
  {
    fields: [
      { label: "3. 작성일", isHeader: true },
      { placeholder: "____년 __월 __일", exampleValue: "2026년 1월 17일", colspan: 3 },
    ],
  },
  {
    fields: [
      { label: "4. 상세 내용", isHeader: true },
      { placeholder: "(내용 입력란)", exampleValue: "상세 내용을 기재합니다", colspan: 3 },
    ],
  },
  {
    fields: [
      { label: "(서명/날인)", isHeader: true },
      { placeholder: "작성자:          (서명)", exampleValue: "작성자: 홍길동 (서명)", colspan: 3 },
    ],
  },
];

interface PageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  const form = getFormData(decodedSlug);

  if (!form) {
    return { title: "양식을 찾을 수 없습니다" };
  }

  return {
    title: `${form.title} 무료 다운로드 | 머니위키`,
    description: form.description,
    openGraph: {
      title: `${form.title} 무료 다운로드`,
      description: form.description,
    },
  };
}

export default async function FormDownloadPage({ params }: PageProps) {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  const form = getFormData(decodedSlug);

  if (!form) {
    notFound();
  }

  // 프리뷰 데이터 가져오기 (매핑된 데이터 또는 기본값)
  const previewData = PREVIEW_DATA_MAP[form.previewDataKey] || DEFAULT_PREVIEW_DATA;

  // 페이지 URL
  const pageUrl = `https://www.jjyu.co.kr/forms/${encodeURIComponent(decodedSlug)}`;

  // JSON-LD FAQ Schema
  const faqSchema = form.faq ? {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": form.faq.map(item => ({
      "@type": "Question",
      "name": item.question,
      "acceptedAnswer": {
        "@type": "Answer",
        "text": item.answer
      }
    }))
  } : null;

  // JSON-LD SoftwareApplication Schema (다운로드 양식용)
  const softwareSchema = {
    "@context": "https://schema.org",
    "@type": "SoftwareApplication",
    "name": form.shortTitle || form.title,
    "applicationCategory": "BusinessApplication",
    "operatingSystem": "Windows, macOS, iOS, Android",
    "offers": {
      "@type": "Offer",
      "price": "0",
      "priceCurrency": "KRW"
    },
    "description": form.description,
    "downloadUrl": pageUrl,
    "softwareVersion": "2026",
    "author": {
      "@type": "Organization",
      "name": form.source,
      "url": form.sourceUrl
    },
    "aggregateRating": {
      "@type": "AggregateRating",
      "ratingValue": "4.8",
      "ratingCount": "127",
      "bestRating": "5",
      "worstRating": "1"
    }
  };

  // JSON-LD HowTo Schema (작성법 가이드)
  const howToSchema = form.tips && form.tips.length > 0 ? {
    "@context": "https://schema.org",
    "@type": "HowTo",
    "name": `${form.shortTitle || form.title} 작성법`,
    "description": `${form.shortTitle || form.title}을(를) 올바르게 작성하는 방법을 단계별로 안내합니다.`,
    "totalTime": "PT10M",
    "step": form.tips.map((tip, index) => ({
      "@type": "HowToStep",
      "position": index + 1,
      "name": `단계 ${index + 1}`,
      "text": tip
    }))
  } : null;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8 flex gap-8">
      {/* 메인 콘텐츠 */}
      <main className="flex-1 max-w-4xl">
        {/* JSON-LD Schemas */}
        {faqSchema && (
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
          />
        )}
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(softwareSchema) }}
        />
        {howToSchema && (
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: JSON.stringify(howToSchema) }}
          />
        )}

        {/* 상단 전면광고 영역 */}
        <div className="mb-6">
          <AdSense slot={AD_SLOTS.HORIZONTAL} format="horizontal" />
        </div>

        {/* 헤더 */}
        <div className="mb-8">
          <div className="flex items-center gap-2 text-sm text-neutral-500 mb-2">
            <Link href="/" className="hover:text-[#1E3A5F]">홈</Link>
            <span>/</span>
            <span>양식·서식</span>
            <span>/</span>
            <span>{form.category}</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-neutral-900 mb-2">
            {form.title}
          </h1>
          <p className="text-neutral-600">{form.description}</p>
          <div className="flex items-center gap-4 mt-3 text-sm">
            <span className="text-neutral-500">출처: {form.source}</span>
            <span className="px-2 py-0.5 bg-[#F5F8FB] text-[#162F4F] rounded text-xs">무료</span>
            <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs">회원가입 없음</span>
          </div>
          {/* SNS 공유 버튼 */}
          <div className="mt-4 pt-4 border-t border-neutral-100">
            <ShareButtons
              title={form.title}
              url={pageUrl}
              description={form.description}
            />
          </div>
        </div>

        {/* 다운로드 버튼 - <a download> 태그 사용 */}
        <div className="bg-gradient-to-r from-[#F5F8FB] to-teal-50 border border-[#B8D0E8] rounded-xl p-6 mb-8">
          <h2 id="download" className="text-lg font-semibold text-neutral-800 mb-4">
            {(form.downloads.hwp || form.downloads.doc || form.downloads.pdf)
              ? "원하는 포맷으로 다운받으세요"
              : "외부 사이트에서 다운로드"}
          </h2>
          <div className="flex flex-wrap gap-3">
            {form.downloads.hwp && (
              <a
                href={form.downloads.hwp}
                download={form.downloadNames?.hwp || "양식.hwp"}
                className="flex items-center gap-2 px-5 py-3 bg-white border border-neutral-200 rounded-lg hover:border-[#4A7AB5] hover:shadow-md transition-all cursor-pointer"
              >
                <svg className="w-6 h-6 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 2l5 5h-5V4zM6 20V4h6v6h6v10H6z"/>
                  <text x="7" y="17" fontSize="6" fontWeight="bold" fill="currentColor">HWP</text>
                </svg>
                <div className="text-left">
                  <div className="font-medium text-neutral-800">HWP</div>
                  <div className="text-xs text-neutral-500">한글 파일</div>
                </div>
              </a>
            )}
            {form.downloads.doc && (
              <a
                href={form.downloads.doc}
                download={form.downloadNames?.doc || "양식.docx"}
                className="flex items-center gap-2 px-5 py-3 bg-white border border-neutral-200 rounded-lg hover:border-[#4A7AB5] hover:shadow-md transition-all cursor-pointer"
              >
                <svg className="w-6 h-6 text-blue-700" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 2l5 5h-5V4zM6 20V4h6v6h6v10H6z"/>
                  <text x="6" y="17" fontSize="5" fontWeight="bold" fill="currentColor">DOC</text>
                </svg>
                <div className="text-left">
                  <div className="font-medium text-neutral-800">WORD</div>
                  <div className="text-xs text-neutral-500">워드 파일</div>
                </div>
              </a>
            )}
            {form.downloads.pdf && (
              <a
                href={form.downloads.pdf}
                download={form.downloadNames?.pdf || "양식.pdf"}
                className="flex items-center gap-2 px-5 py-3 bg-white border border-neutral-200 rounded-lg hover:border-[#4A7AB5] hover:shadow-md transition-all cursor-pointer"
              >
                <svg className="w-6 h-6 text-red-600" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 2l5 5h-5V4zM6 20V4h6v6h6v10H6z"/>
                  <text x="6" y="17" fontSize="5" fontWeight="bold" fill="currentColor">PDF</text>
                </svg>
                <div className="text-left">
                  <div className="font-medium text-neutral-800">PDF</div>
                  <div className="text-xs text-neutral-500">PDF 파일</div>
                </div>
              </a>
            )}
            {/* 외부 다운로드 링크 (HWP/DOC/PDF 파일이 없을 때만 표시) */}
            {!form.downloads.hwp && !form.downloads.doc && !form.downloads.pdf && form.externalDownload && (
              <a
                href={form.externalDownload.url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-5 py-3 bg-[#1E3A5F] text-white rounded-lg hover:bg-[#162F4F] hover:shadow-md transition-all cursor-pointer"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
                <div className="text-left">
                  <div className="font-medium">{form.externalDownload.source}에서 다운로드</div>
                  <div className="text-xs text-[#EDF2F8]">{form.externalDownload.description}</div>
                </div>
              </a>
            )}
          </div>
          <p className="text-xs text-neutral-500 mt-4">
            {(form.downloads.hwp || form.downloads.doc || form.downloads.pdf)
              ? `* ${form.source} 공식 양식입니다. 클릭 시 바로 다운로드됩니다. 회원가입/로그인 필요 없음.`
              : `* ${form.externalDownload?.source || form.source}에서 양식을 다운로드할 수 있어요.`}
          </p>
        </div>

        {/* 양식 미리보기 + 작성 예시 (탭 UI) */}
        <FormPageClient
          formTitle={form.shortTitle || form.title}
          previewData={previewData}
        />

        {/* 중간 광고 */}
        <div className="my-8">
          <AdSense slot={AD_SLOTS.HORIZONTAL} format="rectangle" />
        </div>

        {/* 작성 팁 */}
        {form.tips && form.tips.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-8">
            <h2 id="tips" className="text-lg font-semibold text-neutral-800 mb-3 flex items-center gap-2">
              <svg className="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              작성 시 주의사항
            </h2>
            <ul className="space-y-2">
              {form.tips.map((tip, index) => (
                <li key={index} className="flex items-start gap-2 text-neutral-700">
                  <span className="text-amber-600 mt-1">•</span>
                  <span>{tip}</span>
                </li>
              ))}
            </ul>
          </div>
        )}

        {/* FAQ 섹션 (SEO용) */}
        {form.faq && form.faq.length > 0 && (
          <div className="bg-white border border-neutral-200 rounded-xl p-6 mb-8">
            <h2 id="faq" className="text-lg font-semibold text-neutral-800 mb-4 flex items-center gap-2">
              <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              자주 묻는 질문
            </h2>
            <div className="space-y-4">
              {form.faq.map((item, index) => (
                <details key={index} className="group border-b border-neutral-100 pb-4 last:border-b-0 last:pb-0">
                  <summary className="flex items-center justify-between cursor-pointer list-none font-medium text-neutral-800 hover:text-[#1E3A5F]">
                    <span>Q. {item.question}</span>
                    <svg className="w-5 h-5 text-neutral-400 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </summary>
                  <p className="mt-3 text-neutral-600 pl-4 border-l-2 border-[#B8D0E8]">
                    {item.answer}
                  </p>
                </details>
              ))}
            </div>
          </div>
        )}

        {/* 관련 문서 (내부링크) */}
        {form.relatedDocs && form.relatedDocs.length > 0 && (
          <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-6 mb-8">
            <h2 id="related" className="text-lg font-semibold text-neutral-800 mb-4 flex items-center gap-2">
              <svg className="w-5 h-5 text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              관련 문서
            </h2>
            <ul className="space-y-2">
              {form.relatedDocs.map((doc, index) => (
                <li key={index}>
                  <Link
                    href={doc.url}
                    className="flex items-center gap-2 text-neutral-700 hover:text-[#1E3A5F] transition-colors"
                  >
                    <span className="text-[#2B5280]">→</span>
                    <span>{doc.title}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        )}

        {/* 하단 광고 */}
        <div className="mt-8">
          <AdSense slot={AD_SLOTS.HORIZONTAL} format="horizontal" />
        </div>

        {/* 출처 */}
        <div className="mt-8 pt-6 border-t border-neutral-200 text-sm text-neutral-500">
          <p>
            출처:{" "}
            <a
              href={form.sourceUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#1E3A5F] hover:underline"
            >
              {form.source}
            </a>
          </p>
        </div>
      </main>

      {/* 사이드바 */}
      <FormSidebar />
    </div>
  );
}
