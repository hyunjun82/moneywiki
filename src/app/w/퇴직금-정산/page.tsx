...
// Q1. 퇴직일이 며칠 안 남은 직장인이 퇴직금을 얼마나 받을 수 있는지, 제대로 받는 건지 불안한 상황
// Q2. 정산서를 직접 검증하고 IRP 계좌로 퇴직금을 수령하는 것
// Q3. 1일 평균임금 공식, 상여금 환산 포함 여부, IRP 의무 수령 기준(300만원), 14일 지급 기한, 지연이자 연 20%
// Q4. Steps(절차) + Calculator(금액 확인) + DocTable(서류) + Checklist(체크) + FAQ
//
// MAP:
// - Q1 → 서론: "퇴직일 확정 순간부터 14일 카운트다운" 톤
// - Q2 → H2 순서: 예상액 계산 → 서류 목록 → 4단계 절차 → 체크리스트
// - Q3 → H2 4개 + FAQ(상여금 환산·IRP·지연이자 심화)
// - Q4 → Calculator + DocTable + Steps + Checklist + FAQ
...
const CHECK_ITEMS = [
  ...
  { id: "c2", label: "회사로부터 퇴직금 정산서를 아직 못 받았죠" },
  { id: "c3", label: "급여명세서 최근 3개월치를 준비할 수 있고요" },
  { id: "c4", label: "퇴직금이 300만원을 넘을 것 같아 IRP 계좌가 필요하죠" },
];
...
const STEPS = [
  ...
  },
  {
    ...
    tip: "상여금 환산 누락이 가장 흔한 실수예요. 포함 안 됐으면 재계산 요청할 수 있죠",
  },
  {
    title: "IRP 계좌번호 인사팀 전달",
    desc: "퇴직금이 300만원을 넘으면 IRP 계좌로만 수령할 수 있죠. 은행이나 증권사에서 IRP 계좌를 개설하고 계좌번호를 이메일·문자로 인사팀에 알려줘야 해요.",
    tip: "IRP 계좌는 하나만 있으면 돼요. 없다면 퇴직 전에 미리 개설해두는 게 좋아요",
    link: { label: "IRP 계좌 개설 방법 보기", href: "/w/퇴직금-IRP-계좌" },
  },
  {
    title: "입금 확인 및 이의 제기",
    desc: "IRP 계좌 또는 일반 계좌에 입금됐는지 살펴봐요. 금액이 다르면 즉시 인사팀에 재계산을 요청해요. 퇴직일로부터 14일이 지나도 지급되지 않으면 연 20% 지연이자를 청구할 수 있죠.",
    tip: "입금 후 퇴직소득 원천징수영수증과 금액이 일치하는지 꼭 맞춰봐요",
...
const CHECKLIST = [
  ...
  "퇴직일로부터 14일 이내 입금 여부 점검",
];

const FAQS = [
  {
    q: "퇴직금 정산서는 꼭 받아야 하나요?",
    a: "법적 의무는 없지만 받는 게 훨씬 유리해요. 계산 내역을 직접 검증할 수 있고, 오류가 생겼을 때 증거가 돼요. 인사팀에 요청하면 대부분 발급해줘요.",
  },
  {
    q: "퇴직금이 예상보다 적게 왔어요",
    a: "급여명세서로 직접 계산해봐요. 상여금 월 환산 누락, 근속기간 계산 오류가 가장 흔한 원인이에요. 차이가 10만원 이상이면 인사팀에 재계산을 요청하고, 거부하면 고용노동부(1350)에 진정을 낼 수 있죠.",
  },
  {
    q: "퇴직금을 IRP가 아닌 일반 계좌로 받을 수 있나요?",
    a: "퇴직금이 300만원 이하면 일반 계좌로 받을 수 있죠. 300만원 초과면 의무적으로 IRP 계좌로만 수령해야 해요. 근로자퇴직급여보장법 제9조에 따른 규정이에요.",
  },
  {
    q: "14일이 지나도 퇴직금이 안 들어오면 어떻게 하나요?",
    a: "연 20% 지연이자가 자동으로 발생해요. 고용노동부 민원마당에서 온라인으로 진정을 낼 수 있고요. 진정 접수 후 평균 2~4주 내에 처리돼요.",
  },
  {
    q: "퇴직금 정산 후 세금은 어디서 처리되나요?",
    a: "회사가 퇴직금 지급 시 퇴직소득세를 원천징수하고 처리해요. 근로자 본인이 별도로 납부할 필요는 없어요. 퇴직소득 원천징수영수증을 발급해주니 금액을 살펴봐요.",
  },
  {
    q: "IRP로 받으면 세금이 줄어드나요?",
    a: "IRP로 받으면 수령 시점까지 퇴직소득세 납부가 미뤄지는 과세 이연 효과가 있죠. 나중에 연금으로 인출하면 연금소득세(3~5%)가 적용돼서 일시금 세율보다 유리해요.",
  },
];
...
const RELATED = [
  { slug: "퇴직금-정산서", title: "퇴직금 정산서 항목별 확인 방법", description: "정산서 각 항목이 뭔지, 오류는 어떻게 잡는지 정리했어요." },
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-정산" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        그냥 기다리다가 정산서 숫자만 믿으면 상여금 환산 누락이나 근속기간 오류로 수십만~수백만원을 덜 받는 경우가 꽤 있죠.
      ...
      <p style={body}>
        ...
        이 기한을 넘기면 연 20% 지연이자가 붙고, 고용노동부에 진정도 낼 수 있죠.
        기한 내에 받으려면 퇴직 2주 전부터 정산을 준비하는 게 좋아요.
      </p>
      <p style={body}>
        정산서를 받으면 그냥 믿지 말고 직접 계산해봐야 해요.
        상여금 월 환산이 빠진 경우가 실제로 많아요.
        연간 상여금 ÷ 12가 3개월 임금 합산에 포함됐는지 꼭 점검해야 해요.
      </p>

      <GreenBox>
        상여금 월 환산: 연간 상여금 ÷ 12 × 3개월분이 포함됐는지 점검<br />
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금 정산이 필요한 상황이에요. 아래 계산기로 예상 금액을 먼저 살펴보고 정산 절차를 진행해요."
        partialMatchText="상황이 다를 수 있죠. 고용노동부(1350) 상담을 받아봐요."
      />

      <Divider />

      {/* H2-2: 금액/계산 */}
      <H2>퇴직금 예상액 미리 계산해봐요</H2>
      <p style={body}>
        정산서를 받기 전에 내 예상 금액을 미리 계산해두면 정산서와 비교하기 쉬워요.
        퇴직금 공식은 1일 평균임금 × 30 × 근속연수예요.
        상여금이나 연차수당이 있으면 실제 금액이 계산기 결과보다 더 높을 수 있죠.
      </p>
      <p style={body}>
        차이가 10만원 이상이면 인사팀에 재계산을 요청해요.
      ...
      <p style={body}>
        ...
        이것만 놓치지 않으면 수십만~수백만원 차이를 막을 수 있죠.
      </p>
      ...
      <GreenBox>
        연간 상여금 ÷ 12 = 월 환산액, 이 금액이 3개월 임금 합산에 포함됐는지 꼭 짚어봐야 해요.<br />
        포함 안 됐다면 인사팀에 재계산을 요청할 수 있죠. 거부하면 고용노동부(1350)에 진정을 낼 수 있고요.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법·근로기준법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 살펴봐요." />
    </ArticleLayout>
  );
}
