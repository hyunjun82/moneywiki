...
const STEPS = [
  ...
  {
    title: "최종 퇴직금 계산",
    desc: "공식은 '퇴직 전 3개월 평균임금 × 30일 × 재근속 연수'예요. 중간정산 이후 임금이 올랐다면 현재 임금 기준으로 계산되기 때문에 최종 수령액이 더 클 수도 있어요. 인사팀에 계산 내역을 요청해서 직접 대조해봐요.",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "인사팀에 중간정산 원천징수영수증 재발급을 요청하면 날짜를 파악할 수 있고요. 없다면 홈택스에서 '근로·퇴직소득 지급명세서 조회' 메뉴로 과거 기록을 찾아볼 수도 있죠.",
  },
  {
    q: "중간정산 이후 세금 공제가 더 불리한가요?",
    a: "재근속 기간이 짧으면 근속연수 공제가 줄어서 세금이 더 많이 나올 수 있어요. 전체 근속기간으로 한 번에 받을 때보다 세금 측면에서 불리해지는 경우가 있고요. 중간정산 시 수령한 금액과 합산하면 총액은 비슷하지만 세금 부담이 분산되기도 해요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={body}>
        ...
        '조회/발급 → 기타 조회 → 근로·퇴직소득 지급명세서 조회' 메뉴로 과거 중간정산 내역을 찾을 수 있죠.
      ...
      <p style={body}>
        ...
        퇴직금 지급이 14일을 넘기면{" "}
        <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>연 20% 지연이자</a>를 청구할 수 있죠.
      </p>
