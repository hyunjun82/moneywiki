// Q1: 중간정산을 한 번 이상 받은 근로자가 퇴직을 앞두고 최종 퇴직금이 얼마 나오는지 계산하려는 상황
// Q2: 재근속 기간을 직접 계산하고 인사팀 지급 내역을 대조해서 퇴직금을 정확히 수령한다
// Q3: 재근속 기산일(중간정산일 다음 날), 평균임금×30일×재근속연수 공식, 1년 미만 시 퇴직금 없음, 세금 불리 여부, 서류 확보 방법, 지급기한 14일
// Q4: 공식 GreenBox + 계산기 + 단계별 절차 + 체크리스트 (숫자와 행동 중심)
...
const CHECK_ITEMS = [
  { id: "c1", label: "이전에 퇴직금 중간정산을 받은 기록이 있죠" },
  { id: "c2", label: "중간정산 이후에도 같은 회사에서 계속 근무했고요" },
...
const STEPS = [
  {
    title: "중간정산 이력 및 날짜 파악",
    desc: "중간정산을 받은 날짜와 금액을 먼저 파악해야 해요. 이 날짜가 재근속 기산일의 기준이 되거든요. 원천징수영수증이 없으면 인사팀에 요청하거나 홈택스에서 과거 원천징수 내역을 직접 조회할 수 있고요.",
    tip: "홈택스 → 조회/발급 → 기타 조회 → 근로·퇴직소득 지급명세서 조회",
    link: { label: "홈택스 바로가기", href: "https://www.hometax.go.kr" },
  },
  {
    title: "재근속 기산일 계산",
    desc: "중간정산일 다음 날이 재근속 기산일이에요. 최종 퇴직금은 이 날부터 최종 퇴직일까지의 기간만 기준으로 계산하는 구조거든요. 중간정산 전 기간은 이미 수령했기 때문에 포함되지 않고요.",
    tip: "중간정산일 다음 날 = 재근속 기산일 (이날부터 1년이 채워져야 퇴직금 발생)",
  },
  {
    title: "최종 퇴직금 계산",
    desc: "공식은 '퇴직 전 3개월 평균임금 × 30일 × 재근속 연수'예요. 중간정산 이후 임금이 올랐다면 현재 임금 기준으로 계산되기 때문에 최종 수령액이 더 클 수도 있거든요. 인사팀에 계산 내역을 요청해서 직접 대조해봐요.",
    tip: "인사팀이 재근속 기산일을 잘못 입력하는 경우가 있으니 직접 대조해봐요",
  },
  {
    title: "IRP 계좌로 수령",
    desc: "최종 퇴직금 300만원 초과 시 IRP 계좌로 먼저 받아야 해요. 퇴직소득세는 재근속 기간의 근속연수 공제를 적용해서 원천징수되고요. 회사는 퇴직일로부터 14일 이내에 지급해야 해요.",
    tip: "14일 초과 시 연 20% 지연이자 청구 가능",
  },
];
...
const CHECKLIST = [
  ...
  "재근속 기간 1년 이상 체크: 미만이면 퇴직금 발생 안 됨",
  "최종 퇴직 전 3개월 평균임금 파악: 급여명세서 3개월치 보관",
  "IRP 계좌 사전 개설: 300만원 초과 수령 시 필요",
  "지급기한 14일 체크: 초과 시 지연이자(연 20%) 청구 가능",
];

const FAQS = [
  {
    q: "중간정산 후 최종 퇴직금은 전체 근속기간으로 계산하나요?",
    a: "아니에요. 중간정산 이후 재근속 기간만 기준으로 계산해요. 중간정산 전 기간에 해당하는 퇴직금은 이미 받았기 때문에 다시 포함되지 않는 구조이고요.",
  },
  {
    q: "재근속 기간이 1년 미만이면 퇴직금이 없나요?",
    a: "맞아요. 퇴직금은 근속기간 1년 이상인 근로자에게만 발생해요. 중간정산 이후 1년이 채 안 됐다면 최종 퇴직금은 나오지 않아요. 중간정산 시 받은 금액이 전부가 되는 구조예요.",
  },
  {
    q: "중간정산 날짜를 모르면 어떻게 파악하나요?",
    a: "인사팀에 중간정산 원천징수영수증 재발급을 요청하면 날짜를 파악할 수 있고요. 없다면 홈택스에서 '근로·퇴직소득 지급명세서 조회' 메뉴로 과거 기록을 찾아볼 수도 있거든요.",
  },
  {
    q: "중간정산 이후 세금 공제가 더 불리한가요?",
    a: "재근속 기간이 짧으면 근속연수 공제가 줄어서 세금이 더 많이 나올 수 있거든요. 전체 근속기간으로 한 번에 받을 때보다 세금 측면에서 불리해지는 경우가 있고요. 중간정산 시 수령한 금액과 합산하면 총액은 비슷하지만 세금 부담이 분산되기도 해요.",
  },
  {
    q: "중간정산을 여러 번 받았다면 어떻게 계산하나요?",
    a: "마지막 중간정산일 다음 날부터 최종 퇴직일까지의 기간이 재근속 기간이에요. 중간정산을 세 번 받았다면 세 번째 중간정산일 다음 날이 기산일이에요. 그 이전 기간은 각각 정산 완료된 상태이고요.",
  },
  {
    q: "퇴직금을 14일 안에 안 주면 어떻게 하나요?",
    a: "퇴직일로부터 14일 이내 지급이 원칙이에요. 초과 시 연 20% 지연이자 청구가 가능해요. 고용노동부(1350)에 신고하거나 민사 소송으로 청구할 수도 있고요. 자세한 내용은 퇴직금 지연이자 글에서 살펴볼 수 있고요.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-중간정산-후-퇴직금-계산" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        최종 퇴직금은 중간정산일 다음 날부터 시작된 재근속 기간만 기준으로 계산되는 구조이고요.
      </p>
      ...
      <p style={body}>
        ...
        중간정산일 다음 날부터 근속기간이 0년으로 리셋되고, 새로 쌓이기 시작하죠.
        퇴직금 계산 공식 자체는 똑같이 '평균임금 × 30일 × 근속연수'인데,
        적용되는 근속연수가 전체 기간이 아닌 '재근속 기간'으로 바뀌어요.
      </p>
      <p style={body}>
        중간정산 이후 임금이 올랐다면 최종 퇴직금에 유리한 상황이에요.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="최종 퇴직금 계산이 가능한 상황이에요. 아래 계산기로 재근속 기간 기준 예상액을 살펴봐요."
        partialMatchText="중간정산 이력이나 재근속 기간을 인사팀에 먼저 문의해봐요."
      />
      ...
      <p style={body}>
        ...
        정확한 금액은 퇴직 전 3개월 평균임금(상여금·수당 포함)을 기준으로 계산해야 하고요.
      </p>
      ...
      <Calculator
        ...
        note="※ 월 평균급여 × 재근속 연수 기준 추정값이에요. 실제 금액은 퇴직 전 3개월 평균임금 기준으로 달라질 수도 있고요."
      />
      ...
      <p style={body}>
        중간정산 이력이 있으면 인사팀이 재근속 기산일을 잘못 계산하는 경우가 간혹 있죠.
        원천징수영수증이 재근속 기산일의 핵심 근거라서 반드시 확보해야 해요.
        없다면 홈택스에서 과거 지급명세서를 조회하면 날짜를 찾을 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        '조회/발급 → 기타 조회 → 근로·퇴직소득 지급명세서 조회' 메뉴로 과거 중간정산 내역을 찾을 수 있거든요.
      ...
      <p style={body}>
        받은 퇴직금 지급 명세서에서 적용된 근속기간 항목을 직접 대조해봐요.
        퇴직금 지급이 14일을 넘기면{" "}
        <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>연 20% 지연이자</a>를 청구할 수 있거든요.
      </p>
      ...
      <p style={body}>
        중간정산을 받은 적 있는 분들은 아래 항목을 빠뜨리지 않도록 순서대로 챙겨봐요.
        재근속 기산일과 세금 공제 부분은 인사팀 계산만 믿지 말고 직접 대조하는 게 안전해요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법을 바탕으로 작성됐어요. 법령 개정이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 살펴봐요." />
    </ArticleLayout>
  );
}
