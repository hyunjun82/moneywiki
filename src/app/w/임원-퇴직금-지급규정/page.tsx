...
//     등기 임원(이사·감사)으로 퇴직을 앞두고 "나도 퇴직금 받을 수 있나?" 궁금하거나,
//     법인 대표가 임원 퇴직금 지급규정을 새로 만들어야 하는 상황이에요.
// Q2. 이 사람이 이 글을 읽고 나서 할 수 있어야 하는 행동은?
//     정관·결의 근거를 확인하고, 세법 한도 내에서 퇴직금을 계산한 뒤 주주총회 결의를 거쳐 실제로 수령한다.
// Q3. 이 행동을 하려면 반드시 알아야 하는 정보는?
//     ① 정관/결의 없으면 청구 불가 ② 세법 한도(월보수×재임연수×1/10×최대3배) 초과 시 손금 불산입
//     ③ 300만원 초과 시 IRP 의무 ④ 주주총회 결의 없이 지급 시 법적 문제 ⑤ 실질 근로자성 인정 가능성
// Q4. 이 정보를 가장 잘 전달하는 형태는?
//     조건 체크(EligibilityChecker) + 계산기(Calculator) + 서류 목록(DocTable) + 절차(Steps) + 체크리스트(Checklist) + FAQ
...
const CHECK_ITEMS = [
  { id: "c1", label: "법인에 이사·감사 등 등기 임원으로 등재돼 있죠" },
  { id: "c2", label: "회사 정관 또는 임원퇴직금 지급규정이 마련돼 있죠" },
  { id: "c3", label: "임원으로 재임한 기간이 1년 이상이에요" },
  { id: "c4", label: "주주총회에서 퇴직금 규정이 승인됐어요" },
];
...
const STEPS = [
  {
    title: "정관·지급규정 존재 여부 파악",
    desc: "임원 퇴직금은 법정 의무가 아니에요. 정관에 '임원퇴직금 지급규정에 따른다'는 조항이 있거나, 주주총회·이사회에서 별도 결의한 경우에만 지급 의무가 생겨요. 먼저 정관 해당 조항부터 찾아봐요.",
    tip: "정관 근거가 없으면 청구 자체가 어려워요. 규정이 없다면 주주총회 특별결의로 사후 제정도 가능해요",
  },
  {
    title: "지급액 계산 및 세법 한도 검토",
  ...
  },
  {
    ...
    desc: "지급 전에 주주총회(정관에 이사회 위임 시 이사회) 결의가 필요해요. 결의 없이 지급하면 법인 자금 유용 문제로 이어질 수 있죠. 소규모 법인은 주주 전원 동의서로 대체하기도 해요.",
    tip: "결의 후 의사록을 반드시 작성하고 보관해둬요",
  },
  {
    title: "지급 및 세금 처리",
    desc: "임원 퇴직금도 퇴직소득세 과세 대상이에요. 근속 기간은 임원 재임 기간 기준으로 계산하고, 세율은 근로자 퇴직금과 동일해요. 300만원 초과 시 IRP 계좌로만 수령해야 해요(2022년 4월 이후 퇴직자).",
    tip: "IRP에 넣어두고 55세 이후 연금으로 수령하면 퇴직소득세 30~40% 절감이 가능해요",
  },
];

const CHECKLIST = [
  "정관 조항 파악: 임원 퇴직금 지급 근거 조항 존재 여부",
...
const FAQS = [
  {
    ...
    a: "아니에요. 근로자와 달리 임원 퇴직금은 법정 의무가 없어요. 정관이나 주주총회 결의로 지급 규정이 마련돼야 받을 수 있고, 규정이 없으면 청구 근거 자체가 없어요.",
  },
  {
    q: "임원이면서 실질적으로 근로자 역할도 한 경우엔?",
    a: "사용자의 지휘·감독 아래 근로를 제공한 사실이 인정되면 근로자성을 인정받을 수 있죠. 법원에서도 등기임원이지만 실질 근로자로 본 사례가 있고, 이 경우 근로기준법상 퇴직금도 별도로 청구 가능해요.",
  },
  {
    q: "세법 한도 계산법을 알기 쉽게 설명해줘요.",
    a: "월 평균 보수 × 재임연수 × 1/10 × 지급 배율(최대 3배)이에요. 예를 들어 월 보수 600만원, 재임 5년, 배율 2배라면 600만 × 5 × 0.1 × 2 = 600만원이에요. 한도를 넘으면 초과분은 법인세 비용 처리가 안 돼요.",
  },
  {
    q: "임원 퇴직금에도 IRP 의무가 붙나요?",
    a: "맞아요. 2022년 4월 이후 퇴직자는 300만원 초과 시 IRP 계좌로만 수령해야 해요. 근로자와 동일한 기준이에요.",
  },
  {
    q: "정관에 임원 퇴직금 조항이 없는 소규모 법인이라면?",
    a: "주주 전원 동의로 지급을 결의할 수 있죠. 지급 기준을 명확히 정하고 의사록을 남겨야 해요. 이미 지급한 경우 사후 동의도 가능하지만, 세무상 리스크가 따라요.",
  },
  {
    q: "임원 퇴직금 지급이 지연되면 어떻게 하나요?",
    a: "임원은 근로기준법 14일 규정 적용 대상이 아니에요. 정관에 명시된 기한을 넘기면 상법 연 6% 지연이자를 청구할 수 있고, 계속 미지급이면 민사소송 또는 지급명령 신청이 가능해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        규정이 있더라도 <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>세법 손금 한도</a>를 넘으면 법인세 불이익이 생기고, 주주총회 결의 없이 지급하면 법적 문제로 이어질 수 있죠.
      </p>
      ...
      <p style={body}>
        임원으로 등기돼 있어도 실질적으로 사용자의 지휘·감독을 받으며 근로를 제공했다면 근로자성 인정 가능성이 있죠.
        이 경우 <a href="/w/퇴직금-이란" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자 퇴직금</a>도 별도로 청구할 수 있죠.
        판례에서도 등기임원이지만 실질 근로자로 본 사례가 있고, 해당 판결에서 근로기준법상 퇴직금이 별도 인정됐어요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="임원 퇴직금 지급 절차를 진행할 수 있죠. 아래 계산기로 예상 금액을 뽑아봐요."
        partialMatchText="임원 퇴직금 지급 요건이 복잡할 수 있죠. 세무사나 법무사 상담을 권해요."
      />
      ...
      <p style={body}>
        아래 계산기에서 월 보수와 재임 기간, 정관 배율을 조정하면 예상 퇴직금과 세법 최대 한도를 바로 비교할 수 있죠.
        한도 초과 시 세무조사 리스크가 따라오니 미리 한도 내로 설계해두는 게 좋아요.
      </p>
      ...
      <p style={body}>
        ...
        법인 등기부등본으로 임원 재임 기간을 증빙해야 하고, 최종 보수 자료도 함께 준비해요.
      </p>
      ...
      <p style={body}>
        지급 근거 파악부터 세금 처리까지 순서대로 진행해야 해요.
        결의 없이 지급부터 하면 나중에 법적·세무 문제가 동시에 생길 수 있죠.
      </p>
      ...
      <p style={body}>
        세법 한도와 주주총회 결의를 빠뜨리면 세무조사와 법적 분쟁이 동시에 따라와요.
      ...
      <GreenBox>
        임원 퇴직금도 IRP에 넣어두면 과세 이연 효과가 있죠.
        55세 이후 연금으로 받으면 퇴직소득세를 30~40% 절감할 수 있죠.
      </GreenBox>
