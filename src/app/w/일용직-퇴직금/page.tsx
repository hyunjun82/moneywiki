"use client";
// Q1. 일당제로 같은 사업주 밑에서 1년 넘게 일했는데 "일용직은 퇴직금 없다"는 말을 듣고 억울한 상황
// Q2. 내가 퇴직금 대상인지 판단하고, 맞으면 직접 금액 계산 후 청구(또는 노동청 신고)한다
// Q3. 주 15시간·1년 조건 계산법, 4주 평균 의미, 4대보험 무관, 계속 근로 기준, 계산 공식, 청구 4단계, 소멸시효 3년
// Q4. EligibilityChecker(자격) → Calculator(금액) → Steps(청구 절차) → DocTable(서류) → FAQ → References
// MAP: Q1→서론 공감 Q2→H2순서(조건→계산→청구) Q3→H2깊이(조건 상세, 계속근로 기준 포함) Q4→컴포넌트 매핑
...
const CHECK_ITEMS = [
  ...
  { id: "c4", label: "근무 사실을 입증할 통장 내역 또는 연락 기록이 있죠" },
];
...
const STEPS = [
  {
    ...
    desc: "하루 근무 시간 × 주 근무일수로 '주 15시간'을 맞추는지 먼저 따져요. 하루 3시간씩 주 5일이면 딱 15시간이에요. 1년 이상은 동일 사업주 아래 연속 근로가 원칙인데, 잠깐 쉬었다 재고용됐어도 실질 관계가 유지됐다면 계속 근로로 볼 수 있죠.",
...
const FAQS = [
  {
    ...
    a: "받을 수 있죠. 일당제·일용직이라는 고용 형태는 퇴직금 기준과 무관해요. 같은 사업주 아래서 1년 이상, 주 15시간 이상이라는 두 조건만 충족하면 일반 근로자와 동일하게 퇴직금이 발생해요.",
  },
  {
    q: "여러 현장을 옮겨 다니면 합산이 되나요?",
    a: "같은 고용주(사업장)에서 일한 기간만 합산돼요. 고용주가 다르면 각 사업장별로 1년 기준을 따져야 해요. 건설 일용직이라면 건설근로자공제회 퇴직공제금도 별도로 받을 수 있고요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.45, marginBottom: 14 }}>
        일용직도 퇴직금 받을 수 있나요?<br />
        주 15시간·1년 조건부터 청구 절차까지
      </h1>

      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        "일용직은 퇴직금이 없다"는 말, 그냥 넘어가면 안 돼요.{" "}
        ...
        같은 사업주 아래서 1년 이상, 4주 평균 주 15시간 이상이면 퇴직금이 발생해요.
        계약서나 4대보험이 없어도 통장 내역만 있으면 청구할 수 있고요.
      ...
      <p style={body}>
        ...
        4대보험 미가입이어도 실제 근무 사실을 입증하면 퇴직금을 청구할 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금 청구 조건을 갖추고 있죠. 아래 계산기로 예상 금액을 계산해보세요."
        partialMatchText="조건이 일부 맞지 않을 수 있죠. 고용노동부(1350)에 먼저 전화 상담을 받아보세요."
      />

      <Divider />

      <H2>일당 기준 퇴직금 계산하기</H2>
      <p style={body}>
      ...
      <p style={body}>
        아래에서 일당, 주 근무일수, 근무 기간을 조절해 예상 금액을 계산해보세요.
      ...
      <Calculator
        ...
        note="※ 주 15시간 이상 + 1년 이상 계속 근무 시에만 해당돼요. 실제 퇴직금은 3개월 평균임금 기준으로 재산정될 수 있고요."
      />
      ...
      <p style={body}>
        ...
        있는 것부터 모아두고, 없는 서류가 있어도 청구는 가능해요.
      </p>
      <p style={body}>
        통장 입금 내역 하나만 있어도 진정 접수는 가능해요.
        4대보험 가입이력은 <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 무료로 조회할 수 있고요.
      </p>
      ...
      <H2>신고 전 준비 체크리스트</H2>
      <p style={body}>
        현장을 떠난 뒤에는 증빙 자료를 구하기 어려울 수 있죠.
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여 보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 최신 내용을 받아보세요." />
    </ArticleLayout>
  );
}
