"use client";
// Q1. 퇴사를 앞두고 있거나 이미 그만뒀는데 퇴직금을 받을 수 있는지 확신이 없는 상황
// Q2. 내가 퇴직금 대상인지 판단하고, 대상이면 금액 계산 후 회사에 청구한다
// Q3. 12개월 기준, 주 15시간 기준, 계약직·아르바이트 포함 여부, 1년 못 채웠을 때 대안
// Q4. 조건 체크리스트 → 계산기 → 절차 순서 → 1년 미만 대안 → FAQ
...
const CHECK_ITEMS = [
  { id: "c1", label: "같은 사업주 아래서 만 12개월(1년) 이상 근무했죠" },
  { id: "c2", label: "주 평균 15시간 이상 근무하는 계약으로 일했죠" },
  { id: "c3", label: "정규직·계약직·아르바이트 중 하나로 고용됐죠" },
  { id: "c4", label: "고용 관계가 중간에 끊기지 않고 이어졌죠" },
];
...
const STEPS = [
  {
    ...
    desc: "입사일부터 퇴직일까지 정확히 만 1년이 지나야 퇴직금이 생겨요. 2024년 3월 1일에 입사했다면 2025년 3월 1일이 딱 1년이죠. 11개월 29일은 1년 미만이라서 퇴직금이 0원이에요.",
    tip: "근로계약서의 '입사일'과 고용보험 취득일을 비교해 정확히 계산해요",
  },
  {
    title: "주 평균 15시간 이상 체크",
    desc: "1년을 채웠더라도 주 평균 15시간 미만으로 일했으면 퇴직금이 없어요. 주 15시간은 월 약 65시간 기준이고, 파트타임이어도 주 15시간 이상이면 퇴직금 대상이에요.",
    tip: "근로계약서에 '소정근로시간'이 적혀 있죠. 이 숫자로 판단해요",
  },
  {
  ...
  {
    title: "IRP 계좌 미리 개설",
    desc: "퇴직금이 300만원을 넘으면 IRP(개인형퇴직연금) 계좌로만 받을 수 있죠. 퇴사 전 은행·증권사 앱에서 10분이면 개설 가능하고, IRP 계좌번호를 인사팀에 전달해두세요.",
    tip: "IRP 개설 후 즉시 인출하면 퇴직소득세가 원천징수돼요. 절세하려면 5년 이상 유지해요",
  },
  {
    title: "퇴사 후 14일 이내 수령",
    desc: "회사는 퇴사일로부터 14일 이내에 퇴직금을 지급해야 해요. 기한을 넘기면 연 20%의 지연이자가 붙어요. 14일이 지나도 입금되지 않으면 고용노동부(1350)에 신고할 수 있죠.",
    tip: "14일 기한은 당사자 합의로 연장 가능해요. 단, 서면 합의여야 해요",
...
const CHECKLIST = [
  ...
  "주 평균 15시간 이상: 근로계약서 소정근로시간으로 체크",
...
const FAQS = [
  {
    ...
    a: "만 12개월(1년)이에요. 11개월 29일은 1년 미만이라 퇴직금이 0원이죠. 근로자퇴직급여보장법 제8조에서 '계속근로 1년 이상'을 명시하고 있죠.",
  },
  {
    q: "주 15시간 미만으로 일하면 1년이 넘어도 퇴직금이 없나요?",
    a: "맞아요. 주 평균 15시간 미만 단시간 근로자는 근속 기간과 관계없이 퇴직금 대상에서 제외돼요. 근로계약서에 적힌 소정근로시간을 기준으로 판단하죠.",
  },
  {
    q: "계약직이 계약을 연장했을 때 근속기간은 어떻게 되나요?",
    a: "계약이 여러 번 갱신됐어도 사실상 같은 사업주 아래서 고용이 이어졌다면 전체 기간이 합산돼요. 1년 계약을 두 번 했다면 2년 근속으로 퇴직금이 계산되죠.",
  },
  {
    q: "1년을 채우지 못하고 그만두면 아무것도 못 받나요?",
    a: "퇴직금은 없죠. 하지만 월 개근하면 매달 연차 1개가 발생해요. 11개월 근무했다면 최대 11일치 연차수당을 청구할 수 있고, 비자발적 퇴사라면 실업급여도 고려해봐요.",
  },
  {
    q: "1년 직전에 해고당하면 퇴직금을 못 받는 건가요?",
    a: "고의로 1년을 못 채우게 해고하면 부당해고에 해당할 수 있죠. 노동위원회에 구제신청을 하면 복직 또는 해고수당을 받을 수 있고, 고용노동부(1350)에 먼저 상담하는 게 좋아요.",
  },
  {
    q: "아르바이트도 퇴직금을 받을 수 있나요?",
    a: "받을 수 있죠. 아르바이트도 1년 이상 근무하고 주 평균 15시간 이상이면 퇴직금 대상이에요. 정규직·계약직·아르바이트 구분 없이 같은 기준이 적용돼요.",
  },
  {
    q: "퇴직금이 얼마나 되는지 대략 어떻게 계산하나요?",
    a: "간단히 계산하면 월 평균임금 × 근속연수예요. 월급 300만원으로 2년 근무했다면 약 600만원이죠. 정확하게는 1일 평균임금 × 30일 × (근속일수 ÷ 365)로 계산해요.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-몇개월부터" />}
    >
      ...
      <p style={body}>
        퇴직금을 받으려면 두 조건을 동시에 충족해야 해요. 하나라도 빠지면 퇴직금이 없죠.
        첫째, 같은 사업주 아래서 계속근로 기간이 만 1년 이상이어야 해요.
        둘째, 주 평균 소정근로시간이 15시간 이상이어야 하고요.
      </p>
      <p style={body}>
        '계속근로'는 고용 관계가 끊기지 않고 이어진 기간을 말해요.
        1년 계약직이 계약 갱신으로 2년을 일했다면 2년 전체가 계속근로 기간이죠.
      ...
      <EligibilityChecker
        ...
        partialMatchText="조건 일부를 충족하지 못할 수 있죠. 고용노동부(1350)에 먼저 상담해봐요."
      />
      ...
      <p style={body}>
        ...
        쉽게 이해하면 1년 근무 시 월급 1개월치, 3년이면 3개월치가 기본이죠.
        슬라이더로 월 평균임금과 근무 기간을 조정하면 예상 금액을 바로 볼 수 있죠.
      </p>
      <p style={body}>
        평균임금은 퇴직 직전 3개월간 받은 임금 총액을 그 기간의 총 달력 일수로 나눈 금액이에요.
        기본급 외에 식대·교통비 같은 고정 수당도 포함되고요.
        상여금은 연간 총액의 12분의 3만 반영돼요.
      </p>
      ...
      <Calculator
        ...
        note="※ 월 평균임금 × 근속연수 기준 간이 계산이에요. 상여금·고정수당을 포함하면 실제 퇴직금은 달라질 수 있죠."
      />
      ...
      <p style={body}>
        ...
        하지만 정확한 금액 계산과 분쟁 예방을 위해 서류를 미리 챙겨두는 게 좋죠.
        특히 근로계약서는 입사일과 근무시간을 증명하는 핵심 서류예요.
      </p>
      <p style={body}>
        퇴직금이 300만원을 넘으면 IRP(개인형퇴직연금) 계좌로만 수령해야 해요.
        퇴사 전 은행이나 증권사 앱에서 미리 개설하고 인사팀에 계좌번호를 알려두면 수령이 빨라지고요.
      </p>
      ...
      <H2>퇴직금 받는 절차 5단계</H2>
      <p style={body}>
        퇴직금은 퇴사 결정 전에 미리 준비해야 할 게 있죠.
        IRP 계좌를 퇴사 후에 개설하면 수령이 늦어지는 경우가 많죠.
        아래 순서대로 따라가면 퇴사 후 14일 안에 받을 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        1년 미만 근무자도 월 개근 시 연차가 매달 발생하고, 사용하지 않은 연차는 수당으로 받을 수 있죠.
        비자발적 퇴사라면 <a href="/w/실업급여-피보험기간" style={{ color: "#1D9E75", textDecoration: "underline" }}>피보험기간 180일</a> 이상이면 실업급여도 받을 수 있고요.
      </p>
