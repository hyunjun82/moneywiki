...
const CHECK_ITEMS = [
  ...
  { id: "c3", label: "정규직·계약직·알바·파트타임 등 고용형태는 무관해요" },
...
const STEPS = [
  {
    title: "조건 확인",
    desc: "1년 이상 계속 근로 + 주 15시간 이상 근무 — 두 조건이 모두 충족되면 퇴직금이 발생해요. 4대보험 가입 여부, 사업장 규모, 고용 형태는 관계없죠.",
    tip: "고용24에서 내 고용보험 이력으로 입사일 확인 가능",
  },
  {
    title: "퇴직금 계산",
    desc: "퇴직 전 3개월 평균임금 × 30일 × (근속일수 ÷ 365). 상여금·연차수당도 포함해야 정확해요. 기본급만으로 계산하면 실제보다 낮게 나와요.",
    tip: "위 계산기로 대략적인 금액 확인 후 급여명세서로 검증",
  },
  {
    title: "퇴직금 지급 요청",
    desc: "퇴직 후 14일 이내에 지급해야 해요. 자동으로 안 들어오면 회사에 지급 요청을 해야 하죠. 문자·메일로 요청하면 증거가 남아 유리해요.",
    tip: "IRP 계좌 미리 개설해두면 이체가 빨라요",
  },
  {
    title: "미지급 시 신고",
    desc: "14일이 지났는데 안 주면 연 20% 지연이자가 붙어요. 내용증명 발송 → 고용노동부 진정 접수 순서로 대응하면 대부분 해결되죠.",
    tip: "소멸시효 3년 — 늦어도 퇴직 후 3년 내에 청구해야 해요",
  },
];
...
const FAQS = [
  ...
  },
  {
    ...
    a: "있어요. 2010년부터 5인 미만 사업장에도 퇴직금 지급 의무가 적용됐어요. 1인 사업장도 예외 없죠.",
  },
  {
...
const REFERENCES = [
  {
    category: "법령",
    items: [
      { label: "근로자퇴직급여 보장법 — 퇴직급여제도 설정 의무", url: "https://www.law.go.kr/법령/근로자퇴직급여보장법" },
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        <strong>1년 이상 계속 근무</strong>와 <strong>주 15시간 이상 근무</strong>.
        <a href="https://www.law.go.kr/법령/근로자퇴직급여보장법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여 보장법</a>은 정규직·계약직·알바 구분 없이 동일하게 적용해요.
      ...
      <p style={body}>
        날짜 하루 차이로 조건이 달라질 수 있어요. 꼼꼼하게 확인해보세요.
      </p>
