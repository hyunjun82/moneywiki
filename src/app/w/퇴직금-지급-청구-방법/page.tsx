...
//     퇴직 후 14일이 지났는데 회사가 퇴직금을 안 주거나 미루고 있어서, 어떻게 청구해야 하는지 막막한 상황이에요.
// Q2. 이 사람이 이 글을 읽고 나서 할 수 있어야 하는 행동은?
//     서면으로 퇴직금 청구서를 작성해서 회사에 보내고, 안 받으면 고용노동부 진정까지 신청하는 것이에요.
// Q3. 이 행동을 하려면 반드시 알아야 하는 정보는?
//     청구 금액 계산법(평균임금×근속연수+지연이자), 서면 청구 방법과 증거 남기는 법, 노동청 진정 절차와 소멸시효 3년이에요.
// Q4. 이 정보를 가장 잘 전달하는 형태는?
//     계산기(금액 직접 산출) + 단계별 절차(Steps) + 체크리스트(누락 방지)이에요.
...
const STEPS = [
  {
    ...
    desc: "퇴직 전 3개월 평균임금 × 근속연수로 퇴직금을 계산해요. 14일이 지났다면 지연이자(연 20%)도 함께 계산해서 청구서에 넣어요. 법적으로 정해진 양식은 없고요.",
    tip: "상여금·고정수당도 평균임금에 포함될 수 있으니 꼼꼼히 계산해봐요",
  },
  {
    title: "회사에 서면으로 청구",
    desc: "구두 요청만으로는 나중에 증거가 없어요. 이메일, 문자, 카카오톡으로 '○○년 ○월 ○일 퇴직, 퇴직금 ○○만원 청구'를 명확히 남겨요. 응답 기한(5영업일)도 함께 적어두는 게 좋고요.",
    tip: "내용증명(우체국 발송)이 가장 강력한 증거이에요",
  },
  {
    title: "고용노동부 진정 신청",
  ...
  },
  {
    ...
    desc: "진정 후에도 지급이 안 되면 법원에 지급명령 신청을 낼 수 있죠. 지급명령은 소송보다 비용이 1/10 수준이고 절차도 간단해요. 법률구조공단(132)에서 무료 법률 지원을 받을 수 있고요.",
...
const FAQS = [
  {
    q: "퇴직금 청구서는 꼭 써야 하나요?",
    a: "법적으로 정해진 양식은 없어요. 구두 요청도 되지만 분쟁이 생길 때 증거가 없어서 불리해요. 이메일·문자로라도 남겨두는 게 훨씬 유리하고, 내용증명이 가장 강력한 증거이에요.",
  },
  {
    q: "고용노동부 진정을 내면 회사에 불이익이 가나요?",
    a: "근로감독관이 사업주에게 연락해 사실관계를 파악해요. 퇴직금을 정당하게 지급하지 않으면 3년 이하 징역 또는 3천만원 이하 벌금이 부과될 수 있죠. 진정 자체가 회사에 압력이 되는 경우가 많아요.",
  },
  {
    q: "지연이자는 언제부터 붙나요?",
  ...
  },
  {
    ...
    a: "체당금 제도를 이용해봐요. 고용노동부에 신청하면 퇴직 전 최종 3개월 임금과 퇴직금(최대 3개월치)을 국가가 먼저 지급해줘요. 법원 도산 판정 또는 고용노동부 행정 도산 확인 후 신청할 수 있죠.",
  },
  {
  ...
  },
  {
    ...
    a: "진정이 먼저이에요. 무료고 빠르게 처리돼요. 진정으로 해결이 안 될 때 지급명령 신청이나 민사소송으로 넘어가는 게 일반적인 순서이고요.",
  },
];
...
const RELATED = [
  ...
  { slug: "퇴직금-지연이자", title: "퇴직금 지연이자 계산", description: "연 20% 지연이자를 직접 계산해볼 수 있죠." },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        14일이 지나도 받지 못했다면 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>지연이자(연 20%)</a>와 함께 청구할 수 있죠.
        서면 청구 → 노동청 진정 순서로 밟아가면 대부분 진정 단계에서 해결돼요.
        소멸시효 3년이 지나기 전에 청구 절차를 빠르게 시작하는 게 유리해요.
      </p>
      ...
      <p style={body}>
        ...
        퇴직금이 300만원을 넘으면 IRP 계좌로만 받을 수 있는데, IRP가 없다면 청구 전에
      ...
      <p style={body}>
        ...
        청구 사실을 서면으로 남겨두지 않으면 나중에 분쟁이 생겼을 때 불리해지는 경우가 많아요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금 청구 및 지연이자 청구가 모두 가능해요. 아래 계산기로 금액을 먼저 살펴봐요."
        partialMatchText="상황에 따라 절차가 다를 수 있죠. 고용노동부(1350) 상담을 권해요."
      />
      ...
      <p style={body}>
        월 평균임금과 근속 기간을 입력하면 퇴직금 예상액과 30일 지연 시 이자를 바로 볼 수 있죠.
        평균임금은 퇴직 전 3개월 임금 총액을 그 기간 총 일수로 나눈 금액이에요.
        상여금이 포함됐다면 실제 금액이 더 높을 수 있고요.
      </p>
      ...
      <Calculator
        ...
        note="※ 법정 최저 기준. 상여금·고정수당 포함 시 퇴직금이 더 높을 수도 있죠. 지연이자는 14일 초과분부터 발생해요."
      />
      ...
      <p style={body}>
        ...
        퇴직 후엔 급여명세서를 달라고 해도 회사가 줄 의무가 없는 경우도 있고요.
      ...
      <p style={body}>
        4대보험 가입확인서는 국민건강보험공단(1577-1000) 또는 내 보험 사이트에서 바로 발급받을 수 있죠.
        재직 기간을 객관적으로 증명할 수 있어서 근로계약서가 없을 때 대체 자료로 쓸 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        각 단계에서 필요한 행동이 명확하니 순서대로 따라가봐요.
      </p>

      <Steps steps={STEPS} />

      <p style={body}>
        진정을 내면 근로감독관이 사업주에게 연락해서 사실관계를 파악해요.
      ...
      <p style={body}>
        ...
        기록을 남기지 않으면 나중에 불리해지는 상황이 생길 수 있죠.
      ...
      <GreenBox>
        고용노동부 민원마당(minwon.moel.go.kr)에서 집에서 바로 신고할 수 있죠.<br />
