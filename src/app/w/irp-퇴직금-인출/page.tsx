"use client";
// Q1. 퇴직 후 IRP에 퇴직금이 들어왔는데 생활비나 목돈이 급히 필요해 꺼내야 할지 고민 중인 상황
// Q2. 세금 얼마 내는지 계산하고, 인출 조건 해당 여부 판단한 뒤 금융기관 앱에서 신청
// Q3. 55세 기준 세율 차이(16.5% vs 3.3~5.5%), 법정 사유 4가지, 인출 절차 4단계, 서류 목록
// Q4. GreenBox(세율 요약), Calculator(세금 추정), EligibilityChecker(조건 체크), Steps(절차), DocTable(서류)
// MAP: Q1→서론(상황 공감) Q2→H2순서(조건→세금→서류→절차→체크) Q3→H2깊이(각 섹션 2문단) Q4→컴포넌트

import { H2, SectionBadge, GreenBox, BorderBox, Divider, body, Calculator, EligibilityChecker, Steps, DocTable, Checklist, ArticleLayout, Sidebar, CategoryButton, RelatedArticles, ArticleAd } from "@/components/article-ui";
...
const CHECK_ITEMS = [
  { id: "c1", label: "IRP 계좌에 퇴직금이 들어와 있죠" },
  { id: "c2", label: "만 55세 이상이에요" },
  { id: "c3", label: "IRP 가입 후 5년 이상 지났어요" },
  { id: "c4", label: "무주택자로 주택을 구입하거나 6개월 이상 요양 중이죠 (중도 인출 조건)" },
];
...
const STEPS = [
  {
    title: "인출 사유 파악",
    desc: "법정 사유(무주택자 주택 구입, 6개월 이상 요양, 파산·개인회생, 천재지변)에 해당하면 퇴직소득세만 내고 일부 인출이 가능해요. 그 외 사유는 전액 해지만 가능하고 기타소득세 16.5%가 붙어요. 인출 전에 내 사유가 어디에 해당하는지 먼저 따져봐야 해요.",
    tip: "법정 사유 해당 시 기타소득세 없이 퇴직소득세만 납부",
  },
  {
    title: "세금 추정",
    desc: "55세 이전 일반 해지라면 기타소득세 16.5%가 원금 전체에 붙어요. 55세 이후 연금 수령이라면 연금소득세 3.3~5.5%로 크게 줄죠. 인출 금액과 나이를 위 계산기에 입력해서 실수령액을 먼저 따져봐야 해요.",
    tip: "55세 기준으로 세금이 약 5배 차이 날 수 있죠",
  },
  {
    title: "금융기관 앱 또는 창구에서 신청",
    desc: "IRP를 개설한 금융기관 앱에서 '중도인출' 또는 '해지' 메뉴로 접근해요. 법정 사유가 있으면 증빙 서류를 미리 스캔해서 첨부해야 해요. 창구 방문 시 신분증만 있어도 신청서 작성이 가능해요.",
    tip: "앱 신청이 빠르지만 서류 첨부 기능 먼저 확인",
...
const CHECKLIST = [
  ...
  "인출 사유: 법정 사유 해당 여부 먼저 따지기",
  "세금 계산: 기타소득세 16.5% vs 연금소득세 3.3~5.5%",
  "증빙 서류: 법정 사유라면 서류 미리 스캔해두기",
  "연금 전환 검토: 55세 이후 수령 가능하다면 기다리는 게 유리해요",
...
const FAQS = [
  {
    ...
    a: "기타소득세 16.5%가 IRP 원금 전체에 붙어요. 퇴직소득세와는 별도로 추가 부과되는 세금이에요. 퇴직금 5,000만원을 해지하면 기타소득세만 825만원이죠. 급하게 필요한 돈이 아니라면 가급적 피하는 게 좋아요.",
  },
  {
  ...
  },
  {
    ...
    a: "법정 사유에 해당하면 필요한 금액만 일부 인출이 가능해요. 그 외 일반 해지는 전액 해지만 가능하고요. 부분 인출 후 남은 금액은 계속 IRP에 남아 있죠.",
  },
  {
    q: "IRP를 해지했다가 나중에 다시 가입할 수 있나요?",
    a: "재가입은 가능해요. 하지만 중도 해지로 낸 기타소득세 16.5%는 돌려받지 못하죠. 재가입 후 연금 수령 조건(55세 이상, 5년 이상)을 다시 채워야 해서 실질적으로 손해가 커요.",
  },
  {
    q: "연금 수령 시 세금이 정말 3.3%밖에 안 되나요?",
    a: "55세 이후 연금으로 받으면 퇴직소득세가 30~40% 줄어드는 것에 더해, 매년 받는 연금 금액에는 연금소득세 3.3~5.5%만 붙어요. 55세 이전 해지(16.5%)와 비교하면 세금이 최대 5배 차이 날 수 있죠.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="irp-퇴직금-인출" />}
    >
      ...
      <H2>내 상황에서 인출 가능한지 짚어볼게요</H2>
      <p style={body}>
        IRP는 원칙적으로 55세 이후 노후 자금을 위한 계좌예요.
        55세 이전에 꺼내려면 법정 사유가 있어야 세금 부담을 줄일 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="연금 수령 또는 법정 사유 인출 조건을 갖추고 있죠. 세금이 대폭 줄어드는 방법으로 인출이 가능해요."
      ...
      <p style={body}>
        ...
        같은 5,000만원에서 세금 차이가 650만원 넘게 나죠.
      </p>
      <p style={body}>
        연금소득세는 매년 받는 금액 기준으로 부과되기 때문에 아래 계산기는 단순 추정값이에요.
        정확한 세액은 <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직금 세금 계산</a> 글에서 살펴볼 수 있죠.
      </p>
      ...
      <Calculator
        ...
        note="※ 55세 이전: 기타소득세 16.5% (원금 전체). 55세 이후 연금: 연금소득세 3.3% 기준. 실제 세액은 다를 수 있죠."
      />
      ...
      <p style={body}>
        ...
        법정 사유에 해당한다면 사유별 증빙 서류를 추가로 첨부해야 하죠.
      ...
      <p style={body}>
        사유 파악부터 실제 입금까지 빠르면 2~3 영업일이면 돼요.
        법정 사유가 있는 경우라면 증빙 서류 준비에 시간이 더 걸릴 수 있죠.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        ...
        특히 55세가 가까운 경우라면 조금 더 기다리는 게 수백만 원 절세로 이어질 수 있죠.
      </p>
      ...
      <GreenBox>
        법정 사유(주택 구입, 요양 등) 해당 여부 먼저 따지기<br />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 소득세법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 금융감독원(☎ 1332)에서 직접 문의해봐요." />
    </ArticleLayout>
  );
}
