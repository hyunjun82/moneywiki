...
// Q1. 퇴직 후 2주가 지났는데 퇴직금이 아직 안 들어온 상황 — "14일 지났는데 왜 안 오지? 이자는 얼마지?"
// Q2. 지연이자 금액을 계산하고 인사팀 요청 또는 노동청 신고까지 직접 실행
// Q3. 14일 기산일 계산법, IRP 계좌 조건, 지연이자 20% 산식, 내용증명 발송 방법, 소멸시효 3년
// Q4. 계산기(지연이자 바로 계산) + Steps(신고 절차) + EligibilityChecker + FAQ 6개
...
const STEPS = [
  {
    title: "지급 기한 날짜 계산",
    desc: "퇴직일 다음 날부터 14일을 세요. 14일째 날이 토요일·일요일·공휴일이면 다음 첫 영업일이 기한이에요. 14일은 근무일이 아닌 달력일(캘린더 기준)이고, 퇴직일 자체는 포함하지 않아요.",
    tip: "퇴직일 포함 안 해요. 퇴직 다음 날부터 1일로 세기 시작해요",
  },
  {
    title: "IRP 계좌번호 인사팀 통보",
    desc: "퇴직금 300만원 초과 시 반드시 IRP 계좌로만 받을 수 있죠. IRP 계좌번호를 미리 인사팀에 문자·메일로 알려주세요. 계좌를 안 알려주면 회사가 이체 자체를 할 수 없어서 지연 귀책이 근로자에게 돌아올 수 있죠.",
...
const FAQS = [
  {
    ...
    a: "토요일·일요일·공휴일이 기한이면 다음 첫 영업일이 기한이에요. 14일째 날이 토요일이면 그 다음 월요일까지 입금하면 기한 내 지급으로 처리돼요.",
  },
  {
    q: "지급 기한을 연장할 수 있나요?",
    a: "가능해요. 회사와 근로자가 서면으로 합의하면 기한을 늘릴 수 있죠. 단, 구두 합의는 인정되지 않아요. 연장 합의서를 꼭 서면으로 작성해 두세요.",
  },
  {
    q: "IRP 계좌번호를 아직 안 알려줬는데 14일이 지났어요",
    a: "IRP 계좌를 알려주지 않으면 회사가 이체 자체를 할 수 없죠. 이 경우 지연에 대한 귀책이 근로자에게 있을 수 있고요. 빨리 IRP 계좌번호를 문자·메일로 인사팀에 통보하세요.",
  },
  {
    q: "퇴직금 300만원 이하면 IRP 계좌가 없어도 되나요?",
    a: "맞아요. 퇴직금이 300만원 이하면 IRP 의무 이전 대상이 아니에요. 일반 계좌로 바로 받을 수 있죠. 300만원 초과분부터 IRP 계좌로만 받을 수 있고요.",
  },
  {
  ...
  },
  {
    ...
    a: "퇴직일로부터 3년이에요. 3년이 지나면 퇴직금과 지연이자 청구권이 모두 소멸해요. 내용증명 발송만으로도 소멸시효를 중단시킬 수 있죠.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-지급기한" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        <a href="https://www.law.go.kr/법령/근로자퇴직급여보장법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법 제9조</a>는 퇴직 후 14일 이내 지급 의무를 규정하고 있죠.
        14일은 달력일 기준이고, 이 기한을 넘기면 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>연 20% 지연이자</a>가 자동으로 붙어요.
        지금 내 기한이 언제인지, 이미 초과했다면 이자가 얼마인지 이 글에서 바로 계산할 수 있죠.
      </p>

      <Divider />
      <ArticleAd position="intro" />

      <H2>퇴직금 지급 기한, 내가 해당되나요?</H2>
      <p style={body}>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="지연이자 청구 및 노동청 신고가 가능한 상황이에요. 아래 계산기로 지연이자를 먼저 계산해 보세요."
        partialMatchText="상황에 따라 다를 수 있죠. 고용노동부(1350) 상담을 먼저 받아보세요."
      />

      <Divider />

      <H2>지급 기한 초과, 지연이자는 얼마일까요?</H2>
      <p style={body}>
      ...
      <p style={body}>
        슬라이더로 내 퇴직금 총액과 지연 일수를 조정하면 청구 가능한 금액을 바로 볼 수 있죠.
      ...
      <GreenBox>
        퇴직금 300만원 초과 시 IRP 계좌로만 받을 수 있죠.<br />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 근로기준법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 확인해 주세요." />
    </ArticleLayout>
  );
}
