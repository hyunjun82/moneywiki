...
// Q1. 1년도 안 됐는데 퇴사한 상황 퇴직금을 받을 수 있는지 모르는 상태
// Q2. 취업규칙·DC형 연금 납입금·실업급여 중 내가 받을 수 있는 게 무엇인지 판단해서 청구한다
// Q3. 법 조건(1년+주15시간), 취업규칙 예외, DC형 연금 수령 방법, 실업급여 대안 이 4가지를 알아야 행동할 수 있다
// Q4. GreenBox 핵심 요약 → Steps 대응 절차 → DocTable 서류 목록 → FAQ 오해 해소 순서로 전달
...
const STEPS = [
  {
    ...
    tip: "취업규칙이나 근로계약서에 '1년 미만도 지급' 조항이 있으면 받을 수 있죠.",
  },
  {
    title: "회사에 자발적 지급 요청",
    desc: "법적 의무가 없어도 회사가 자발적으로 줄 수 있죠. 인사팀에 퇴직금 지급 가능 여부를 문의해봐요. 이 경우에도 퇴직소득으로 처리돼서 퇴직소득세가 부과돼요. 금액이 300만원 초과면 IRP로만 수령해야 해요.",
    tip: "내규에 근거 없이 지급하더라도 법적으로 문제없어요.",
  },
  {
    title: "DC형 퇴직연금 납입금 수령",
    desc: "회사에 DC형(확정기여형) 퇴직연금이 있다면 1년 미만이어도 납입된 금액을 받을 수 있죠. 금융감독원 통합연금포털(100lifeplan.fss.or.kr)에서 내 퇴직연금 유형과 적립금을 조회할 수 있고요.",
    tip: "DC형은 사용자가 매년 연봉의 1/12 이상을 근로자 계좌에 납입하는 방식이에요.",
    link: { label: "퇴직연금 유형 조회하기", href: "https://100lifeplan.fss.or.kr" },
  },
  {
    title: "실업급여 신청 (비자발적 퇴직일 때)",
    desc: "퇴직금을 못 받더라도 고용보험 피보험기간 180일 이상 + 비자발적 퇴직이면 실업급여를 신청할 수 있죠. 퇴직 후 12개월 내에 고용센터에 방문해서 수급 자격 인정 신청을 해야 해요.",
    tip: "계약 만료, 권고사직, 회사 귀책 사유 퇴직은 비자발적으로 인정돼요.",
    link: { label: "실업급여 조건 보기", href: "/w/실업급여-피보험기간" },
  },
];

const CHECKLIST = [
  "근로자퇴직급여보장법 제4조: 1년 이상이 법적 지급 요건",
  "취업규칙·근로계약서: 1년 미만 지급 규정이 있는지 살펴봐요",
...
const FAQS = [
  {
    ...
    a: "근로자퇴직급여보장법 제4조는 계속 근로 1년 이상을 지급 요건으로 규정해요. 1년 미만이면 사업주에게 법적 지급 의무가 없죠. 단, 취업규칙이나 단체협약에 별도 규정이 있으면 그 규정에 따라 받을 수 있고요.",
  },
  {
    q: "회사가 1년 미만인데도 퇴직금을 주겠다고 하면?",
    a: "받을 수 있죠. 법적 의무는 없지만 회사가 자발적으로 주는 건 아무 문제 없어요. 이 경우 퇴직소득으로 처리돼서 퇴직소득세가 부과돼요. 금액이 300만원을 넘으면 IRP로만 수령해야 해요.",
  },
  {
  ...
  },
  {
    ...
    a: "취업규칙에 별도 규정을 두는 회사는 많지 않아요. 입사 전 근로계약서와 취업규칙에 이런 조항이 있는지 살펴보는 게 가장 정확해요. 고용노동부 공식 자료에 따르면 법적 의무 외 자발적 지급은 회사 재량이에요.",
  },
  {
    q: "5인 미만 사업장도 1년 미만이면 퇴직금 없나요?",
    a: "5인 미만이어도 동일해요. 근로자퇴직급여보장법은 사업장 규모와 무관하게 적용돼요. 1년 이상 근무해야 퇴직금 지급 의무가 생기고요. 1년 미만이면 5인 미만 사업장도 법적 지급 의무가 없어요.",
  },
  {
...
const RELATED = [
  ...
  { slug: "1년-미만-퇴직금-지급-규정-조건-기준", title: "1년 미만 퇴직금 조건과 DC형 수령", description: "DC형 퇴직연금이면 1년 미만도 받을 수 있죠." },
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        <a href="/w/퇴직금-기준" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법 제4조</a>는 퇴직금 지급 요건으로 '계속 근로 1년 이상'을 명시하고 있죠.
        1년 미만이면 법적으로 사업주에게 지급 의무가 없죠.
        하지만 취업규칙에 별도 규정이 있거나, 회사에 <a href="/w/퇴직금-DC형-계산법" style={{ color: "#1D9E75", textDecoration: "underline" }}>DC형 퇴직연금</a>이 있다면 예외가 생겨요.
        정확한 지급 규정과 내가 할 수 있는 조치를 아래에서 살펴봐요.
      </p>
      ...
      <p style={body}>
        ...
        두 조건을 모두 충족해야 사업주에게 퇴직금을 지급할 의무가 생기죠.
        근속 기간이 1년 미만이면 아무리 열심히 일했어도 이 조건을 충족하지 못해요.
      </p>
      <p style={body}>
        법이 이렇게 정한 건 퇴직금 제도의 취지가 장기 근속에 대한 보상이기 때문이에요.
        그래서 1년을 기준선으로 뒀죠.
        다만 법적 의무가 없다는 것과 회사가 줄 수 없다는 건 다른 얘기예요. 회사가 자발적으로 줄 수는 있고요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="1년 미만이라서 법정 퇴직금 지급 의무는 없어요. 단, 취업규칙 및 DC형 연금 납입금 수령 가능 여부를 살펴봐요."
        partialMatchText="상황에 따라 다를 수 있죠. 아래 내용을 읽어보거나 고용노동부(1350)에 문의해봐요."
      />
      ...
      <p style={body}>
        1년 미만이라 못 받는 상황이라면 '1년을 채웠을 때 얼마를 받았을까'를 계산해볼 수 있죠.
        퇴직금은 평균임금 × 30일 × 근속연수로 계산하죠.
      ...
      <Calculator
        ...
        note="※ 퇴직 전 3개월 평균임금 × 30일 × (근속일수 ÷ 365) 기준. 실제 금액은 평균임금 산정 방식에 따라 달라질 수 있죠."
      />
      ...
      <p style={body}>
        취업규칙이나 근로계약서에 1년 미만 지급 규정이 있는지 살펴보려면 관련 서류가 필요해요.
      ...
      <p style={body}>
        법정 퇴직금을 받기 어렵더라도 할 수 있는 조치가 있죠.
        취업규칙 확인부터 DC형 수령, 실업급여 신청까지 단계적으로 챙겨봐요.
      </p>
      ...
      <GreenBox>
        ...
        금융감독원 통합연금포털(100lifeplan.fss.or.kr)에서 내 연금 적립금을 바로 조회할 수 있고요.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법을 바탕으로 작성됐어요. 법 개정이 있을 수 있으니 최신 기준은 고용노동부(1350) 또는 법제처(www.law.go.kr)에서 살펴봐요." />
    </ArticleLayout>
  );
}
