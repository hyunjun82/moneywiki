...
// Q1. 퇴직 예정인데 IRP 계좌가 없거나 만들기 싫어서, 그냥 일반 통장으로 받을 수 있는지 검색 중
// Q2. 내가 예외 조건에 해당하는지 판단하고, 해당하면 인사팀에 일반 계좌 번호 전달
// Q3. 예외 5가지 조건 + 55세 생일 당일 판정 기준 + 300만원 세전·세후 구분 + 절차 4단계
// Q4. EligibilityChecker(체크 → 즉시 판단) + Calculator(금액 확인) + Steps(절차) + Checklist(요약)
//
// MAP:
// - Q1 → 서론 톤: "IRP 없이도 받을 수 있는 경우가 있죠"
// - Q2 → H2 순서: 예외 조건 확인 → 금액 계산 → 서류 → 절차 → 체크리스트
// - Q3 → H2 개수/깊이: 5개 섹션, 55세 생일·세전 기준 명시
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직일 기준으로 만 55세 생일이 이미 지났죠" },
  { id: "c2", label: "퇴직금(세전 총액)이 300만원 이하고요" },
  { id: "c3", label: "외국인 근로자로 퇴직 후 출국 예정이에요" },
  { id: "c4", label: "사망 또는 장해로 본인이 IRP 개설을 할 수 없는 상태고요" },
];
...
const STEPS = [
  {
    ...
    desc: "퇴직일 기준 만 55세 이상인지, 퇴직금 세전 총액이 300만원 이하인지 먼저 따져봐요. 55세는 생일이 퇴직일 이전이어야 해요. 생일 당일 퇴직은 예외에 해당하지 않고요.",
    tip: "하루 차이로 의무 여부가 바뀌니 퇴직일 전에 꼭 따져보세요",
  },
  {
    title: "회사 인사팀에 예외 사유 전달",
    desc: "예외에 해당하면 인사팀에 나이·금액·출국 등 사유를 알리고 본인 명의 일반 계좌 번호를 전달해요. 문자나 이메일로 남겨두면 나중에 증거로 쓸 수 있죠.",
    tip: "이메일이나 문자로 전달 기록을 꼭 남겨두세요",
  },
  {
    title: "퇴직소득세 원천징수 후 입금 확인",
    desc: "회사가 퇴직소득세를 원천징수한 뒤 나머지를 일반 계좌로 송금해요. 퇴직일로부터 14일 이내에 입금되지 않으면 연 20% 지연이자를 청구할 수 있죠.",
    tip: "원천징수영수증 발급 요청도 함께 해두세요",
    link: { label: "퇴직금 지급기한 14일 기준 보기", href: "/w/퇴직금-지급-기한" },
  },
  {
    title: "원천징수영수증 수령 및 보관",
    desc: "퇴직소득 원천징수영수증은 종합소득세 신고나 퇴직금 분쟁 시 꼭 필요한 서류예요. 인사팀에서 발급받아 보관해두고, 홈택스에서도 사후 조회가 되죠.",
    tip: "최소 5년 이상 보관하는 걸 권해요",
...
const FAQS = [
  {
    ...
    a: "근로자퇴직급여보장법 제9조 기준으로 5가지예요. 퇴직급여 300만원 이하, 만 55세 이상 퇴직, 외국인 근로자 출국, 근로자 사망, 장해로 IRP 개설이 불가한 경우고요. 이 중 하나라도 해당하면 일반 계좌로 수령할 수 있죠.",
  },
  {
    q: "55세 생일 당일에 퇴직하면 예외가 되나요?",
    a: "안 돼요. 퇴직일 기준으로 생일이 이미 지난 상태여야 해요. 생일 당일 퇴직은 '만 55세 미만'으로 처리돼서 IRP 의무 대상이에요. 하루 이후에 퇴직해야 예외에 해당하죠.",
  },
  {
    q: "300만원 기준이 세전인가요, 세후인가요?",
    a: "세전 퇴직금 총액 기준이에요. 퇴직소득세를 공제하기 전 금액이 300만원 이하여야 하죠. 세후로 따지지 않으니 세전 총액을 먼저 계산해보는 게 중요해요.",
  },
  {
    q: "예외인데도 IRP로 받는 게 더 유리한 경우가 있나요?",
    a: "그렇죠. 만 55세 이상이라도 IRP를 선택해서 연금으로 수령하면 퇴직소득세를 30% 줄일 수 있어요. 퇴직금을 노후 자금으로 쓸 계획이라면 IRP가 훨씬 유리하죠.",
  },
  {
    q: "회사가 예외 적용을 거부하면 어떻게 해야 하나요?",
    a: "예외 조건이 명확히 해당됨에도 거부하면 고용노동부(1350)에 신고할 수 있죠. 이메일이나 문자로 예외 사유를 전달한 기록이 분쟁에서 유리하게 쓰이고요.",
  },
  {
    q: "외국인 근로자는 어떤 서류가 필요한가요?",
    a: "외국인등록증이나 여권으로 신원 확인이 돼요. 출국 예정임을 회사에 알리면 IRP 없이 일반 계좌로 퇴직금을 받을 수 있죠. 출국 후에는 수령이 어려우니 퇴직 전에 정산하는 게 좋아요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금을 받을 때 IRP 계좌가 없으면 못 받는다고 알고 있는 분들이 많은데, 예외가 있죠.
        <a href="/w/퇴직금-irp-의무" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법 제9조</a>에서 정한 5가지 조건 중 하나라도 해당하면 일반 은행 계좌로 바로 받을 수 있죠.
      ...
      <H2>IRP 의무 예외, 내가 해당되는지 따져보는 방법</H2>
      <p style={body}>
        IRP 의무 예외 조건은 총 5가지예요. 퇴직급여 300만원 이하, 만 55세 이상 퇴직, 외국인 근로자 출국, 근로자 사망, 장해로 IRP 개설이 불가한 경우죠.
        이 중 하나라도 해당하면 회사 인사팀에 일반 계좌 번호를 전달하면 돼요.
      </p>
      <p style={body}>
        55세 기준은 퇴직일 기준이에요. 생일이 퇴직일보다 하루라도 앞서야 예외에 해당하죠.
      ...
      <EligibilityChecker
        ...
        partialMatchText="일부 조건만 체크됐어요. 해당하는 항목이 하나라도 있으면 예외 적용이 가능하니 인사팀과 얘기해봐요."
      />
      ...
      <p style={body}>
        ...
        1년 미만 근속이나 단시간 근로자라면 퇴직금 자체가 낮을 수 있으니 먼저 계산해보는 게 좋죠.
      </p>
      ...
      <p style={body}>
        ...
        외국인 출국 예외라면 외국인등록증이나 여권이 필요하고, 장해·사망 예외라면 관련 증빙 서류가 추가되죠.
      </p>
      <p style={body}>
        별도 신청서 없이 문자나 이메일로 처리되는 경우가 많아요.
        전달 기록을 남겨두면 나중에 지급 지연 분쟁 시 증거로 쓸 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        퇴직일로부터 14일 이내에 입금이 이뤄져야 하고, 지연되면 연 20% 이자를 청구할 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        수령 전에 아래 항목을 하나씩 짚어보면 실수 없이 받을 수 있죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법 제9조를 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 안내를 받으세요." />
    </ArticleLayout>
  );
}
