"use client";
// Q1. 건설현장 일용직으로 여러 현장을 옮겨다니며 일했는데, 퇴직금이 쌓이는지도 모르고 그냥 지나친 상황
// Q2. 건설근로자공제회 퇴직공제금을 직접 조회하고 신청까지 완료한다
// Q3. 퇴직공제금은 일반 퇴직금과 별개 제도(건설근로자법 적용), 적립 주체는 사업주, 수령 조건 252일, 신청처 공제회 앱/홈페이지
// Q4. EligibilityChecker(조건확인) → Calculator(예상금액) → DocTable(서류) → Steps(신청절차) → Checklist(사전점검)
// MAP: Q1→서론(여러현장 옮겨도 합산) Q2→H2순서(제도이해→금액확인→서류→절차→체크) Q3→H2깊이(적립구조/252일조건/미적립대처) Q4→컴포넌트
...
const CHECK_ITEMS = [
  { id: "c1", label: "건설현장에서 일용직·일당직으로 근무 중이죠" },
  { id: "c2", label: "건설근로자공제회에 공제부금이 적립 중이죠" },
  { id: "c3", label: "건설업 종사 기간이 252일 이상이에요" },
  { id: "c4", label: "퇴직공제금 수령 방법을 모르겠죠" },
];
...
const STEPS = [
  {
    title: "공제부금 적립 내역 조회",
    desc: "원청 또는 하청 사업주가 근무일당 9,600원씩 적립해줘요. 미적립 기간이 있으면 사업주에게 소급 적립을 요청해야 해요.",
    tip: "건설근로자공제회 앱에서 적립 내역 실시간 조회 가능",
    link: { label: "적립 내역 조회하기", href: "https://eum.cw.or.kr/web/fir/WEBFIR090M00" },
  },
  {
    title: "수령 자격 252일 충족 여부 점검",
    desc: "퇴직공제금은 건설업 종사 기간이 252일 이상이어야 신청 가능해요. 여러 현장에서 근무한 일수를 모두 합산해서 계산하죠. 현장이 달라도 건설업이면 모두 포함되고요.",
    tip: "252일은 약 1년치 건설업 근무 일수예요",
  },
  {
    title: "온라인 또는 방문 신청",
    desc: "방문 신청은 전국 지부에서 진행 가능해요. 신분증과 통장 사본만 갖추면 되고, 공인인증서로 본인 확인이 돼요.",
    tip: "온라인 신청이 방문보다 빠르게 처리돼요",
    link: { label: "온라인 신청하기", href: "https://eum.cw.or.kr/web/fir/WEBFIR130M00" },
  },
  {
    title: "퇴직공제금 수령",
    desc: "신청 후 약 7~14일 이내에 통장으로 입금돼요. 퇴직공제금은 퇴직소득세 대상이라 근속일수에 따라 세금이 원천징수된 뒤 지급되죠. 세후 금액이 실제 입금액이고요.",
...
const CHECKLIST = [
  ...
  "퇴직소득세: 공제금 수령 시 원천징수 금액 확인",
];

const FAQS = [
  {
    q: "건설근로자공제회 퇴직공제금이 뭔가요?",
    a: "건설현장 일용근로자를 위한 퇴직금 제도예요. 공사 발주자·원청·하청이 근무일당 9,600원씩 적립하죠. 여러 현장을 옮겨도 합산해서 받을 수 있고요.",
  },
  {
    q: "건설현장 일용직도 일반 퇴직금을 받을 수 있나요?",
    a: "같은 사업주 밑에서 1년 이상 계속 근무하면 일반 퇴직금도 받을 수 있죠. 건설공제회 퇴직공제금과는 별개예요. 단, 일용직은 계속 근무 인정이 어려운 경우가 많고요.",
  },
  {
    q: "공제부금이 미적립된 기간이 있으면 어떻게 하나요?",
    a: "사업주가 적립 의무를 이행하지 않은 거예요. 건설근로자공제회에 신고하면 사업주에게 소급 납부 명령이 내려지죠. 미적립 기간 공제금도 돌려받을 수 있고요.",
  },
  {
    q: "퇴직공제금과 실업급여를 동시에 받을 수 있나요?",
    a: "퇴직공제금은 퇴직급여고, 실업급여는 고용보험에서 나오는 거예요. 중복 수급 제한은 없으니 퇴직공제금을 받아도 실업급여 신청이 가능해요.",
  },
  {
    q: "252일이 안 됐는데 적립된 공제부금은 어떻게 되나요?",
    a: "252일 미만이면 지금 당장 퇴직공제금으로 받을 수는 없어요. 다시 건설업에 취업하면 이전 적립 일수에 합산되죠. 건설업을 완전히 그만둘 경우에는 일정 요건에서 반환 신청도 가능하고요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        건설현장을 여러 곳 옮겨 다니면 퇴직금을 받기 어렵다고 생각하는 분들이 많죠.
        건설근로자는 일반 근로자와 다른 퇴직금 제도가 적용돼요.
        바로 <a href="https://cw.or.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>건설근로자공제회</a> 퇴직공제금이에요.
        현장이 바뀌어도 근무일수가 합산되는 구조라서, 여러 곳에서 일했더라도 252일이 채워지면 받을 수 있죠.
      ...
      <p style={body}>
        ...
        현장을 옮길 때마다 퇴직금이 끊기는 문제를 해결하기 위해 만들어졌고요.
      </p>
      <p style={body}>
        공제부금은 사업주(원청·하청)가 근무일마다 적립해요. 근로자가 따로 신청하거나 납부할 필요가 없죠.
        여러 현장에서 일한 일수를 모두 합산하기 때문에, 오늘은 A 현장, 내일은 B 현장에서 일해도 두 곳의 근무일수가 모두 내 계좌에 쌓이고요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직공제금 신청 조건을 갖췄어요. 아래 계산기로 예상 금액을 먼저 따져보세요."
      ...
      <p style={body}>
        연간 근무일수와 종사 기간을 입력하면 예상 퇴직공제금을 바로 계산해볼 수 있죠.
        적립 단가는 2024년 기준 일당 9,600원이에요.
        실제 금액은 건설근로자공제회 홈페이지나 앱에서 내 계좌를 직접 조회하면 정확하게 볼 수 있죠.
      </p>
      ...
      <Calculator
        ...
        note="※ 실제 적립금은 연도별 적립 단가 변동, 미적립 기간에 따라 달라져요. 공제회 앱에서 정확한 잔액을 조회하세요."
      />
      ...
      <p style={body}>
        ...
        신분증과 통장 사본만 갖추면 기본적으로 신청이 가능하죠.
        온라인 신청이라면 공제회 앱에 로그인해서 공인인증서로 본인 확인만 하면 되고요.
      </p>
      ...
      <BorderBox>
        ...
        사업주에게 소급 납부 명령이 내려지고, 미적립 기간 공제금도 돌려받을 수 있죠.
      </BorderBox>
      ...
      <p style={body}>
        신청 전에 내 공제부금이 제대로 적립됐는지 먼저 봐야 해요.
        사업주가 적립을 빠뜨린 기간이 있으면 받을 수 있는 금액이 줄어들거든요.
        4단계를 순서대로 따라가면 수령까지 무리 없이 진행되죠.
      </p>
      ...
      <H2>신청 전 체크리스트</H2>
      <p style={body}>
        신청 전에 아래 항목들을 하나씩 점검해봐야 해요.
        특히 미적립 기간 확인은 놓치기 쉽지만 나중에 추가 청구가 어려워지거든요.
        한 번에 챙겨서 신청하는 게 훨씬 편하죠.
      </p>
      ...
      <GreenBox>
        지금 당장 못 받더라도 나중에 다시 건설업에 취업하면 이전 일수에 합산되죠.<br />
        건설업을 완전히 그만둘 경우에는 일정 요건에서 반환 신청도 가능하고요.<br />
      ...
      <p style={{ ...body, marginBottom: 14 }}>
        건설근로자 퇴직공제금에 대해 많이 물어보는 질문들을 모았어요.
      </p>
      <FAQ items={FAQS} />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 건설근로자의 고용개선 등에 관한 법률을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니, 최신 기준은 건설근로자공제회(1588-0075)에서 알아보세요." />
    </ArticleLayout>
  );
}
