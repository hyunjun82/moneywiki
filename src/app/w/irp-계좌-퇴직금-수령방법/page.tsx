...
// Q1. IRP 계좌에 퇴직금이 들어왔는데, 어떻게 꺼내야 할지 막막한 상황. 그냥 꺼내면 세금을 다 내야 한다는 건 아는데 연금 조건이 뭔지, 신청은 어디서 하는지 모름.
// Q2. 일시금 vs 연금 선택 결정 후 금융사 앱에서 수령 신청을 완료한다.
// Q3. ① 연금 수령 조건(55세+5년), ② 세금 차이(퇴직소득세 전액 vs 연금소득세 3.3~5.5%), ③ 수령 기간별 감면율(5년 30% / 10년 40%), ④ 55세 이전 꺼내면 기타소득세 16.5%, ⑤ 4단계 신청 절차.
// Q4. 조건 요약(GreenBox) → 체크(EligibilityChecker) → 계산기 → 4단계 Steps → 서류표 → 체크리스트 → FAQ.
// MAP: Q1→서론, Q2→H2 순서(조건→세금→절차→체크리스트), Q3→H2 깊이, Q4→컴포넌트 선택.
...
const CHECK_ITEMS = [
  { id: "c1", label: "IRP 계좌에 퇴직금이 입금됐고요" },
  { id: "c2", label: "만 55세 이상이에요" },
  { id: "c3", label: "IRP 가입 후 5년 이상 지났고요" },
...
const STEPS = [
  {
    title: "IRP 계좌 입금 여부 체크",
    desc: "퇴직 후 14일 이내에 IRP 계좌로 퇴직금이 들어와야 해요. 금융기관 앱에서 입금 내역을 먼저 봐주세요. 14일이 지나도 입금이 없으면 인사팀에 요청하고, 그래도 안 되면 고용노동부(☎ 1350)에 신고할 수 있죠.",
    tip: "퇴직 후 14일 초과 시 연 20% 지연이자 발생",
  },
  {
    title: "수령 방법 결정 (일시금 vs 연금)",
    desc: "일시금은 즉시 꺼낼 수 있지만 퇴직소득세를 전액 내야 해요. 연금은 55세 이후 5년 이상 나눠 받으면 퇴직소득세가 30% 줄고, 10년 이상 받으면 40% 줄어요. 목돈이 급하게 필요한 상황이 아니라면 연금이 유리해요.",
  ...
  {
    title: "세금 원천징수 후 수령",
    desc: "일시금은 신청 즉시 퇴직소득세가 빠진 금액이 들어와요. 연금은 매월(또는 분기별) 연금소득세 3.3~5.5%를 뺀 금액이 들어와요. 세후 실수령액이 얼마인지 미리 계산해두면 재정 계획 세우기가 편해요.",
    tip: "연금소득세 3.3~5.5%는 퇴직소득세보다 훨씬 낮아요",
  },
];

const CHECKLIST = [
  "입금 체크: 퇴직 후 14일 이내 IRP 계좌 입금 여부 확인",
  "수령 방법 결정: 목돈 필요 여부 따라 일시금/연금 선택",
  "연금 조건: 만 55세 이상 + IRP 가입 5년 이상",
  "세금 차이: 일시금(퇴직소득세 전액) vs 연금(30~40% 감면)",
  "금융기관 앱 접속: 신청서 작성 전 공동인증서 준비",
  "수수료 체크: 증권사 IRP는 운용 수수료 연 0.2% 수준",
];

const FAQS = [
  {
    q: "IRP 계좌로 퇴직금이 들어오면 바로 꺼낼 수 있나요?",
    a: "꺼낼 수는 있죠. 다만 55세 이전에 일시금으로 꺼내면 기타소득세 16.5%가 원금 전체에 붙어요. 55세 이후에 연금으로 수령하면 퇴직소득세가 30~40% 줄어들기 때문에, 급하게 필요한 돈이 아니라면 55세까지 기다리는 게 훨씬 유리해요.",
  },
  {
  ...
  },
  {
    ...
    a: "가능해요. 퇴직급여가 300만원 미만이거나 법정 퇴직금(퇴직연금 미가입 사업장)이라면 일반 계좌로 받을 수 있죠. 다만 IRP로 받으면 과세가 이연돼서 나중에 연금 수령 시 세금이 줄어드는 혜택이 있죠.",
  },
  {
    q: "연금 수령 중에 일부를 일시금으로 꺼낼 수 있나요?",
    a: "원칙적으로 어려워요. 연금 개시 이후에는 설정된 수령 계획대로 받는 게 기본이에요. 무주택자 주택 구입, 6개월 이상 요양 같은 부득이한 사유가 있으면 일부 중도 인출이 가능한데, 그 부분에는 기타소득세가 붙어요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        이걸 어떻게 꺼내야 할지 막막하죠. 그냥 꺼내면 세금을 다 내야 하고, 연금으로 받으면 절세가 되는데 조건이 뭔지도 헷갈리고요.
        일시금과 연금 중 어떤 걸 선택하느냐에 따라 세금 차이가 수백만 원까지 나요.
      ...
      <p style={body}>
        ...
        이 두 조건을 모두 충족해야 <a href="/w/퇴직금-소득세" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a> 30~40% 감면 혜택을 받을 수 있죠.
      </p>
      <p style={body}>
        55세 이전에 꺼내면 감면 혜택 없이 세금을 전액 내야 해요.
        55세가 됐어도 IRP 가입 5년이 안 됐다면 연금 개시를 미뤄야 해요.
        IRP를 퇴직 직전에 만든 경우라면 가입일 기준 5년이 지난 뒤 신청하면 돼요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="연금 수령 조건을 모두 갖추고 있죠. 연금을 선택하면 퇴직소득세를 40%까지 아낄 수 있죠."
      ...
      <p style={body}>
        일시금으로 꺼내면 <a href="/w/퇴직금-소득세" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a>를 그 자리에서 전액 내야 해요.
        연금으로 나눠 받으면 퇴직소득세가 30~40% 줄어들고, 매년 받는 연금 금액에는 낮은 세율(3.3~5.5%)이 적용돼요.
        퇴직금이 클수록 절세 효과도 커지죠.
      </p>
      <p style={body}>
        퇴직소득세는 근속연수와 총급여에 따라 달라지기 때문에 아래 계산기는 단순 추정값이에요.
        정확한 세액은 <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직금 세금 계산</a> 글에서 볼 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        창구 방문 시에는 수령 신청서를 직접 작성하는 경우도 있죠.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        IRP를 처음 만든 직후라면 금융사 앱 접속과 공동인증서 설정에 시간이 걸릴 수 있죠.
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 소득세법을 바탕으로 작성됐고요, 제도 변경이 있을 수 있으니 최신 기준은 금융감독원(☎ 1332)에서 볼 수 있죠." />
    </ArticleLayout>
  );
}
