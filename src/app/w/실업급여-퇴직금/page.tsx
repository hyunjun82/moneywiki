"use client";
// Q1. 퇴직금을 받으면 실업급여가 줄어들까 봐 걱정하며 퇴직을 앞둔 직장인
// Q2. 퇴직금과 실업급여를 모두 신청하고 둘 다 수령한다
// Q3. 두 제도가 완전히 별개임을 이해하고, 각각의 조건·기한·절차를 파악해야 함 (자진퇴사 예외, IRP 처리, 이직확인서 발급 포함)
// Q4. GreenBox로 핵심 결론 먼저, Steps로 절차 순서, EligibilityChecker로 조건 자가진단, Calculator로 금액 예측
// MAP: Q1→서론 첫 문장(퇴직금 받으면 실업급여 줄어들지 않나요?) Q2→H2순서(조건→계산→서류→절차→체크리스트) Q3→H2깊이(자진퇴사·IRP·이직확인서 각각 별도 설명) Q4→컴포넌트(GreenBox·EligibilityChecker·Calculator·Steps·Checklist)
...
const CHECK_ITEMS = [
  { id: "c1", label: "비자발적으로 퇴직했죠 (권고사직·계약만료·폐업 등)" },
  { id: "c2", label: "고용보험에 180일 이상 가입되어 있죠" },
  { id: "c3", label: "같은 직장에서 1년 이상 일해서 퇴직금 조건을 충족하죠" },
  { id: "c4", label: "퇴직 후 적극적으로 구직 활동을 할 의사가 있죠" },
];
...
const STEPS = [
  {
    title: "퇴직금 수령 준비",
    desc: "퇴직일로부터 14일 이내에 퇴직금을 받아야 해요. 퇴직금이 300만원을 초과하면 IRP 계좌로 이체되죠. 미리 IRP 계좌번호를 인사팀에 전달해두면 이체가 빠르게 처리돼요. 퇴직금 수령 시기는 실업급여 신청과 무관해요.",
    tip: "IRP 계좌는 은행·증권사·보험사 어디서든 개설 가능해요",
  },
  {
    title: "이직확인서 발급 확인",
    desc: "실업급여 신청 전에 회사가 고용24에 이직확인서를 제출했는지 봐야 해요. 회사가 10일 이내에 제출하지 않으면 고용24에서 직접 발급 요청을 할 수 있죠. 이직확인서 없이는 실업급여 신청이 안 돼요.",
    tip: "고용24 > 나의 서비스 > 이직확인서 처리 여부 조회에서 진행 상황을 볼 수 있죠",
    link: { label: "고용24에서 이직확인서 조회", href: "https://www.work.go.kr" },
  },
  {
    title: "실업급여 신청",
    desc: "퇴직 다음 날부터 고용센터 방문 또는 고용24 온라인으로 신청해요. 신청 기한은 퇴직 후 12개월이에요. 이 기간이 지나면 남은 수급 일수가 소멸하죠. 퇴직 직후 바로 신청하는 게 가장 유리해요.",
    tip: "온라인 수급자 교육을 먼저 이수하면 방문 없이 신청할 수 있죠",
    link: { label: "고용24 실업급여 신청", href: "https://www.work.go.kr" },
  },
  {
    title: "구직 활동 및 실업인정",
    desc: "실업급여 수급 중에는 4주에 1회 이상 구직활동을 증명해야 해요. 입사지원, 면접, 직업훈련 등이 인정돼요. 취업하면 실업급여가 종료되고, 남은 수급 일수에 따라 조기재취업수당을 받을 수도 있죠. 퇴직금은 이미 받은 금액이라 수급 기간 중에도 영향이 없어요.",
    tip: "퇴직금은 이미 받은 금액이라 실업급여 수급 기간 중 영향 없죠",
  },
];
...
const FAQS = [
  ...
  },
  {
    ...
    a: "원칙적으로 그래요. 하지만 임금 미지급, 직장 내 괴롭힘, 건강 악화, 회사 이전 등 정당한 사유가 있으면 자진 퇴사도 실업급여를 받을 수 있죠. 고용센터에서 사유를 확인받아야 해요.",
  },
  {
  ...
  {
    q: "실업급여 신청 기한이 얼마인가요?",
    a: "퇴직 후 12개월이에요. 이 기간 안에 신청해야 남은 수급 일수를 받을 수 있죠. 12개월이 지나면 잔여 수급 기간이 소멸하니 퇴직 후 빠르게 신청하는 게 좋아요.",
  },
  {
    q: "회사가 이직확인서를 안 내줄 때는 어떻게 하나요?",
    a: "고용24에서 직접 이직확인서 발급 요청을 할 수 있죠. 회사에 발급 요청 후 10일이 지나도 처리가 안 되면 고용24를 통해 고용센터가 직접 사업주에게 요청해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={body}>
        ...
        실업급여는 고용보험법에 따라 고용보험 기금에서 지급하는 별개의 사회보험 급여죠.
        재원도, 지급 주체도, 조건도 완전히 달라서 퇴직금 수령이 실업급여를 막거나 줄이지 않아요.
      </p>
      <p style={body}>
        실업급여를 받으려면 비자발적 이직(권고사직·계약만료·폐업 등)과 고용보험 180일 이상 가입이 핵심 조건이에요.
        자진 퇴사라면 임금 미지급·직장 내 괴롭힘 등 <a href="/w/실업급여-정당한-퇴사-사유" style={{ color: "#1D9E75", textDecoration: "underline" }}>정당한 퇴직 사유</a>가 있어야 수급이 가능하죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금과 실업급여를 동시에 받을 수 있죠. 아래 계산기로 실업급여 예상액을 봐봐요."
        partialMatchText="실업급여 수급 조건 충족 여부를 고용센터(1350)에서 먼저 봐야 해요."
      />
      ...
      <p style={body}>
        ...
        슬라이더로 일 평균임금과 고용보험 가입 기간을 입력하면 1일 지급액과 예상 수급 기간을 볼 수 있죠.
      ...
      <Calculator
        ...
        note="※ 일 평균임금의 60% 기준. 하한액(최저임금 80%)이 적용되면 실제 금액이 더 높을 수 있죠. 퇴직금은 별도 수령."
      />
      ...
      <p style={body}>
        ...
        회사가 제출을 미루면 고용24에서 직접 발급 요청을 할 수 있죠.
      ...
      <p style={body}>
        퇴직금 수령 준비 → 이직확인서 조회 → 실업급여 신청 → 구직활동 순서로 진행해요.
      ...
      <p style={body}>
        ...
        퇴직금 지급 기한 14일도 봐야 하고, 미지급 시 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>지연이자</a>를 청구할 수 있죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 고용보험법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 봐야 해요." />
    </ArticleLayout>
  );
}
