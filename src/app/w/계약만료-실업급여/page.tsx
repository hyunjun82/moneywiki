...
export default function Page() {
  return (
    ...
    >
      ...
      <p style={body}>
        얼마나 오래, 얼마를 받는지가 가장 궁금하죠. 수급기간은 <strong>고용보험 가입기간</strong>과 <strong>퇴직 시 나이</strong> 두 가지로 결정돼요. 가입기간이 길수록, 나이가 50세 이상이면 더 오래 받을 수 있죠. 아래 표를 보면 바로 내 상황이 어디에 해당하는지 파악이 되실 거예요.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        하나 더요. 실업급여를 받는 중에 새 일자리를 구하면 <strong>조기재취업수당</strong>이 나와요. 수급일수가 절반 이상 남은 상태에서 12개월 이상 고용이 확정된 곳에 취업하면, 남은 금액의 절반을 한꺼번에 받는 거예요. 다음 계약직이라도 적용되니까 빠른 재취업이 오히려 유리할 수 있어요.
      </p>
      <p style={body}>
