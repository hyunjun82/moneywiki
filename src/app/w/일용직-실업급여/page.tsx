  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        건설현장이 끝나거나 계약이 만료돼서 일이 끊겼을 때, 일용직이라고 실업급여를 포기할 필요 없어요.{" "}
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법 제43조</a>에 따라
        일용직도 피보험단위기간 <strong>180일</strong>을 충족하면 실업급여를 받을 수 있죠.
        일반 직장인은 7일 대기기간이 있는데, 일용직은 <strong>대기기간 없이</strong> 바로 지급이 시작돼요.
      ...
      <p style={body}>
        일용직 실업급여의 첫 번째 관문은 <strong>고용보험 가입 여부</strong>예요.
        하루 단위로 일해도 고용보험 적용 대상이에요.
        건설현장, 공장, 행사장, 물류센터 등 어디서 일하든 고용주가 일용근로자 고용보험 취득 신고를 하면 자동으로 가입되죠.
      </p>
      <p style={body}>
        문제는 일부 고용주가 신고를 안 하는 경우예요.
        특히 소규모 현장이나 단기 일감에서 이런 일이 많죠.
        내가 가입돼 있는지 확인하려면{" "}
        <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 피보험자격 이력내역서를 조회해보면 돼요.
        내가 일한 현장별로 취득일과 상실일이 전부 나와요.
      </p>
      <p style={body}>
        누락된 이력이 보인다면 고용센터(1350)에 신고할 수 있어요.
        급여이체 내역, 출근부, 작업일지 등 일한 증거를 제출하면 사업주 미신고분도 소급 가입이 가능하죠.
        건설 일용직이라면{" "}
        <a href="https://www.cwma.or.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>건설근로자공제회</a>의 퇴직공제금도 별도로 확인해보세요.
      </p>

      <GreenBox title="고용보험 가입 확인 방법">
        고용24(ei.go.kr) → 피보험자격 이력내역서 조회<br />
        현장별 취득일·상실일 확인 가능<br />
        누락 이력은 고용센터(1350) 신고로 소급 가입
      </GreenBox>
      ...
      <p style={body}>
        일용직의 피보험단위기간 <strong>180일</strong>은 일반 직장인과 계산 방식이 달라요.
        일반 직장인은 주말·공휴일 포함 재직일수를 세지만, 일용직은 <strong>실제로 일한 날</strong>만 세요.
        한 달에 20일 일했으면 20일로 계산하는 거죠.
        그래서 일용직은 180일을 채우는 데 시간이 더 걸릴 수 있어요.
      </p>
      <p style={body}>
        수급액 계산도 다르죠.
        일반 직장인은 퇴직 전 3개월 평균임금의 60%인데, 일용직은 <strong>최근 1개월 총 임금을 근무일수로 나눈 1일 평균임금</strong>에 60%를 곱해요.
        2026년 기준 상한액 68,100원, 하한액 66,048원이 적용돼요.
        일당이 높아도 상한에 걸리고, 낮아도 하한이 보장되는 구조예요.
      </p>
      <p style={body}>
        예를 들어 한 달 총 임금 300만원에 20일 일했다면, 1일 평균임금은 15만원이에요.
        15만원의 60%는 9만원이지만, 상한액(68,100원)을 넘기니까 하루 68,100원을 받게 되죠.
        반대로 일당이 낮더라도 하루 66,048원(월 약 198만원)은 보장돼요.
      </p>

      <BorderBox title="일용직 vs 일반 직장인 핵심 차이">
        피보험단위기간: 일반 = 재직일수 / 일용직 = 실제 근무일수<br />
        평균임금: 일반 = 퇴직 전 3개월 / 일용직 = 최종 1개월<br />
        대기기간: 일반 = 7일 / 일용직 = 0일 (바로 지급)<br />
        이직확인서: 일반 = 필요 / 일용직 = 불필요
      </BorderBox>
      ...
      <p style={body}>
        일용직 실업급여 신청은 일반 직장인보다 간단해요.
        이직확인서가 필요 없으니까 회사에 서류를 요청할 필요가 없죠.{" "}
        <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서
        피보험단위기간을 확인하고, 수급자격 신청자 온라인 교육을 이수한 뒤, 관할 고용센터에 방문하면 돼요.
      </p>
      <p style={body}>
        고용센터에서 피보험 이력을 확인하고 수급자격을 인정해줘요.
        일반 직장인은 여기서 7일 대기기간이 있지만, 일용직은 대기기간이 없어요.
        실업인정을 받으면 바로 다음 날부터 구직급여가 지급되기 시작하죠.
        이후에는 1~4주 단위로 고용센터에 출석해서 구직활동을 보고하면 계속 받을 수 있어요.
      </p>
      <p style={body}>
        수급기간은 고용보험 가입기간과 나이에 따라 120일(약 4개월)에서 최대 270일(약 9개월)까지예요.
        여러 현장의 가입기간을 합산할 수 있으니까 고용24에서 전체 이력을 조회해보세요.
        퇴직일로부터 <strong>12개월 이내</strong>에 신청해야 하니까 미루지 말고 빨리 움직이세요.
      </p>
      ...
      <p style={body}>
        일용직은 일이 불규칙하잖아요.
        실업급여 수급 중에 간간이 현장에서 일하는 경우가 많죠.
        이건 허용돼요.
        다만 <strong>일한 날은 반드시 실업인정 때 신고</strong>해야 해요.
        일한 날에는 구직급여가 안 나오고, 그만큼 수급기간이 뒤로 밀리는 구조예요.
      </p>
      <p style={body}>
        신고를 안 하면 <strong>부정수급</strong>이에요.
        부정수급이 적발되면 받은 금액의 최대 5배를 추징당할 수 있죠.
        "며칠 일한 건데 괜찮겠지"라고 생각하면 안 돼요.
        고용보험 전산에 일용근로 취득 신고가 올라가면 바로 잡혀요.
        적발 안 되더라도 이후 다시 실업급여를 신청할 때 불이익을 받을 수 있어요.
      </p>
      <p style={body}>
        한 가지 더 중요한 게 있어요.
        실업급여 수급 중에 <strong>월 60시간(주 15시간) 이상</strong> 취업하면 취업으로 간주돼서 수급이 중단될 수 있어요.
        단기 일감을 여러 개 잡다 보면 이 기준을 넘길 수 있으니까 근무시간을 잘 관리하세요.
      </p>

      <GreenBox title="부정수급 예방 핵심 3가지">
        일한 날은 실업인정 때 빠짐없이 신고<br />
        미신고 적발 시 받은 금액의 최대 5배 추징<br />
        월 60시간(주 15시간) 이상 일하면 수급 중단 가능
      </GreenBox>
      ...
      <p style={body}>
        "나는 고용보험에 가입된 적이 없는데?"라면 사업주가 신고를 안 한 거예요.
        특히 소규모 건설현장, 단기 아르바이트, 개인 사업장에서 이런 경우가 많죠.
        그렇다고 실업급여를 포기할 필요는 없어요.
      </p>
      <p style={body}>
        고용센터(1350)에 사업주 미신고를 신고하면 <strong>소급 가입</strong>이 가능해요.
        일한 증거(급여이체 내역, 출근부, 작업일지, 문자 내역 등)를 제출하면 고용센터에서 사실관계를 확인하고 가입 처리를 해주죠.
        소급 가입된 기간은 피보험단위기간에 합산돼요.
      </p>
      <p style={body}>
        사업주가 고용보험료를 체납한 경우에도 근로자의 수급자격에는 영향이 없어요.{" "}
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서
        사업주의 보험료 체납과 근로자의 급여 수급을 분리해서 규정하고 있죠.
        사업주가 보험료를 안 냈더라도 내 수급자격은 유지돼요.
      </p>
