...
// Q1. 퇴직이 결정됐거나 임박해서 IRP 계좌를 어디서 어떻게 만들어야 할지 모르는 직장인
// Q2. IRP 계좌를 개설하고 계좌번호를 인사팀에 전달한다
// Q3. 의무 대상 여부(300만원 기준), 금융사별 수수료 차이, 개설 절차 4단계, 세액공제 계산, 55세 전 해지 페널티
// Q4. Steps(개설 절차) + Calculator(세액공제) + Checklist(주의사항) + EligibilityChecker(의무 체크)
//
// MAP:
// - Q1 → 서론: 퇴직 결정 나면 바로 만들어야 하는 이유 공감
// - Q2 → H2 순서: 의무 여부 → 금융사 선택 → 개설 절차 → 세액공제 → 주의사항
// - Q3 → H2 5개, Steps 4단계, 페널티 섹션 포함
// - Q4 → EligibilityChecker + Steps + Calculator + Checklist
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직금이 300만원을 초과하죠" },
  { id: "c2", label: "퇴직이 확정됐거나 이미 퇴직했죠" },
  { id: "c3", label: "만 55세 미만이죠" },
  { id: "c4", label: "IRP로 연간 납입해서 세액공제도 받고 싶죠" },
];
...
const STEPS = [
  {
    title: "금융기관 수수료 비교",
    desc: "은행, 증권사, 보험사 중 수수료를 먼저 비교하세요. 증권사(미래에셋·삼성증권·NH투자증권)는 연 0.2% 수준이고 은행은 연 0.4~0.5% 수준이죠. ETF 투자까지 원한다면 증권사가 유리해요. 퇴직금 수령만 목적이라면 수수료 0% 상품도 있죠.",
    tip: "수수료 0.3% 차이가 10년이면 수십만 원 차이죠",
  },
  {
    title: "앱으로 계좌 개설",
    desc: "선택한 금융사 앱을 설치하고 IRP 계좌 개설 메뉴를 찾아요. 신분증 촬영 → 본인인증 → 계좌 연결 순서로 10분 안에 끝나죠. 공동인증서나 카카오·PASS 간편인증으로 처리 가능하고요. 일부 은행은 비대면 한도 제한으로 영업점 방문이 필요하기도 해요.",
    tip: "비대면 개설이 안 되면 영업점 방문 시 신분증 한 장만 챙기면 돼요",
  },
  {
    title: "운용 방식 선택",
    desc: "계좌가 만들어지면 어떻게 운용할지 설정해야 해요. 원리금보장형(정기예금·ELB)은 원금이 보장되고, 실적배당형(ETF·펀드)은 수익률이 달라지죠. 운용 지시를 따로 안 하면 기본 원리금보장형으로 배정돼요. 나중에 언제든지 바꿀 수 있고요.",
    tip: "20~40대라면 ETF 분산 투자가 장기적으로 유리해요",
  },
  {
    title: "계좌번호 인사팀에 전달",
    desc: "퇴직이 확정되면 IRP 계좌번호(금융사명 + 계좌번호 + 예금주)를 인사팀에 알려줘요. 회사는 퇴직일로부터 14일 이내에 IRP 계좌로 퇴직금을 이체해야 하죠. 구두 전달보다 메일이나 문자로 남겨두면 나중에 증거로 쓸 수 있죠.",
    tip: "14일 이후에도 안 들어오면 지연이자(연 20%) 청구가 가능해요",
...
const FAQS = [
  {
    ...
    a: "수수료가 낮은 증권사를 권해요. 미래에셋·삼성증권·NH투자증권이 연 0.2% 수준으로 낮죠. 은행은 0.4~0.5%로 차이가 나고요. 퇴직금 수령만 목적이라면 수수료 0% 상품도 있죠. ETF 투자까지 원한다면 증권사가 유리하죠.",
  },
  {
    q: "IRP 계좌를 여러 개 만들 수 있나요?",
    a: "만들 수는 있죠. 하지만 세액공제 한도는 IRP + 연금저축 합산 연 900만원으로 고정이죠. 여러 계좌에 나눠 납입해도 한도 합산이기 때문에 하나로 관리하는 게 편하죠.",
  },
  {
    q: "IRP 세액공제를 받으면 나중에 세금이 더 나오나요?",
    a: "연금 수령 시 연금소득세(3.3~5.5%)가 붙어요. 하지만 지금 돌려받는 세액공제(16.5% 또는 13.2%)가 훨씬 크기 때문에 대부분 유리하죠. 일시 인출 시에는 기타소득세 16.5%가 부과돼요.",
  },
  {
    q: "회사가 IRP로 이체를 안 해주면 어떻게 하나요?",
    a: "300만원 초과 퇴직금을 IRP 외 계좌로 주거나 현금으로 주는 건 위법이에요. 고용노동부(1350)에 신고하거나 노동청에 진정을 넣을 수 있죠.",
  },
  {
    q: "IRP 납입 한도가 얼마예요?",
    a: "연 1,800만원까지 납입 가능해요. 세액공제는 최대 900만원(IRP + 연금저축 합산)까지 적용되죠. 퇴직금 이체분은 이 한도와 별개로 들어와요.",
  },
  {
    q: "퇴직 전에 IRP 계좌를 만들어야 하나요?",
    a: "퇴직 전에 미리 만들어두는 게 좋아요. 계좌가 없으면 회사가 퇴직금을 이체할 수 없어서 지급이 늦어질 수 있죠. 퇴직 결정 나면 그날 바로 앱으로 개설하는 걸 권해요.",
  },
];
    ...
    >
      ...
      <p style={body}>
        ...
        <a href="/w/퇴직금-irp-의무" style={{ color: "#1D9E75", textDecoration: "underline" }}>
          근로자퇴직급여보장법 제9조
        </a>에 따라 회사는 반드시 IRP 계좌로 퇴직금을 이체해야 하죠.
        300만원 이하인 경우에만 일반 통장으로 받을 수 있죠.
      </p>
      <p style={body}>
        IRP는 퇴직금 수령 통로 역할만 하는 게 아니에요.
        본인이 추가 납입하면 연말정산에서 세액공제도 받을 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="IRP 계좌 개설이 필요해요. 아래 계산기로 세액공제 혜택부터 보세요."
      ...
      <p style={body}>
        납입액과 소득 구간을 선택하면 예상 환급액을 바로 볼 수 있죠.
        퇴직금 이체분은 납입 한도와 별개로 들어오기 때문에 세액공제 계산에 포함되지 않죠.
      ...
      <p style={body}>
        ...
        기존 은행 계좌번호가 있으면 자금 이체 연결도 바로 할 수 있죠.
      ...
      <p style={body}>
        ...
        퇴직 전에 미리 만들어두고 계좌번호를 인사팀에 전달하는 게 핵심이죠.
      </p>
      ...
      <p style={body}>
        ...
        세액공제 한도도 놓치면 매년 수십만 원 손해이니 납입 계획을 미리 세워두면 좋죠.
      </p>
      ...
      <GreenBox>
        ...
        앱으로 10분이면 개설되고, 개설 후 계좌번호를 인사팀에 메일로 남겨두면 끝이죠.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 소득세법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 금융감독원(1332)에서 보세요." />
    </ArticleLayout>
  );
}
