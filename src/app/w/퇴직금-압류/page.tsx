...
// Q1. 채권자로부터 압류 통지를 받았거나, 퇴직 전에 퇴직금을 빼앗길까 봐 겁나는 사람
// Q2. 퇴직금이 얼마나 보호되는지 파악하고, IRP 계좌로 이체 요청하거나 이의신청 기한을 잡는다
// Q3. 1/2 압류 금지 한도(민사집행법 제246조), IRP 전액 보호(퇴직연금법 제7조),
//     압류 통지 후 이의신청 기한, 개인회생 시 강제집행 중지 여부
// Q4. GreenBox(핵심 규칙 요약) + Calculator(보호 금액 계산) + Steps(4단계 절차) +
//     DocTable(서류 목록) + Checklist + FAQ
//
// MAP:
// - Q1 → 서론 톤: "날아왔을 때" 공감 → 전액 아님을 바로 말해줌
// - Q2 → H2 순서: 보호 한도 파악 → 계산기 → 서류 → 대응 절차 → 체크리스트 → FAQ
// - Q3 → H2 6개 (압류 한도, 계산기, 서류, 4단계 절차, 체크리스트, FAQ)
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직금 압류 통지를 받았거나 압류가 우려되는 상황이죠" },
  { id: "c2", label: "아직 퇴직금을 수령하지 않은 상태이죠" },
  { id: "c3", label: "퇴직 전 IRP 계좌를 아직 개설하지 않았죠" },
  { id: "c4", label: "채권자에게 빼앗기지 않고 퇴직금을 지키고 싶죠" },
];
...
const STEPS = [
  ...
  },
  {
    ...
    tip: "은행·증권사 앱에서 IRP 계좌 개설은 당일 가능하죠",
    link: { label: "IRP 계좌 개설 방법", href: "/w/퇴직금-IRP-계좌" },
  },
  {
    title: "압류 통지 받은 경우 이의신청",
    desc: "압류 명령이 회사에 도달하면 회사는 압류 가능 금액을 공탁할 수 있죠. 1/2을 초과한 과도한 압류라면 이의신청으로 다툴 수 있고, 이의신청 기한이 짧으니 결정문을 받은 날 바로 대한법률구조공단(132)에 전화해 무료 상담을 받으세요.",
    tip: "이의신청은 결정문 수령일로부터 1주일 이내가 원칙이에요",
    link: { label: "대한법률구조공단 무료 상담", href: "https://www.klac.or.kr" },
  },
  {
    title: "개인회생·파산 절차 활용",
    desc: "채무가 많아서 압류 자체를 막고 싶다면 개인회생이나 파산 신청을 검토해볼 수 있죠. 개인회생 개시 결정 이후에는 채권자들의 개별 강제집행이 중지되고, 압류 금지 재산 목록에 퇴직금 1/2이 포함돼서 나머지 절반도 한 번 더 보호받아요.",
    tip: "대한법률구조공단(132)에서 무료로 절차 안내를 받을 수 있죠",
  },
];

const CHECKLIST = [
  "압류 범위 파악: 퇴직금의 1/2만 압류 가능 (민사집행법 제246조)",
  "IRP 계좌 개설: 퇴직 전에 미리 개설해두는 게 핵심이에요",
  "회사 인사팀에 IRP 이체 서면 요청 제출",
  "압류 통지 수령 시 이의신청 기한 즉시 파악",
...
const FAQS = [
  {
    ...
    a: "아니에요. 민사집행법 제246조에 따라 퇴직금의 1/2은 압류 금지 재산이죠. 채권이 아무리 커도 퇴직금 절반은 반드시 내 손에 남아야 해요.",
  },
  {
    q: "IRP 계좌로 받으면 전혀 압류가 안 되나요?",
    a: "맞아요. 근로자퇴직급여보장법 제7조에 따라 IRP 계좌 잔액은 압류·양도·담보 제공이 모두 금지돼요. 일반 계좌(1/2 보호)보다 훨씬 강한 전액 보호죠. 퇴직 전에 IRP 계좌를 개설하고 인사팀에 서면으로 요청해두는 게 핵심이에요.",
  },
  {
  ...
  },
  {
    ...
    a: "회사는 압류 명령 범위 내에서 채권자에게 직접 지급하거나 법원에 공탁할 의무가 생겨요. 회사가 압류 명령을 무시하고 임의로 전액 지급하면 회사에 채무불이행 책임이 생길 수 있죠.",
  },
  {
    q: "개인회생 신청하면 퇴직금 압류가 중지되나요?",
    a: "개인회생 개시 결정 이후 강제집행이 중지돼요. 퇴직금도 이 범위에 포함될 수 있죠. 법무사나 변호사에게 정확한 적용 범위를 묻거나 대한법률구조공단(132)에 무료 상담을 받으세요.",
  },
  {
    q: "퇴직연금(DB·DC)도 압류가 안 되나요?",
    a: "퇴직연금은 근로자퇴직급여보장법 제7조에 따라 계좌 잔액 전액이 압류 금지예요. 회사가 적립해둔 퇴직연금이 있다면 일반 퇴직금보다 더 강하게 보호되죠.",
  },
];
...
const RELATED = [
  { slug: "퇴직금-가압류", title: "퇴직금 가압류 대응 방법", description: "가압류 통지 받았을 때 단계별 대처법이죠." },
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-압류" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        퇴직금의 1/2을 압류 금지 재산으로 보호하죠. 퇴직금 3,000만원이라면 최대 1,500만원까지만 압류할 수 있고,
      ...
      <p style={body}>
        퇴직금에는 두 가지 보호 장치가 있죠. 일반 계좌로 받을 경우 민사집행법 제246조가 퇴직금 전액의 1/2을 압류 금지 재산으로 못 박아요.
      ...
      <p style={body}>
        ...
        IRP 계좌 잔액 전체를 압류·양도·담보 제공 불가로 막죠. 1/2이 아니라 전부 보호되는 구조예요.
      </p>
      <p style={body}>
        퇴직 전에 IRP 계좌를 개설하고 인사팀에 서면으로 IRP 이체를 요청해두는 것만으로 전액 보호가 가능해요.
        이것만 챙겨도 채권자로부터 퇴직금 전체를 지킬 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="지금 압류 대응이 필요한 상황이죠. 아래 계산기로 보호 금액부터 파악해보세요."
        partialMatchText="상황에 따라 다를 수 있죠. 대한법률구조공단(132) 무료 상담을 권해요."
      />
      ...
      <p style={body}>
        퇴직금 총액을 입력하면 법적으로 보호되는 금액과 최대 압류 가능 금액을 바로 볼 수 있죠.
        일반 계좌로 수령했을 때 기준이에요.
      </p>
      <p style={body}>
        퇴직금 5,000만원이라면 채권자가 가져갈 수 있는 건 최대 2,500만원이에요.
        나머지 2,500만원은 반드시 내 손에 남아야 하고, IRP 계좌로 받으면 금액 상관없이 전액 0원이죠.
      </p>
      ...
      <p style={body}>
        ...
        은행·증권사 앱에서 당일 개설과 확인서 발급이 모두 가능하죠.
      </p>
      ...
      <p style={body}>
        압류 통지를 받은 순간부터 할 일이 생겨요. 무작정 기다리면 회사가 압류 가능 금액을 공탁해버려서 나중에 돌려받기 어려워지죠.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        ...
        이미 압류 통지가 온 상태라면 이의신청 기한부터 즉시 파악하세요.
      </p>
      ...
      <GreenBox>
        퇴직금을 IRP 계좌로 받으면 근로자퇴직급여보장법 제7조에 따라 계좌 잔액 전액이 압류·양도·담보 제공 금지 대상이죠.<br />
