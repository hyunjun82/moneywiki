...
//     아기를 낳고 퇴직했거나, 출산·육아휴직 중 퇴직을 앞두고 실업급여를 받을 수 있는지, 기한이 지났는지 불안한 상황이에요.
// Q2. 이 사람이 이 글을 읽고 나서 할 수 있어야 하는 행동은?
//     수급기간 연장 신청서를 들고 고용센터에 가거나 고용24에서 온라인 신청을 완료하는 것이에요.
// Q3. 이 행동을 하려면 반드시 알아야 하는 정보는?
//     ① 출산전후휴가 급여 종료 후에야 실업급여 신청 가능(중복 불가), ② 수급기간 12개월 만료 전에 연장 신청해야 함,
//     ③ 육아휴직 후 자발적 퇴직이면 정당한 사유 심사 필요, ④ 구직활동은 워크넷 온라인 지원으로 대체 가능이에요.
// Q4. 이 정보를 가장 잘 전달하는 형태는?
//     GreenBox로 요약 조건 제시 → EligibilityChecker → BorderBox로 중복 불가 구분 → Checklist로 신청 절차 → FAQ이에요.
...
const CHECK_ITEMS = [
  ...
  { id: "c3", label: "출산예정일 또는 출산일을 알고 있죠" },
  { id: "c4", label: "고용센터에 수급기간 연기(연장) 신청을 할 수 있는 상황이에요" },
];

const CHECKLIST = [
  "고용24(ei.go.kr)에서 피보험자격 이력 조회: 가입기간 180일 체크",
  "출산전후휴가 급여 수급이 완전히 종료됐는지 체크 (중복 수급 불가)",
  "이직확인서 퇴직 사유가 비자발적으로 기재됐는지 체크",
...
const FAQS = [
  {
    ...
    a: "받을 수 있죠. 다만 동시에는 안 돼요. 출산전후휴가 급여가 끝난 뒤에 실업급여를 신청해야 해요. 둘 다 고용보험에서 나오는 급여라 중복 수급이 법으로 금지돼 있거든요.",
  },
  {
    q: "임신 중에 퇴직하면 바로 실업급여를 신청해야 하나요?",
    a: "바로 신청할 필요 없어요. 고용센터에 출산 서류를 제출하고 수급기간 연장을 받으면 최대 4년까지 미룰 수 있죠. 출산하고 몸이 회복된 뒤에 천천히 신청해도 돼요.",
  },
  {
    q: "육아휴직 쓰다가 복직 안 하고 퇴직하면 실업급여 받을 수 있나요?",
    a: "받을 수 있죠. 육아휴직 기간도 고용보험 가입기간으로 인정돼요. 다만 회사가 복직을 받아줬는데 본인이 거부한 경우, 자발적 퇴직으로 분류될 수 있거든요. 퇴직 사유를 잘 정리해두는 게 중요하죠.",
  },
  {
  ...
  },
  {
    ...
    a: "완전히 다른 제도예요. 출산급여는 건강보험에서 지급하는 일시금이고, 실업급여는 고용보험에서 지급하는 구직급여예요. 재원도 다르고 관할 기관도 달라서 둘 다 받을 수 있죠.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        출산·육아로 구직이 어려운 상황을 위해 수급기간을 <strong>최대 4년까지 연장</strong>해주는 제도를 두고 있죠.
      ...
      <p style={body}>
        기본 조건은 일반 실업급여와 같아요. 퇴직 전 18개월 내 <strong>고용보험 가입기간 180일 이상</strong>, 그리고 <strong>비자발적 퇴직</strong>이면 되죠. 출산을 했든 안 했든, 이 두 가지가 맞으면 수급자격이 생겨요.
      </p>
      <p style={body}>
        놓치기 쉬운 부분이 하나 있죠. 출산전후휴가나 육아휴직 기간도 <strong>고용보험 가입기간에 포함</strong>돼요. 출산휴가 90일에 육아휴직 12개월을 썼다면 그 기간이 전부 피보험기간으로 잡히거든요. 오히려 180일 채우기가 더 수월해지는 구조예요.
      </p>
      <p style={body}>
        &ldquo;비자발적 퇴직&rdquo;이라는 조건도 빠뜨리면 안 돼요. 계약만료, 권고사직, 근무조건 불리변경은 당연히 해당되죠. 그런데 회사가 복직을 제안했는데 본인이 거부한 경우라면 자발적 퇴직으로 분류될 수 있거든요. 이직확인서에 퇴직 사유가 어떻게 적히는지 반드시 체크해봐요.
      </p>
      ...
      <SectionBadge>내 상황에 해당되는지 체크해봐요</SectionBadge>
      <EligibilityChecker
        items={CHECK_ITEMS}
      ...
      <p style={body}>
        헷갈리기 쉬운 게 <strong>출산급여(건강보험 일시금)</strong>예요. 건강보험공단에서 지급하는 별도 제도이고, 고용보험 재원이 아니에요. 그래서 출산급여와 실업급여를 동시에 받는 건 가능하죠. 이름이 비슷해서 혼동하는 분이 많은데, 재원 자체가 달라요.
      </p>
      ...
      <H2>수급기간 연장 기준에 따라 최대 4년까지 미룰 수 있죠</H2>
      <p style={body}>
        실업급여는 퇴직 다음 날부터 <strong>12개월 이내</strong>에 소진해야 해요. 기한을 넘기면 남은 수급일수가 남아도 그냥 날아가죠. 그런데 신생아를 안고 면접을 보러 다니라는 건 말이 안 되잖아요.
      </p>
      <p style={body}>
        그래서 <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법 제52조</a>가 출산과 육아를 수급기간 연장 사유로 인정하고 있죠. 고용센터에 출생증명서나 모자보건수첩 등을 내면{" "}
      ...
      <p style={body}>
        신청 시점을 놓치면 안 돼요. 원래 수급기간 12개월이 만료되기 <strong>전에</strong> 연장을 신청해야 하죠. 기한이 지나고 나서 &ldquo;연장해주세요&rdquo; 해봤자 소용없어요. 기준이 퇴직일이지 출산일이 아니라는 것도 빠뜨리면 안 되고, 퇴직하면 곧바로 고용센터에 연장 가능 여부를 체크하는 게 가장 안전하죠.
      </p>
      ...
      <p style={body}>
        출산 뒤 출산휴가와 육아휴직을 쓰다가 복직 없이 퇴직하는 분이 꽤 많죠. 이 경우에도 실업급여를 받을 수 있죠. 육아휴직 기간 전부가 고용보험 가입기간으로 잡히기 때문에 180일 조건은 오히려 넉넉하게 넘기는 편이에요.
      </p>
      <p style={body}>
        갈림길은 <strong>퇴직 사유</strong>예요. 회사가 복직을 거부했거나 복귀 뒤 근무조건이 나빠졌다면 비자발적 퇴직이죠. 실업급여 수급이 가능해요. 반면 회사가 원래 자리로 돌아오라는데 본인이 &ldquo;안 가겠다&rdquo;고 했다면 자발적 퇴직으로 잡힐 수 있거든요.
      </p>
      <p style={body}>
        자발적 퇴직이라고 길이 완전히 막힌 건 아니에요. 육아를 이유로 한 퇴직이{" "}
        <a href="/w/실업급여-정당한-퇴사-사유" style={{ color: "#1D9E75", textDecoration: "underline" }}>정당한 이직 사유</a>로 인정받는 경우가 있거든요. 어린이집 대기가 걸려서 아이를 맡길 곳이 없는 상황 등이 해당될 수 있죠. 고용센터(1350)에 사전 상담을 꼭 받아봐야 해요.
      </p>
      ...
      {/* 섹션 5: 구직활동 현실적 방법 */}
      <H2>출산 후 수급기간 연장, 이렇게 신청해요</H2>
      <p style={body}>
        면접장을 돌아다녀야만{" "}
        <a href="/w/실업급여-구직활동-횟수" style={{ color: "#1D9E75", textDecoration: "underline" }}>구직활동</a>이 인정되는 건 아니에요. <strong>워크넷(work.go.kr) 온라인 입사지원</strong>만으로도 충분히 인정되거든요. 아기 재우고 나서 컴퓨터 앞에 앉아 입사지원 몇 건 넣으면 되는 거예요. 고용센터에 매번 가지 않아도 되는 방법이 여럿 있죠.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        방문 자체가 힘든 상황이면 담당자에게 전화로 사정을 설명하세요. 전화상담이나 화상상담으로 구직활동을 대체할 수 있는지 물어봐요. 인정 기준에 고용센터 재량이 있으니, 본인 상황을 솔직하게 말하는 게 가장 나은 방법이에요.
      </p>
