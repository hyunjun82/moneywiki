...
const STEPS = [
  {
    ...
    desc: "퇴직일 기준 만 55세 미만이고 퇴직금이 세전 300만원을 초과하면 IRP 의무 대상이에요. 두 조건 중 하나라도 해당 안 되면 일반 계좌로 받을 수 있어요. 55세 생일이 퇴직일 당일이면 의무 대상에서 빠져요.",
  ...
  },
  {
    ...
    desc: "은행명·계좌번호·예금주를 문자나 메일로 인사팀에 알려줘요. 회사는 퇴직일 이후 14일 이내에 이 계좌로 퇴직금을 이체해야 해요. 14일을 넘기면 연 20% 지연이자를 청구할 수 있어요.",
    tip: "구두 전달은 나중에 증거가 안 돼요 — 문자·메일로 기록을 남겨요",
  },
];

const CHECKLIST = [
  "의무 여부 확인: 만 55세 미만 + 퇴직금 세전 300만원 초과인지 체크",
  "금융사 수수료 비교: 수령만 목적이면 수수료 0% 상품 선택",
  "IRP 개설: 모바일 앱으로 퇴직 전에 미리 개설",
  "계좌번호 전달: 인사팀에 문자·메일로 은행명·계좌번호·예금주 전달",
  "지급 기한 체크: 퇴직일 기준 14일 이내 이체 여부 확인",
];

const FAQS = [
  {
    q: "IRP 의무 이전 제도가 언제부터 시행됐나요?",
    a: "2022년 4월 14일부터예요. 근로자퇴직급여보장법이 개정되면서 퇴직금 300만원 초과 시 IRP 계좌로만 지급하도록 바뀌었어요. 이전에는 본인 명의 계좌면 어디든 받을 수 있었어요.",
  },
  {
    q: "IRP 의무 예외가 되는 조건은 뭐가 있나요?",
    a: "크게 네 가지예요. ① 퇴직일 기준 만 55세 이상, ② 퇴직금이 세전 300만원 이하, ③ 근로자 사망, ④ 퇴직연금 담보대출 상계로 지급되는 경우예요. 이 중 하나에 해당하면 일반 계좌로 받을 수 있어요.",
  },
  {
  ...
  },
  {
    ...
    a: "의무를 어기는 주체는 회사예요. 회사에 과태료가 부과될 수 있어요. 근로자가 동의하더라도 법적으로 허용되지 않아요. 받은 금액 자체는 유효하지만 회사가 제재를 받을 수 있어요.",
  },
  {
  ...
  },
  {
    ...
    a: "가능해요. 퇴직금 수령분 외에도 연간 1,800만원까지 추가 납입할 수 있어요. 이 중 900만원까지는 세액공제(연봉 5,500만원 이하 16.5%, 초과 13.2%)를 받을 수 있어요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        만 55세 미만이고 퇴직금이 세전 300만원을 초과하면 IRP 계좌 없이는 퇴직금을 받을 수 없어요.
        IRP를 미리 개설해두지 않으면 지급이 14일 기한을 넘겨 연 20% 지연이자가 붙을 수도 있어요.
      ...
      <p style={body}>
        ...
        두 조건 중 하나라도 해당하지 않으면 일반 통장으로 받을 수 있어요.
      </p>
      <p style={body}>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="IRP 의무 대상이에요. 퇴직 전에 IRP 계좌를 개설해두세요. 아래 계산기로 퇴직금 금액과 의무 여부를 확인해보세요."
        partialMatchText="예외에 해당할 수 있어요. 나이와 퇴직금 금액 조건을 정확히 확인해보세요."
      />
      ...
      <p style={body}>
        월 평균급여와 근속 기간을 입력하면 예상 퇴직금과 IRP 의무 여부를 바로 볼 수 있어요.
      ...
      <p style={body}>
        ...
        재직증명서는 일부 금융사에서만 요구하니 미리 연락해서 확인하는 게 편해요.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        ...
        수수료 비교에 하루, 개설에 10분, 계좌번호 전달까지 하면 퇴직 전에 전부 마칠 수 있어요.
      </p>
