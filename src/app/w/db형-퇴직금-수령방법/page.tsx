...
const CHECK_ITEMS = [
  { id: "c1", label: "회사에서 DB형(확정급여형) 퇴직연금에 가입되어 있죠" },
  { id: "c2", label: "현 직장에서 1년 이상 근무했죠" },
  { id: "c3", label: "퇴직이 확정됐거나 퇴직을 앞두고 있죠" },
  { id: "c4", label: "퇴직급여 300만원 초과가 예상되죠" },
];
...
const STEPS = [
  ...
  },
  {
    ...
    desc: "퇴직이 확정되면 IRP 계좌번호(은행명·계좌번호·예금주)를 인사팀에 메일이나 문자로 전달하세요. 회사는 퇴직일로부터 14일 이내에 DB형 퇴직급여를 IRP로 이체해야 하죠. 구두 통보보다 문서로 남겨야 나중에 증거가 되고요.",
    tip: "14일을 넘기면 연 20% 지연이자를 청구할 수 있죠.",
  },
  {
    title: "IRP 계좌로 입금 확인",
    desc: "이체 후 IRP 계좌에서 입금 내역을 파악해보세요. DB형은 회사가 이체 주체라 금융사가 아닌 인사팀이 처리하죠. 금액이 예상과 다르다면 인사팀에 산정 내역서를 요청하세요.",
    tip: "퇴직소득 원천징수영수증도 함께 받아두면 연말정산에 유용하죠.",
  },
  {
    title: "일시금 또는 연금 수령 선택",
    desc: "IRP에 들어온 퇴직급여는 일시금으로 빼거나 55세 이후 연금으로 수령할 수 있죠. 연금으로 10년 이상 받으면 퇴직소득세를 40% 감면받고요. 시간 여유가 있다면 IRP에서 ETF 운용도 가능하죠.",
    tip: "55세 이전에 일시금으로 빼면 퇴직소득세를 전액 납부해야 하죠.",
  },
];
...
const FAQS = [
  ...
  },
  {
    ...
    a: "법정 최저 기준은 퇴직 직전 3개월 평균임금 × 근속연수이죠. 예를 들어 월 350만원으로 10년 일했다면 약 3,500만원이고요. 회사 규정이 이보다 높으면 그 기준을 따르고, 낮으면 근로자퇴직급여보장법 위반이죠.",
  },
  {
  ...
  {
    q: "DB형 가입 여부를 어떻게 알 수 있죠?",
    a: "인사팀에 직접 문의하거나 금융감독원 통합연금포털(100lifeplan.fss.or.kr)에서 조회하면 되죠. 회사 규모에 따라 DB와 DC를 혼합 적용하는 경우도 있고요.",
  },
  {
...
export default function Page() {
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1.5rem",
      ...
      <p style={body}>
        DB형 수령의 핵심은 IRP 계좌번호를 인사팀에 전달하는 거이죠.
        회사가 이체 주체라서 금융사가 아닌 인사팀이 처리하고요.
        퇴직 전에 미리 IRP를 개설해두면 이체 처리가 빠르게 진행되죠.
      </p>
      ...
      <p style={body}>
        IRP 계좌 개설을 미루다 이체가 지연되는 경우가 많죠. 퇴직이 확정되면 바로 개설하고 계좌번호를 전달하세요.
        수수료 낮은 증권사 IRP가 장기 운용에 유리하고요.
      </p>
