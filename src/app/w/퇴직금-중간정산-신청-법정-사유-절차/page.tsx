...
const CHECK_ITEMS = [
  { id: "c1", label: "무주택자이고, 주택을 구입하거나 전세 계약을 체결했어요" },
  { id: "c2", label: "본인·배우자·부양가족의 의료비가 6개월치 월급을 넘었어요" },
  { id: "c3", label: "법원에서 파산 또는 개인회생 결정을 받았어요" },
  { id: "c4", label: "천재지변·재해로 재산 피해가 발생했어요" },
  { id: "c5", label: "임금피크제 적용으로 임금이 줄었어요" },
];
...
const STEPS = [
  {
    ...
    desc: "중간정산은 근로자퇴직급여보장법 시행령 제3조에 열거된 사유가 있어야만 신청할 수 있어요. 주택 구입, 전세 계약, 의료비 과다, 파산·개인회생, 천재지변, 임금피크제가 주요 사유예요. 사유에 해당하지 않으면 신청 자체가 불가하고 회사가 해줬어도 나중에 문제가 돼요.",
    tip: "DC형 퇴직연금은 중간정산이 아닌 중도인출 제도를 이용해요",
  },
  {
    title: "사유별 증빙서류 준비",
    desc: "사유를 입증하는 서류가 없으면 회사가 승인을 거부할 수 있어요. 주택 구입이라면 매매계약서, 의료비라면 진단서와 치료비 영수증, 파산이라면 법원 결정문이 필요해요. 서류가 불완전하면 처음부터 다시 준비해야 해요.",
  ...
  {
    title: "세금 원천징수 후 수령",
    desc: "승인되면 회사가 퇴직소득세를 원천징수하고 14일 이내에 지급해요. 중간정산 시점에 근속 기간이 리셋되기 때문에 나중에 최종 퇴직 시 퇴직소득세 공제가 줄어들 수 있어요. IRP로 받으면 과세 이연이 되니 세금 부담을 줄일 수 있어요.",
    tip: "14일이 지나도 지급이 없으면 연 20% 지연이자를 청구할 수 있어요",
  },
];

const CHECKLIST = [
  "법정 사유: 시행령 제3조 7가지 중 해당 항목 확인",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "중간정산 시점부터 근속 기간이 다시 시작돼요. 최종 퇴직 시 퇴직금은 중간정산 이후 기간만으로 계산해요. 근속 기간이 짧아지면 퇴직소득세 공제(근속공제)가 줄어서 세금이 더 나올 수 있어요.",
  },
  {
  ...
  },
  {
    ...
    a: "위법이에요. 사유 없이 중간정산을 한 경우, 해당 금액은 퇴직금 지급으로 인정되지 않을 수 있어요. 나중에 퇴직금 분쟁이 생기면 회사가 다시 지급해야 하는 상황이 될 수 있어요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금 중간정산은 <a href="/w/퇴직금-중간정산-조건" style={{ color: "#1D9E75", textDecoration: "underline" }}>법정 사유</a>가 있어야만 신청할 수 있어요.
        주택 구입, 전세 계약, 의료비 과다, 파산·개인회생, 천재지변, 임금피크제가 대표 사유예요.
        사유가 있어도 증빙서류를 갖춰야 하고, 중간정산 후 근속 기간이 리셋돼서 <a href="/w/퇴직금-중간정산-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a>가 늘어날 수 있으니 신청 전에 꼭 계산해보세요.
      </p>
      ...
      <p style={body}>
        퇴직금 중간정산은 <a href="/w/퇴직금-중간정산" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법</a> 시행령 제3조에서 허용 사유를 정해두고 있어요.
        이 사유에 해당하지 않으면 회사가 중간정산을 해줬더라도 나중에 법적 분쟁이 생길 수 있어요.
        사유별로 입증해야 하는 서류가 다르기 때문에 어떤 사유에 해당하는지 먼저 확인하는 게 중요해요.
      </p>
      <p style={body}>
        임금피크제 적용도 사유 중 하나예요. 임금이 줄어든 시점에 중간정산을 신청할 수 있고, 회사에서 임금 조정 확인서를 받아서 증빙으로 내면 돼요.
      ...
      <H2>중간정산 예상액과 세금, 미리 계산해보세요</H2>
      <p style={body}>
        중간정산 후에는 근속 기간이 리셋돼서 최종 퇴직 시 퇴직소득세 공제가 줄어요.
        지금 받는 금액과 나중에 늘어나는 세금을 비교해봐야 해요.
      ...
      <Calculator
        ...
        note="※ 월 평균임금 × 근속연수 추정치. 실제 금액은 인사팀 확인 기준으로 다를 수 있어요. IRP 이전 시 퇴직소득세 과세 이연 가능."
      />
      ...
      <p style={body}>
        ...
        서류가 불완전하면 회사가 승인을 거부할 수 있고, 처음부터 다시 준비해야 할 수 있어요.
      ...
      <BorderBox>
        ...
        계약서나 진단서의 날짜가 신청 시점과 맞아야 해요. 이미 계약이 끝난 뒤에 신청하면 사유 인정이 거부될 수 있어요.
      </BorderBox>
      ...
      <p style={body}>
        ...
        지금 받는 금액이 나중에 늘어날 세금보다 크다면 신청이 유리하지만, 그렇지 않으면 손해가 날 수 있어요.
      </p>
