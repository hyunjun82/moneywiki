...
// Q1. 일용직으로 오래 일했는데 퇴직금을 받을 수 있는지 몰라서 그냥 넘어간 상황
// Q2. 내가 퇴직금 대상인지 판단하고, 조건이 맞으면 사업주에게 청구하거나 고용노동청에 신고한다
// Q3. 계속 근로 인정 기준(1년+7일 공백), 주 15시간 기준(4주 평균), 평균임금 계산법,
//     건설일용직 퇴직공제금, 소멸시효 3년, 지급기한 14일, 지연이자 연 20%
// Q4. EligibilityChecker(조건 자가진단) + Calculator(예상액) + DocTable(서류) + Steps(청구절차) + Checklist
//
// MAP:
// - Q1 → 서론: "일용직이라고 들었는데도" 상황 공감 톤
// - Q2 → H2 순서: 조건 → 예상액 계산 → 서류 → 청구절차 → 체크리스트
// - Q3 → H2 5개, 건설일용직 별도 안내 포함
...
const CHECK_ITEMS = [
  { id: "c1", label: "같은 사업장에서 반복적으로 계속 일했죠" },
  { id: "c2", label: "1년(365일) 이상 근무 기간이 됐죠" },
  { id: "c3", label: "주 15시간 이상 일한 주가 대부분이었죠" },
  { id: "c4", label: "공백 기간이 7일을 넘은 적이 거의 없었죠" },
];
...
const STEPS = [
  {
    title: "계속 근로 여부 판단",
    desc: "일용직이어도 반복적으로 같은 사업장에서 일했다면 '계속 근로'로 인정될 수 있죠. 고용노동부 기준은 근무 공백이 7일을 넘지 않으면 계속 근로로 봐요. 1년 이상이고 주 15시간 이상인 주가 대부분이라면 퇴직금 요건을 충족해요.",
    tip: "4대 보험 가입 기간도 계속 근로 증거로 사용할 수 있죠",
  },
  {
    title: "증거 수집",
    desc: "출근부, 통장 거래내역, 업무 지시 카카오톡·문자를 모아요. 국민건강보험공단이나 국민연금공단 앱에서 가입 기간을 조회하면 근무 기간을 입증하는 강력한 증거가 되죠. 사업주가 준 작업 지시 메시지도 계속 근로 관계를 보여줘요.",
    tip: "공단 앱에서 가입 기간 조회는 1분이면 끝나요",
  },
  {
    title: "퇴직금 계산",
    desc: "1일 평균임금은 퇴직 전 3개월 총임금 ÷ 해당 기간 총 일수로 계산해요. 일용직은 일한 날만 임금을 받으니 실제 근무일수를 기준으로 하고요. 퇴직금 = 1일 평균임금 × 30일 × 근속연수예요.",
    tip: "근무일이 불규칙하면 고용노동부(1350)에 계산 방법을 먼저 문의해요",
  },
  {
    title: "고용노동청 진정 접수",
    desc: "사업주가 지급을 거부하면 고용노동청에 체불 진정을 내요. 고용24(work.go.kr) 온라인 또는 가까운 고용노동지청 방문으로 접수할 수 있죠. 진정 후 근로감독관이 조사하고 지급 명령을 내려요. 소멸시효 3년 이내라면 퇴직 후에도 청구 가능해요.",
    tip: "진정 접수는 무료이고, 변호사 없이 직접 할 수 있죠",
...
const FAQS = [
  {
    ...
    a: "받을 수 있죠. 계약서에 '일용직'이라고 써 있어도, 같은 사업장에서 1년 이상 계속 일하고 주 15시간 이상 근무했다면 퇴직금 지급 대상이에요. 고용 형태가 아니라 실제 근무 형태가 기준이에요.",
  },
  {
    q: "일용직의 계속 근로는 어떻게 판단하나요?",
    a: "고용노동부 기준으로 7일 이상 공백 없이 반복적으로 일했다면 계속 근로로 봐요. 같은 사업주 아래 현장이 달라져도 합산될 수 있고, 4대 보험 가입 기간이 증거로 쓰이죠.",
  },
  {
    q: "근로계약서가 없어도 퇴직금 청구가 가능한가요?",
    a: "가능해요. 통장 거래내역, 업무 지시 메시지, 출근 기록이 있다면 근무 사실을 충분히 입증할 수 있죠. 계약서가 없어도 실제 근무 사실이 증거가 돼요.",
  },
  {
    q: "건설 현장 일용직은 퇴직금 기준이 다른가요?",
    a: "건설 현장 일용직은 건설근로자공제회 퇴직공제금 제도가 별도로 있죠. 같은 현장에서 1년 이상 일했다면 일반 퇴직금도 함께 청구할 수 있고요.",
  },
  {
  ...
  {
    q: "주 15시간 이상 못 채운 주가 있는데도 받을 수 있나요?",
    a: "4주 평균으로 판단해요. 일부 주에 15시간을 못 채워도 4주 평균이 15시간 이상이라면 퇴직금 지급 의무가 발생하죠. 전체 기간 중 대부분이 주 15시간 이상이었다면 청구할 수 있죠.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        일용직이어도 같은 사업장에서 1년 이상, 주 15시간 이상 근무했다면 <a href="/w/퇴직금-수령방법" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직금</a>을 받을 수 있죠.
      ...
      <p style={body}>
        ...
        같은 사업장에서 반복적으로 일하면서 공백이 7일을 넘지 않고 1년을 채우면 '계속 근로'로 인정되죠.
        그리고 주 15시간 이상인 주가 대부분이어야 해요.
      </p>
      <p style={body}>
        사업주가 "일용직이라 퇴직금이 없다"고 해도 법적으로 틀린 말이에요.
        법원과 고용노동부 모두 실제 근무 형태를 기준으로 판단하죠.
        증거만 충분하다면 청구할 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금 청구 조건에 해당해요. 아래 계산기로 예상 금액을 먼저 파악해보세요."
        partialMatchText="일부 조건이 달라질 수 있죠. 고용노동부(1350) 상담을 먼저 받아보세요."
      />

      <Divider />

      <H2>일용직 퇴직금 예상액, 직접 계산해보세요</H2>
      <p style={body}>
        일당과 연간 근무일수, 근속 기간을 입력하면 1일 평균임금과 예상 퇴직금이 나와요.
        실제 계산은 퇴직 전 3개월 총임금 ÷ 해당 기간 총 일수로 1일 평균임금을 구하죠.
      ...
      <p style={body}>
        근로계약서가 없어도 통장 거래내역만으로 청구할 수 있죠.
        업무 지시 카카오톡·문자, 출근부 사본이 있으면 더 강력하고요.
        4대 보험 가입 기간은 국민건강보험공단이나 국민연금공단 앱에서 1분이면 조회할 수 있죠.
      </p>
      <p style={body}>
        건강보험 피부양자 자격 상실·취득 내역도 근무 기간을 보여주는 증거로 쓸 수 있죠.
      ...
      <p style={body}>
        계속 근로 여부 파악부터 고용노동청 신고까지 단계별로 대응하면 돼요.
        증거가 충분하면 대부분 신고 단계에서 해결되고, 보통 4~8주 안에 결론이 나죠.
      </p>
      ...
      <p style={body}>
        소멸시효 3년이 지나면 청구가 막혀요. 퇴직 후에도 3년 안이라면 지금 당장 움직여야 하죠.
        증거 수집이 먼저이고, 계속 근로 요건 충족 여부를 하나씩 따져보면 돼요.
      </p>
      ...
      <GreenBox>
        ...
        1년 이상, 주 15시간 이상이라면 고용노동청에 신고해서 청구할 수 있죠.
      </GreenBox>
