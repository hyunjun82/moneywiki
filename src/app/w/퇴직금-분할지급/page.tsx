"use client";
// Q1. 퇴직 후 회사가 퇴직금을 나눠서 주겠다고 해 어떻게 대응해야 할지 모르는 상황
// Q2. 분할지급 합의 여부를 판단하고, 필요하면 지연이자를 청구하는 행동
// Q3. 서면 합의 조건, 이자 포기 조항 위험성, 지연이자 계산법, IRP 수령 의무, 고용노동부 진정 절차
// Q4. GreenBox(합법/위법 기준), Calculator(지연이자 계산), Steps(대응 절차), DocTable(서류), Checklist(서명 전 체크)
// MAP: Q1→서론(분할지급 제안 받은 상황 공감) Q2→H2순서(합법기준→이자계산→서류→대응절차→서명전체크) Q3→H2깊이(서면합의+이자조항+IRP+진정) Q4→컴포넌트매핑
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직 후 14일이 지났는데 퇴직금을 아직 못 받은 상황이에요" },
  { id: "c2", label: "회사가 퇴직금을 나눠서 주겠다고 했죠" },
  { id: "c3", label: "분할지급 합의서에 서명을 요청받았고요" },
  { id: "c4", label: "합의서에 이자 포기 조항이 들어가 있죠" },
];
...
const STEPS = [
  {
    title: "14일 기한 계산",
    desc: "퇴직 후 14일 이내에 퇴직금 전액을 지급받아야 해요. 14일은 달력일 기준으로, 공휴일과 주말도 포함돼요. 퇴직일이 월요일이라면 14일 뒤인 그다음 월요일까지가 기한이에요. 기한이 지난 날부터 연 20% 지연이자가 자동으로 붙기 시작해요.",
    tip: "퇴직일을 기준으로 정확히 14일 뒤 날짜를 달력에서 짚어보세요",
  },
  {
    title: "분할지급 합의 여부 결정",
    desc: "근로자가 동의하면 분할지급으로 합의할 수 있죠. 합의 없이 회사가 일방적으로 나눠 지급하면 위법이에요. 합의할 때는 각 회차 지급일과 금액을 서면에 명확히 적어야 해요. 이자 포기 조항이 들어가 있다면 삭제를 요청하거나 서명을 거부하면 되고요.",
    tip: "합의서에 서명하기 전 이자 관련 조항이 있는지 꼼꼼히 살펴보세요",
  },
  {
    title: "내용증명 발송",
    desc: "14일이 지났는데 지급이 안 됐다면 내용증명으로 퇴직금과 지연이자 지급을 요청해요. '퇴직금 ○○만원 및 연 20% 지연이자 지급 요청'이라는 내용이면 충분해요. 발송 사실이 법적 청구 증거가 되고, 나중에 소송이나 진정 시 유리하게 작용해요.",
    tip: "카카오 전자내용증명으로 10분 이내 발송 가능",
    link: { label: "카카오 전자내용증명 바로가기", href: "https://ecertifit.kakaocert.com" },
  },
  {
    title: "고용노동부 진정 신청",
    desc: "내용증명 발송 이후에도 지급이 안 되면 고용노동부 민원마당에서 온라인 진정을 접수해요. 근로감독관이 회사를 조사하고 지급 명령을 내려요. 회사가 명령을 이행하지 않으면 형사 처벌 대상이 되고요.",
...
const CHECKLIST = [
  ...
  "분할지급 합의: 근로자 동의 없는 회사 일방 분할은 위법",
...
const FAQS = [
  ...
  {
    q: "합의서에 이미 서명했는데 이자 포기 조항이 들어가 있죠",
    a: "퇴직금 지연이자는 근로기준법 제37조에서 보장하는 법적 권리예요. 근로자에게 불리한 조항은 법에 따라 무효가 될 수도 있고요. 법률구조공단(132) 무료 상담을 받아보세요. 서명 자체가 이자 청구권을 완전히 박탈하지는 않을 수도 있죠.",
  },
  {
    q: "회사 사정이 어려워 퇴직금을 못 준다고 해요",
    a: "회사 사정과 무관하게 퇴직금 지급 의무는 유지돼요. 회사가 폐업하거나 도산한 경우에는 체당금 제도를 통해 고용보험에서 일부를 받을 수 있죠. 고용노동부(1350)에 문의하면 구체적인 절차를 안내받을 수 있고요.",
  },
  {
    q: "지연이자를 꼭 받아야 하나요, 포기해도 되나요?",
    a: "지연이자 청구는 선택이에요. 회사와 원만하게 해결하고 싶다면 지연이자를 포기하고 원금만 받는 합의도 가능해요. 다만 지연이자가 법적 권리라는 걸 알고 협상하는 게 유리해요. 소멸시효(3년) 내라면 언제든 청구할 수 있죠.",
  },
  {
    q: "퇴직금 청구권 소멸시효가 얼마나 되나요?",
    a: "퇴직 후 3년이에요. 3년 이내에 청구하지 않으면 법적 권리가 소멸해요. 지연이자 청구권도 동일하게 3년이에요. 회사가 '이미 시효가 지났다'고 주장할 수도 있으니 퇴직 후 시간이 지나기 전에 청구하는 게 좋아요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        동의하더라도 14일을 넘긴 기간에 대해서는 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>연 20% 지연이자</a>를 따로 청구할 수 있죠.
        합의서에 서명하기 전에 이자 포기 조항이 있는지 꼭 살펴봐야 해요.
      </p>
      ...
      <EligibilityChecker
        ...
        partialMatchText="상황에 따라 다를 수 있죠. 고용노동부(1350) 또는 법률구조공단(132)에서 상담해보세요."
      />
      ...
      <p style={body}>
        분할지급 합의서에 서명하기 전에 아래 항목을 하나씩 살펴보세요.
        특히 이자 조항은 한 번 서명하면 번복하기 까다로울 수 있어서 사전에 꼭 짚어봐야 해요.
      </p>
      ...
      <GreenBox>
        합의서에 '이자 포기', '지연이자 면제' 등의 조항이 있다면 삭제 요청하거나 서명을 거부할 수 있죠.<br />
        지연이자는 근로기준법 제37조에서 보장하는 법적 권리예요. 포기 조항은 무효가 될 수도 있고요.<br />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 문의하세요." />
    </ArticleLayout>
  );
}
