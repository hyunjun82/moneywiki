...
const CHECK_ITEMS = [
  { id: "c1", label: "정기적으로 받는 상여금이 있어요" },
  { id: "c2", label: "식대·교통비를 매월 받고 있어요" },
  { id: "c3", label: "퇴직 전 3개월 급여명세서가 있어요" },
  { id: "c4", label: "연차수당을 정산받은 내역이 있어요" },
];
...
const STEPS = [
  {
    title: "3개월 임금 총합 계산",
    desc: "퇴직일 기준으로 역산한 3개월간의 임금을 모두 더해요. 기본급, 정기 상여금(월 환산), 각종 고정수당이 포함돼요. 연차수당은 퇴직 전 3개월 내 지급됐으면 포함해요.",
    tip: "상여금은 지급된 달 금액이 아니라 연 총액 ÷ 12로 환산",
  },
  {
    title: "1일 평균임금 계산",
    desc: "3개월 총임금 ÷ 3개월 총 일수(보통 89~92일)예요. 달력에서 직접 세는 게 정확해요. 2월이 포함된 경우 총 일수가 달라지니 주의하세요.",
    tip: "편의상 91일로 계산하면 크게 차이 안 나요",
  },
  {
    title: "퇴직금 공식 적용",
    desc: "1일 평균임금 × 30 × 근속연수예요. 근속기간이 딱 떨어지지 않으면 일 단위로 계산해요. 예: 근속 547일이면 547÷365=1.498년이에요.",
    tip: "소수점 근속 기간도 일 단위로 환산해야 정확",
  },
  {
    title: "세금 계산 및 IRP 확인",
    desc: "퇴직금에서 퇴직소득세가 차감돼요. 근속기간이 길수록 세금이 적어요. 퇴직금이 300만원을 넘으면 IRP 계좌로만 받을 수 있어요.",
...
const FAQS = [
  {
    ...
    a: "1일 평균임금을 30일로 곱해 한 달치 임금을 산정하기 위해서예요. 실제 달의 일수(28~31일)가 아니라 법정 기준 30일이에요.",
  },
  {
    q: "상여금을 1년에 한 번 받으면 어떻게 계산하나요?",
    a: "연간 상여금 ÷ 12로 월 환산해요. 예를 들어 연 상여금 600만원이면 월 50만원을 3개월 임금에 포함시켜요. 지급 시기와 관계없이 월 환산 포함이에요.",
  },
  {
    q: "3개월 총 일수가 왜 중요한가요?",
    a: "동일한 3개월치 임금이어도 총 일수가 89일인지 92일인지에 따라 1일 평균임금이 달라져요. 2월이 포함되면 총 일수가 줄어 평균임금이 높아질 수 있어요.",
  },
  {
    q: "입사일 바로 당일부터 근속기간이 시작되나요?",
    a: "맞아요. 입사일 당일부터 계산해요. 퇴직일은 마지막 근무일 포함이에요. 정확한 날짜는 고용24에서 확인할 수 있어요.",
  },
  {
    q: "공식 계산과 회사 계산이 다르면 어떻게 하나요?",
    a: "급여명세서를 기준으로 직접 계산한 뒤 차액을 확인해요. 10만원 이상 차이가 나면 고용노동부에 진정을 낼 수 있어요.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} currentSlug="퇴직금-계산-평균임금-공식" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금 법정 공식은 <strong>1일 평균임금 × 30일 × 근속연수</strong>예요.
      ...
      <p style={body}>
        ...
        기본급, 정기 상여금(월 환산), 연차수당, 고정수당이 포함되고, 실비 변상 항목이나 일시적으로 지급된 금품은 제외돼요.
        상여금을 분기별로 받거나 연 1회 받는 경우에는 연간 총액을 12로 나눠 월 환산해서 포함해야 해요.
      </p>
      <p style={body}>
        회사에서 기본급만으로 퇴직금을 계산하는 경우가 꽤 있어요.
        이때는 상여금 월 환산액을 직접 계산해서 차이를 확인하고, 수정 요청을 해야 하죠.
        아래 계산기에서 3개월 총임금(상여금 환산 포함)을 넣어보세요.
      </p>

      <GreenBox title="평균임금 공식 한 줄 정리">
        <strong>1일 평균임금 = 3개월 총임금 ÷ 3개월 총 일수(89~92일)</strong><br />
        <strong>퇴직금 = 1일 평균임금 × 30 × 근속연수</strong>
      </GreenBox>

      <SectionBadge>내 상황 체크해보세요</SectionBadge>
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="공식 적용에 필요한 항목을 갖추고 있어요. 아래 계산기로 직접 확인해보세요."
        partialMatchText="일부 항목이 다를 수 있어요. 고용노동부(1350) 상담을 권해요."
      />

      <Divider />

      <H2>3개월 총임금으로 바로 계산해보세요</H2>
      <p style={body}>
        3개월 급여명세서를 합산한 총임금과 근속기간을 입력하면 퇴직금을 바로 계산할 수 있어요.
        상여금이 있다면 연간 총액 ÷ 12 × 3을 더해서 총임금에 포함시키면 더 정확해요.
      </p>
      ...
      <p style={body}>
        ...
        상여금 지급 규정이 있으면 포함 여부 판단에 도움이 되고, 분쟁 시 증빙자료가 돼요.
      </p>
      ...
      <p style={body}>
        ...
        상여금 월 환산과 3개월 총 일수만 정확히 처리하면 나머지는 공식 대입이에요.
      </p>
      ...
      <GreenBox title="공식보다 중요한 건 포함 항목">
        공식 자체는 단순해요. 핵심은 어떤 항목이 3개월 총임금에 들어가느냐예요.
