"use client";
// Q1. 국민은행 월급통장을 쓰는 직장인이 퇴직을 앞두고, 퇴직금을 어떻게 받는지 몰라서 검색 중
// Q2. KB스타뱅킹 앱으로 IRP 계좌 개설 → 인사팀 계좌번호 통보 → 퇴직금 수령
// Q3. 300만원 초과 IRP 의무 이전, 앱 개설 10분, 14일 지급기한 + 연 20% 지연이자, 일시금 vs 연금 세금 30% 차이, 원리금보장형 수수료 0%
// Q4. EligibilityChecker(준비 여부) + Steps(4단계) + Calculator(세금 비교) + Checklist(놓치면 안 되는 항목) + FAQ
// MAP: Q1→서론 첫 문장(퇴직 앞두고 막막한 상황) Q2→H2순서(개설→통보→수령→절세) Q3→H2깊이(각 단계 구체적 설명) Q4→컴포넌트 배치
...
const CHECK_ITEMS = [
  ...
  { id: "c3", label: "KB국민은행 IRP 계좌를 개설했거나 개설 가능한 상태예요" },
...
const STEPS = [
  ...
  },
  {
    ...
    desc: "퇴직이 확정되면 IRP 계좌번호(은행명·계좌번호·예금주)를 인사팀에 문자나 메일로 알려줘요. 회사는 퇴직일로부터 14일 이내에 이 계좌로 퇴직금을 이체해야 해요. 14일을 넘기면 연 20% 지연이자 청구가 가능해요.",
    tip: "구두보다 문자·메일로 통보해야 증거가 남아요",
  },
  {
    title: "운용 상품 선택",
    desc: "퇴직금이 IRP에 들어오면 어떻게 운용할지 선택해야 해요. 원리금보장형(예금·채권)과 실적배당형(ETF·펀드) 중 고를 수 있고, 운용 지시 없이 두면 기본 원리금보장형으로 배정돼요.",
    tip: "단기 보유라면 원리금보장형, 장기 운용이라면 ETF 분산 투자도 고려해보세요",
  },
  {
    title: "일시금 인출 또는 연금 수령 선택",
    desc: "IRP에서 바로 인출하면 퇴직소득세가 부과돼요. 55세 이후 연금으로 받으면 퇴직소득세의 30%를 줄일 수 있죠. 인출은 KB스타뱅킹 앱 또는 영업점에서 신청해요.",
    tip: "55세 미만에 중도 해지하면 기타소득세(16.5%)가 추가 부과될 수 있으니 주의하세요",
  },
];
...
const FAQS = [
  {
    ...
    a: "원리금보장형은 수수료 0%예요. 실적배당형(ETF·펀드)은 연 0.2~0.5% 수준이고요. 퇴직금 수령만 목적이라면 수수료 부담이 없어요. 증권사 IRP보다 수수료가 약간 높을 수 있으니 비교해보는 게 좋아요.",
  },
  {
  ...
  },
  {
    ...
    a: "퇴직소득세가 부과돼요. 근속기간이 길수록 세금이 적죠. 55세 이후 연금으로 받으면 퇴직소득세의 30%를 절감할 수 있고요.",
  },
  {
    q: "KB국민은행 IRP에서 ETF에 투자할 수 있나요?",
    a: "가능해요. KB스타뱅킹 앱에서 ETF·펀드·예금 등 다양한 상품을 선택해 운용할 수 있죠. 실적배당형은 원금 손실 가능성이 있으니 본인 투자 성향에 맞게 선택하세요.",
  },
  {
  ...
  {
    q: "퇴직 후에 IRP를 개설해도 되나요?",
    a: "가능하지만, 퇴직 전에 미리 만들어두는 게 낫죠. 퇴직 후 개설하면 회사가 계좌를 확인하는 데 시간이 걸려서 14일 이내 이체가 늦어지는 경우가 생겨요. 미리 개설해서 계좌번호를 인사팀에 전달하는 게 가장 안전해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직을 앞두고 퇴직금을 어떻게 받는지 막막하죠.
        퇴직금이 300만원을 넘으면 <a href="/w/퇴직금-IRP-계좌" style={{ color: "#1D9E75", textDecoration: "underline" }}>IRP 계좌</a>로만 받을 수 있고,
        KB국민은행 IRP는 KB스타뱅킹 앱으로 10분이면 개설돼요.
        55세 이후 연금으로 받으면 <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a>의 30%를 줄일 수 있죠.
      ...
      <H2>내가 지금 받을 수 있는 상태인지 먼저 체크해요</H2>
      <p style={body}>
        퇴직금이 300만원을 초과하면 <a href="/w/퇴직금-irp-의무" style={{ color: "#1D9E75", textDecoration: "underline" }}>IRP 계좌 이전이 의무</a>예요.
        KB국민은행 IRP는 기존 국민은행 계좌가 있으면 앱에서 바로 연동되고, 오프라인 영업점도 전국에 많아서 접근성이 좋죠.
        원리금보장형을 선택하면 수수료가 0%라서 퇴직금 수령만 목적이라면 비용 부담이 없어요.
      </p>
      <p style={body}>
        퇴직 전에 IRP를 미리 만들어두는 게 핵심이에요.
        퇴직 후에 개설하면 회사가 계좌를 확인하는 데 시간이 걸려서 14일 이내 이체가 늦어질 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="국민은행 IRP로 퇴직금을 받을 준비가 됐어요. 아래 계산기로 세후 수령액을 계산해보세요."
        partialMatchText="조건이 다를 수 있으니 KB국민은행(1588-9999) 또는 금융감독원(1332)에 상담해보세요."
      />

      <Divider />

      <H2>일시금 vs 연금 수령, 세금 차이를 계산해봐요</H2>
      <p style={body}>
      ...
      <p style={body}>
        ...
        퇴직 확정 전에 미리 개설해두는 게 가장 중요한 포인트예요.
      </p>
      ...
      <p style={body}>
        14일 이내 입금이 안 되면 연 20% 지연이자 청구가 가능해요.
        날짜를 체크해두고, 미입금 시 바로 인사팀에 연락하세요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 소득세법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 금융감독원(1332)에서 확인해주세요." />
    </ArticleLayout>
  );
}
