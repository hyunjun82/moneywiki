...
// Q1. 퇴직금을 일시금으로 받은 직후, 원천징수된 세금이 맞게 계산된 건지 의심스러운 상황이에요.
// Q2. 근속연수 공제가 제대로 적용됐는지 직접 계산해보고, 과다 납부 시 경정청구까지 할 수 있는 행동이에요.
// Q3. 근속연수 구간별 공제 금액, 분리과세 구조, IRP 연금 절세 방법, 경정청구 기한 5년이에요.
// Q4. 계산기(퇴직소득세 예상), GreenBox(공제 기준표), Steps(납부 절차), Checklist(최종 점검)
//
// MAP:
// - Q1 → 서론 톤: "세금이 맞게 나왔을까?" 의심에서 시작
// - Q2 → H2 순서: 계산 구조 → 직접 계산 → 서류 → 납부 절차 → 체크리스트
// - Q3 → H2 개수/깊이: 5개 섹션, 공제 기준 수치 상세 서술
// - Q4 → 계산기+GreenBox+Steps+Checklist+DocTable 배치
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직금이 IRP 계좌에 입금됐어요" },
  { id: "c2", label: "퇴직 후 IRP 계좌로 퇴직금이 들어왔어요" },
  { id: "c3", label: "근속연수와 퇴직소득을 알고 있어요" },
...
const STEPS = [
  {
    ...
    desc: "회사 인사팀에 퇴직소득 원천징수영수증을 요청해요. 퇴직 후에도 재발급 요청이 가능하고, 홈택스에서도 조회할 수 있죠. 이 영수증에 근속연수 공제와 실납부 세액이 모두 나와요.",
    tip: "퇴직 직후 받아두는 게 좋아요. 경정청구 기한(5년)이 지나기 전에 챙기세요",
    link: { label: "홈택스에서 조회하기", href: "https://www.hometax.go.kr" },
  },
  {
    title: "근속연수 공제 적용 확인",
    desc: "영수증에서 근속연수 공제가 맞게 적용됐는지 직접 계산해봐요. 5년 이하 30만원/년, 10년 이하 50만원/년(150만원 기본), 20년 이하 80만원/년(400만원 기본), 20년 초과 120만원/년(1,200만원 기본)이 기준이에요.",
    tip: "중간정산 이력이 있으면 정산 이후 근속연수로 다시 계산해야 해요",
  },
  {
    title: "세액 이의 있으면 경정청구",
    desc: "공제가 잘못 적용됐거나 세금이 과다하게 나왔다면 홈택스에서 경정청구를 진행해요. 납부 기준일로부터 5년 이내면 언제든 신청 가능하고, 과다 납부 금액을 환급받을 수 있죠.",
    tip: "홈택스 메뉴: 세금신고 → 경정청구",
    link: { label: "경정청구 신청하기", href: "https://www.hometax.go.kr" },
  },
  {
    title: "IRP 연금 수령 절세 여부 판단",
    desc: "IRP 계좌에 퇴직금이 있다면 55세 이후 연금으로 수령하면 퇴직소득세를 30~40% 줄일 수 있죠. 일시금으로 빼면 감면 없이 세금 전액을 납부해요. 퇴직금 규모가 클수록 절세 금액 차이가 커요.",
...
const CHECKLIST = [
  ...
  "근속연수 공제: 5·10·20년 구간별 적용 여부 직접 계산",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "국세청 홈택스에서 경정청구를 하면 돼요. 근속연수 공제 계산 오류, 중간정산 이력 처리 누락 등이 원인인 경우가 많아요. 납부일로부터 5년 이내면 신청할 수 있죠.",
  },
];
...
export default function Page() {
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        회사가 원천징수로 처리해주지만, 공제가 제대로 적용됐는지는 본인이 직접 살펴봐야 해요.
        잘못 적용됐으면 5년 이내에 경정청구로 환급받을 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        두 단계 공제를 거치면 최종 실질 세율이 퇴직금의 5~10% 수준까지 낮아지는 경우가 많죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직소득세를 확인해야 하는 상황이에요. 아래 계산기로 예상 세금을 먼저 살펴보세요."
        partialMatchText="상황에 따라 다를 수 있죠. 국세청(126) 또는 세무사 상담을 권해요."
      />
      ...
      <p style={body}>
        월 평균급여와 근속 기간을 조정하면 예상 퇴직금, 근속연수 공제액, 퇴직소득세 참고값을 바로 볼 수 있죠.
        실제 세액은 환산급여공제 계산이 별도로 적용돼서 아래 수치보다 낮게 나오는 경우가 많죠.
        정확한 금액은 <a href="https://www.hometax.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>홈택스 퇴직소득 모의계산</a>을
        통해 직접 계산해보세요.
      </p>
      ...
      <Calculator
        ...
        note="※ 환산급여공제 미적용 참고값. 실제 세금은 홈택스 모의계산으로 계산하세요."
      />

      <p style={body}>
        근속 기간 슬라이더를 조정해보면 공제액이 늘어나면서 세금이 줄어드는 게 보여요.
        같은 퇴직금이라도 10년과 20년 근속의 세금 차이가 큰 게 바로 근속연수 공제 효과예요.
      </p>
      ...
      <p style={body}>
        퇴직소득세는 회사가 원천징수로 처리해요. 별도 신고는 필요 없지만, 공제가 제대로 적용됐는지 보려면
      ...
      <p style={body}>
        경정청구는 납부일로부터 5년 이내면 언제든 신청할 수 있죠.
      ...
      <H2>퇴직소득세 납부 절차</H2>
      <p style={body}>
        대부분은 회사가 처리해줘서 별도 행동이 필요 없지만, 세금이 제대로 계산됐는지 직접 따져보는 두 번째 단계가 중요해요.
      ...
      <p style={body}>
        ...
        중간정산 이력이 있거나 근속기간 계산에 오류가 있었다면 납부 세금을 돌려받을 수 있죠.
        퇴직 직후에 챙기는 게 편해요.
      </p>
      ...
      <p style={body}>
        회사가 처리해주는 부분도 있지만, 직접 챙겨야 하는 항목도 있죠.
        놓치면 불필요하게 세금을 더 낼 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        퇴직금이 5,000만원 이상이고 55세까지 시간 여유가 있다면 연금 수령 방식을 진지하게 검토할 만하죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 소득세법을 바탕으로 작성됐어요. 세법 개정이 있을 수 있으니 최신 기준은 국세청(126)에서 직접 확인해 주세요." />
    </ArticleLayout>
  );
}
