...
const STEPS = [
  ...
  {
    title: "합의서 내용 꼼꼼히 검토",
    desc: "합의서에 '지연이자 포기' 문구가 있는지 반드시 확인해요. 포기 문구가 없으면 합의 기간이 지난 이후부터 연 20% 지연이자를 청구할 수 있어요. 포기 문구가 있고 자발적 서명이면 이자 청구가 제한될 수 있어요.",
  ...
  },
  {
    ...
    desc: "지급이 계속 없으면 사업장 관할 지방고용노동청에 임금체불 진정을 접수해요. 진정서에 '지연이자 연 20% 청구'를 명시해야 이자도 처리돼요. 3,000만원 이하라면 법원 소액심판도 이용할 수 있어요.",
...
const CHECKLIST = [
  ...
  "지연이자 포기 조항 확인: 포기 문구 없으면 합의 기한 후 이자 청구 가능",
  "합의서 사본 보관: 서명 전에 반드시 사본을 받아두세요",
...
const FAQS = [
  {
    ...
    a: "거부할 수 있어요. 연장 합의는 근로자 자유의사로 이뤄져야 해요. 거부하면 14일 기한이 그대로 적용되고, 초과 시 연 20% 지연이자를 청구할 수 있어요.",
  },
  {
    q: "합의서에 서명했어도 지연이자를 받을 수 있나요?",
    a: "합의서에 지연이자 포기 문구가 없다면 청구 가능해요. 포기 문구가 있더라도 강요로 서명했다면 무효 주장이 가능해요. 합의서 내용을 먼저 확인하세요.",
  },
  {
  ...
  },
  {
    ...
    a: "없어요. 근로기준법 제36조는 서면 합의를 요건으로 해요. 전화나 말로 한 합의는 인정되지 않아요. 14일 기한이 그대로 적용돼요.",
  },
  {
    q: "퇴직 후에 사업주가 연장 합의서를 가져왔어요.",
    a: "퇴직 후 사후 합의는 법적 효력이 없어요. 이미 14일 기한이 적용되는 상황이라면 14일 경과분부터 연 20% 지연이자를 청구할 수 있어요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        단, 사업주와 근로자가 서면으로 합의하면 기한을 연장할 수 있고, 합의 기간 내에는 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>지연이자</a>가 붙지 않아요.
        하지만 합의서에 '지연이자 포기' 문구가 있는지, 합의가 퇴직 전에 이뤄진 건지를 반드시 확인해야 해요.
        조건을 모르고 서명하면 정당한 권리를 잃을 수 있어요.
      </p>
      ...
      <SectionBadge>합의 유효성 체크해보세요</SectionBadge>
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="합의 기한 초과분에 대해 지연이자 청구가 가능해요. 아래 계산기로 금액을 확인해보세요."
        partialMatchText="상황에 따라 다를 수 있어요. 합의서 내용과 서명 시점을 먼저 확인하세요."
      />
      ...
      <p style={body}>
        합의서는 양측이 서명한 서면이어야 해요. 작성 후 반드시 사본을 보관하세요.
        합의서 외에도 퇴직 확인서와 급여명세서를 챙겨야 퇴직금 금액을 입증할 수 있어요.
      ...
      <p style={body}>
        합의 여부를 확인하고, 기한이 지나도 지급이 없으면 지급 요청 후 노동청에 진정을 넣으면 돼요.
        진정서에 지연이자를 명시해야 이자도 받을 수 있어요.
      </p>
      ...
      <p style={body}>
        합의서에 서명하기 전에 지연이자 포기 조항이 없는지 꼭 확인하세요.
        서명 후에도 강요가 있었다면 무효 주장이 가능하고, 합의 기한 후 이자는 청구할 수 있어요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법 제36조·제37조를 바탕으로 작성됐어요. 개별 사안에 따라 적용이 달라질 수 있으니 고용노동부(1350) 또는 관할 노동청에서 확인하세요." />
    </ArticleLayout>
  );
}
