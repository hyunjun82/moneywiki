...
const CHECK_ITEMS = [
  { id: "c1", label: "계약직(기간제)으로 근무하고 있거나 근무했어요" },
  { id: "c2", label: "같은 사업장에서 1년 이상 계속 근무했어요" },
  { id: "c3", label: "4주 평균 주 소정 근로시간이 15시간 이상이에요" },
  { id: "c4", label: "계약 종료일로부터 아직 3년이 지나지 않았어요" },
];
...
const STEPS = [
  {
    title: "계속근로 기간 확인",
    desc: "입사일부터 계약 종료일까지 정확히 계산해요. 계약을 반복 갱신했다면 공백 없이 이어진 전체 기간을 합산해요. 고용24(ei.go.kr)에서 고용보험 가입 이력으로 입사일을 확인할 수 있어요.",
    tip: "계약 사이에 공백이 있어도 동일 업무·동일 장소라면 실질로 판단해요.",
  },
  {
    title: "퇴직금 금액 계산",
    desc: "1일 평균임금 × 30일 × 근속연수로 계산해요. 평균임금은 퇴직 전 3개월 급여 총액을 해당 기간 총 일수로 나눈 금액이에요. 상여금이 있다면 연간 총액을 12로 나눠서 월 급여에 더하세요.",
    tip: "상여금 환산: 연간 상여금 총액 ÷ 12 = 월 환산액",
  },
  {
    title: "IRP 계좌 개설 및 지급 요청",
    desc: "퇴직금 300만원 초과 시 IRP 계좌로만 받아야 해요. 은행·증권사·보험사 앱에서 무료로 열 수 있어요. 계좌번호를 인사팀에 문자나 메일로 전달하면 계약 만료 후 14일 이내에 이체돼요.",
    tip: "계약 만료 전에 미리 IRP를 개설해두면 지급 지연을 막을 수 있어요.",
    link: { label: "IRP 계좌 개설 방법", href: "/w/퇴직금-IRP-계좌" },
  },
  {
    title: "미지급 시 고용노동부 신고",
    desc: "14일이 지났는데도 퇴직금이 안 들어오면 고용노동부 민원마당(minwon.moel.go.kr)에서 임금체불 진정을 접수해요. 지연이자(연 20%)도 함께 청구할 수 있어요. 소멸시효는 퇴직일로부터 3년이에요.",
...
const CHECKLIST = [
  "계속근로 1년 확인: 입사일~종료일 정확히 계산 (갱신 합산)",
...
const FAQS = [
  {
    ...
    a: "공백 없이 이어졌다면 합산돼요. 최초 입사일부터 최종 계약 종료일까지 전체 기간이 근속기간이에요. 갱신 횟수는 관계없어요.",
  },
  {
    q: "계약 사이에 2주 공백이 있었는데 계속근로로 인정되나요?",
    a: "법원은 실질을 봐요. 동일 업무·동일 장소에서 계속 일했다면 짧은 공백은 계속근로로 인정될 수 있어요. 반대로 명확히 고용관계가 끊겼다면 공백 전후를 별도로 산정해요.",
  },
  {
    q: "계약 종료 후 바로 재계약하면 퇴직금은 어떻게 되나요?",
    a: "공백 없이 재계약하면 계속근로로 볼 수 있어요. 다만 실질적인 근로 단절이 있었다고 회사가 입증하면 계약별로 퇴직금이 정산돼요. 재계약 시 퇴직금 중간정산 여부를 꼭 확인하세요.",
  },
  {
    q: "4대보험에 미가입된 계약직인데 퇴직금을 받을 수 있나요?",
    a: "받을 수 있어요. 4대보험 가입 여부는 퇴직금 수령 조건과 무관해요. 실제 근무 사실이 증명되면 퇴직금 청구권이 생겨요. 문자, 카카오톡, 통장 입금 내역 모두 증거로 쓸 수 있어요.",
  },
  {
    q: "계약직 퇴직금 포기각서를 써줬는데 퇴직금을 못 받나요?",
    a: "받을 수 있어요. 퇴직금 포기각서는 법적 효력이 없어요. 근로자퇴직급여보장법에서 정한 권리는 계약이나 각서로 없애지 못해요. 서명했더라도 퇴직금 청구권은 그대로 유지돼요.",
  },
];
...
export default function Page() {
  return (
    ...
    >
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="계약직 퇴직금 발생 조건을 갖췄어요. 아래 계산기로 예상 금액을 확인해보세요."
        partialMatchText="조건 미충족 항목이 있어요. 1년 근속과 주 15시간은 필수예요."
      />

      <Divider />

      <H2>계약직 퇴직금 예상액, 직접 계산해보세요</H2>
      <p style={body}>
        퇴직금은 1일 평균임금 × 30일 × 근속연수로 계산해요.
        실제 퇴직금에는 기본급뿐 아니라 상여금도 포함돼요. 연간 상여금이 있다면 총액을 12로 나눠서 월 급여에 더하면 더 정확해요.
        계약 갱신이 있었다면 전체 근속 개월 수를 합산해서 입력하세요.
      </p>
      ...
      <p style={body}>
        퇴직금 청구에 복잡한 서류는 필요 없어요.
        근로계약서와 급여명세서만 있으면 기본적으로 처리할 수 있어요.
        4대보험 미가입 계약직이라도 실제 근무 사실을 증명하면 퇴직금 청구가 가능해요.
        문자, 카카오톡, 통장 입금 내역도 증거로 쓸 수 있어요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법과 근로기준법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부 고객상담센터(1350)에서 확인하세요." />
    </ArticleLayout>
  );
}
