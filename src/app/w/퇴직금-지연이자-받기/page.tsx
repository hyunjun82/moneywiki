...
// ─── Q1-Q4 필수 사고 ─────────────────────────────────
// Q1. 퇴직금을 받지 못한 지 14일이 넘었고, 지연이자가 붙는다는 건 알지만 어떻게 청구하는지 모르는 상황
// Q2. 지연이자 금액을 계산하고, 내용증명 → 노동청 진정 순서로 실제 청구를 완료한다
// Q3. 지연이자 발생 조건(14일 초과·서면 합의 없음), 계산 공식(연 20%·일할), 청구 4단계 절차, 필요 서류, 소멸시효 3년
// Q4. EligibilityChecker(자격 체크) + Calculator(금액 계산) + Steps(단계별 절차) + DocTable(서류) + Checklist(행동 정리)
//
// MAP:
// Q1 → 서론: 이자가 자동으로 들어오지 않는다는 사실 공감
// Q2 → H2 순서: 자격 체크 → 금액 계산 → 서류 준비 → 4단계 절차 → 체크리스트
// Q3 → H2 5개 + FAQ 6개
...
const STEPS = [
  {
    title: "지연이자 금액 계산",
    desc: "미지급 퇴직금 × 20% ÷ 365 × 지연 일수로 계산해요. 14일 초과 시점부터 실제 지급일까지 매일 이자가 쌓이죠. 퇴직금 1,500만원을 45일 지연하면 이자만 약 37만원이고요. 청구서 발송 전에 금액을 먼저 계산해두는 게 좋아요.",
    tip: "위 계산기로 지연이자를 바로 산출할 수 있죠",
  },
  {
    title: "내용증명으로 퇴직금 + 지연이자 청구",
    desc: "퇴직금 원금과 지연이자를 합산해서 내용증명으로 청구해요. '퇴직일로부터 14일이 경과했으므로 근로기준법 제37조에 따라 연 20% 지연이자를 포함한 OOO원을 청구해요'라고 명시하면 돼요. 내용증명은 소멸시효 3년도 중단시키고요.",
    tip: "우체국 방문 또는 카카오 전자내용증명으로 간편 발송 가능해요",
  },
  {
    title: "고용노동부 진정 신청",
    desc: "내용증명 발송 후 3~5영업일 내 응답이 없으면 고용노동부 민원마당에서 임금체불 진정을 접수해요. 진정 시 지연이자도 함께 청구할 수 있죠. 근로감독관이 조사 후 지급 명령을 내리고, 보통 2~4주 내 처리돼요.",
    tip: "minwon.moel.go.kr → 민원신청 → 임금체불 진정",
    link: { label: "고용노동부 민원마당 바로가기", href: "https://minwon.moel.go.kr" },
  },
  {
    title: "형사 고발 또는 민사 소송",
    desc: "진정 후에도 지급이 없으면 근로기준법 위반(2년 이하 징역 또는 2,000만원 이하 벌금)으로 형사 고발하거나 법원에 지급명령 신청을 낼 수 있죠. 대한법률구조공단(132)에서 무료 법률 지원을 받을 수 있고요.",
...
const FAQS = [
  {
    q: "지연이자는 자동으로 들어오나요?",
    a: "법적으로는 자동 발생하지만, 실제로 받으려면 직접 청구해야 해요. 퇴직금만 받고 넘어가면 이자는 포기하게 되죠. 내용증명이나 노동청 진정 시 지연이자를 명시해서 함께 청구해야 해요.",
  },
  {
    q: "퇴직금을 받았는데 지연이자도 따로 청구할 수 있나요?",
    a: "청구 가능해요. 지연이자는 퇴직금과 별도로 청구할 수 있죠. 퇴직금을 받았더라도 14일 초과분 이자는 따로 내용증명이나 노동청 진정으로 청구하면 받을 수 있고요.",
  },
  {
    q: "회사가 퇴직금만 주겠다고 하면 어떻게 하나요?",
    a: "지연이자는 근로기준법 제37조에 따라 법으로 보장된 권리예요. 이자 포기 서면 합의를 강요받았다면 무효 주장이 가능하죠. 서명하기 전에 고용노동부(1350)에 상담해봐요.",
  },
  {
    q: "IRP 계좌번호를 안 알려줬는데 지연이자를 받을 수 있나요?",
    a: "IRP 계좌번호를 알려주지 않으면 이체 자체가 불가능해서 지연 귀책이 근로자에게 생길 수 있고요. 먼저 IRP 계좌번호를 인사팀에 전달하고, 그 이후에도 입금이 없으면 지연이자를 청구할 수 있죠.",
  },
  {
    q: "소멸시효 3년이 지났으면 정말 못 받나요?",
    a: "원칙적으로 3년이 지나면 청구권이 소멸해요. 내용증명, 진정, 소송 등으로 시효를 중단시킬 수는 있죠. 3년이 임박했다면 내용증명을 먼저 발송해서 시효를 중단시키는 게 우선이에요.",
  },
  {
    q: "회사가 폐업했는데 지연이자도 받을 수 있나요?",
    a: "체당금 제도로 퇴직금 원금 일부는 받을 수 있고요. 지연이자는 체당금 지급 대상이 아닐 수 있죠. 고용노동부(1350) 또는 근로복지공단에서 체당금 신청 방법을 안내받아봐요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        이자는 자동으로 들어오지 않고, 직접 청구해야 받을 수 있고요.{" "}
        <a href="https://www.law.go.kr/법령/근로기준법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로기준법 제37조</a>에 명시된 권리이며, 소멸시효 3년 안에 청구하면 퇴직금과 별도로 받을 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        회사와 서면으로 기한 연장에 합의했다면 그 기간까지는 이자가 붙지 않죠.
        구두 합의만 있었다면 인정되지 않아서 14일 초과분부터 청구할 수 있고요.
      </p>
      <p style={body}>
        소멸시효는 퇴직일로부터 3년이에요. 3년이 지나면 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>지연이자</a>와 퇴직금 청구권이 함께 소멸하죠.
        기한이 임박했다면 내용증명 한 통만 보내도 소멸시효를 중단시킬 수 있고요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="지연이자 청구 및 노동청 신고가 가능한 상황이에요. 아래 계산기로 이자를 먼저 계산해봐요."
        partialMatchText="상황에 따라 다를 수 있고요. 고용노동부(1350) 상담을 먼저 받아봐요."
      />
      ...
      <p style={body}>
        미지급 퇴직금 금액과 14일을 초과한 지연 일수를 입력하면 청구 가능한 지연이자를 바로 산출할 수 있고요.
        이자는 퇴직금이 실제로 지급된 날까지 매일 쌓이는 구조이에요.
        퇴직금 1,500만원을 45일 지연하면 이자만 약 37만원이죠.
      </p>
      <p style={body}>
        이 금액이 내용증명과 노동청 진정서 작성 시 청구 근거 금액이 돼요.
        지급 전까지 날마다 이자가 늘어나니 빨리 행동하는 게 유리하죠.
      </p>
      ...
      <p style={body}>
        ...
        퇴직 날짜를 증명하는 사직서·퇴직확인서와 급여 수준을 파악할 수 있는 명세서가 핵심이에요.
        지급 요청 기록을 따로 보관해두면 노동청 진정 시 유리한 증거가 되고요.
      </p>
      ...
      <p style={body}>
        4단계예요. 지연이자 계산 → 내용증명 → 노동청 진정 → 형사 고발·소송 순서이고요.
        4단계까지 가는 경우는 드물고, 대부분 2~3단계에서 지급이 이뤄져요.
        각 단계에서 서면 기록을 남겨두는 게 핵심이죠.
      </p>
      ...
      <p style={body}>
        ...
        법으로 보장된 권리이고, 청구하지 않으면 자동으로 들어오지 않죠.
      ...
      <GreenBox>
        ...
        14일 초과분 지연이자는 따로 청구해야 받을 수 있고요.<br />
        내용증명 한 통으로 청구권과 소멸시효를 동시에 보전해요.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 안내받아봐요." />
    </ArticleLayout>
  );
}
