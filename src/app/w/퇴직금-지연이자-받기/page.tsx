...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직 후 14일이 지났는데 퇴직금이 아직 안 들어왔어요" },
  { id: "c2", label: "회사에 지급 요청했는데 묵묵부답이에요" },
...
const DOCS = [
  ...
  { name: "지급 요청 기록 (문자·메일 캡처)", required: false, where: "직접 보관" },
];

const STEPS = [
  {
    title: "지연이자 금액 계산",
    desc: "미지급 퇴직금 × 20% ÷ 365 × 지연 일수로 계산해요. 14일 초과 시점부터 실제 지급일까지 매일 이자가 쌓여요. 퇴직금 1,000만원을 45일 지연하면 이자만 약 24만원이에요. 청구서 발송 전에 금액을 먼저 계산해두세요.",
    tip: "위 계산기로 지연이자를 바로 확인할 수 있어요",
  },
  {
    title: "내용증명으로 퇴직금 + 지연이자 청구",
    desc: "퇴직금 원금과 지연이자를 합산한 금액을 내용증명으로 청구해요. '퇴직일로부터 14일이 경과했으므로 근로기준법 제37조에 따라 연 20% 지연이자를 포함한 OOO원을 청구합니다'라고 명시하면 돼요. 내용증명은 소멸시효도 중단시켜요.",
    tip: "우체국 방문 또는 카카오 전자내용증명으로 간편 발송 가능해요",
  },
  {
    title: "고용노동부 진정 신청",
    desc: "내용증명 발송 후 3~5영업일 내 응답이 없으면 고용노동부 민원마당에서 임금체불 진정을 접수해요. 진정 시 지연이자도 함께 청구할 수 있어요. 근로감독관이 조사 후 지급 명령을 내려요. 보통 2~4주 내에 처리돼요.",
    tip: "minwon.moel.go.kr → 민원신청 → 임금체불 진정",
  },
  {
    title: "형사 고발 또는 민사 소송",
    desc: "진정 후에도 지급이 없으면 근로기준법 위반(2년 이하 징역 또는 2,000만원 이하 벌금)으로 형사 고발하거나 법원에 지급명령 신청을 낼 수 있어요. 법률구조공단(132)에서 무료 법률 지원을 받을 수 있어요.",
    tip: "형사 고발 직후 합의가 이뤄지는 경우가 많아요",
  },
];

const CHECKLIST = [
  "지연이자 계산: 14일 초과분부터 연 20% 일할 계산",
  "내용증명 발송: 퇴직금 원금 + 지연이자 합산 청구",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "지연이자는 근로기준법 제37조에 따라 법으로 보장된 권리예요. 퇴직금만 받고 이자를 포기하는 서면 합의를 강요받았다면 무효 주장이 가능해요. 노동청(1350)에 상담해보세요.",
  },
  {
  ...
  {
    q: "노동청 진정에서 지연이자도 같이 받을 수 있나요?",
    a: "네, 진정 시 퇴직금 원금과 지연이자를 함께 청구할 수 있어요. 진정서 작성 시 지급 기한 초과 일수와 계산된 이자 금액을 명시해두면 더 유리해요.",
  },
  {
...
const RELATED = [
  { slug: "퇴직금-지연이자", title: "퇴직금 지연이자 연 20% 계산법", description: "지연이자가 발생하는 기준과 계산 공식을 정리했어요." },
  { slug: "퇴직금-지급-기한-초과", title: "퇴직금 지급 기한 14일 초과 대응", description: "기한 초과 시 단계별 대응 방법을 안내해요." },
  { slug: "퇴직금-지급-청구-방법", title: "퇴직금 지급 청구 방법", description: "내용증명부터 노동청 신고까지 청구 절차를 설명해요." },
];

export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} currentSlug="퇴직금-지연이자-받기" />}
    >
      <p style={{ fontSize: 13, color: "#1D9E75", fontWeight: 600, marginBottom: 10 }}>퇴직금 · 지연이자 · 청구방법</p>

      <h1 style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.45, marginBottom: 14 }}>
        퇴직금 지연이자, 어떻게 받나요?<br />
        청구 방법과 노동청 신고까지 단계별 정리
      </h1>

      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금을 14일 안에 못 받았다면 <a href="/w/퇴직금-지연이자" style={{ color: "#1D9E75", textDecoration: "underline" }}>지연이자(연 20%)</a>를 청구할 권리가 있어요.
        이자는 자동으로 들어오지 않아요. 직접 청구해야 받을 수 있어요.
        <a href="https://www.law.go.kr/법령/근로기준법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로기준법 제37조</a>에 명시된 권리이니 포기하지 마세요.
      </p>

      <Divider />
      <ArticleAd position="intro" />

      <H2>지연이자 청구 가능한 상황인지 확인해보세요</H2>
      <p style={body}>
      ...
      <p style={body}>
        소멸시효는 퇴직일로부터 3년이에요. 3년이 지나면 퇴직금과 지연이자 청구권 모두 소멸해요.
        내용증명 발송만으로도 소멸시효를 중단시킬 수 있어요.
      </p>
      ...
      <H2>지연이자가 얼마인지 계산해보세요</H2>
      <p style={body}>
        미지급 퇴직금 금액과 14일을 초과한 지연 일수를 입력하면 청구 가능한 지연이자를 바로 확인할 수 있어요.
        지연이자는 퇴직금이 실제로 지급된 날까지 매일 쌓여요.
        퇴직금 1,000만원을 45일 지연하면 이자만 약 24만원이에요.
      </p>
      ...
      <p style={body}>
        ...
        핵심은 퇴직 날짜를 증명하는 서류와 급여 수준을 확인할 수 있는 명세서예요.
      ...
      <p style={body}>
        지연이자 계산 → 내용증명 청구 → 노동청 진정 → 형사 고발·소송 순서예요.
      ...
      <H2>지연이자 청구 전 체크리스트</H2>
      <p style={body}>
        퇴직금만 받고 지연이자를 포기하는 경우가 많아요.
        법으로 보장된 권리이니 꼭 함께 청구하세요.
      ...
      <GreenBox title="지연이자는 청구해야 받을 수 있어요">
        퇴직금이 들어왔다고 끝이 아니에요.<br />
