...
const CHECK_ITEMS = [
  ...
  { id: "c4", label: "DC형 퇴직연금 또는 퇴직금 제도(DB형 제외)를 적용받고 있어요" },
];
...
const STEPS = [
  ...
  {
    title: "사유별 증빙서류 준비",
    desc: "사유마다 필요한 서류가 달라요. 주택 구입이면 매매계약서, 전세면 임대차계약서와 주민등록등본, 요양이면 진단서가 필수예요. 서류가 불완전하면 회사가 처리를 미룰 수 있으니 한 번에 완비하는 게 유리해요. 가족관계증명서, 주민등록등본은 정부24에서 온라인으로 바로 발급받을 수 있어요.",
    tip: "서류가 완비되면 회사는 정당한 이유 없이 거부할 수 없어요",
    link: { label: "정부24 서류 발급", href: "https://www.gov.kr" },
  },
  {
    title: "회사(또는 금융기관)에 신청서 제출",
    desc: "신청서와 증빙서류를 인사팀에 제출해요. DC형 퇴직연금이라면 회사뿐 아니라 퇴직연금 운용 금융기관에도 별도로 신청해야 할 수 있어요. 처리 기간은 회사마다 다르지만 보통 2~4주 내외예요. 퇴직금 제도(DB형 아님)면 회사에서 직접 지급해요.",
  ...
  },
  {
    ...
    desc: "중간정산 후엔 퇴직금 계산의 기산점이 중간정산 시점으로 리셋돼요. 이후 퇴직할 때 받는 퇴직금은 중간정산 이후 기간만 기준이 되죠. 임금이 오르는 직장이라면 중간정산 없이 퇴직 시까지 놔두는 게 최종 퇴직금이 더 클 수 있어요.",
...
const CHECKLIST = [
  ...
  "증빙서류 완비: 서류가 빠지면 회사가 처리를 미룰 수 있어요",
  "DC형이면 금융기관도 별도 신청 필요해요",
  "세금 처리: 중간정산 금액에도 퇴직소득세가 원천징수돼요",
  "IRP 이전 여부: 수령 전에 결정해야 해요. 지급 후 변경 불가예요",
  "기산점 초기화: 중간정산 후엔 새로운 기산점부터 다시 계산돼요",
  "임금 상승 예상 시: 퇴직 시까지 놔두는 게 더 유리할 수 있어요",
];

const FAQS = [
  {
    q: "중간정산을 요청하면 회사가 거부할 수 있나요?",
    a: "법정 사유가 명확하고 증빙서류를 갖추면 회사는 정당한 이유 없이 거부할 수 없어요. 근로자퇴직급여 보장법 제8조에서 이를 보장해요. 단, 서류가 부족하거나 사유가 법정 범위 밖이면 회사가 반려할 수 있어요.",
  },
  {
    q: "DB형 퇴직연금도 중간정산이 되나요?",
    a: "DB형은 원칙적으로 중간정산이 안 돼요. 중간정산은 DC형 퇴직연금 또는 퇴직금 제도에서만 가능해요. 내 퇴직연금 유형이 DB형이라면 고용노동부에 예외 적용 여부를 상담해보세요.",
  },
  {
    q: "중간정산 후 퇴직금이 줄어드나요?",
    a: "기산점이 초기화되기 때문에 최종 퇴직금에서 중간정산 기간 분이 빠져요. 임금이 계속 오르는 직장이라면 퇴직 시 마지막 임금 기준으로 전체 기간 퇴직금을 받는 게 더 크죠. 반대로 임금이 줄어들 상황이라면 중간정산이 유리할 수 있어요.",
  },
  {
  ...
  },
  {
    ...
    a: "퇴직소득세가 원천징수돼요. 세율은 근속기간과 금액에 따라 달라요. 근속기간이 길수록 환산급여 공제가 커져서 실제 세율이 낮아지는 구조예요. IRP로 이전하면 퇴직소득세를 나중에 낼 수 있어요(과세 이연).",
  },
  {
    q: "이미 중간정산을 받은 적이 있는데 또 받을 수 있나요?",
    a: "법정 사유가 다시 발생하면 가능해요. 이전에 주택 구입으로 한 번 받았더라도, 이번에 부양가족 요양이 새로 생기면 다시 신청할 수 있어요. 다만, 같은 사유로 중복 신청은 안 돼요.",
  },
  {
    q: "파산 신청을 했는데 중간정산을 받을 수 있나요?",
    a: "파산선고 또는 개인회생 인가결정이 났다면 법정 사유에 해당해요. 법원에서 발급한 결정문을 회사에 제출하면 신청할 수 있어요.",
  },
];
...
const RELATED = [
  ...
  { slug: "퇴직금-중간정산-사유별-증빙서류", title: "중간정산 사유별 필요 서류 목록", description: "사유에 따라 서류가 달라요. 미리 확인하고 준비하세요." },
  { slug: "퇴직금-세금", title: "퇴직금 세금 얼마나 내나요", description: "중간정산 시 퇴직소득세 계산 방법과 절세 전략." },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        정한 법정 사유에 해당하면 재직 중에 미리 받을 수 있어요.
      ...
      <p style={body}>
        ...
        이 사유들을 근로자퇴직급여 보장법 시행령 제3조에서 구체적으로 규정해요.
      </p>
      <p style={body}>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="중간정산 신청 조건을 갖추고 있어요. 아래 계산기로 예상 금액을 먼저 확인해보세요."
        partialMatchText="조건이 일부 달라요. 고용노동부 고객상담센터(1350)에 먼저 상담받아보세요."
      />
      ...
      <p style={body}>
        ...
        평균임금은 최근 3개월치 임금을 기준으로 하고, 상여금과 연차수당도 포함될 수 있어요.
      </p>
      <p style={body}>
        중간정산 후엔 기산점이 리셋되고 이후 근속분부터 다시 쌓여요. 임금이 꾸준히 오르는 직장이라면 중간정산이 불리할 수 있어요.
        퇴직 시 더 높아진 임금을 기준으로 전체 근속 기간 퇴직금을 받는 게 더 크죠.
        아래 계산기로 두 시나리오를 비교해보세요.
      </p>
      ...
      <Calculator
        ...
        note="※ 월 평균임금 × 근속연수 기준 추정치예요. 상여금·연차수당 포함 시 실제 금액이 달라질 수 있어요. 중간정산 후 기산점이 리셋돼요."
      />
      ...
      <p style={body}>
        중간정산에서 서류 준비가 가장 중요해요. 사유에 따라 필요한 서류가 달라지기 때문에,
        미리 파악하지 않으면 신청 후 반려되거나 처리가 늦어질 수 있어요.
      </p>
      <p style={body}>
        공통으로 급여명세서 최근 3개월분과 재직증명서가 필요해요.
        가족관계증명서·주민등록등본은 정부24(gov.kr)에서 무료로 온라인 발급받을 수 있어요.
        법원 서류가 필요한 파산·회생 사유는 법원 인터넷 등기소에서 발급받으면 돼요.
      </p>
      ...
      <p style={body}>
        ...
        단계별로 준비하면 처리가 빨라지고 반려될 가능성이 낮아요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여 보장법 및 동법 시행령을 바탕으로 작성됐어요. 법정 사유나 절차는 변경될 수 있으니 최신 기준은 고용노동부(1350)에서 확인하세요." />
    </ArticleLayout>
  );
}
