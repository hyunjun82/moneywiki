  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        "실업급여를 최대로 받으면 총 얼마나 되죠?"<br />
        2026년 기준 <strong>1일 상한액은 68,100원</strong>이에요. 월로 환산하면 약 204만원이죠.<br /><br />
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서 정한
        상한액에 최대 수급일수 270일을 곱하면 총 18,387,000원이 나오죠.
        다만 이 금액을 전부 받으려면 나이와 가입기간 조건을 모두 충족해야 해요.
      </p>
      ...
      <p style={body}>
        계산 공식은 단순해요. <strong>1일 수급액 x 수급일수</strong>가 최대 수령 총액이죠.
        2026년 1일 상한액은 68,100원이에요. <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 매년 초에 고시하는 금액이에요.
        이 상한액은 퇴직 전 평균임금의 60%가 상한선을 넘을 때 적용되죠.
      </p>
      <p style={body}>
        수급일수는 나이와 고용보험 가입기간에 따라 120일부터 270일까지 달라져요.
        가장 긴 270일을 적용받으면 68,100원 x 270일 = <strong>18,387,000원</strong>이에요.
        약 9개월 동안 매달 204만원씩 받는 셈이죠.
      </p>
      <p style={body}>
        반대로 가장 짧은 120일만 인정받으면 68,100원 x 120일 = 8,172,000원이에요.
        같은 상한액이라도 수급일수 차이에 따라 총액이 1,000만원 이상 벌어지는 거예요.
        그래서 "내 수급일수가 며칠인지"를 먼저 파악하는 게 핵심이죠.
      </p>

      <GreenBox title="2026년 실업급여 최대금액 공식">
      ...
      <p style={body}>
        수급일수를 결정하는 두 가지 요소가 <strong>퇴직 당시 나이</strong>와 <strong>고용보험 피보험기간</strong>이에요.
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서 50세를 기준으로 수급일수를 나누고 있죠.
        50세 이상이거나 장애인이면 같은 가입기간이라도 수급일수가 더 길어요.
      </p>
      <p style={body}>
        구체적으로 보면, 가입기간 <strong>1년 미만</strong>이면 나이에 관계없이 120일이에요. 상한액 기준 최대 약 817만원이죠.
        <strong>1년~3년</strong>이면 50세 미만은 150일(약 1,022만원), 50세 이상은 180일(약 1,226만원)이에요.
        <strong>3년~5년</strong>이면 50세 미만 180일(약 1,226만원), 50세 이상 210일(약 1,430만원)이죠.
      </p>
      <p style={body}>
        <strong>5년~10년</strong>이면 50세 미만 210일(약 1,430만원), 50세 이상 240일(약 1,634만원)이에요.
        그리고 <strong>10년 이상</strong>이면 50세 미만 240일(약 1,634만원), 50세 이상은 270일(약 <strong>1,839만원</strong>)로 최대치에 도달하죠.
        50세 미만은 아무리 오래 일해도 240일이 천장이에요.
      </p>

      <BorderBox title="50세 기준 최대금액 비교">
        50세 미만, 10년 이상 → 240일 → 약 1,634만원<br />
        50세 이상, 10년 이상 → 270일 → 약 <strong>1,839만원</strong><br />
        차이: 30일분 = 약 204만원
      </BorderBox>
      ...
      <p style={body}>
        세 가지를 모두 갖춰야 해요. 첫째, <strong>고용보험 가입기간 10년 이상</strong>이에요.
        여러 직장에서 일해도 가입기간은 합산돼요. 다만 이전 직장에서 실업급여를 이미 수급했다면 그 이전 기간은 합산에서 빠지니까 주의하세요.
        중간에 1년쯤 쉬었다고 해서 기간이 리셋되지는 않아요.
      </p>
      <p style={body}>
        둘째, <strong>퇴직 시점 나이가 만 50세 이상</strong>이어야 해요.
        49세 364일에 퇴직하면 240일이 최대이고, 50세 생일 이후에 퇴직해야 270일을 적용받죠.
        딱 하루 차이로 약 204만원이 갈리는 셈이에요.
      </p>
      <p style={body}>
        셋째, <strong>비자발적 퇴직</strong>이어야 해요.
        해고, 권고사직, 계약만료, 폐업 등이 대표적이죠.
        자발적으로 그만뒀더라도 임금체불이나 직장 내 괴롭힘 같은 정당한 사유가 있으면 인정받을 수 있어요.
        정당한 사유에 해당하는지 애매하면 퇴직 전에 고용센터(1350)에 미리 상담하세요.
      </p>
      ...
      <p style={body}>
        네, 취업하는 순간 남은 수급일수의 실업급여는 소멸돼요.
        270일을 인정받았는데 150일째에 재취업했다면 나머지 120일분은 못 받죠.
        68,100원 x 120일 = 약 817만원이 사라지는 셈이에요.
      </p>
      <p style={body}>
        그런데 여기서 <strong>조기재취업수당</strong>이라는 제도를 활용할 수 있어요.
        수급일수의 절반 이상을 남기고 안정적인 직장에 취업하면, 남은 수급일수의 절반을 일시금으로 지급하죠.
        위 예시라면 120일의 절반인 60일분, 약 409만원을 한 번에 받을 수 있어요.
      </p>
      <p style={body}>
        조기재취업수당을 받으려면 몇 가지 조건을 갖춰야 해요.
        12개월 이상 고용이 보장되는 직장이어야 하고, 실업급여 신청 전에 채용이 확정된 곳은 안 되죠.
        자영업을 시작하는 경우에도 일정 요건을 충족하면 받을 수 있어요.
        빨리 재취업할수록 받는 금액이 커지니까, 구직활동을 미루지 않는 게 유리하죠.
      </p>
      ...
      <p style={body}>
        본인의 나이와 고용보험 가입기간만 알면 바로 계산할 수 있어요.
        고용보험 가입기간은 <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에 로그인해서 "피보험기간 조회"를 하면 정확히 나오죠.
        여러 직장의 기간이 자동으로 합산돼서 표시되니까 따로 계산할 필요 없어요.
      </p>
      <p style={body}>
        주의할 점은, 상한액을 받는다고 해서 모든 사람의 1일 수급액이 68,100원은 아니라는 거예요.
        실업급여 1일 지급액은 퇴직 전 평균임금의 60%인데, 이 금액이 상한액보다 높으면 상한액으로 잘리고, 하한액보다 낮으면 하한액(66,048원)으로 올라가죠.
        월 평균임금이 약 341만원 이상이었다면 상한액에 해당될 가능성이 높아요.
      </p>
      <p style={body}>
        정리하면, <strong>50세 이상 + 10년 이상 가입 + 비자발적 퇴직</strong>일 때 최대 18,387,000원을 받을 수 있어요.
        50세 미만이라면 최대 16,344,000원이 한도이고요.
        정확한 예상 수령액이 궁금하면 위의 계산기를 활용하거나 고용24의 모의계산 서비스를 이용해보세요.
      </p>
