...
const STEPS = [
  ...
  },
  {
    ...
    desc: "평균임금이 통상임금보다 낮으면 통상임금을 평균임금으로 사용할 수 있어요. 근로기준법 제2조 제2항에서 이를 명시해요. 두 금액을 계산해서 큰 쪽을 퇴직금 계산에 써요.",
...
const CHECKLIST = [
  "3개월 총임금: 상여금 월 환산액 포함했는지 확인",
  "실비 변상·임시 지급분: 총임금에서 제외했는지 확인",
  "통상임금: 기본급 + 고정수당으로 정확히 산출했는지 확인",
  "두 기준 비교: 평균임금과 통상임금 중 큰 쪽을 적용했는지 확인",
  "근속기간: 일 단위로 환산해서 소수점 처리했는지 확인",
  "IRP: 퇴직금 300만원 초과 시 IRP 계좌 개설 여부 확인",
];

const FAQS = [
  {
    q: "평균임금과 통상임금이 왜 다른가요?",
    a: "평균임금은 실제로 지급된 임금(상여금, 연차수당 포함)을 3개월로 나눈 실수령 기반이에요. 통상임금은 매달 정기적으로 나오는 기본급+고정수당만 포함해요. 그래서 상여금이 많은 해는 평균임금이 높고, 그 3개월에 상여금이 없으면 통상임금이 더 높게 나오기도 해요.",
  },
  {
    q: "퇴직 직전에 무급휴가나 결근이 있으면 어떻게 되나요?",
    a: "평균임금이 낮아질 수 있어요. 이때 통상임금이 더 높게 나오는 경우가 많아요. 근로기준법 제2조 제2항에 따라 통상임금을 기준으로 퇴직금을 계산할 수 있으니 꼭 비교해보세요.",
  },
  {
  ...
  },
  {
    ...
    a: "DB(확정급여형)은 일반 퇴직금과 동일하게 퇴직 시점 평균임금 기준으로 계산해요. DC(확정기여형)은 매년 연봉의 1/12이 적립되는 방식이라 산정 기준이 달라요. 본인이 어떤 유형인지 먼저 확인해봐요.",
  },
  {
    q: "회사가 계산한 금액이 내 계산보다 적어요. 어떻게 해야 하나요?",
    a: "먼저 인사팀에 계산 근거를 요청해서 항목별로 비교하세요. 차이가 있으면 재계산을 공식 요청해요. 거부하거나 해결이 안 되면 고용노동부 고객상담센터(1350)에 진정을 낼 수 있어요. 소멸시효 3년 이내라면 퇴직 후에도 청구 가능해요.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금 산정 방식에서 가장 많이 헷갈리는 게 평균임금이냐 통상임금이냐예요.
        ...
        즉, 무조건 평균임금이 아니라 두 기준을 비교해서 근로자에게 유리한 쪽을 써야 해요.
      </p>
      ...
      <p style={body}>
        ...
        상여금이나 실적급은 지급 조건에 따라 통상임금에 포함될 수도, 안 될 수도 있어요.
        퇴직 직전 3개월에 상여금이 없었거나 무급휴가가 있었다면 평균임금이 낮게 나올 수 있어요.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="아래 계산기로 두 기준을 비교해서 유리한 쪽으로 퇴직금을 확인해보세요."
        partialMatchText="상황에 따라 적용 기준이 달라질 수 있어요. 고용노동부(1350) 상담을 권해요."
      />
      ...
      <p style={body}>
        슬라이더로 3개월 평균임금 총액, 월 통상임금, 근속기간을 설정하면 두 기준의 퇴직금을 동시에 볼 수 있어요.
        실제 적용 퇴직금은 두 값 중 더 높은 쪽으로 나와요.
      </p>
      <p style={body}>
        3개월 총임금에는 상여금 월 환산액을 꼭 포함해야 해요.
        연간 상여금 총액을 12로 나눠 월 환산한 뒤 3을 곱한 값이에요.
        이 항목을 빠뜨리면 평균임금이 낮게 나와서 퇴직금을 덜 받을 수 있어요.
      </p>
      ...
      <p style={body}>
        ...
        근로계약서에서 고정수당 항목을 확인해야 통상임금을 정확히 계산할 수 있어요.
      </p>
      <p style={body}>
        회사 인사팀에 자료 요청이 어렵다면 고용노동부(1350)에 임금대장 열람을 요청할 수 있어요.
      ...
      <p style={body}>
        ...
        특히 퇴직 직전 3개월에 상여금이 없었거나 무급 기간이 있었다면 통상임금 기준이 유리할 수 있어요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 확인해봐요." />
    </ArticleLayout>
  );
}
