...
// Q1. 퇴직금을 연금저축계좌로 직접 받을 수 있는지 몰라서 IRP와 연금저축 중 어디서 받아야 하는지 헷갈리는 상황
// Q2. IRP에서 연금 개시를 신청해 퇴직소득세 30% 아끼면서 월 연금을 수령하는 행동
// Q3. 퇴직금→연금저축 직접이전 불가 / IRP 연금 수령 조건(만55세+5년) / 세율 구조(10년 이하 5.5%, 초과 3.3%) / 연 1,200만원 분리과세 기준 / 중도해지 시 16.5%
// Q4. GreenBox(결론 요약) + EligibilityChecker(조건 체크) + Calculator(세금 계산) + Steps(신청 절차) + Checklist(주의사항) + FAQ
//
// MAP:
// - Q1 → 서론 톤: "연금저축으로 받을 수 있냐" 질문에 곧장 답
// - Q2 → H2 순서: 가능여부 → 조건체크 → 세금계산 → 서류 → 절차 → 주의사항 → FAQ
// - Q3 → H2 개수/깊이: 불가 이유 + 조건 + 세율 + 1,200만원 기준 + 중도해지 모두 커버
// - Q4 → 컴포넌트: GreenBox 결론 강조, Calculator 세금 시뮬, Steps 절차, Checklist 체크
...
const CHECK_ITEMS = [
  ...
  { id: "c2", label: "IRP 또는 연금저축 계좌를 보유하고 있죠" },
  { id: "c3", label: "해당 계좌에 가입한 지 5년이 지났죠" },
...
const STEPS = [
  {
    ...
    desc: "IRP에는 퇴직금 이전분(사업주 납입)과 본인 추가 납입금이 들어 있죠. 연금저축에는 본인이 납입한 금액만 있는 구조고요. 두 계좌의 잔액과 납입 유형을 먼저 구분해야 해요.",
    tip: "퇴직금 이전분은 IRP에서만 연금으로 받을 수 있죠. 연금저축으로 옮기는 건 불가한 구조에요.",
  },
  {
    title: "연금 수령 기간·금액 설계",
    desc: "연간 1,200만원 이하로 받으면 분리과세(연금소득세 3.3~5.5%)가 적용돼요. 초과하면 다른 소득과 합산해서 종합소득세 신고 대상이 되고요. 퇴직소득세 30% 절감 혜택을 유지하려면 연금 수령 기간을 10년 이상으로 설정하는 게 유리해요.",
    tip: "IRP와 연금저축 합산 연금소득이 1,200만원을 넘지 않도록 나눠서 설계해야 해요.",
  },
  {
    title: "55세 이후 연금 개시 신청",
    desc: "만 55세가 되면 IRP 또는 연금저축에서 연금 개시를 신청할 수 있죠. 금융사 앱 또는 영업점 방문으로 신청 가능하고, 수령 기간과 방식(정액형·자유인출형)을 선택하면 돼요.",
    tip: "앱으로 신청하면 보통 당일 처리돼요. 수령 방식은 나중에 변경이 어려울 수 있죠.",
  },
  {
    title: "연금소득세 납부 및 수령",
    desc: "연금 수령 시 금융사가 원천징수로 연금소득세(3.3~5.5%)를 떼고 지급해요. 연간 1,200만원 이하면 분리과세로 끝나고요. 초과분은 다음 해 5월 종합소득세 신고를 해야 해요.",
    tip: "연금소득세는 금융사가 자동으로 처리해요. 별도로 납부하지 않아도 되죠.",
  },
];
...
const FAQS = [
  {
    ...
    a: "불가해요. 퇴직금은 반드시 IRP 계좌로만 수령할 수 있죠. 사업주가 납입하는 퇴직금 이전분은 IRP에서만 연금으로 받을 수 있고, 연금저축계좌로 직접 이전은 안 돼요. IRP에서 연금 개시를 신청하면 돼요.",
  },
  {
    q: "IRP와 연금저축, 어디서 먼저 받는 게 유리한가요?",
    a: "IRP에 퇴직금이 있다면 IRP에서 먼저 연금을 수령하는 게 유리해요. IRP로 연금을 받으면 퇴직소득세를 30% 절감할 수 있죠. 연금저축은 세액공제 받은 납입분에 연금소득세만 붙어요.",
  },
  {
    q: "연금 수령 기간이 길수록 세금이 낮아지나요?",
    a: "맞아요. 10년 초과 수령 시 연금소득세율이 5.5%에서 3.3%로 낮아지죠. 80세 이후에는 3.3%가 적용돼요. 기간을 길게 설정할수록 매년 내는 세금이 줄어요.",
  },
  {
    q: "연간 1,200만원을 넘게 받으면 어떻게 되나요?",
    a: "초과 금액은 종합소득세 신고 대상이 돼요. 다른 소득(근로소득, 사업소득 등)과 합산해서 세율이 결정되기 때문에 세금이 크게 늘 수 있죠. 가능하면 연간 1,200만원 이하로 수령액을 조정하는 게 좋아요.",
  },
  {
    q: "연금 수령 중에 계좌를 해지하면 어떻게 되나요?",
    a: "중도 해지하면 세액공제 받은 금액과 운용 수익에 기타소득세 16.5%가 부과돼요. IRP에서 퇴직금 이전분을 해지하면 퇴직소득세 30% 절감 혜택도 사라지죠. 세금 손실이 크니 해지는 피하는 게 맞아요.",
  },
  {
    q: "55세 미만인데 연금저축에서 돈을 꺼낼 수 있나요?",
    a: "연금 외 수령으로 꺼낼 수는 있죠. 하지만 세액공제 받은 납입분과 운용 수익에 기타소득세 16.5%가 붙어요. 퇴직금 이전분이 있는 IRP도 마찬가지에요. 55세까지 기다리는 게 세금 면에서 훨씬 유리해요.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="퇴직금-연금저축계좌-수령" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금은 원칙적으로 <a href="/w/퇴직금-IRP-계좌" style={{ color: "#1D9E75", textDecoration: "underline" }}>IRP 계좌</a>로 수령해야 해요.
        연금저축계좌로 직접 이전은 불가하지만, IRP에서 55세 이후 연금을 수령하면 <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a>를 30% 아낄 수 있죠.
      ...
      <p style={body}>
        ...
        사업주가 납입하는 퇴직금 이전분은 반드시 IRP 계좌로만 수령해야 하죠.
        연금저축계좌는 본인이 직접 납입한 금액만 있는 계좌에요.
      </p>
      <p style={body}>
        다만 IRP에서 연금 개시를 신청하면 55세 이후부터 연금 형태로 퇴직금을 받을 수 있죠.
        IRP 연금 수령 시 퇴직소득세를 30% 아낄 수 있고, 연금소득세(3.3~5.5%)만 내면 돼요.
        연금저축과 IRP는 합산해서 연 900만원 세액공제 한도가 적용되는 구조에요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="IRP·연금저축 연금 수령 조건을 갖추고 있죠. 아래 계산기로 예상 수령액과 세금을 계산해보세요."
        partialMatchText="조건 일부가 충족되지 않았죠. 금융감독원(1332) 또는 국세청(126)에 문의해보세요."
      />
      ...
      <p style={body}>
        총 연금 수령액과 수령 기간을 조정하면 연간 수령액과 예상 세금을 볼 수 있죠.
        수령 기간이 10년을 넘으면 세율이 5.5%에서 3.3%로 낮아져요.
        연간 1,200만원 이하로 받으면 분리과세 적용으로 종합소득세 신고가 필요 없고요.
      </p>
      ...
      <p style={body}>
        금융사 앱으로 신청하면 신분증 없이 본인 인증만으로 처리되는 경우가 많죠.
        영업점 방문 시에는 신분증을 반드시 지참해야 해요.
        퇴직소득원천징수영수증은 연금 신청 시 필요할 수 있어서 미리 보관해두는 게 좋아요.
      </p>
      ...
      <p style={body}>
        ...
        수령 기간과 금액을 미리 설계해두면 세금을 최소화할 수 있죠.
      </p>
      ...
      <GreenBox>
        ...
        일시금으로 받을 때보다 수백만 원 이상 세금이 줄 수 있죠.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 소득세법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 세율은 수령 방식·기간에 따라 달라질 수 있으니 최신 기준은 국세청(126)에 문의해요." />
    </ArticleLayout>
  );
}
