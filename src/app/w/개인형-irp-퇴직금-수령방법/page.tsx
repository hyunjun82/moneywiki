...
// Q1. 퇴직을 앞두거나 방금 퇴직한 사람이 IRP 계좌에 퇴직금을 어떻게 넣고, 어떻게 받아야 세금을 줄일 수 있는지 모르는 상황
// Q2. 개인형 IRP 계좌를 개설하고, 퇴직금을 이체받아서, 연금 수령 방식을 설정한다
// Q3. 퇴직 전 IRP 개설 타이밍 / 4단계 절차 / 55세 이후 10년+ 연금 수령 시 퇴직소득세 30% 감면 / 수수료 차이(증권사 vs 은행) / 운용 지시 방법 / 중도인출 불가 원칙과 예외
// Q4. Steps(4단계 절차) + Calculator(연금 수령액 시뮬레이터) + Checklist(놓치기 쉬운 5가지) + FAQ(의문 해소)
// MAP: Q1→서론(IRP 왜 필요한지 상황 공감) / Q2→H2 순서(개설→이체→운용→수령) / Q3→H2 깊이(세금 감면 조건, 수수료, 운용 지시) / Q4→Steps+Calculator+Checklist+FAQ
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직금이 300만 원을 초과해요" },
  { id: "c2", label: "만 55세 이상이에요" },
  { id: "c3", label: "IRP에 추가 납입 여력이 있어요" },
  { id: "c4", label: "퇴직금이 300만 원을 초과해요" },
];
...
const FAQS = [
  {
    ...
    a: "개인형은 본인이 직접 가입하는 계좌, 기업형(DC·DB형)은 회사가 운용하는 구조예요. 퇴직 시 기업형 계좌 잔액이 개인형 IRP로 이전돼요.",
  },
  {
  ...
  },
  {
    ...
    a: "IRP+연금저축 합산 연 900만원까지 세액공제 대상이에요. 소득 5,500만원 이하면 16.5%, 초과면 13.2%예요.",
  },
  {
    q: "운용 중 손실이 나면 퇴직금이 줄어드나요?",
    a: "실적배당형(ETF·펀드)을 선택하면 손실이 날 수 있고요. 안전하게 원리금보장형을 선택하는 방법도 있죠.",
  },
  {
    q: "퇴직 후 연금 수령 전까지 IRP 관리를 안 해도 되나요?",
    a: "운용 지시를 따로 안 하면 원리금보장형으로 자동 운용돼요. 수수료는 계속 부과되니 주기적으로 내역을 살펴보는 게 좋아요.",
  },
];
...
export default function Page() {
  ...
  return (
    <ArticleLayout sidebar={sidebar}>
      ...
      <p style={body}>
        퇴직금을 받는 방법은 크게 두 가지예요. 그냥 일시불로 받거나, 개인형 IRP 계좌에 넣어서 연금으로 나눠 받거나요. 그냥 받으면 퇴직소득세를 한꺼번에 내야 하는데, IRP를 거치면 세금 납부를 미루면서 운용 수익까지 노릴 수 있죠.
      </p>
      <p style={body}>
        개인형 IRP(Individual Retirement Pension)는 근로자가 직접 개설하는 퇴직연금 계좌예요. 회사가 운용하는 DB형·DC형과 달리 내가 직접 금융기관을 골라서 만들고, 운용 방법도 내가 정하는 구조예요. 퇴직할 때 회사에서 퇴직금을 이 계좌로 이체해줘요.
      </p>
      <p style={body}>
      ...
      <GreenBox>
        IRP 계좌는 퇴직 전에 미리 만들어두는 게 좋아요. 퇴직 후에 회사가 퇴직금을 이체할 때 계좌가 없으면 지연될 수 있고요. 증권사 앱으로 10분이면 개설 가능해요.
      </GreenBox>
      ...
      <p style={body}>
        특히 3단계 '운용 지시'를 많이들 놓쳐요. 아무것도 안 하면 원리금보장형(예금 수준)으로 자동 배정되는데, 10~20년 장기로 굴리는 돈이라면 ETF나 펀드로 분산해두는 게 더 유리할 수 있죠. 물론 원금 손실 가능성도 있으니 본인의 성향에 맞게 골라야 해요.
      </p>
      ...
      <H2>연금 수령액, 미리 계산해 봐요</H2>

      <p style={body}>
        퇴직금 규모와 수령 기간을 바꿔가며 연간 수령액을 미리 살펴볼 수 있죠. 운용 수익은 포함되지 않은 단순 분할 기준이에요.
      </p>
      <p style={body}>
        세액공제는 본인이 IRP에 추가로 납입한 금액에 대해 적용돼요. 퇴직금 자체가 아니라 '추가 납입분'에 한해서 돌려받는 거라서, 납입 금액에 따라 실제 혜택이 달라질 수 있죠.
      </p>
      ...
      <p style={body}>
        개인형 IRP 계좌 개설은 앱으로도 가능해요. 신분증과 인증서(공동인증서 또는 금융인증서)만 있으면 대부분 비대면으로 처리돼요. 퇴직 확인서는 회사에서 받아야 하니 퇴직 처리 전에 미리 챙겨두세요.
      </p>
      ...
      <p style={body}>
        수수료는 생각보다 크게 차이 나요. 증권사는 연 0.2% 수준인데, 은행은 0.5%까지 올라가는 경우도 있고요. 퇴직금 5,000만원을 20년 운용한다면 수수료 차이만으로 수백만원이 갈릴 수 있죠. <a href="https://www.fss.or.kr" style={{ color: "#1D9E75" }}>금융감독원 퇴직연금 비교공시</a>에서 금융기관별로 비교해볼 수 있고요.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법을 바탕으로 작성됐어요. 최신 기준은 금융감독원(1332)에서 안내받을 수 있죠." />
    </ArticleLayout>
  );
}
