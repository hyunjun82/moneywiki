...
// Q1. 회사에서 퇴직금 정산서를 받았는데 금액이 맞는지 모른 채 서명해야 할지 고민 중인 상황
// Q2. 정산서 항목을 직접 검증하고 오류가 있으면 이의를 제기한다
// Q3. 필수 기재 항목(근속기간·평균임금), 상여금 환산 포함 여부, 퇴직소득세 원천징수, 소멸시효 3년, 이의 제기 절차
// Q4. GreenBox(핵심 체크 3가지) + EligibilityChecker + Calculator + DocTable + Steps + Checklist + FAQ
//
// MAP:
// - Q1 → 서론 톤: "서명하기 전에 잠깐" 공감형
// - Q2 → H2 순서: 확인 항목 → 직접 계산 → 필요 서류 → 이의 제기 → 서명 전 체크
// - Q3 → H2 개수/깊이: 5개 H2, 상여금·소멸시효·지연이자 별도 설명
// - Q4 → 컴포넌트 배치: GreenBox(핵심 3가지) > 계산기 > DocTable > Steps > Checklist
...
const CHECK_ITEMS = [
  { id: "c1", label: "회사에서 퇴직금 정산서를 받은 상황이에요" },
...
const DOCS = [
  ...
  { name: "상여금·수당 지급 내역", required: false, where: "인사팀 또는 급여명세서" },
];

const STEPS = [
  {
    title: "입사일·퇴직일 확인",
    desc: "정산서를 받으면 맨 먼저 입사일과 퇴직일이 정확한지 봐요. 하루만 틀려도 근속연수가 달라지고 퇴직금도 바뀌어요. 달력으로 직접 세어봐서 총 일수가 정산서와 맞는지 대조하는 게 좋아요.",
    tip: "근속기간 1일 차이도 퇴직금에 영향을 줘요",
  },
  {
    title: "평균임금 구성 항목 대조",
    desc: "정산서에 표시된 3개월 총임금이 급여명세서 합계와 일치하는지 직접 더해봐요. 정기 상여금은 연간 총액 ÷ 12 × 3으로 환산해 포함시켜야 해요. 식대·교통비 같은 고정수당도 빠지지 않았는지 봐야 해요.",
    tip: "상여금 누락이 퇴직금 차이의 가장 흔한 원인이에요",
  },
  {
    title: "공식으로 직접 계산",
    desc: "1일 평균임금 × 30일 × 근속연수 공식으로 직접 계산해봐요. 1일 평균임금은 퇴직 전 3개월 총임금 ÷ 그 기간 총 일수예요. 정산서 금액과 10만원 이상 차이가 나면 인사팀에 산정 내역 설명을 요청하세요.",
    tip: "위 계산기로 바로 비교해볼 수 있죠",
    link: { label: "퇴직금 평균임금 계산 기준 보기", href: "/w/퇴직금-평균임금" },
  },
  {
    title: "오류 발견 시 이의 제기",
    desc: "차액이 확인되면 먼저 인사팀에 서면으로 재계산을 요청해요. 거부하거나 이유 없이 차액을 인정하지 않으면 고용노동부 민원마당에서 온라인 진정을 낼 수 있죠. 퇴직 후에도 소멸시효 3년 이내라면 청구가 가능해요.",
...
const FAQS = [
  {
    q: "퇴직금 정산서를 안 줘도 되나요?",
    a: "법적 발급 의무는 없어요. 그래도 요청하면 대부분 줘요. 안 주면 급여명세서와 근속 기간으로 직접 계산할 수 있죠. 퇴직소득 원천징수영수증은 의무 발급 서류라 반드시 받을 수 있고요.",
  },
  {
    q: "정산서에 상여금이 빠진 것 같아요",
    a: "정기적으로 지급된 상여금은 연간 총액 ÷ 12 × 3으로 환산해 3개월 총임금에 포함시켜야 해요. 빠졌다면 인사팀에 항목 추가 후 재계산을 요청하는 게 맞아요.",
  },
  {
    q: "정산서 금액과 실제 입금액이 달라요",
    a: "퇴직소득세가 원천징수된 금액이 입금돼요. 정산서는 세전 퇴직금이고, 실제 입금은 세후 금액이에요. 원천징수영수증에서 원천징수된 세금 금액을 보면 차이가 설명돼요.",
  },
  {
    q: "퇴직금 정산서와 원천징수영수증은 다른 건가요?",
    a: "달라요. 정산서는 퇴직금 계산 내역이고, 원천징수영수증은 세금 납부 내역이에요. 둘 다 받아두는 게 좋죠.",
  },
  {
    q: "서명 후에 오류를 발견했어요",
    a: "서명 후라도 명백한 계산 오류라면 차액 청구가 가능해요. 퇴직 후 3년 이내라면 내용증명 발송 또는 고용노동부 진정으로 청구할 수 있죠.",
  },
  {
    q: "이미 퇴직한 지 2년이 됐는데 청구할 수 있나요?",
    a: "퇴직금 청구권 소멸시효는 3년이에요. 퇴직일 기준 3년 이내라면 지금도 청구가 가능해요. 소멸시효 완성이 임박하면 진정을 먼저 낸 뒤 협의를 진행하면 돼요.",
  },
];
...
const RELATED = [
  ...
  { slug: "퇴직금-미지급-신고", title: "퇴직금 미지급 신고 방법", description: "고용노동부 진정 절차를 정리했어요." },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        서명 전 숫자를 직접 검증해두지 않으면 나중에 오류를 발견해도 추가 청구가 복잡해져요.{" "}
      ...
      <p style={body}>
        ...
        달력으로 직접 세어봐서 정산서에 표시된 총 일수와 맞는지 대조하는 게 기본이에요.
      </p>
      <p style={body}>
        다음으로 볼 건 3개월 총임금이에요. 퇴직 전 3개월 급여명세서를 꺼내서 합계를 직접 더해봐요.
        정산서 수치와 차이가 나면 상여금·고정수당이 빠진 경우가 많아요.
        정기 상여금은 연간 총액 ÷ 12 × 3으로 환산해서 포함해야 하고, 식대·교통비 같은 고정수당도 마찬가지예요.
      </p>
      <p style={body}>
        정산서 하단에 "이의를 제기하지 않겠다"는 문구가 있는지도 꼭 봐야 해요.
        이런 문구에 서명하면 나중에 오류를 발견해도 청구하기가 훨씬 어려워져요.
      </p>

      <GreenBox>
        근속기간: 입사일·퇴직일 정확한지, 총 일수 맞는지<br />
        평균임금: 3개월 총임금에 상여금·고정수당 포함 여부<br />
        이의 제기 문구: 서명 전 제한 문구 유무 반드시 봐야 해요
      </GreenBox>

      <SectionBadge>내 상황 체크해보세요</SectionBadge>
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="해당 상황이에요. 아래 계산기로 예상 퇴직금을 직접 계산하고 정산서 금액과 비교해봐요."
        partialMatchText="일부 항목이 해당돼요. 해당 부분부터 집중해서 보면 돼요."
      />
      ...
      <Calculator
        ...
        note="※ 월 평균급여 × 근속연수 기준 간략 계산이에요. 실제 퇴직금은 퇴직 전 3개월 평균임금 기준으로 달라져요."
      />
      ...
      <p style={body}>
        정산서를 제대로 보려면 비교할 기준 서류가 필요해요. 가장 중요한 건 퇴직 전 3개월 급여명세서예요.
        정산서에 표시된 총임금이 명세서 합계와 일치하는지 직접 대조할 수 있죠.
      </p>
      <p style={body}>
        퇴직소득 원천징수영수증도 꼭 받아두세요. 정산서 금액과 실제 입금액이 다른 이유가 이 서류에 나와 있고요.
        퇴직소득세가 얼마나 원천징수됐는지, 세율 적용이 맞는지 볼 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        각 단계에서 메일·문자 등 서면으로 기록을 남겨두면 나중에 증거로 쓸 수 있죠.
      </p>
      <p style={body}>
        회사가 거부하거나 이유 없이 차액을 인정하지 않으면 그때 고용노동부 진정을 넣으면 돼요.
        소멸시효 만료가 임박하다면 진정을 먼저 낸 뒤 협의를 진행하는 방식도 가능해요.
      </p>
      ...
      <H2>서명 전 이것만 보면 돼요</H2>
      <p style={body}>
        정산서를 받은 날 바로 서명하지 않아도 돼요. 검토 시간을 달라고 요청하는 건 당연한 권리예요.
        아래 6가지를 순서대로 보고 이상 없을 때 서명하면 돼요.
      </p>
      <p style={body}>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여보장법·근로기준법을 바탕으로 작성됐어요. 제도 변경이 생길 수 있으니 최신 기준은 고용노동부(1350)에서 보세요." />
    </ArticleLayout>
  );
}
