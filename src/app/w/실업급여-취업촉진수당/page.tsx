...
export default function Page() {
  return (
    ...
    >
      ...
      <p style={body}>
        구직급여는 실직 후 구직활동을 하는 동안 <strong>매달 정기적으로</strong> 받는 기본 급여예요. 반면에 취업촉진수당은 특정 조건을 충족했을 때 <strong>추가로 지급</strong>되는 인센티브 성격의 수당이에요. 제도의 취지가 다르죠.
      </p>
      <p style={body}>
        고용보험법은 실업급여를 "구직급여"와 "취업촉진수당"으로 나눠요. 대부분의 사람이 구직급여만 알고 취업촉진수당은 모르고 지나가죠. 그 이유는 간단해요. 구직급여는 신청하면 자동으로 나오지만, 취업촉진수당은 <strong>본인이 조건을 충족하고 별도로 신청</strong>해야 하기 때문이에요.
      </p>
      <p style={body}>
        네 종류를 간략히 정리하면 이래요. <strong>조기재취업수당</strong>은 빨리 취업한 보상, <strong>직업능력개발수당</strong>은 직업훈련 참여 지원금, <strong>광역구직활동비</strong>는 먼 거리 면접 교통비, <strong>이주비</strong>는 취업 목적 이사 비용이에요. 각각 대상과 신청 방식이 다르니 하나씩 살펴볼게요.
      </p>
      ...
      <p style={body}>
        조기재취업수당은 취업촉진수당 중에서 금액이 가장 커요. 구직급여 수급기간의 <strong>절반 이상을 남겨놓고 재취업</strong>하면 받을 수 있죠. 핵심 조건이 하나 더 있는데, 재취업한 직장에서 <strong>12개월 이상 고용이 유지</strong>돼야 해요. 12개월 채우기 전에 그만두면 지급 안 되죠.
      </p>
      <p style={body}>
        계산 방식은 단순해요. <strong>남은 수급일수 x 50% x 1일 구직급여액</strong>이에요. 예를 들어 수급기간 180일 중 60일만 사용하고 취업했다면, 남은 120일의 절반인 60일분이 지급돼요. 1일 구직급여가 66,048원이면 60일 x 66,048원 = <strong>약 396만원</strong>을 한꺼번에 받는 셈이죠.
      </p>
      <p style={body}>
        여기서 전략적으로 따져볼 게 있어요. 남은 수급일수가 너무 적으면 조기재취업수당보다 구직급여를 끝까지 받는 게 유리해요. 남은 일수의 50%만 받으니까, 최소한 수급기간 절반 이상은 남겨야 의미가 있죠. 아래 계산기로 본인 상황을 직접 비교해보세요.
      </p>
      ...
      <p style={body}>
        <strong>직업능력개발수당</strong>은 고용센터 소장이 지시한 직업훈련을 받을 때 지급돼요. 국민내일배움카드로 수강할 수 있는 훈련과 연계되는 경우가 많죠. 금액 자체는 하루 수천 원 수준으로 크지 않지만, 교통비와 식비를 보조하는 성격이에요. 훈련 참여일수에 비례해서 지급되고, 별도 신청 없이 <strong>실업인정일에 자동 반영</strong>되니 편해요.
      </p>
      <p style={body}>
        <strong>광역구직활동비</strong>는 거주지에서 <strong>50km 이상 떨어진 곳</strong>에 면접을 보러 갈 때 교통비를 실비로 지원해요. 서울에서 부산 면접을 본다든지, 대전에서 광주로 이동하는 경우가 해당되죠. 숙박이 필요한 거리면 숙박비도 나와요. 중요한 건 <strong>면접 출발 전에 고용센터에서 사전 승인</strong>을 받아야 한다는 거예요. 먼저 다녀오고 나중에 신청하면 한 푼도 못 받아요.
      </p>
      <p style={body}>
        <strong>이주비</strong>도 비슷한 구조예요. 취업하거나 직업훈련을 받기 위해 주거를 이전할 때 <strong>이사 비용을 실비로 지급</strong>해요. 가족이 함께 이사하면 가족분 이사비까지 포함돼요. 이것도 이사 전에 고용센터에서 <strong>사전 승인이 필수</strong>예요. 지방에서 수도권으로, 혹은 다른 도시로 이동하는 경우에 활용하면 돼요.
      </p>
      ...
      <GreenBox title="사전 승인이 필요한 수당 — 순서를 지키세요">
        ...
        먼저 하고 나중에 신청하면 지급 대상에서 빠져요. 순서가 핵심이에요.
      </GreenBox>
      ...
      <p style={body}>
        조건만 맞으면 <strong>여러 취업촉진수당을 동시에</strong> 챙길 수 있어요. 예를 들어 직업훈련을 받으면서 직업능력개발수당을 받고, 훈련 후 수급기간 절반 이상을 남기고 취업해서 조기재취업수당까지 받는 식이에요. 먼 곳에 면접을 보러 가서 광역구직활동비를 받은 뒤, 그 회사에 취업해서 이사하면 이주비까지 가능하죠.
      </p>
      <p style={body}>
        다만 한 가지 주의할 점이 있어요. 조기재취업수당을 받으면 <strong>남은 구직급여는 소멸</strong>돼요. 빨리 취업한 보너스를 받는 대신 나머지 구직급여를 포기하는 구조이기 때문이에요. 남은 수급일수가 30일밖에 안 남았다면 조기재취업수당(15일분)보다 구직급여를 끝까지 받는 게 더 유리하죠.
      </p>
      <p style={body}>
        실수하기 쉬운 부분도 짚어드릴게요. 광역구직활동비나 이주비는 "일단 하고 나중에 청구하면 되겠지"라는 생각이 가장 위험해요. <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서 사전 승인을 요건으로 정해놓았기 때문에, 승인 없이 진행하면 지급 대상에서 완전히 빠지죠. "사후 신청 불가"라는 점을 반드시 기억하세요.
      </p>

      <BorderBox title="수당별 신청 시점과 방법">
        <strong>조기재취업수당</strong> → 재취업 후 12개월 고용 유지 확인 → 고용24 온라인 신청<br />
      ...
      <p style={body}>
        가장 큰 금액은 단연 조기재취업수당이에요. 남은 수급일수가 많을수록 유리하니까, 빨리 재취업할 자신이 있다면 구직급여를 오래 끌지 말고 적극적으로 취업활동을 하세요. 수급기간 150일 중 30일만 쓰고 취업하면 남은 120일의 50%인 60일분, 약 <strong>396만원</strong>을 한 번에 받을 수 있어요.
      </p>
      <p style={body}>
        직업훈련을 계획하고 있다면 고용센터에 먼저 상담하세요. 고용센터 소장이 "지시"한 훈련이어야 직업능력개발수당이 나와요. 본인이 알아서 수강한 훈련은 대상이 아니에요. 국민내일배움카드 훈련과 연계하면 수강료 지원 + 직업능력개발수당까지 이중으로 챙길 수 있죠.
      </p>
      <p style={body}>
      ...
      <GreenBox title="수당 최대 활용 전략">
        1. 구직급여 수급 시작 → 직업훈련 참여 → <strong>직업능력개발수당</strong> 수령<br />
        2. 먼 거리 면접 시 → 출발 전 사전 승인 → <strong>광역구직활동비</strong> 수령<br />
        3. 수급기간 절반 이상 남기고 재취업 → 12개월 유지 → <strong>조기재취업수당</strong> 수령<br />
        4. 취업 목적 이사 → 이사 전 사전 승인 → <strong>이주비</strong> 수령
      </GreenBox>
