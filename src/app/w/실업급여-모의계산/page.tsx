...
const STEPS = [
  {
    ...
    desc: "고용보험 공식 사이트에 들어가세요. 로그인 없이도 모의계산 메뉴를 이용할 수 있죠. 메인 페이지에서 '모의계산' 또는 '예상 급여 조회'를 찾으면 돼요.",
  ...
  {
    title: "결과 확인",
    desc: "1일 수급액, 수급기간, 총 예상 수령액이 나와요. 이 금액은 참고용이에요. 실제로는 고용센터 심사 후 확정되니까 정확한 금액은 신청 후에 알 수 있죠.",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "상한이 걸려요. 2026년 기준 1일 최대 68,100원, 월로 치면 약 204만원이에요. 월급이 500만원이든 700만원이든 최대 204만원까지만 받아요. 그래서 고소득자일수록 실업급여 체감 금액은 적죠.",
  },
  {
...
const RELATED = [
  ...
  {
    slug: "실업급여-평균임금-계산",
    title: "실업급여 평균임금 산정 방법",
    description: "퇴직 전 3개월 평균임금을 기준으로 계산돼요. 상여금, 식대, 교통비가 포함되는지 여부에 따라 금액이 달라지죠.",
  },
  {
    slug: "실업급여-최대금액",
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        근데 여기에 상한액·하한액이 걸려서 월급이 높다고 무한정 많이 받는 건 아니에요.<br />
      ...
      <p style={body}>
        슬라이더에서 나이를 49세 → 50세로 바꿔보세요. 수급기간이 확 늘어나면서 총액도 크게 달라져요. 50세 이상은 재취업이 어려운 연령대라 수급기간을 더 길게 주죠. 이 차이만 해도 수백만원이에요.
      </p>

      <GreenBox title="이것만 기억해요">
        월급의 60%가 기본이지만, 상한·하한이 있어서 대부분 월 198~204만원이에요<br />
        차이를 만드는 건 수급기간이에요. 나이와 가입기간이 길수록 유리하죠<br />
      ...
      <p style={body}>
        위 계산기로 대략적인 금액을 확인했으면, 정확한 수치는 고용보험 공식 모의계산기에서 한 번 더 확인해보세요. 고용24(ei.go.kr)에서 제공하는데, 로그인 없이 바로 사용할 수 있죠.
      </p>
      ...
      {/* 섹션 4 — 모의계산과 실제 차이 */}
      <H2>계산 결과랑 실제 지급액이 다를 수 있죠</H2>
      <p style={body}>
        모의계산에서 나온 금액과 실제로 통장에 찍히는 금액이 다를 수 있죠. 당황하지 마세요, 정상이에요. 차이가 나는 이유는 크게 세 가지예요.
      </p>
      <p style={body}>
        첫째, 평균임금 산정 방식이 달라질 수 있죠. 모의계산은 기본급만 넣지만, 실제로는 상여금, 복리후생비, 식대 등이 포함될 수 있죠. 퇴직 전 3개월 사이에 보직 변경이나 급여 조정이 있었다면 더 복잡해져요.
      </p>
      <p style={body}>
      ...
      <p style={body}>
        급여명세서가 없으면 홈택스에서 근로소득 원천징수영수증을 뽑으면 돼요. 가입기간은 고용24에서 바로 확인할 수 있죠. 이 두 개만 준비하면 5분 안에 모의계산 끝나요.
      </p>
