...
// Q1. 퇴직금을 못 받은 채 퇴직한 지 시간이 꽤 지났는데 아직 청구 가능한지 모르는 상황
// Q2. 소멸시효 기산일을 파악하고, 지연이자를 계산해서 노동청에 진정을 접수한다
// Q3. 소멸시효 3년(퇴직일 다음 날 기산), 14일 초과 시 연 20% 지연이자, 내용증명으로 시효 중단, 노동청 진정 절차
// Q4. GreenBox(핵심 숫자 3줄) + Calculator(지연이자 계산) + Steps(청구 4단계) + DocTable(서류) + FAQ
//
// MAP:
// - Q1 → 서론: "퇴직하고 시간이 좀 지났는데 아직 청구할 수 있는지" 공감형 톤
// - Q2 → H2 순서: 소멸시효 → 지연이자 계산 → 청구 절차 → 준비 서류
// - Q3 → H2 4개, 소멸시효 중단 방법 상세히
// - Q4 → Calculator + Steps + DocTable + EligibilityChecker
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직일로부터 3년이 아직 지나지 않았죠" },
  { id: "c2", label: "퇴직 후 14일이 지났는데 퇴직금을 아직 못 받았죠" },
  { id: "c3", label: "회사에 지급 요청한 기록(문자·이메일)이 남아 있죠" },
  { id: "c4", label: "미지급 퇴직금 원금이 얼마인지 파악하고 있죠" },
];
...
const STEPS = [
  {
    ...
    tip: "내용증명 발송·노동청 진정 신청 시 소멸시효가 중단되어 새로 3년이 카운트돼요",
  },
  {
    title: "지연이자 계산",
  ...
  {
    title: "내용증명 발송",
    desc: "퇴직금 원금과 지연이자를 합산한 금액을 내용증명으로 회사에 청구해요. 내용증명은 소멸시효 중단 효과가 생겨요. 3년이 다 되어간다면 응답 여부와 상관없이 내용증명을 먼저 발송하세요.",
    tip: "내용증명은 우체국에서 직접 보내거나 인터넷우체국(온라인)으로도 발송 가능해요",
  },
  {
    title: "노동청 진정 접수 또는 소송",
    desc: "내용증명에 응답이 없으면 사업장 관할 지방노동청 또는 고용노동부 민원마당(온라인)에 진정을 접수해요. 진정 후 보통 2~4주 내에 처리돼요. 그래도 지급하지 않으면 법원 지급명령·소액사건 소송으로 강제 회수해요.",
    tip: "법률구조공단(132)에서 무료 법률 지원을 받을 수 있고, 형사처벌도 가능해요(근로기준법 위반 2년 이하 징역 또는 2,000만원 이하 벌금)",
  },
];
...
const FAQS = [
  {
    ...
    a: "퇴직일 다음 날부터예요. 퇴직일이 2023년 3월 1일이면 2026년 3월 2일까지 청구해야 해요. 이후엔 회사가 소멸시효 항변을 주장할 수 있죠.",
  },
  {
    q: "3년이 다 되어가는데 소멸시효를 중단하는 방법이 있나요?",
    a: "내용증명 발송, 노동청 진정 접수, 법원 지급명령 신청이 모두 시효 중단 사유예요. 내용증명을 발송하면 시효가 중단되고, 그 시점부터 새로 3년이 카운트돼요.",
  },
  {
    q: "지연이자를 회사가 거부하면 어떻게 하나요?",
    a: "노동청에 지연이자도 포함해서 진정을 내면 돼요. 근로기준법 제37조로 보장된 권리라 노동청이 지급을 명령할 수 있죠.",
  },
  {
    q: "퇴직 후 3년이 지나면 정말 받을 수 없나요?",
    a: "원칙적으로는 그렇죠. 소멸시효가 완성되면 회사가 시효 이익을 주장할 수 있죠. 다만 회사가 자발적으로 지급하거나 시효 이익을 포기하면 여전히 받을 수 있고요.",
  },
  {
    q: "회사가 폐업했는데 지연이자도 청구할 수 있나요?",
    a: "체당금 제도로 퇴직금 원금은 일부 회수할 수 있죠. 지연이자는 체당금 지급 대상이 아닐 수 있고요. 고용노동부(1350)에 체당금 신청 방법을 문의하세요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        기한이 촉박하다면 내용증명 발송만으로 소멸시효를 중단시킬 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        내용증명 발송·노동청 진정 신청이 있으면 시효가 중단되어 그 시점부터 새로 3년이 시작돼요.
      </p>
      <p style={body}>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금 + 지연이자 청구가 가능해요. 아래 계산기로 지연이자를 계산해봐요."
        partialMatchText="상황에 따라 다를 수 있죠. 고용노동부(1350) 상담을 권해요."
      />

      <Divider />

      <H2>지연이자 총액, 직접 계산해봐요</H2>
      <p style={body}>
        미지급 퇴직금과 14일 초과 지연 일수를 입력하면 지연이자와 총 청구액을 바로 계산할 수 있죠.
      ...
      <p style={body}>
        ...
        4대보험 자격상실 확인서는 가입자 포털(4insure.or.kr)에서 무료로 내려받을 수 있죠.
      </p>
      ...
      <p style={body}>
        소멸시효 파악 → 지연이자 계산 → 내용증명 발송 → 노동청 진정·소송 순서예요.
      ...
      <H2>청구 전 체크리스트</H2>
      <p style={body}>
        소멸시효 3년을 놓치지 않는 게 가장 중요해요. 지연이자도 퇴직금과 함께 청구하면 추가로 받을 수 있죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 파악해보세요." />
    </ArticleLayout>
  );
}
