  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        예전에 실업급여를 받은 적이 있는데, 이번에 또 퇴직하셨죠.<br />
        "한 번 받았으면 끝 아닌가?" 걱정되실 거예요.<br /><br />
        결론부터 말하면, <strong>조건만 맞으면 다시 받을 수 있어요</strong>.{" "}
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>은
        실업급여 수급 횟수를 제한하지 않아요.
        핵심은 이전 수급 이후 새로 쌓은 <strong>고용보험 가입기간 180일</strong>이에요.
      </p>
      ...
      <p style={body}>
        처음 받을 때와 기본 구조는 같아요. 핵심은 딱 하나, <strong>이전 수급 종료 후 새로 고용보험 180일 이상을 채우는 것</strong>이에요. 여기서 "180일"은 달력상 날짜가 아니라 실제 근무일(유급일) 기준이죠. 주 5일제 회사라면 대략 8~9개월 정도 다녀야 채워져요.
      </p>
      <p style={body}>
        그리고 퇴직 사유도 다시 따져요. 비자발적 퇴사(권고사직, 계약만료, 정당한 사유 등)여야 하죠. 본인이 원해서 그냥 나온 단순 자발적 퇴사면 이번에도 실업급여 대상이 아니에요. <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에 따르면 실업급여를 수급하면 그 시점까지의 피보험기간은 소진돼요. 이전 가입기간은 합산되지 않는다는 뜻이죠.
      </p>
      <p style={body}>
        두 번째 신청 때는 이전 수급 종료일 이후부터 새로 쌓은 기간만 인정돼요. 그래서 재취업 후 오래 다닐수록 수급일수가 길어지고, 단기 근무 후 퇴직하면 수급일수가 확 줄어드는 구조예요. 마지막으로 퇴직 후 <strong>12개월 이내</strong>에 신청해야 한다는 점도 잊지 마세요.
      </p>

      <GreenBox title="재신청 4가지 조건">
      ...
      <p style={body}>
        이 부분이 실질적으로 가장 중요해요. 실업급여 수급기간(받을 수 있는 날수)은 <strong>나이</strong>와 <strong>고용보험 가입기간</strong>으로 정해지죠. 이전에 10년 가입해서 240일을 받았더라도, 두 번째 신청 때 가입기간이 2년이면 150일(50세 미만 기준)밖에 안 돼요.
      </p>
      <p style={body}>
        예를 들어볼게요. 첫 직장에서 7년 근무 후 퇴직해서 실업급여 210일을 수급했어요. 재취업해서 새 직장에서 3년 일하다 다시 퇴직하면? 이번 가입기간은 3년이니까 수급일수는 180일(50세 미만 기준)로 줄어들죠. 가입기간 리셋은 피할 수 없는 구조예요.
      </p>
      <p style={body}>
        수급액도 새 직장 기준으로 재계산돼요. 퇴직 전 3개월 평균임금의 60%가 1일 수급액이에요. 2026년 기준 상한액은 1일 66,000원, 하한액은 63,104원이죠. 아래 계산기에서 본인 조건을 넣어보세요.
      </p>
      ...
      <BorderBox title="수급일수 비교 예시 (50세 미만)">
      ...
      <p style={body}>
        수급 중간에 취업해서 남은 급여가 있었다면, 그 <strong>남은 일수는 소멸</strong>돼요. 예를 들어 180일 받을 수 있었는데 60일만 받고 취업했다면, 나머지 120일은 사라지죠. 나중에 "남은 거 이어서 받겠다"는 건 불가능해요.
      </p>
      <p style={body}>
        대신 <strong>조기재취업수당</strong>이라는 제도가 있어요. 수급기간의 절반 이상을 남기고 재취업하면, 남은 급여의 절반을 일시금으로 받을 수 있죠. 180일 중 60일만 쓰고 취업하면 120일의 절반인 60일분을 한꺼번에 받는 구조예요. 놓치면 아까우니까 해당되는지 꼭 확인하세요.
      </p>
      <p style={body}>
        재취업한 직장에서 다시 퇴직했다면? 새 직장에서의 고용보험 가입기간이 180일 이상이고 비자발적 퇴사라면 또다시 실업급여를 신청할 수 있어요. 결국 매번 같은 조건을 새로 충족해야 하는 거죠. 가입기간이 모자라면 이전 직장에서 수급받지 않은 잔여 피보험기간이 있는지도 따져봐야 해요.
      </p>

      <SectionBadge>재신청 전 반드시 확인하세요</SectionBadge>
      <Checklist items={[
      ...
      <p style={body}>
        절차는 처음 받을 때와 동일해요.{" "}
        <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 온라인 교육을 이수하고, 관할 고용센터에 방문해서 수급자격 인정을 받은 뒤, 정기적으로 실업인정(구직활동 보고)을 하면 돼요. 한 번 받아본 경험이 있으니 절차 자체는 낯설지 않을 거예요.
      </p>
      <p style={body}>
        다만 이번에 받을 수 있는 금액과 기간은 이전과 다를 수 있어요. 새 직장 기준의 퇴직 전 3개월 평균임금과 가입기간으로 다시 계산하기 때문이에요. 이전 직장보다 월급이 높았다면 수급액이 올라갈 수도 있고, 반대일 수도 있죠.
      </p>
      <p style={body}>
        주의할 점 하나. 퇴직 후 <strong>12개월 이내</strong>에 신청해야 해요. 이 기한이 지나면 수급 자격 자체가 사라지니까, 퇴직하면 미루지 말고 빠르게 움직이세요. 이전에 받았던 경험을 살려서 서류도 미리 챙겨두면 훨씬 수월하죠.
      </p>
      ...
      <p style={body}>
        실업급여 재신청에서 가장 많이 실수하는 부분은 "이전 가입기간이 합산될 거다"라는 오해예요. 한 번 수급하면 피보험기간은 완전히 리셋되니까, 새 직장에서의 기간만으로 수급일수가 결정되죠. 짧게 다니고 나오면 수급일수도 짧아져요.
      </p>
      <p style={body}>
        그래서 재취업 후에는 <strong>가능한 오래 다니는 게 유리</strong>해요. 가입기간이 길수록 수급일수가 늘어나고, 퇴직 전 평균임금이 높을수록 1일 수급액도 올라가니까요. 물론 비자발적 퇴사 요건도 갖춰야 하는 건 마찬가지예요.
      </p>
      <p style={body}>
        마지막으로 <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 본인의 피보험기간과 수급 이력을 미리 조회해보세요. 관할 고용센터(1350)에 전화 상담을 받으면 재신청 가능 여부를 확인할 수 있어요. 미리 준비할수록 수급까지 걸리는 시간을 줄일 수 있죠.
      </p>
