...
const CHECK_ITEMS = [
  { id: "c1", label: "회사가 DC형(확정기여형) 퇴직연금 제도를 채택했어요" },
  { id: "c2", label: "회사가 매년 내 IRP 계좌에 연봉의 1/12 이상을 납입했어요" },
  { id: "c3", label: "내가 직접 운용 지시를 내려 펀드나 예금으로 운용하고 있어요" },
  { id: "c4", label: "퇴직 시 IRP 계좌로 잔액 전액이 이전될 예정이에요" },
];
...
const STEPS = [
  {
    title: "적립 금융기관 확인",
    desc: "회사 인사팀에 DC형 퇴직연금을 운용 중인 금융기관(은행·증권사·보험사)을 확인해요. 해당 기관 앱에 로그인하면 현재 적립 잔액과 운용 상품 목록을 바로 볼 수 있어요.",
    tip: "인사팀에 '퇴직연금 운용 기관명과 계좌번호'를 요청하세요",
  },
  {
    title: "운용 지시 선택",
    desc: "적립된 원금을 어떤 상품으로 운용할지 직접 결정해요. 원리금보장형(정기예금·ELB)과 실적배당형(ETF·TDF·펀드) 중 선택할 수 있어요. 운용 지시를 하지 않으면 기본형(원리금보장) 상품으로 자동 배정돼요.",
    tip: "TDF(타깃데이트펀드)는 퇴직 시점이 가까울수록 자동으로 안전자산 비중이 높아지는 상품이에요",
  },
  {
    title: "퇴직 시 IRP 계좌로 이전",
    desc: "퇴직하면 DC형 계좌 전액이 본인 IRP 계좌로 자동 이전돼요. IRP 계좌가 없으면 퇴직 처리가 지연되니, 퇴직 전에 미리 개설해두는 게 좋아요. 이전된 금액은 IRP 계좌 안에서 계속 운용돼요.",
    tip: "IRP 계좌는 은행·증권사·보험사 어디서든 개설 가능해요. 수수료가 낮은 곳을 고르세요",
  },
  {
    title: "일시금 또는 연금 수령",
    desc: "IRP 잔액을 일시금으로 인출하면 퇴직소득세가 원천징수돼요. 55세 이후 연금으로 10년 이상 나눠 받으면 퇴직소득세의 30%(10년 초과 시 40%)를 줄일 수 있어요. 55세 이전 인출은 기타소득세 16.5%가 적용돼서 불리해요.",
    tip: "연금 수령 기간이 길수록 절세 효과가 커요",
  },
];

const CHECKLIST = [
  "DC형 여부 확인: 인사팀에 퇴직연금 제도 방식(DB/DC) 문의",
  "연간 납입 확인: 연봉 ÷ 12 이상이 실제로 납입됐는지 체크",
  "운용 지시: 원리금보장형 vs 실적배당형 선택 (방치 금지)",
  "IRP 계좌 사전 개설: 퇴직 직전 아닌 재직 중에 미리 만들기",
  "수령 방식 결정: 연금(55세 이후 10년 이상) vs 일시금 비교",
  "미납 여부 점검: 중도 퇴직·휴직 기간 납입 공백 확인",
];

const FAQS = [
  {
    q: "DC형과 DB형 중 어느 게 퇴직금이 더 많이 나오나요?",
    a: "임금 인상률이 높을수록 DB형이 유리해요. DC형은 운용 수익을 직접 가져가기 때문에 ETF 수익률이 임금 인상률보다 높으면 DC형이 더 많아질 수 있어요. 임금이 정체된 직장이라면 DC형을 적극 운용하는 게 유리한 경우가 많아요.",
  },
  {
    q: "회사가 납입 의무를 안 지키면 어떻게 해야 하나요?",
    a: "근로자퇴직급여보장법 제20조에 따라 사업주는 매년 연봉의 1/12 이상을 납입할 법적 의무가 있어요. 미납이 확인되면 고용노동부(1350)에 신고할 수 있고, 회사는 과태료 대상이 돼요. 미납 금액은 퇴직 시 추가 청구할 수 있어요.",
  },
  {
    q: "운용 지시를 한 번도 안 했는데 괜찮나요?",
    a: "운용 지시를 하지 않으면 기본 원리금보장형 상품으로 자동 배정돼요. 원금 손실 위험은 없지만 수익률이 낮아 물가상승률에 못 미칠 수 있어요. 지금이라도 금융기관 앱에서 운용 지시를 변경할 수 있어요.",
  },
  {
    q: "DC형은 중도 인출(중간정산)이 되나요?",
    a: "법정 사유에 해당할 때만 가능해요. 무주택자의 주택 구입, 주거 목적 전세 보증금, 본인·부양가족 의료비(6개월 이상 요양), 천재지변 등이 인정돼요. 중도 인출 시 퇴직소득세가 원천징수되고, 인출 후 해당 금액은 운용에서 빠져요.",
  },
  {
    q: "퇴직 후 DC형 잔액이 IRP로 이전됐는데, 바로 인출해도 되나요?",
    a: "가능해요. 다만 퇴직소득세가 원천징수돼요. 55세 이전에 인출하면 기타소득세 16.5%가 추가로 붙어서 손해예요. 여유가 있다면 55세 이후 연금으로 나눠 받는 방식이 세금을 30~40% 줄여줘요.",
  },
  {
    q: "DC형 적립금을 ETF로 운용하다가 손실이 나면 어떻게 되나요?",
    a: "DC형은 운용 손익이 전부 근로자 본인에게 귀속돼요. 손실이 나면 수령액이 원금보다 줄어들 수 있어요. DB형은 회사가 리스크를 부담하지만, DC형은 근로자가 직접 감당해요. 퇴직이 가까울수록 안전자산 비중을 높이는 게 좋아요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        퇴직 시 받는 금액은 납입 원금과 운용 수익의 합계라서, 어떤 상품을 골랐느냐에 따라 최종 수령액이 달라져요.
        <a href="https://www.law.go.kr/법령/근로자퇴직급여보장법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법 제20조</a>에 따라 사업주는 매년 연봉의 1/12 이상을 납입할 의무가 있어요.
      </p>
      ...
      <p style={body}>
        ...
        같은 회사라도 DB형을 쓰는 곳이 있고 DC형을 쓰는 곳이 있어서, 먼저 인사팀에 확인하는 게 첫 번째예요.
      </p>
      <p style={body}>
        DB형(확정급여형)은 회사가 퇴직금 운용을 책임지고 최종 평균임금 기준으로 지급해요.
        반면 DC형은 근로자가 직접 운용하고, 수익도 손실도 본인 몫이에요.
      ...
      <p style={body}>
        ...
        실적배당형 상품의 수익률은 보장이 아닌 예측치라서 실제와 다를 수 있어요.
      ...
      <p style={body}>
        ...
        퇴직 시에는 IRP 계좌로 잔액이 이전되기 때문에, 퇴직 전에 IRP 계좌를 미리 개설해두는 게 중요해요.
      </p>
      ...
      <p style={body}>
        DC형 퇴직연금은 적립만 되고 방치하면 수익률이 낮아져요.
        아래 절차대로 하면 납입 원금을 최대한 불리고, 퇴직 후 세금도 줄일 수 있어요.
      </p>
      ...
      <p style={body}>
        ...
        운용 지시 방치, IRP 계좌 미개설, 납입 공백 등은 퇴직 후 수령액에 직접 영향을 줘요.
      </p>
