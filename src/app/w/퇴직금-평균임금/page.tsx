...
//     퇴직 후 퇴직금을 받았는데 기본급만 기준으로 계산된 것 같아 실제 금액이 맞는지 의심스러운 상황이에요.
// Q2. 이 사람이 이 글을 읽고 나서 할 수 있어야 하는 행동은?
//     평균임금 포함 항목과 계산 4단계를 직접 적용해 회사가 준 퇴직금과 비교하고, 차액이 있으면 신고할 수 있어야 해요.
// Q3. 이 행동을 하려면 반드시 알아야 하는 정보는?
//     포함 항목(상여금·연차수당·고정수당), 3개월 총 일수 산정 방법, 통상임금 비교 선택권, 서류 준비, 신고 절차예요.
// Q4. 이 정보를 가장 잘 전달하는 형태는?
//     포함/제외 GreenBox + 계산기 + 4단계 Steps + 체크리스트 + FAQ 순서로 전달해요.
...
const CHECK_ITEMS = [
  ...
  { id: "c2", label: "상여금을 정기적으로 받았죠 (분기·반기·연간 포함)" },
  { id: "c3", label: "고정 야근수당이나 직무수당을 매월 받았죠" },
  { id: "c4", label: "회사가 기본급만 기준으로 퇴직금을 계산해서 줬죠" },
];
...
const STEPS = [
  ...
  {
    title: "3개월 총 일수 파악",
    desc: "달력 기준 실제 일수를 세야 해요. 1~3월은 90일, 2월이 포함된 분기는 89일(윤년 90일)이에요. 단순히 90일로 고정하거나 30일로 나누면 틀려요. 달력에서 퇴직 전 3개월의 시작일~퇴직일까지 일수를 직접 세야 정확한 값이 나와요.",
    tip: "2월이 포함된 기간은 총 일수가 달라지니 꼭 달력으로 세봐요",
  },
  {
    title: "1일 평균임금 계산",
    desc: "3개월 임금 총액 ÷ 3개월 총 일수 = 1일 평균임금이에요. 예를 들어 3개월 임금 합계가 900만원이고 총 일수가 91일이면 1일 평균임금은 약 98,901원이에요.",
    tip: "기본급만 계산한 경우 상여금·수당 포함 시 실제보다 10~30% 낮게 나올 수 있죠",
  },
  {
    title: "퇴직금 산정 및 통상임금과 비교",
    desc: "1일 평균임금 × 30일 × 근속연수 = 퇴직금이에요. 근속 1년 6개월이면 × 1.5를 해요. 산출한 평균임금이 통상임금보다 낮으면 통상임금을 선택할 수 있죠. 두 가지 모두 계산해서 높은 쪽을 택하는 게 유리해요.",
...
const FAQS = [
  {
    ...
    a: "3개월 기간 중에 상여금이 한 번 들어왔다고 해서 그 금액만 넣으면 안 돼요. 연간 상여금 총액 ÷ 12로 월 환산해서 3개월치를 포함해야 해요. 예를 들어 연 600만원 상여금이면 월 50만원, 3개월분 150만원을 포함하는 거죠.",
  },
  {
    q: "식대·교통비도 평균임금에 포함되나요?",
    a: "근로의 대가로 정기적·일률적으로 지급된다면 포함돼요. 취업규칙에 명시됐거나 매월 빠짐없이 지급됐다면 포함 가능성이 높죠. 반면 실비 보전 성격으로 영수증 제출 후 지급되는 건 제외돼요.",
  },
  {
    q: "평균임금이 통상임금보다 낮으면 어떻게 하나요?",
    a: "통상임금을 평균임금으로 사용할 수 있죠. 근로기준법 제2조 2항이 이를 허용하고 있죠. 두 방법을 모두 계산한 뒤 높은 쪽을 선택하면 돼요. 기본급에 고정수당만 더한 통상임금이 평균임금보다 높게 나오는 경우도 있거든요.",
  },
  {
    q: "퇴직 전에 무급 병가나 휴직이 있었는데 어떻게 되나요?",
    a: "무급 기간이 포함된 달은 임금 총액이 줄어서 평균임금이 낮아질 수 있죠. 업무상 부상·질병으로 인한 휴업 기간은 평균임금 산정에서 제외할 수 있죠. 출산휴가·육아휴직 기간도 마찬가지예요.",
  },
  {
    q: "회사가 평균임금을 잘못 계산했는데 어떻게 하나요?",
    a: "고용노동부(1350)에 진정을 접수하면 돼요. 근로감독관이 급여명세서와 근로계약서를 기준으로 재산정해줘요. 차액이 발생하면 추가 지급 시정 명령이 내려지고, 퇴직금 청구권은 3년 안에 행사해야 해요.",
  },
  {
    q: "3개월 총 일수를 어떻게 세나요?",
    a: "퇴직일 기준으로 역산해요. 퇴직일이 3월 31일이면 1월 1일부터 3월 31일까지 90일이에요. 12월 31일 퇴직이면 10월 1일부터 12월 31일까지 92일이고요. 달마다 일수가 다르니 달력으로 직접 세는 게 정확해요.",
  },
];
...
const RELATED = [
  { slug: "퇴직금-계산법", title: "퇴직금 계산법, 얼마나 받을까?", description: "평균임금 기준 계산기로 직접 금액을 뽑아볼 수 있죠." },
...
export default function Page() {
  return (
    ...
    >
      ...
      <p style={body}>
        식대·교통비는 회사 규정에 따라 달라요. 취업규칙에 명시됐거나 매월 일률적으로 지급됐으면 포함될 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="상여금·수당까지 포함한 정확한 평균임금을 계산할 수 있죠. 아래 계산기로 예상액을 뽑아봐요."
        partialMatchText="항목별로 따져볼 부분이 있죠. 고용노동부(1350) 상담을 받아보는 게 좋아요."
      />
      ...
      <p style={body}>
        월 기본급, 상여금 월 환산액, 고정수당을 입력하면 월 평균임금과 퇴직금 예상액을 바로 뽑아볼 수 있죠.
      ...
      <p style={body}>
        회사가 평균임금을 잘못 계산했다면 직접 따져보고 이의를 제기할 수 있죠.
        아래 서류가 있으면 정확한 계산이 가능하고, 분쟁이 생겼을 때 핵심 증거가 돼요.
        퇴직 전에 미리 챙겨두는 게 중요해요.
      </p>
      <p style={body}>
        급여명세서는 퇴직 후에도 회사에 요청할 수 있죠.
      ...
      <p style={body}>
        ...
        이 두 가지만 제대로 해도 회사 계산과 차이를 잡아낼 수 있죠.
      </p>
      ...
      <H2>계산 전 꼭 챙겨야 할 것들</H2>
      <p style={body}>
        하나라도 빠지면 퇴직금이 실제보다 낮게 계산돼요.
        특히 상여금과 연차수당은 빠뜨리는 경우가 많고, 3개월 총 일수를 90일로 고정하는 실수도 흔하죠.
      </p>
      ...
      <GreenBox>
        산출한 평균임금이 통상임금보다 낮을 때는 통상임금을 기준으로 계산할 수 있죠.<br />
        근로기준법 제2조 2항이 보장하는 권리예요. 두 가지를 모두 계산해서 높은 쪽을 선택해요.
      </GreenBox>
