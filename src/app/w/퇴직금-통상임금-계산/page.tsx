...
// Q1. 퇴직 후 급여명세서를 보니 고정 수당이 있었는데, 회사가 기본급만으로 퇴직금을 계산해서 지급한 것 같아 차액을 돌려받을 수 있는지 궁금한 상황.
// Q2. 통상임금과 평균임금을 직접 계산해서 더 높은 쪽을 기준으로 회사에 차액을 청구한다.
// Q3. 통상임금 포함·제외 항목 구분 / 두 기준 계산 공식 / 차액 청구 절차와 소멸시효 / 회사가 거부할 때 대응 방법.
// Q4. EligibilityChecker(내 상황 진단) → Calculator(두 기준 비교) → DocTable(필요 서류) → Steps(청구 절차) → Checklist(최종 점검).
//
// MAP:
// - Q1 → 서론 톤: 회사가 기본급만 계산했을 때 독자 공감
// - Q2 → H2 순서: 포함항목 판단 → 계산기 → 서류 → 절차 → 체크리스트
// - Q3 → H2 개수/깊이: 5개 H2, 포함항목·절차 섹션에 설명 충분히
// - Q4 → 컴포넌트 배치: EligibilityChecker, Calculator, DocTable, Steps, Checklist, FAQ
...
const CHECK_ITEMS = [
  { id: "c1", label: "기본급 외 매월 고정으로 식대·교통비·직책수당을 받고 있죠" },
  { id: "c2", label: "육아휴직, 무급휴직, 장기병가로 3개월 임금이 줄었죠" },
  { id: "c3", label: "회사가 기본급만으로 퇴직금을 계산해서 지급했죠" },
  { id: "c4", label: "퇴직한 지 아직 3년이 지나지 않았죠" },
];
...
const CALC_RESULTS = [
  ...
  },
  {
    ...
    format: (v: number) => `약 ${Math.round(v / 10000).toLocaleString()}만원 추가 수령 가능`,
  },
];
...
const STEPS = [
  {
    title: "통상임금 구성 항목 판단",
    desc: "기본급에 정기적·일률적·고정적으로 지급되는 수당을 더해요. 식대(고정분), 교통비(고정분), 직책수당, 고정 자격수당이 포함되죠. 성과급·인센티브·실적에 따른 항목은 빠지고요.",
    tip: "근로계약서나 취업규칙에 '매월 정기 지급'으로 명시된 수당만 포함돼요",
  },
  {
    title: "평균임금과 통상임금 모두 계산",
    desc: "1일 평균임금 = 퇴직 전 3개월 총임금 ÷ 3개월 총 일수. 1일 통상임금 = 월 통상임금 ÷ 30일. 두 금액을 각각 구해서 어느 쪽이 높은지 비교하는 거예요.",
    tip: "육아휴직·무급휴직 기간이 있으면 평균임금이 낮게 나와 통상임금이 유리해요",
  },
  {
    title: "높은 쪽으로 퇴직금 계산",
    desc: "근로기준법 제2조 제2항은 평균임금이 통상임금보다 낮은 경우 통상임금을 평균임금으로 사용하도록 규정해요. 두 값 중 높은 쪽에 '× 30일 × 근속연수'를 곱하면 퇴직금이 나와요.",
    tip: "고정 수당이 월급의 20% 이상이면 통상임금 기준이 유리한 경우가 많아요",
  },
  {
    title: "회사에 서면 청구 또는 고용노동부 진정",
    desc: "회사가 기본급만으로 지급했다면 차액을 서면으로 청구해요. 내용증명으로 발송하면 법적 증거가 되죠. 거부할 경우 고용노동부 고객상담센터(1350)에 진정을 낼 수 있고요. 소멸시효는 퇴직일로부터 3년이에요.",
...
const FAQS = [
  {
    ...
    a: "통상임금은 기본급 + 정기·일률·고정 수당이에요. 매달 조건 없이 받는 금액이죠. 평균임금은 퇴직 전 3개월에 실제로 받은 금액의 평균이고요. 야근이 많았던 달이 포함되면 평균임금이 높고, 육아휴직 기간이 있으면 평균임금이 낮아져요.",
  },
  {
    q: "식대 20만원이 통상임금에 포함되나요?",
    a: "매월 모든 근로자에게 조건 없이 정기 지급된다면 통상임금이에요. 단, 실제로 식사한 날만 지급하거나 금액이 달라지는 실비 식대는 제외되죠. 근로계약서에 '월 20만원 정기 지급'으로 명시돼 있으면 포함돼요.",
  },
  {
    q: "성과급은 통상임금인가요?",
    a: "아니에요. 지급 여부와 금액이 실적에 따라 달라지면 고정성이 없어서 통상임금에서 빠지죠. 단, 최소한의 금액을 무조건 지급하는 구조라면 그 최소 금액은 포함될 수 있고요.",
  },
  {
    q: "육아휴직 중에 퇴직하면 통상임금 기준이 유리한가요?",
    a: "맞아요. 육아휴직 기간에는 임금을 받지 않거나 적게 받기 때문에 3개월 평균임금이 낮게 나와요. 이 경우 통상임금이 평균임금보다 높으면 통상임금 기준으로 퇴직금을 계산하는 거예요.",
  },
  {
    q: "회사가 통상임금 기준 적용을 거부하면?",
    a: "고용노동부 고객상담센터(1350)에 진정을 낼 수 있죠. 근로계약서, 급여명세서, 취업규칙을 증빙으로 갖추고 신청하면 되고요. 퇴직 후 3년 내라면 청구 권리가 살아 있죠.",
  },
  {
    q: "기본급 외 수당을 회사가 인정 안 하면?",
    a: "급여명세서에 수당 항목이 찍혀 있다면 그 자체가 증거예요. 근로계약서나 취업규칙에 명시된 항목도 마찬가지죠. 서류가 없다면 계좌 이체 내역으로도 수당 지급 사실을 입증할 수 있고요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        회사가 기본급만으로 퇴직금을 계산했다면, 고정 수당까지 포함한 통상임금 기준으로 다시 계산하면 차액이 나오죠.
        <a href="https://www.law.go.kr/법령/근로기준법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로기준법 제2조 제2항</a>은
        평균임금이 통상임금보다 낮으면 통상임금을 기준으로 하도록 명시하고 있죠.
        육아휴직, 무급휴직, 장기 병가로 3개월 임금이 줄었다면 통상임금 기준이 유리할 가능성이 높고요.
      </p>

      <Divider />
      <ArticleAd position="intro" />

      <H2>통상임금 기준이 유리한 상황인지 먼저 짚어봐요</H2>
      <p style={body}>
        통상임금은 기본급에 정기적·일률적·고정적으로 지급되는 수당을 더한 금액이에요.
        매달 변동 없이 받는 식대·교통비·직책수당 같은 항목들이 포함되죠.
        반면 평균임금은 퇴직 전 3개월에 실제로 받은 금액의 평균이라서, 그 기간에 임금이 줄었다면 통상임금보다 낮게 나오고요.
      </p>
      <p style={body}>
        회사가 기본급만으로 퇴직금을 계산해서 지급했다면 차액 청구가 가능해요.
        퇴직금 청구권 소멸시효는 퇴직일로부터 3년이에요.
        이미 퇴직했어도 3년 내라면 차액을 돌려받을 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="통상임금 기준으로 계산하면 퇴직금이 더 높게 나올 가능성이 높아요. 아래 계산기로 두 기준을 비교해봐요."
      ...
      <p style={body}>
        기본급, 고정 수당, 근속 기간을 입력하면 두 기준의 퇴직금 차이를 바로 볼 수 있죠.
        고정 수당이 월 60만원이면 5년 근무 시 300만원 차이가 나요.
        수당이 클수록, 근속기간이 길수록 격차가 커지고요.
      </p>
      ...
      <p style={body}>
        ...
        근로계약서에 수당 내역이 명시돼 있다면 가장 강력한 근거가 되죠.
        급여명세서에 수당 항목이 매월 동일하게 찍혀 있어도 증거로 쓸 수 있고요.
      </p>
      ...
      <p style={body}>
        ...
        회사가 기본급만으로 계산해서 지급했다면 차액 청구가 가능하죠.
        거부할 경우 고용노동부에 진정을 낼 수 있고요.
      </p>
      ...
      <p style={body}>
        고정 수당이 있는데도 회사가 기본급만으로 계산했다면 이의 제기로 차액을 돌려받을 수 있죠.
        청구 기한인 3년이 지나면 권리가 소멸되니 서두르는 게 좋아요.
      </p>
      ...
      <GreenBox>
        평균임금 기준과 통상임금 기준을 모두 계산해서 높은 쪽을 선택할 수 있죠.<br />
        근로기준법 제2조 제2항이 보장하는 권리예요. 회사가 거부하면 고용노동부(1350)에 진정을 낼 수 있고요.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에 문의해봐요." />
    </ArticleLayout>
  );
}
