"use client";
// Q1. 1년 이상 일한 알바가 퇴직금 대상인지 몰라서, 사장에게 말 꺼내기 전 본인이 받을 수 있는지 먼저 따져보려는 상황
// Q2. 조건 충족 여부를 판단하고, 예상 금액을 계산해서 사장에게 직접 청구한다
// Q3. ① 1년+주15시간 조건(4대보험 무관), ② 시급제 평균임금 계산법, ③ 300만원 초과 시 IRP 필수, ④ 미지급 시 내용증명→진정 순서, ⑤ 소멸시효 3년
// Q4. EligibilityChecker(조건 체크) + Calculator(시급 계산) + Steps(대응 절차) + FAQ(예외상황) + DocTable(서류)
// MAP: Q1→서론 첫 문장(상황 공감) Q2→H2순서(조건→계산→서류→대응→체크리스트) Q3→H2깊이(5개 섹션) Q4→컴포넌트 선택
...
const STEPS = [
  {
    ...
    desc: "아래 계산기로 예상 금액을 미리 파악하세요. 그다음 사장에게 문자나 메일로 '퇴직금 지급 요청'을 보내고 날짜와 금액을 기록해두세요. 회사는 퇴직일 후 14일 이내에 지급해야 해요.",
    tip: "요청 메시지는 카카오톡보다 문자·이메일로 보내면 증거로 활용하기 더 좋아요",
  },
  {
    title: "IRP 계좌 개설 (300만원 초과 시)",
    desc: "퇴직금이 300만원을 초과하면 IRP(개인형퇴직연금) 계좌로만 받을 수 있죠. 은행이나 증권사 앱에서 10분이면 개설 가능해요. 계좌번호를 사장에게 전달하면 돼요.",
    tip: "300만원 이하면 일반 통장으로 받을 수 있죠",
    link: { label: "퇴직금 IRP 계좌 개설 방법", href: "/w/퇴직금-IRP-계좌" },
  },
  {
    title: "내용증명 발송 (미지급 시)",
    desc: "14일이 지났는데 연락이 없으면 내용증명을 보내세요. '퇴직금 미지급으로 고용노동부에 신고할 예정'이라는 내용이면 충분해요. 우체국 또는 카카오 전자내용증명으로 빠르게 발송할 수 있죠.",
    tip: "대부분 이 단계에서 지급 약속을 받아요",
  },
  {
    title: "고용노동부 진정 접수",
    desc: "내용증명 이후에도 무응답이면 고용노동부 민원마당에서 온라인으로 진정을 접수하세요. 무료이고 근로감독관이 사업장을 직접 조사해요. 보통 2~4주 내에 처리돼요.",
...
const FAQS = [
  {
    ...
    a: "주 15시간 이상 근무했다면 받을 수 있죠. 편의점, 카페, 식당, 학원 등 업종은 상관없어요. 1년 이상 같은 곳에서 계속 일했다는 게 핵심이에요.",
  },
  {
  ...
  },
  {
    ...
    a: "방학마다 계속 같은 곳에서 일했다면 합산이 가능해요. 다만 중간에 완전히 그만뒀다가 재입사했다면 기간이 끊길 수 있죠. 계약서와 급여 기록으로 연속성을 입증하는 게 중요해요.",
  },
  {
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="알바-퇴직금" />}
    >
      ...
      <p style={body}>
        여기서 "계속 근로"가 핵심이에요. 방학마다 같은 카페에서 일하면서 총 기간이 1년을 넘었다면 합산해서 퇴직금이 발생할 수 있죠.
        4대보험에 미가입됐어도 실제 근무 사실만 증명하면 받을 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="퇴직금을 받을 수 있는 조건을 갖췄어요. 아래 계산기로 예상 금액을 파악해보세요."
        partialMatchText="조건 일부가 맞지 않을 수 있죠. 고용노동부(1350)에 상담부터 받아보세요."
      />
      ...
      <p style={body}>
        상여금이나 연차수당이 있으면 평균임금에 포함돼서 실제 퇴직금이 더 높아질 수 있죠.
        계산기는 순수 시급 기준 추정치예요.
      </p>
      ...
      <Calculator
        ...
        note="※ 시급·근무시간 기준 추정치. 상여금·연차수당 포함 시 실제 금액이 더 높아질 수 있죠."
      />
      ...
      <p style={body}>
        4대보험에 미가입된 알바라도 실제 근무 사실만 증명하면 청구할 수 있죠.
      ...
      <GreenBox>
        ...
        4대보험 가입 이력은 고용24(ei.go.kr)에서 무료로 조회할 수 있죠.
      </GreenBox>
