...
const CHECK_ITEMS = [
  ...
  { id: "c2", label: "상여금을 분기별 또는 연 단위로 받아요" },
  { id: "c3", label: "야근수당이 포함된 달이 있어요" },
  { id: "c4", label: "퇴직 전 3개월 기준이 맞는지 확인하고 싶어요" },
];
...
const STEPS = [
  ...
  {
    title: "포함 항목 정리",
    desc: "기본급, 직무·직책 수당, 고정 야근수당, 상여금(월 환산), 연차수당(3개월 내 지급분)을 모두 합산해요. 임시 지급 항목이나 실비 변상은 제외해요.",
    tip: "상여금은 연간 총액 ÷ 12 × 3으로 3개월분 환산",
  },
  {
    title: "총합 ÷ 총 일수",
    desc: "3개월 총임금을 3개월의 총 일수(89~92일)로 나눠요. 달력에서 직접 세는 게 가장 정확해요. 2월이 포함된 분기는 총 일수가 줄어드니 주의하세요.",
    tip: "편의상 91일로 나눠도 큰 차이 없어요",
  },
  {
    title: "근속일수 적용",
    desc: "1일 평균임금 × 30 × (근속일수 ÷ 365)로 최종 퇴직금을 계산해요. 근속기간이 딱 1년·2년이 아니라도 일 단위로 정확히 계산할 수 있어요.",
...
const FAQS = [
  ...
  },
  {
    ...
    a: "그땐 통상임금을 평균임금으로 사용할 수 있어요. 근로기준법 제2조 2항이 보호 조항으로 명시하고 있어요.",
  },
];
...
const RELATED = [
  ...
  { slug: "퇴직금-계산법", title: "퇴직금 계산기", description: "월급·근속기간으로 빠르게 예상 금액 확인." },
  { slug: "퇴직금-상여금-포함", title: "상여금 퇴직금 포함 여부", description: "상여금 환산 방법을 자세히 설명해요." },
];
...
export default function Page() {
  return (
    ...
    >
      <p style={{ fontSize: 13, color: "#1D9E75", fontWeight: 600, marginBottom: 10 }}>퇴직금 · 평균임금 · 3개월산정</p>
      ...
      <p style={body}>
        ...
        연간 상여금 총액을 12로 나눈 뒤 3을 곱해서(= 3개월분) 넣어야 제대로 된 계산이에요.
      </p>
      ...
      <H2>평균임금 산정 4단계</H2>
      <p style={body}>
        단계별로 따라가면 헷갈리던 상여금 환산과 일수 계산이 명확해져요.
        특히 2단계 포함 항목 정리가 가장 중요해요.
      </p>
      ...
      <GreenBox title="평균임금 vs 통상임금, 뭐가 더 유리하죠?">
        계산해보면 알 수 있어요. 평균임금이 낮으면 통상임금으로 퇴직금을 계산할 수 있어요.
        두 가지 모두 계산해서 높은 쪽을 선택하세요.
      </GreenBox>
