"use client";
// Q1. 퇴직 후 몇 주 또는 몇 달이 지나도 퇴직금이 입금되지 않아 불안하고 어떻게 해야 할지 막막한 상황
// Q2. 내용증명 발송 → 노동청 진정 순서로 직접 행동해서 퇴직금을 실제로 받아낸다
// Q3. 청구 자격 요건, 내용증명 발송 방법, 노동청 진정 절차, 지연이자 계산법, 폐업 시 체당금 신청, 소멸시효 3년
// Q4. EligibilityChecker(자격체크) + Calculator(지연이자) + Steps(4단계 절차) + DocTable(서류) + Checklist(놓치지 말아야 할 것) + FAQ
// MAP: Q1→서론(막막함 공감) Q2→H2순서(청구→지연이자→서류→절차→체크리스트) Q3→H2깊이(각 단계 구체화) Q4→위 컴포넌트 조합
...
const CHECK_ITEMS = [
  { id: "c1", label: "1년 이상 계속 근무하고 주 15시간 이상 일했죠" },
  { id: "c2", label: "퇴직 후 14일이 지났는데 퇴직금이 입금되지 않았죠" },
  { id: "c3", label: "퇴직일로부터 3년(소멸시효)이 아직 지나지 않았죠" },
  { id: "c4", label: "회사에 지급 요청했지만 무응답이거나 거부하고 있죠" },
];
...
const STEPS = [
  {
    title: "회사에 직접 청구 기록을 남겨야 하죠",
    desc: "문자·이메일·카카오톡으로 '○월 ○일까지 퇴직금 ○○만원을 지급해 주세요'라고 요청하는 게 먼저예요. 전화는 기록이 남지 않아 나중에 불리하죠. 요청 메시지는 반드시 캡처해서 저장해두세요. 이 기록이 나중에 핵심 증거가 되죠.",
    tip: "요청 메시지에 금액을 명시하면 나중에 분쟁 시 유리하죠",
  },
  {
    title: "내용증명 발송 지연이자 청구 의사 명시",
    desc: "문자 요청 후에도 지급이 없으면 내용증명을 발송해야 해요. '퇴직금 ○○만원 및 14일 초과분 연 20% 지연이자를 ○일 이내 지급해 달라'는 내용을 넣어야 하죠. 우체국 방문 또는 카카오 전자내용증명으로 10분 안에 발송 가능하고, 발송 즉시 소멸시효가 6개월 중단돼요.",
    tip: "지연이자 청구 의사를 명시하지 않으면 원금만 받게 될 수 있죠",
  },
  {
    title: "고용노동부 임금체불 진정 접수",
    desc: "고용24(ei.go.kr)에서 온라인으로 무료 접수할 수 있죠. 진정서에 '퇴직금 ○○만원 및 연 20% 지연이자 포함 청구'를 명시해야 해요. 접수 즉시 소멸시효가 완전히 중단되고, 근로감독관이 배정돼서 사업주에게 시정 명령을 내리죠. 대부분 이 단계에서 해결돼요.",
    tip: "진정 접수는 무료. 방문은 관할 고용노동청 근로감독과로 가세요",
    link: { label: "고용24에서 진정 접수하기", href: "https://www.ei.go.kr" },
  },
  {
    title: "소액심판 또는 민사소송 폐업 시 체당금",
    desc: "진정 이후에도 지급하지 않으면 법원에 소액심판(3,000만원 이하)을 신청할 수 있죠. 변호사 없이 직접 신청 가능하고, 인지대는 수만 원 수준이에요. 회사가 폐업했다면 고용노동부에 체당금(임금채권보장제도)을 신청하면 정부가 대신 지급해주죠. 대한법률구조공단(132)에서 무료 법률 지원도 받을 수 있죠.",
...
const CHECKLIST = [
  ...
  "내용증명에 '연 20% 지연이자 포함' 명시 없으면 원금만 받죠",
...
const FAQS = [
  {
    ...
    a: "법적 청구 의사를 공식적으로 전달하는 거예요. 많은 회사가 내용증명을 받으면 바로 지급하죠. 소멸시효도 6개월 중단되는 효과가 있고요.",
  },
  {
    q: "노동청 진정은 얼마나 걸리나요?",
    a: "보통 1~3개월이에요. 단순 미지급 건은 더 빠르게 처리되는 경우가 많죠. 민사소송(6개월~1년)보다 훨씬 빠르고 무료예요.",
  },
  {
    q: "회사가 폐업했으면 어떻게 받나요?",
    a: "체당금 제도로 고용노동부에서 대신 받을 수 있죠. 파산 선고일 또는 폐업일로부터 2년 내 신청해야 하고요. 퇴직금 전액이 아닌 일정 한도 내에서 지급돼요.",
  },
  {
    q: "지연이자도 같이 받을 수 있나요?",
    a: "받을 수 있죠. 진정서나 소송장에 '연 20% 지연이자 포함'을 명시해야 받을 수 있고요. 자동으로 주지 않으니 반드시 명시하세요.",
  },
  {
    q: "소액심판이 뭔가요?",
    a: "3,000만원 이하 분쟁을 법원에서 빠르게 해결하는 절차예요. 변호사 없이 직접 신청 가능하고 인지대가 수만 원이죠. 판결이 나면 강제집행도 가능해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        내용증명 → 노동청 진정 순서로만 밟으면 대부분 3단계 이내에서 해결되죠.
      ...
      <p style={body}>
        ...
        이 조건을 갖췄다면 퇴직일로부터 3년 안에 언제든 청구할 수 있죠.
      </p>
      <p style={body}>
        14일 기한이 지났다면 이미 지연이자가 쌓이고 있죠.
      ...
      <EligibilityChecker
        ...
        partialMatchText="상황에 따라 다를 수 있으니 고용노동부(1350)에 먼저 상담하세요."
      />
      ...
      <p style={body}>
        ...
        노동청 진정 단계에서 함께 청구해야 받을 수 있고요. 빠뜨리면 원금만 받게 되죠.
      </p>
      <p style={body}>
        지연이자 계산은 퇴직 후 14일 초과 다음 날부터 시작해요.
        아래 계산기에서 기간을 조정하면 예상 지연이자를 바로 볼 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        근로 사실과 미지급 사실만 증명하면 충분하고, 없는 서류는 온라인으로 대체할 수 있죠.
      </p>
      ...
      <BorderBox>
        통장 입금 내역 + 카카오톡 대화 + 동료 증언만으로도 진정 접수는 가능해요.
        4대보험 가입 이력은{" "}
      ...
      <p style={body}>
        ...
        소멸시효는 내용증명 발송 시 6개월 중단되고, 진정 접수 시 완전 중단되죠.
      </p>
      ...
      <GreenBox>
        진정서나 소송장에 "퇴직금 ○○만원 및 이에 대한 연 20% 지연손해금"으로 명시해야 받을 수 있죠.<br />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 임금채권보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 직접 안내받으세요." />
    </ArticleLayout>
  );
}
