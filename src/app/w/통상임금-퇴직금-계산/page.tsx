...
//     퇴직금을 받았거나 받을 예정인데, 기본급만으로 계산된 금액이 적다는 느낌이 들어서 통상임금 기준으로 더 받을 수 없는지 따져보려는 상황이에요.
// Q2. 이 사람이 이 글을 읽고 나서 할 수 있어야 하는 행동은?
//     고정 수당이 통상임금에 포함되는지 판단하고, 두 기준을 직접 비교해서 회사에 재계산을 요청하는 것이에요.
// Q3. 이 행동을 하려면 반드시 알아야 하는 정보는?
//     통상임금 포함 기준(정기·일률·고정성), 평균임금과의 비교 방법, 수당별 포함 여부 판단법, 재계산 요청 절차와 거부 시 대응법이에요.
// Q4. 이 정보를 가장 잘 전달하는 형태는?
//     상황 체크 → 비교 계산기 → 단계별 절차 → FAQ 순서로, 독자가 바로 행동할 수 있는 구조예요.
...
const CHECK_ITEMS = [
  { id: "c1", label: "기본급 외 고정 수당(식대·직책수당·교통비 등)이 매월 지급되고 있죠" },
  { id: "c2", label: "육아휴직·무급휴직 기간이 퇴직 전 3개월에 포함돼 있죠" },
  { id: "c3", label: "퇴직 전 3개월 중 특정 달에 급여가 크게 줄거나 비정상적이었죠" },
  { id: "c4", label: "회사가 기본급만으로 퇴직금을 계산했죠" },
];
...
const STEPS = [
  {
    ...
    tip: "병가·육아휴직·무급휴직 기간이 포함된 달은 평균임금이 낮아지는 경우가 많아요",
  },
  {
    title: "통상임금 계산",
    desc: "기본급 + 정기적·일률적·고정적으로 지급되는 수당의 합이에요. 성과급처럼 지급 여부가 불확실한 건 빠지고, 매월 모든 직원에게 빠짐없이 지급되는 식대·교통비·직책수당이 대표적인 포함 항목이에요.",
  ...
  },
  {
    ...
    tip: "두 금액을 모두 계산해서 비교해봐요",
  },
  {
    title: "회사에 재계산 요청",
    desc: "통상임금 기준이 유리하다면 회사에 수당을 포함한 퇴직금 재계산을 요청해요. 회사가 거부하면 고용노동부(1350)에 진정을 낼 수 있고, 증빙 서류(급여명세서, 취업규칙)를 미리 갖춰두는 게 좋아요.",
    tip: "거부 시 내용증명을 먼저 보내고 그래도 안 되면 노동청 진정",
  },
];

const CHECKLIST = [
  "통상임금 항목 체크: 식대·교통비·직책수당의 정기성·일률성·고정성 여부",
...
const FAQS = [
  {
    ...
    a: "육아휴직·무급병가 등으로 퇴직 전 3개월 임금이 평소보다 줄었을 때, 또는 고정 수당이 크고 평균임금 산정기간에 비정상적인 달이 끼었을 때 통상임금이 더 높게 나와요.",
  },
  {
    q: "식대 20만원이 통상임금에 포함되나요?",
    a: "매월 모든 근로자에게 정기적·일률적으로 지급된다면 통상임금에 해당해요. 취업규칙에 명시됐거나 근로계약서에 '고정 식대'로 표기된 경우 더 확실하게 인정받을 수 있죠.",
  },
  {
    q: "성과급은 통상임금인가요?",
    a: "원칙적으로 아니에요. 지급 여부나 금액이 실적에 따라 달라지는 성과급은 고정성이 없어 통상임금에 해당하지 않죠. 다만 일정 요건만 충족하면 무조건 지급되는 조건부 상여금은 통상임금으로 볼 수도 있죠.",
  },
  {
    q: "회사가 통상임금 기준 적용을 거부하면 어떻게 하나요?",
    a: "급여명세서·취업규칙을 갖추고 고용노동부(1350)에 진정을 낼 수 있죠. 대법원 판례에서도 정기·일률·고정 수당은 통상임금으로 인정한 판결이 다수고, 증빙 서류가 있으면 인용 가능성이 높아요.",
  },
  {
    q: "고정 수당이 월 50만원이면 퇴직금 차이가 얼마나 나나요?",
    a: "근속 5년 기준으로 약 250만원(50만원 × 5년) 더 받게 돼요. 근속이 길수록, 고정 수당이 많을수록 차이가 커지죠.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="통상임금-퇴직금-계산" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        그런데 육아휴직이나 무급휴직이 끼어 있거나, 고정 수당이 많다면 통상임금 기준이 더 높게 나올 수도 있죠.
        <a href="https://www.law.go.kr/법령/근로기준법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로기준법 제2조 2항</a>은 평균임금이 통상임금보다 낮으면 통상임금으로 계산하도록 규정하고 있죠.
        기본급만으로 퇴직금을 받았다면 지금 바로 두 기준을 비교해봐요.
      </p>
      ...
      <p style={body}>
        ...
        통상임금으로 계산한 퇴직금이 평균임금 기준보다 높다면 통상임금 기준을 선택할 수 있죠.
      </p>
      <p style={body}>
        회사가 기본급만으로 퇴직금을 계산했다면 고정 수당이 누락된 거예요.
        이런 경우 통상임금 기준으로 재계산을 요청할 수 있고, 거부 시 <a href="/w/퇴직금-미지급-신고" style={{ color: "#1D9E75", textDecoration: "underline" }}>노동청 진정</a>을 낼 수 있죠.
      </p>

      <GreenBox>
        월 고정 식대 20만원 + 직책수당 30만원이 있다면<br />
        통상임금 = 기본급 + 50만원이에요.<br />
        이를 기준으로 퇴직금을 다시 계산해봐요.
      </GreenBox>

      <SectionBadge>내 상황 체크해봐요</SectionBadge>
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="통상임금 기준 계산이 더 유리할 수 있죠. 아래 계산기로 두 기준을 비교해봐요."
        partialMatchText="상황에 따라 다를 수 있죠. 고용노동부(1350) 상담을 권해요."
      />
      ...
      <p style={body}>
        월 기본급과 고정 수당, 근속연수를 입력하면 두 기준의 퇴직금 차이를 바로 비교할 수 있죠.
        고정 수당이 많을수록, 근속이 길수록 차이가 커지는 구조예요.
      </p>
      ...
      <p style={body}>
        ...
        취업규칙이나 근로계약서에 수당 내역이 명시돼 있으면 증빙이 쉬워지죠.
      </p>
      <p style={body}>
        급여명세서에 수당 항목이 별도로 표시된 경우에도 통상임금 포함 여부를 따질 수 있죠.
        서류가 없으면 회사 인사팀에 요청하거나, 근로복지공단 고용보험 내역에서 보완할 수 있죠.
      </p>
      ...
      <p style={body}>
        회사가 기본급만으로 계산했다면 먼저 수정 요청을 해봐요.
        거부하면 내용증명 발송 후 노동청에 진정을 내는 방법으로 해결할 수 있죠.
      </p>
      <p style={body}>
        증빙 서류를 갖추고 있으면 대부분 해결되는 편이에요.
        아래 4단계 절차를 따르면 어렵지 않아요.
      </p>
      ...
      <p style={body}>
        ...
        IRP 계좌 개설도 미리 해두면 퇴직금 이체가 빨라지죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 살펴봐요." />
    </ArticleLayout>
  );
}
