"use client";
// Q1. 퇴직을 앞둔 공무원이 일반 퇴직금과 뭐가 다른지, 얼마를 받는지 모르는 상황
// Q2. 본인이 받을 퇴직급여 종류를 파악하고 공무원연금공단에 신청한다
// Q3. 재직기간별 수령 형태(연금/일시금), 계산 방식, 명예퇴직수당, 신청 절차 4단계
// Q4. GreenBox(재직기간별 분기), Calculator(추정 계산), Steps(신청 절차), Checklist(퇴직 전 체크), FAQ
// MAP: Q1→서론(일반직장인과 차이) Q2→H2순서(종류→계산→서류→절차→체크) Q3→H2깊이(연금/일시금 분기 상세) Q4→컴포넌트
...
const STEPS = [
  {
    ...
    desc: "퇴직 후 소속기관이 공무원연금공단에 자동으로 신고해요. 개인이 직접 신고할 필요는 없고요. 기관 인사팀에 자동 처리 여부를 미리 확인해두는 게 좋아요.",
    tip: "퇴직 예정이라면 인사팀에 퇴직 신고 일정을 미리 물어보세요",
  },
  {
    title: "연금 vs 일시금 선택",
    desc: "10년 이상 재직한 공무원은 퇴직연금(평생 매월 지급)과 퇴직일시금(한 번에 지급) 중 선택할 수 있죠. 건강 상태와 기대 수명, 퇴직 후 소득 계획을 종합해서 결정해요. 선택 후 변경이 어렵기 때문에 신중하게 결정해야 하고요.",
    tip: "80세 이상 장수 가능성이 높다면 연금이 누적 금액 면에서 유리해요",
  },
  {
    title: "지급 신청",
    desc: "공무원연금공단 앱 또는 geps.or.kr에서 온라인으로 신청해요. 청구서, 신분증, 통장 사본을 준비해서 올리면 되고요. 방문 신청도 가능해요.",
    tip: "온라인 신청이 가장 빠르게 처리돼요",
    link: { label: "공무원연금공단 신청하기", href: "https://www.geps.or.kr" },
  },
  {
    title: "수령",
    desc: "신청 후 1~2개월 안에 지급돼요. 퇴직일시금은 일반 계좌로, 연금은 매월 지정 계좌로 입금되고요. 연금 수급 중 소득이 생기면 일부 감액될 수 있죠.",
...
const CHECKLIST = [
  ...
  "연금 vs 일시금 비교: 장기 수령 시 연금이 유리한 경우가 많죠",
...
const FAQS = [
  {
    q: "공무원도 퇴직금을 받나요?",
    a: "받죠. 다만 일반 근로자의 퇴직금(근로자퇴직급여보장법)이 아니라 공무원연금법에 따른 퇴직급여를 받아요. 이름만 다를 뿐, 퇴직 시 받는 급여라는 점은 같아요.",
  },
  {
    q: "5년 미만 재직했을 때도 퇴직급여가 있나요?",
    a: "있죠. 재직 기간에 상관없이 퇴직일시금으로 받을 수 있고요. 연금은 10년 이상 재직해야 선택할 수 있죠.",
  },
  {
    q: "연금과 일시금 중 어떤 게 유리한가요?",
    a: "오래 살수록 연금이 유리해요. 건강 상태가 좋고 퇴직 후 다른 소득이 없다면 연금을 선택하는 게 일반적으로 더 유리하고요. 반대로 목돈이 필요하거나 건강에 우려가 있다면 일시금도 고려할 수 있죠.",
  },
  {
  ...
  },
  {
    ...
    a: "퇴직수당, 사망조위금, 부조금 등 부가 급여가 있죠. 공무원연금공단 홈페이지(geps.or.kr)에서 급여 종류를 확인할 수 있고요.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="공무원-퇴직금" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        10년 이상 재직했다면 연금과 일시금 중 선택할 수 있고, 10년 미만이라도 퇴직일시금은 받을 수 있죠.
      ...
      <p style={body}>
        가장 큰 차이는 연금 선택권이에요. 10년 이상 재직한 공무원은 매월 연금을 받을지, 한 번에 일시금으로 받을지 직접 선택할 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="공무원 퇴직급여 신청 준비가 된 상태예요. 아래 계산기로 예상 금액을 먼저 보세요."
      ...
      <p style={body}>
        ...
        퇴직연금은 재직연수와 기준소득월액을 바탕으로 매월 지급 금액이 산정되고요.
        기준소득월액은 연간 소득을 12로 나눈 금액으로, 공무원연금공단에서 직접 조회할 수 있죠.
      </p>
      <p style={body}>
        아래 슬라이더로 기준소득월액과 재직 기간을 조정하면 추정치를 볼 수 있죠.
        실제 금액은 가입 이력, 기여금 납부 내역 등에 따라 달라지기 때문에 정확한 수치는 공무원연금공단에서 조회해야 하고요.
      </p>
      ...
      <p style={body}>
        ...
        신고가 완료되면 퇴직급여 선택과 지급 신청은 본인이 직접 해야 하고요.
      ...
      <p style={body}>
        ...
        연금과 일시금은 한 번 선택하면 되돌리기 어렵기 때문에 충분히 비교하고 결정해요.
      </p>
      ...
      <GreenBox>
        연금 수급 중 소득이 발생하면 연금이 일부 감액될 수 있죠.<br />
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 공무원연금법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니, 최신 기준은 공무원연금공단(1588-4321)에서 문의하세요." />
    </ArticleLayout>
  );
}
