...
// Q1. 퇴직을 앞두고 또는 이미 퇴직했는데 회사가 제시한 퇴직금이 맞는지 의심스럽고, 상여금·소수점 근속기간이 제대로 반영됐는지 직접 검증하려는 상황이에요.
// Q2. 법정 공식으로 직접 계산해서 회사 제시 금액과 비교하고, 차액이 있으면 재계산을 청구한다.
// Q3. 평균임금 포함 항목(상여금 환산법), 실제 총 일수 세는 법, 소수점 근속기간 계산, 통상임금 비교, 차이 발생 시 청구 방법이 반드시 필요해요.
// Q4. 공식 GreenBox(즉시 답) + 계산기(직접 적용) + Steps(순서) + FAQ(예외 상황) 조합으로 전달해요.
// MAP: Q1→서론 첫 문장(상황 공감) / Q2→H2 순서(공식→계산→검증→청구) / Q3→H2 개수와 깊이 / Q4→섹션별 컴포넌트
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직 전 3개월 급여명세서를 보관하고 있죠" },
  { id: "c2", label: "연간 상여금을 받았고 금액을 알고 있죠" },
  { id: "c3", label: "근속기간이 딱 떨어지지 않죠 (예: 3년 5개월)" },
  { id: "c4", label: "회사가 제시한 퇴직금 금액이 맞는지 직접 검증하고 싶죠" },
];
...
const STEPS = [
  ...
  {
    title: "1일 평균임금 계산",
    desc: "3개월 총임금 ÷ 3개월 총 일수(보통 89~92일)로 1일 평균임금을 구해요. 달력에서 실제 날짜를 세는 게 정확해요. 2월이 포함된 분기는 일수가 줄어 평균임금이 높아질 수 있죠.",
  ...
  },
  {
    ...
    tip: "고용24(work.go.kr)에서 입사일·퇴직일 기준 근속일수를 조회할 수 있죠",
    link: { label: "고용24에서 근속일수 조회", href: "https://www.work.go.kr" },
  },
  {
    title: "퇴직금 공식 적용 및 검증",
    desc: "1일 평균임금 × 30일 × 근속연수로 최종 퇴직금을 구해요. 회사 계산값과 차이가 5만원 이상이면 인사팀에 재계산을 요청하세요. 거부하면 고용노동부 고객상담센터(1350)에 진정을 낼 수 있죠. 평균임금이 통상임금보다 낮으면 통상임금 기준으로 재계산해요.",
...
const FAQS = [
  {
    q: "퇴직금 법정 계산 공식이 뭔가요?",
    a: "1일 평균임금 × 30일 × 근속연수이에요. 근로자퇴직급여보장법 제8조에 명시돼 있고요. 1일 평균임금은 퇴직 전 3개월 총임금 ÷ 3개월 총 일수로 구해요.",
  },
  {
    q: "상여금이 평균임금에 포함되나요?",
  ...
  },
  {
    ...
    a: "통상임금을 평균임금으로 대신 써요. 근로기준법 제2조 제2항이 이를 허용하고 있고요. 육아휴직, 무급휴직 기간이 포함된 경우 이 상황이 자주 생겨요.",
  },
  {
    q: "회사가 기본급만으로 계산했는데 어떻게 해요?",
    a: "상여금·고정수당까지 포함해서 직접 계산하고 차액을 청구해요. 인사팀이 거부하면 고용노동부(1350) 진정을 낼 수 있죠. 퇴직일로부터 3년 내라면 청구 가능해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.45, marginBottom: 14 }}>
        회사가 계산해준 퇴직금, 맞는지 모르겠죠?<br />
        평균임금·근속연수 직접 계산해서 검증하는 법
      </h1>

      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        퇴직금 계산에서 막히는 지점은 거의 정해져 있죠. "상여금이 평균임금에 들어가나요?", "3년 5개월은 어떻게 계산하나요?" 이 두 가지예요.
        법정 공식은 <a href="https://www.law.go.kr/법령/근로자퇴직급여보장법" style={{ color: "#1D9E75", textDecoration: "underline" }}>근로자퇴직급여보장법 제8조</a>에 명시된
        '1일 평균임금 × 30일 × 근속연수'이에요. 상여금은 포함되고, 근속기간은 일 단위로 계산해요.
      </p>
      ...
      <p style={body}>
        평균임금이 통상임금보다 낮게 계산될 때는 통상임금을 대신 쓸 수 있죠.
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="직접 퇴직금을 계산해볼 수 있죠. 아래 계산기로 예상 금액을 바로 적용해봐요."
        partialMatchText="상황에 따라 계산 방식이 달라질 수 있죠. 고용노동부(1350)에 상담해봐요."
      />
      ...
      <p style={body}>
        ...
        급여명세서에 상여금이 따로 표기되지 않으면 인사팀에 연간 상여금 지급 내역을 별도로 요청해요.
      </p>
      ...
      <p style={body}>
        ...
        계산 결과가 회사 제시액과 5만원 이상 차이가 나면 인사팀에 재계산을 요청할 수 있죠.
      </p>
      ...
      <H2>계산 전에 이 다섯 가지만 챙겨요</H2>
      <p style={body}>
        퇴직금 계산에서 실수가 생기는 지점은 거의 정해져 있죠.
        상여금 빠뜨리기, 총 일수를 30일로 나누기, 소수점 근속기간 반올림 처리가 대표적이에요.
        아래 다섯 가지를 순서대로 짚어보면 빠뜨린 항목을 찾을 수 있죠.
      </p>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로기준법과 근로자퇴직급여보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350)에서 확인해요." />
    </ArticleLayout>
  );
}
