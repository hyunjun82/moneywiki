...
// Q1. 퇴직을 앞두거나 방금 퇴직한 사람이 하나은행에서 퇴직금을 어떻게 받는지 몰라서 검색하는 상황
// Q2. IRP 계좌를 개설하고 회사에 계좌번호를 전달해 퇴직금을 수령한다
// Q3. IRP 의무 대상 기준(300만원 초과), 개설 방법(앱/영업점), 서류, 14일 입금 기한, 일시금·연금 선택 기준
// Q4. 4단계 Steps + 체크리스트 + 계산기 + FAQ
...
const CALC_RESULTS = [
  ...
  },
  {
    ...
    format: (v: number) => `약 ${Math.round(v / 10000).toLocaleString()}만원 (근속·공제에 따라 달라져요)`,
  },
  {
    label: "세후 예상 수령액",
...
const STEPS = [
  {
    ...
    desc: "하나원큐 앱 또는 영업점에서 IRP 계좌를 개설해요. 신분증만 있으면 10분 내 완료되고, 앱에서 비대면 개설도 되죠.",
    tip: "하나원큐 앱 → 자산관리 → IRP 개설 메뉴에서 진행해요",
    link: { label: "하나원큐 앱 이동", href: "https://www.kebhana.com" },
  },
  {
    title: "회사에 IRP 계좌번호 제출",
    desc: "퇴직 전 또는 퇴직 직후 회사 인사·급여 담당자에게 하나은행 IRP 계좌번호를 알려줘요. 회사는 이 계좌로 퇴직금을 이체하죠.",
    tip: "계좌번호 + 은행명(하나은행)을 문자나 이메일로 전달하면 돼요",
  },
  {
    title: "퇴직금 IRP 계좌 입금 체크",
    desc: "퇴직일로부터 14일 이내에 회사가 IRP 계좌로 퇴직금을 이체해야 해요. 퇴직소득세는 이 시점에 원천징수되고, 하나원큐 앱에서 입금 내역을 바로 볼 수 있죠.",
    tip: "14일이 지나도 입금 안 되면 지연이자(연 20%) 청구 가능해요",
  },
  {
    title: "수령 방식 선택 (일시금 또는 연금)",
    desc: "IRP 계좌에 입금된 퇴직금은 일시금으로 바로 받거나, 55세 이후 연금으로 나눠 받는 방식을 고를 수 있죠. 연금 수령 시 퇴직소득세의 30~40%가 감면돼요.",
    tip: "55세 미만이면 일시금 수령만 되죠. 연금은 55세 이후부터 신청 가능해요",
  },
];
...
const CHECKLIST = [
  ...
  "퇴직일로부터 14일 이내 입금 여부 체크",
  "IRP 입금 후 수령 방식 결정 (일시금/연금)",
  "55세 이후 연금 수령 선택 시 퇴직소득세 30~40% 감면",
  "운용 중인 펀드가 있으면 환매 후 수령 일정을 미리 살펴봐요",
];

const FAQS = [
  {
    q: "하나은행 IRP 계좌가 없으면 퇴직금을 못 받나요?",
    a: "아니에요. IRP 계좌가 없으면 회사는 일반 예금 계좌로 입금할 수도 있죠. 다만 퇴직금 300만원 초과는 IRP로 받는 게 원칙이에요. IRP로 받으면 세금 납부를 나중으로 미룰 수 있어서 유리하고요.",
  },
  {
    q: "하나은행 IRP 개설 수수료가 있나요?",
    a: "계좌 개설 자체는 무료예요. 다만 IRP 내 운용 상품(펀드, ETF 등)에 따라 운용 수수료가 붙을 수 있죠. 원금보장형 상품(정기예금)을 선택하면 수수료가 낮아요.",
  },
  {
    q: "퇴직금을 IRP 말고 일반 통장으로 받으면 어떻게 되나요?",
    a: "퇴직금 300만원 초과는 원칙적으로 IRP로 받아야 해요. 일반 통장으로 받으면 회사가 퇴직소득세를 즉시 원천징수하고 나머지를 입금하죠. IRP로 받으면 수령 시점까지 세금 납부를 미룰 수 있고요.",
  },
  {
    q: "하나은행 IRP에서 일시금 인출이 가능한가요?",
    a: "가능해요. IRP 해지 신청을 하면 퇴직소득세 납부 후 잔액을 받을 수 있죠. 단, 중도 해지 시 세액 감면 혜택은 사라지고요. 55세 이후 연금 수령 시 세금이 줄어드는 점도 함께 살펴봐요.",
  },
  {
    q: "다른 은행 IRP로 이전할 수 있나요?",
    a: "가능해요. IRP 계좌는 금융기관 간 이전이 자유롭고요. 하나은행 IRP에서 다른 금융기관 IRP로 이전하거나, 다른 금융기관 IRP를 하나은행으로 통합하는 것도 되죠.",
  },
];
...
export default function Page() {
  return (
    <ArticleLayout
      sidebar={<Sidebar heading="퇴직금 가이드" items={퇴직금_SIDEBAR} highlightSlugs={퇴직금_HIGHLIGHT} currentSlug="하나은행-퇴직금-수령방법" />}
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        하나은행 IRP 계좌를 개설하고 회사에 계좌번호만 알려주면 퇴직금을 받을 수 있죠.
        ...
        하나원큐 앱에서 10분이면 IRP 계좌 개설이 완료되죠.
      </p>
      ...
      <p style={body}>
        퇴직금 300만원 초과는 <a href="/w/퇴직금-IRP-계좌" style={{ color: "#1D9E75", textDecoration: "underline" }}>IRP 계좌</a>로만 받을 수 있죠.
        하나은행 IRP는 하나원큐 앱 또는 영업점에서 신분증만 있으면 개설되고, 계좌 개설 후 회사 담당자에게 계좌번호를 전달하면 퇴직일로부터 14일 이내에 입금돼요.
      </p>
      <p style={body}>
        IRP 계좌에 퇴직금이 입금되면 일시금으로 바로 받거나, 55세 이후 연금으로 나눠 받는 방식을 고를 수 있죠.
        연금 방식을 선택하면 퇴직소득세의 30~40%를 감면받고, 하나은행 외에 다른 금융기관 IRP로 이전도 자유롭게 되고요.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="하나은행 IRP로 퇴직금을 받을 준비가 됐어요. 아래 계산기로 예상 수령액을 살펴봐요."
        partialMatchText="일부 조건을 아직 갖추지 못했고요. 하나은행 고객센터(1588-1111)에서 안내를 받을 수 있죠."
      />
      ...
      <p style={body}>
        퇴직금은 월 평균 임금 × 근속연수로 계산하죠.
        실제 퇴직소득세는 근속연수 공제, 환산 배수 등으로 계산이 복잡하지만 일반적으로 5~10% 수준이에요.
        슬라이더로 월급과 근속연수를 조정하면 예상 금액을 바로 볼 수 있죠.
      </p>
      ...
      <Calculator
        ...
        note="※ 퇴직소득세율 5.5% 적용 추정치. 실제 세율은 근속연수·퇴직금 규모에 따라 달라져요. IRP 연금 수령 시 세율이 더 낮아지고요."
      />
      ...
      <p style={body}>
        하나은행 IRP 개설은 신분증 하나만 있으면 되죠.
        앱에서 비대면으로도 바로 개설되어 별도 서류 준비가 거의 필요 없고, 회사에 제출하는 계좌번호는 IRP 계좌 개설 즉시 앱에서 바로 볼 수 있죠.
      </p>
      ...
      <p style={body}>
        ...
        대부분 하나원큐 앱에서 해결되고, 회사에 계좌번호만 전달하면 회사가 알아서 입금해주죠.
      </p>
      ...
      <GreenBox>
        ...
        세금을 줄이고 싶다면 55세 이후 연금 수령 방식을 선택해봐요.
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 근로자퇴직급여 보장법을 바탕으로 작성됐어요. 제도 변경이 있을 수 있으니 최신 기준은 고용노동부(1350) 또는 하나은행(1588-1111)에서 살펴봐요." />
    </ArticleLayout>
  );
}
