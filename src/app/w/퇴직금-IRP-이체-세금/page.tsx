...
// Q1. 퇴직 후 IRP 계좌로 퇴직금이 이체됐는데, 세금이 바로 빠졌는지 불안하고 나중에 얼마나 내야 하는지 모르는 상황
// Q2. IRP 수령 방법(일시금 vs 연금)을 선택하고, 연금 수령 신청까지 직접 진행할 수 있어야 함
// Q3. 이체 시 과세 이연(세금 없음), 일시금 인출 시 퇴직소득세 전액, 연금 수령 시 30~40% 감면(만 55세 이후), 중도 인출 시 기타소득세 16.5% 추가
// Q4. GreenBox(핵심 요약) + EligibilityChecker(상황 체크) + Calculator(세금 비교) + Steps(절차) + DocTable(서류) + Checklist(절세 체크)
//
// MAP:
// - Q1 → 서론 톤: "IRP에 들어온 순간엔 세금이 없어요. 꺼낼 때가 문제예요"
// - Q2 → H2 순서: 이체 시 세금 원리 → 수령 방법별 세금 비교 → 절차 → 서류 → 절세 체크리스트
// - Q3 → H2 5개, 과세 이연·감면율·중도인출 조건 깊이 있게
...
const CHECK_ITEMS = [
  { id: "c1", label: "퇴직 후 퇴직금이 IRP 계좌로 이체됐고요" },
  { id: "c2", label: "회사에서 퇴직소득 원천징수영수증을 받았죠" },
  { id: "c3", label: "IRP 가입 금융사에서 수령 방법(일시금/연금)을 아직 결정 중이고요" },
  { id: "c4", label: "연금 수령 절세 조건(만 55세 이후)을 충족하거나 충족 예정이죠" },
];
...
const STEPS = [
  {
    title: "IRP 이체 확인",
    desc: "퇴직 후 회사가 IRP 계좌로 퇴직금을 이체해요. 이때 퇴직소득세는 원천징수되지 않아요. IRP 앱에서 이체 금액을 보고, 회사에서 퇴직소득 원천징수영수증을 받아두세요.",
    tip: "원천징수영수증에 퇴직소득세 계산 내역이 나오죠 나중에 절세 계획 세울 때 꼭 필요해요",
  },
  {
    title: "수령 방법 결정 (일시금 vs 연금)",
    desc: "IRP 금융사 앱에서 일시금 또는 연금을 선택할 수 있죠. 일시금을 선택하면 퇴직소득세 전액을 납부 후 수령해요. 만 55세 이후 연금을 선택하면 퇴직소득세의 30%(10년 이상 수령 시 40%)를 감면받아요.",
    tip: "55세 이전이라면 IRP 안에서 운용하면서 기다리는 게 세금 면에서 유리하죠",
  },
  {
    title: "연금 수령 신청 (연금 선택 시)",
    desc: "IRP 금융사 앱에서 연금 수령 신청서를 작성해요. 연금 지급 주기(월·분기·연간)와 수령 기간을 선택할 수 있죠. 수령 기간이 10년 이상이면 퇴직소득세 40% 감면 혜택을 받아요.",
    tip: "연간 연금 수령액이 1,200만원 이하면 분리과세 선택이 가능하죠",
  },
  {
    title: "세금 납부 및 환급 처리",
    desc: "일시금 인출 시 금융사가 퇴직소득세를 원천징수하고 나머지를 지급해요. 세금이 과오납됐다면 홈택스에서 경정청구로 환급받을 수 있죠. 경정청구 기한은 5년이에요.",
...
const FAQS = [
  {
    ...
    a: "아니에요. IRP로 이체하는 것 자체는 과세 없이 그대로 들어가죠. 세금은 IRP에서 돈을 꺼낼 때 납부해요. 이걸 과세 이연이라고 해요. IRP 안에서 운용 수익이 생겨도 꺼낼 때까지 과세되지 않아요.",
  },
  {
    q: "IRP에서 바로 일시금으로 빼면 세금이 얼마나 나오나요?",
    a: "퇴직소득세 전액이 원천징수돼요. 금액과 근속 기간에 따라 다른데, 근속 10년·퇴직금 3,000만원이면 수십만 원에서 수백만 원 수준이에요. 정확한 금액은 회사에서 받은 퇴직소득 원천징수영수증으로 볼 수 있죠.",
  },
  {
  ...
  },
  {
    ...
    a: "맞아요. IRP에서 중도 인출을 하면 퇴직소득세에 더해 기타소득세 16.5%가 추가로 부과될 수 있죠. 법에서 허용하는 사유(무주택자 주택 구입, 요양비 등)가 아니면 중도 인출이 제한되고 세금도 많아요.",
  },
  {
    q: "55세 이전인데 IRP에서 연금을 받을 수 있나요?",
    a: "안 돼요. 연금 수령은 만 55세 이후부터 가능해요. 55세 이전에 인출하면 연금 수령이 아닌 일시금 또는 중도 인출로 처리돼서 세금 감면 혜택을 받을 수 없죠.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        퇴직금이 들어온 순간엔 세금이 없어요.
        <a href="/w/퇴직금-세금" style={{ color: "#1D9E75", textDecoration: "underline" }}>퇴직소득세</a>는
        IRP에서 돈을 꺼낼 때 납부하는 구조예요.
        꺼내는 방법이 세금 규모를 결정하는데, 만 55세 이후 연금으로 받으면 퇴직소득세의 30~40%를 감면받을 수 있죠.
      ...
      <p style={body}>
        ...
        이걸 '과세 이연'이라고 해요. IRP 계좌 안에 쌓이는 동안엔 운용 수익도 포함해서 세금이 발생하지 않아요.
      ...
      <p style={body}>
        ...
        규정 외 사유로 중도 인출하면 퇴직소득세에 기타소득세 16.5%까지 더 나올 수 있죠.
      </p>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="과세 이연 혜택과 연금 수령 절세 조건을 갖추고 있죠. 아래 계산기로 절세 효과를 비교해 보세요."
        partialMatchText="상황에 따라 세금 처리가 달라질 수 있죠. 국세청(126) 또는 IRP 금융사 상담을 권해요."
      />
      ...
      <p style={body}>
        퇴직금 규모와 근속 기간을 입력하면 일시금 인출 시 퇴직소득세 추정액과 연금 수령 시 절세 효과를 비교할 수 있죠.
        퇴직금이 클수록, 근속 기간이 짧을수록 절세 효과가 더 크게 나타나요.
      </p>
      <p style={body}>
        계산기는 단순화된 추정치예요. 실제 퇴직소득세는 근속공제와 환산산식을 적용하기 때문에 달라질 수 있죠.
      ...
      <Calculator
        ...
        note="※ 추정치. 실제 퇴직소득세는 근속공제·환산산식 적용으로 다를 수 있죠. 퇴직소득 원천징수영수증으로 정확한 세액을 봐야 해요."
      />
      ...
      <p style={body}>
        ...
        55세까지 시간이 남아 있다면 IRP 안에서 운용하면서 기다리는 게 세금 측면에서 유리하죠.
      </p>
      ...
      <p style={body}>
        연금 수령이 핵심이에요. 55세가 되면 IRP 금융사에 연금 수령 신청을 하고, 수령 기간을 10년 이상으로 설정하면 최대 40% 감면을 받아요.
      </p>
      ...
      <GreenBox>
        ...
        연간 1,200만원 이하 수령 시 분리과세 선택 가능하죠
      </GreenBox>
      ...
      <Disclaimer text="이 글은 2026년 3월 기준 소득세법 및 근로자퇴직급여 보장법을 바탕으로 작성됐어요. 세법 개정이 있을 수 있으니 최신 기준은 국세청(126)에서 봐야 해요." />
    </ArticleLayout>
  );
}
