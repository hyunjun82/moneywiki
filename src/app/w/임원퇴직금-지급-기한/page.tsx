...
// Q1. 법인 등기 임원으로 퇴직했는데 퇴직금이 안 나왔고, 14일 지급 기한이 적용되는지 모르는 상황이에요.
// Q2. 지급 기한을 파악하고, 기한이 지났으면 지연이자 포함해서 법적으로 청구하는 행동이에요.
// Q3. 임원은 근로기준법 14일 규정 적용 안 됨, 정관·주총 결의 기준, 기한 없으면 상당한 기간, 지연이자 상법 연 6%, 미지급 시 민사소송만 가능(노동부 신고 불가).
// Q4. GreenBox(기준 요약), EligibilityChecker(해당 여부), Calculator(지연이자), Steps(청구 절차), DocTable(서류), Checklist(점검), FAQ.
//
// MAP:
// - Q1 → 서론 톤: 14일 적용 안 된다는 충격 포인트 먼저
// - Q2 → H2 순서: 지급기한 기준 → 지연이자 → 서류 → 절차 → 체크리스트
// - Q3 → H2 개수/깊이: 5개 H2, 정관/주총/상당기간 3단계 구분 상세히
// - Q4 → 컴포넌트: GreenBox + EligibilityChecker + Calculator + DocTable + Steps + Checklist + FAQ
...
const CHECK_ITEMS = [
  { id: "c1", label: "법인 등기 임원(이사·감사)으로 재임한 이력이 있죠" },
  { id: "c2", label: "퇴직 후 약속한 기한에 퇴직금이 지급되지 않은 상황이죠" },
  { id: "c3", label: "정관이나 주주총회 결의에 지급 시기가 명시돼 있죠" },
  { id: "c4", label: "지급 지연에 따른 이자나 법적 청구를 검토하고 있죠" },
];
...
const STEPS = [
  {
    title: "정관·결의에서 지급 기한 파악",
    desc: "정관에 '퇴직 후 30일 이내' 같은 기한이 적혀 있으면 그 기간이 기준이에요. 기한이 없으면 주주총회 결의에서 정한 날짜가 기준이 되죠. 정관과 결의 모두 기한이 없다면 상법·민법상 '상당한 기간' 원칙이 적용돼요.",
    tip: "기한 명시 여부에 따라 이자 청구 가능 시점이 달라져요",
  },
  {
    title: "내용증명 발송으로 지급 촉구",
    desc: "기한이 지났다면 내용증명으로 지급을 요청해야 해요. 내용증명은 법적 효력은 없지만 지급 요청 사실을 공식적으로 기록으로 남길 수 있죠. 발송일 기준으로 지연이자 기산점을 명확히 하는 데도 도움이 돼요.",
    tip: "내용증명 발송 후 2주 이내 회신이 없으면 법적 절차를 준비해야 해요",
  },
  {
    title: "지연이자 청구",
    desc: "약정한 지급 기한을 넘겼다면 이자를 청구할 수 있죠. 임원은 근로기준법의 연 20% 지연이자가 아닌 상법상 법정이율(연 6%)이 적용돼요. 약정 이율이 별도로 명시돼 있으면 그 이율이 우선이에요.",
    tip: "약정 지급일이 없으면 '상당한 기간' 초과 여부를 다퉈야 해서 소송 난이도가 높아져요",
  },
  {
    title: "법적 청구 (지급명령·민사소송)",
    desc: "지연이 계속되면 법원에 지급명령 신청 또는 민사소송을 제기할 수 있죠. 임원 퇴직금 분쟁은 근로기준법이 아닌 상법과 민법을 적용해요. 3,000만 원 이하라면 소액심판으로 빠르게 처리할 수 있죠.",
    tip: "변호사 상담 없이 진행하면 주장 근거가 약해져요",
  },
];

const CHECKLIST = [
  "정관 기한 파악: '퇴직 후 N일 이내' 조항 존재 여부",
  "주주총회 결의서: 지급 시기 명시 여부",
  "지연이자율: 상법 연 6% (약정이율이 있으면 우선 적용)",
  "내용증명 발송: 지급 요청 사실 공식화",
  "소송 유형: 소액심판(3,000만 원 이하) vs 민사소송",
];

const FAQS = [
  {
    q: "임원 퇴직금도 14일 이내에 줘야 하나요?",
    a: "아니에요. 14일 이내 지급 규정은 근로기준법 조항이라서 근로자에게만 적용돼요. 등기 임원은 위임 관계라서 정관이나 주주총회에서 정한 기한이 기준이 되죠.",
  },
  {
  ...
  },
  {
    ...
    a: "약정 이율이 없으면 상법 법정이율 연 6%가 적용돼요. 근로자(연 20%)보다 낮죠. 정관이나 결의에 지연이율을 별도로 정해두면 그 이율이 우선이에요.",
  },
  {
    q: "임원이 근로자성도 인정되면 어떻게 되나요?",
    a: "실질적으로 사용자 지휘 아래 근로를 제공한 사실이 입증되면 근로기준법 적용 대상이 돼요. 그 경우 14일 지급 의무와 연 20% 지연이자 규정이 적용되죠. 법원이나 노동위원회에서 다툴 수 있죠.",
  },
  {
    q: "소액심판으로 청구할 수 있나요?",
    a: "퇴직금 청구액이 3,000만 원 이하라면 소액심판 절차를 쓸 수 있죠. 비용이 적고 처리 기간이 빠른 편이에요. 3,000만 원 초과라면 일반 민사소송으로 진행해야 해요.",
  },
  {
    q: "내용증명을 꼭 보내야 하나요?",
    a: "법적 의무는 아니에요. 다만 지급 요청 사실을 공식적으로 기록으로 남길 수 있고, 이후 소송에서 지연이자 기산점을 명확히 하는 데 도움이 돼요. 가능하면 발송해두는 게 유리해요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.45, marginBottom: 14 }}>
        임원 퇴직금 지급 기한, 14일 규정 적용 안 돼요<br />
        정관 기준·지연이자·법적 청구 방법
      </h1>
      ...
      <p style={body}>
        ...
        정관에 "퇴직 후 30일 이내" 같은 지급 시기 조항이 있으면 그 기한이 기준이 되죠.
        정관에 기한이 없다면 주주총회 결의에서 지급 시기를 별도로 정할 수 있죠.
      </p>
      <p style={body}>
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="임원 퇴직금 지급 기한 문제에 해당해요. 아래 계산기로 지연이자를 파악해보세요."
        partialMatchText="상황에 따라 다를 수 있죠. 세무사 또는 변호사 상담을 권해요."
      />
      ...
      <p style={body}>
        약정 지급 기한을 넘겼다면 상법 법정이율 연 6%로 이자를 청구할 수 있죠.
        근로자에게 적용되는 연 20%보다 낮지만, 금액이 크면 무시할 수 없어요.
        아래 계산기로 미지급 퇴직금과 지연 기간을 입력하면 청구 가능한 이자를 파악할 수 있죠.
      </p>
      ...
      <Calculator
        ...
        note="※ 상법 법정이율 연 6% 기준. 약정 이율이 있으면 약정 이율이 우선이에요. 실제 금액은 소송 결과에 따라 달라져요."
      />
      ...
      <p style={body}>
        정관 기한 파악부터 법적 청구까지 단계별로 접근해야 해요.
      ...
      <GreenBox>
        지급 기한이 없으면 '상당한 기간'이 얼마인지 다퉈야 해서 소송 기간이 길어지죠.
