...
const DOCS = [
  { name: "퇴직 증빙 서류 (사직서 또는 해고통지서)", required: true, where: "본인 보관 또는 회사 인사팀" },
  { name: "근로계약서", required: true, where: "입사 시 수령본 또는 인사팀 요청" },
  { name: "급여명세서 (최근 3개월)", required: true, where: "회사 인사팀 요청" },
  { name: "지급 요청 기록 (문자·이메일·카카오톡)", required: false, where: "본인 보관 캡처" },
  { name: "내용증명 발송 확인서", required: false, where: "우체국 또는 카카오 전자내용증명 발급" },
];

const STEPS = [
  {
    title: "지연 일수 정확히 계산하기",
    desc: "퇴직금 지급 기한은 퇴직일 다음 날부터 14일이에요. 15일째 날부터 이자 계산이 시작돼요. 달력으로 퇴사일 +14일을 세고, 오늘까지 며칠이 더 지났는지 확인하면 돼요. 주말과 공휴일도 일수에 포함돼요.",
    tip: "위 계산기에 퇴직금 금액과 초과 일수를 넣으면 이자 금액이 바로 나와요",
  },
  {
    title: "내용증명 발송 (지연이자 금액 명시)",
    desc: "'퇴직금 ○○만원이 14일 기한을 초과했으므로 근로기준법 제37조에 따른 연 20% 지연이자와 함께 지급을 요청합니다'라는 내용으로 내용증명을 발송해요. 이 시점부터 소멸시효가 중단되고 법적 청구 기록이 생겨요.",
    tip: "카카오 전자내용증명(등기 효력)으로 집에서 5분 내 발송 가능해요",
  },
  {
    title: "고용노동부 진정 접수 (지연이자 항목 포함)",
    desc: "노동청 진정서에 퇴직금 원금과 지연이자를 별도 항목으로 명시해야 해요. 근로감독관이 시정 명령을 낼 때 지연이자도 함께 포함시켜 줘요. 온라인 접수는 고용노동부 민원마당(minwon.moel.go.kr)에서 가능하고, 보통 2~4주 내에 처리돼요.",
    tip: "진정서 '체불 금품' 항목에 '퇴직금 ○○만원 + 지연이자 ○○만원'을 구체적으로 적어야 해요",
    link: { label: "고용노동부 민원마당 바로가기", href: "https://minwon.moel.go.kr" },
  },
  {
    title: "소액심판 또는 민사소송 제기",
    desc: "진정 처리 후에도 지급하지 않으면 법원에 소액심판을 청구해요. 청구 금액 3,000만원 이하라면 변호사 없이 본인이 직접 가능해요. 지연이자는 원금에 포함해 한꺼번에 청구할 수 있어요. 대한법률구조공단(132)에서 무료 법률 지원도 받을 수 있어요.",
...
const FAQS = [
  {
    ...
    a: "법적으로는 자동 발생하지만 실제로 받으려면 반드시 명시적으로 청구해야 해요. 진정서나 소장에 지연이자 항목을 빠뜨리면 안 줘요. 내용증명 단계부터 금액을 명시해두는 게 중요해요.",
  },
  {
    q: "지연이자 계산 시작일이 정확히 언제인가요?",
    a: "퇴직일 다음 날부터 14일을 세고, 그다음 날(15일째)부터 이자가 붙어요. 예를 들어 3월 1일 퇴직이라면 3월 15일부터 이자 계산이 시작돼요. 주말과 공휴일도 일수에 포함돼요.",
  },
  {
    q: "지연이자도 소멸시효가 있나요?",
    a: "있어요. 퇴직금 원금과 동일하게 퇴직일로부터 3년이에요. 3년이 지나면 지연이자 청구권도 소멸해요. 내용증명을 발송하면 소멸시효를 중단시킬 수 있어요.",
  },
  {
  ...
  {
    q: "지연이자에도 세금이 붙나요?",
    a: "퇴직금 지연이자는 기타소득으로 분류될 수 있어요. 회사가 지급할 때 원천징수하거나, 소송으로 받을 때 세금 문제가 생길 수 있어요. 금액이 크다면 세무사에게 확인하는 게 좋아요.",
  },
  {
    q: "서면 합의로 기한을 연장했다면 이자가 안 붙나요?",
    a: "서면으로 합의한 기간까지는 지연이자가 발생하지 않아요. 단, 구두(말)로만 합의했거나 서면 없이 미루기만 했다면 효력이 없어서 이자를 청구할 수 있어요.",
  },
];
  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        ...
        다만 이자는 기다린다고 자동으로 들어오지 않아요. <a href="/w/퇴직금-지연이자-받기" style={{ color: "#1D9E75", textDecoration: "underline" }}>내용증명과 진정</a>으로 명시적으로 청구해야 받을 수 있어요.
      </p>
      ...
      <p style={body}>
        ...
        구두로만 "좀 기다려달라"는 말을 들었다면 효력이 없어서 이자를 청구할 수 있어요.
      </p>
      <p style={body}>
        소멸시효는 퇴직금 원금과 동일하게 3년이에요.
        3년이 지나기 전에 내용증명을 발송하면 소멸시효를 중단시킬 수 있어요.
        지금 당장 아래 항목을 체크해보세요.
      </p>

      <GreenBox title="지연이자 발생 기준 (근로기준법 제37조)">
        이율: 연 20% (일할 계산)<br />
      ...
      <EligibilityChecker
        items={CHECK_ITEMS}
        allMatchText="지연이자 청구가 가능한 상황이에요. 아래 계산기로 이자 금액을 먼저 확인해보세요."
      ...
      <p style={body}>
        계산 공식은 단순해요. 미지급 퇴직금 × 20% ÷ 365 × 지연 일수예요.
      ...
      <H2>청구에 필요한 서류</H2>
      <p style={body}>
        지연이자 청구에도 퇴직 사실과 미지급 금액을 증명할 서류가 필요해요.
        진정 접수 전에 미리 준비해두면 처리 속도가 빨라져요.
        필수 서류 3가지와 있으면 유리한 서류 2가지예요.
      </p>
      ...
      <p style={body}>
        서류가 없는 경우엔 고용노동부 진정 접수 전에 회사에 공식 요청 이메일을 보내세요.
        요청과 거부 사실 자체가 나중에 증거로 쓰일 수 있어요.
      </p>
      ...
      <p style={body}>
        ...
        원금만 적으면 원금만 받게 돼요. 소멸시효 3년도 반드시 챙겨야 해요.
      </p>

      <SectionBadge>체크리스트</SectionBadge>
      <Checklist items={CHECKLIST} />

      <GreenBox title="내용증명에 지연이자 금액을 반드시 명시하세요">
        내용증명에 "퇴직금 ○○만원 + 연 20% 지연이자 ○○만원"을 함께 적어야 해요.<br />
        노동청 진정이나 소송으로 이어질 때 청구 근거가 명확해지고, 이자를 누락 없이 받을 수 있어요.
      </GreenBox>
