  ...
  return (
    ...
    >
      ...
      <p style={{ ...body, fontSize: 15, lineHeight: 2.1 }}>
        실업급여 받으면서 직업훈련 다니기로 했는데, 교통비랑 점심값이 부담되죠.{" "}
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서는
        고용센터 지시 훈련에 출석하면 교통비·식비 명목으로 <strong>월 최대 11만6천원</strong>을 추가로 지급해요.
        실업급여와 별개이기 때문에 둘 다 수령할 수 있죠.
      ...
      <p style={body}>
        <strong>훈련 참여 수당</strong>이에요. 실업급여를 받는 사람이 고용센터에서 지시한 직업훈련에 출석하면, 교통비와 식비 명목으로 별도 수당을 지급하는 제도죠. 구직급여와는 완전히 별개 항목이라 두 가지를 동시에 수령하게 돼요.
      </p>
      <p style={body}>
        <a href="https://www.law.go.kr/법령/고용보험법" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용보험법</a>에서 규정한 정식 명칭이에요. 현장에서는 &quot;훈련장려금&quot;이라고 부르기도 하는데, 같은 제도를 가리키는 거죠. 실업급여 수급자가 받을 수 있는 부가급여 중 하나이고, 조기재취업수당이나 광역구직활동비와 같은 카테고리에 속해요.
      </p>
      <p style={body}>
        두 가지 조건을 모두 충족해야 받을 수 있어요. 첫째, <strong>고용센터 소장이 지시한 훈련</strong>이어야 하죠. 본인이 직접 선택한 내일배움카드 훈련은 해당이 안 돼요. 둘째, <strong>훈련 출석률 80% 이상</strong>이어야 해요. 한 달에 20일 훈련이면 최소 16일은 출석해야 하죠.
      </p>
      ...
      <p style={body}>
        <strong>1일 약 5,800원</strong>이에요. 한 달에 20일 훈련을 받으면 5,800원 x 20일 = <strong>11만6천원</strong>이 되는 구조예요. 실제 훈련 일수에 비례해서 달라지니까, 월마다 금액이 조금씩 차이가 나죠.
      </p>
      <p style={body}>
        금액 자체는 크지 않아 보이지만, 실업급여에 더해서 받는 추가 수당이에요. 3개월 훈련이면 약 35만원, 6개월이면 약 70만원이 쌓이죠. 훈련비 자체는 내일배움카드에서 별도로 지원되니까, 직업능력개발수당은 순수하게 교통비와 식비를 보전해주는 역할을 해요.
      </p>
      <p style={body}>
        아래 계산기에서 본인의 월 출석 일수와 훈련 기간을 입력하면 예상 수당을 바로 확인할 수 있어요. 주말과 공휴일은 훈련이 없으니까, 보통 월 18~22일 정도를 기준으로 잡으면 돼요.
      </p>
      ...
      <p style={body}>
        <strong>별도 신청이 필요 없어요.</strong> 고용센터 지시 훈련을 받고 출석률 80% 이상이면 자동으로 지급되죠. 훈련기관에서 매달 출석 정보를 고용센터에 전송하고, 고용센터에서 조건 충족 여부를 확인한 뒤 지급하는 구조예요.
      </p>
      <p style={body}>
        지급 시기는 <strong>훈련 월 종료 후</strong>예요. 1월에 훈련을 받았으면 2월에 입금되는 식이죠. 실업급여 지급일과는 다를 수 있으니, 통장에 별도 항목으로 들어오는지 확인해보세요.
      </p>
      <p style={body}>
        혹시 수당이 안 들어왔다면 훈련기관에 출석 데이터가 제대로 전송됐는지 확인하는 게 먼저예요. 출석 인정에 문제가 생겼을 수도 있으니까요. <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 수급 내역을 조회하거나, 관할 고용센터(1350)에 문의하면 확인할 수 있죠.
      </p>
      ...
      <p style={body}>
        직업능력개발수당은 <strong>교통비·식비</strong> 명목이에요. 훈련 수강료 자체는 내일배움카드에서 별도로 지원되죠. 두 가지가 성격이 다르기 때문에 동시에 수령하게 돼요. 내일배움카드로 훈련비를 지원받으면서 직업능력개발수당도 함께 받는 구조예요.
      </p>
      <p style={body}>
        훈련연장급여와도 구분해야 해요. 훈련연장급여는 실업급여 수급 기간이 끝난 뒤에도 훈련 중이면 급여를 연장해주는 제도이고, 직업능력개발수당은 훈련 기간 내내 교통비·식비를 보전해주는 제도예요. 훈련연장급여를 받는 기간에도 직업능력개발수당은 함께 나오죠.
      </p>
      <p style={body}>
        정리하면, 직업훈련 기간에 받을 수 있는 지원금은 세 가지예요. <strong>실업급여(또는 훈련연장급여) + 직업능력개발수당 + 내일배움카드 훈련비 지원</strong>. 세 가지 모두 중복 수령이 가능하니까, 훈련받기로 결정했다면 빠짐없이 챙기세요.
      </p>
      ...
      <p style={body}>
        가장 많이 하는 실수가 <strong>&quot;내일배움카드 훈련 = 지시 훈련&quot;</strong>이라고 착각하는 거예요. 내일배움카드로 등록한 훈련이라도, 고용센터 소장이 &quot;이 과정을 수강하세요&quot;라고 지시한 게 아니면 직업능력개발수당 대상이 아니에요. 반드시 고용센터 상담 시 지시 훈련 해당 여부를 확인하세요.
      </p>
      <p style={body}>
        출석률 관리도 중요해요. 80%는 생각보다 빡빡한 기준이죠. 20일 훈련 중 5일만 빠져도 75%로 떨어져서 그 달 수당을 못 받아요. 정당한 사유(병원 진료, 경조사)가 있으면 훈련기관에 사전 신고하고 증빙을 제출하세요.
      </p>
      <p style={body}>
        마지막으로, 수당이 자동 지급이라고 해서 아무것도 안 해도 되는 건 아니에요. 훈련기관의 출석 데이터 전송이 지연되거나 누락되면 수당이 안 나올 수 있어요. <a href="https://www.ei.go.kr" style={{ color: "#1D9E75", textDecoration: "underline" }}>고용24</a>에서 수급 내역을 주기적으로 확인하는 습관을 들이세요.
      </p>
