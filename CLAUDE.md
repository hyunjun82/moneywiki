...
1. **문제해결 100%** — 독자가 타이틀 보고 들어왔을 때 다른 글 안 봐도 되는 수준. 구체적 숫자, 실전 예시, 행동 단계까지 빠짐없이. 이게 안 되면 노출 안 됨
2. **내부링크 거미줄** — 본문에서 다른 주제 언급 시 내부 글 링크 우선 (나무위키 스타일). 외부 법령 링크는 보조
3. **구어체 필수** — ~해요, ~이에요 (합니다/입니다 절대 금지)
4. **숫자에는 출처** — 출처 없는 숫자 생성 금지, 공식 기관 URL 필수
5. **텍스트가 주인공** — 컴포넌트 앞 2~3문단, 뒤 1~2문단, 1~2줄 섹션 금지
6. **금지 단어** — 또한·결론적으로·다양한·매우중요·총정리·확인하세요 (상세: SKILL.md `## 6` 참조)
7. **계산기 불가침** — Calculator, EligibilityChecker 등 인터랙티브 컴포넌트와 데이터 상수(CALC_SLIDERS, CALC_RESULTS, getDays 등)는 글 수정 시 절대 건드리지 않음
