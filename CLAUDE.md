...
**타이틀이 나열한 항목 = 소제목. 개수까지 1:1이다.**
타이틀이 3개를 나열하면 소제목도 3개다. 8개는 목표가 아니라 상한이다.
소제목을 늘리려면 타이틀에 그 항목을 먼저 넣는다 — 반대로 채우면 곁가지가 섞인다.
타이틀만 갈아 끼우면 리라이트가 아니다.

**3. 본문 블록** (순서 고정, 톤은 합니다체)
```
lead → heroHook(마지막 문장은 아래 버튼을 누를 이유) → heroCta(대형 버튼 1개)
목차 → keyFacts 7~9행 → 소제목(타이틀 항목 수만큼) → FAQ → summary 3줄 → 스포크
