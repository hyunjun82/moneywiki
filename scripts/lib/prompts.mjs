...
export function planPrompt({ slug, topic, category, categories, keywords, registry, related, today, rewrite, oldTitle, retryNote, titleRule, titleExamples }) {
...
2. **타이틀은 아래 「저장된 타이틀 형식」과 「실제 예시」를 그대로 따릅니다.** 제가 형식을 새로 만들지 않습니다.
...
3. primaryKeywords 2~3개: 검색량이 큰 메인 검색어. **사람이 실제로 치는 짧은 말(2~4어절, 12자 이내)** 이어야 합니다 — "실업급여 건강보험"(O), "실업급여 수급 중 건강보험"(X, 아무도 이렇게 안 칩니다). 그중 2개 이상이 타이틀에 글자 그대로 들어가야 하므로, 키워드가 길면 타이틀이 따라 길어집니다.
