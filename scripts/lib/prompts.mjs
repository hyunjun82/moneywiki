...
7. ctas: 독자가 누를 버튼 2~4개. 신청·조회·계산·발급을 실제로 하는 화면. label 은 행동형("임의계속가입 신청하기"). "보기" 로 끝나는 열람형 금지. 어느 대제목 아래에 둘지 forSection 에 h2 문장을 적습니다. "hero": true(첫 화면 대형 버튼)는 그 화면이 **이 글의 주제 자체**를 처리할 때만 붙입니다(상병급여 글 → 상병급여 청구). 모든 실업급여 글에 걸리는 수급자격 신청서 화면처럼 주제와 한 단계 떨어진 화면뿐이면 hero 를 두지 않습니다 — 기계가 떼어 냅니다. 주소는 Playwright 가 실제로 열어 신청·조회 요소가 있는지 봅니다 — 안내·개편·점검 페이지나 열리지 않는 주소는 버려집니다. 고용24·노동포털 버튼은 「행동 화면 등록부」의 주소만 쓰고 버튼 이름에 그 화면의 말을 넣습니다(예: 상병급여 청구 화면 → "고용24에서 상병급여 청구하기"). 등록부에 그 일을 하는 화면이 없으면 그 버튼은 넣지 않습니다 — 기관 첫 화면·제도 안내 화면·다른 일을 하는 화면으로 보내는 버튼은 기계가 거부합니다. 실손24(insu24.or.kr)처럼 첫 화면이 곧 그 일을 하는 화면이면 도메인만 있는 주소도 됩니다.
