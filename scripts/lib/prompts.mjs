...
export function planPrompt({ slug, topic, category, categories, keywords, registry, related, today, rewrite, oldTitle, retryNote, titleRule, titleExamples, fixedTitle, fixedItems }) {
  ...
  const titleBlock = fixed
    ? [
        ...
        items.length
          ? [`이 타이틀이 약속한 항목은 ${items.length}개입니다. **군집(대제목)을 정확히 ${items.length}개** 만들고, 순서도 타이틀 순서 그대로 둡니다.`,
             ...items.map((x, i) => `  ${i + 1}) ${x}  → 대제목 ${i + 1} 이 이 항목에 답합니다`),
             "타이틀에 없는 주제로 군집을 만들지 않고, 항목 하나도 빼지 않습니다. 이 글은 본문이 타이틀과 어긋나 있어 다시 씁니다 — 본문을 타이틀에 맞춥니다."].join("\n")
          : "군집(대제목) 수는 타이틀이 쉼표·와/과/·/및·'부터…까지' 로 약속한 항목 수와 같아야 합니다.",
      ].join("\n")
    : [
        "## 저장된 타이틀 형식 (scripts/plan-articles.md — 이 프로젝트가 정한 것)",
        titleRule,
        "",
        "## 실제 예시 (scripts/plan-new21.md — 타이틀과 그 글의 대제목들. 개수까지 1:1)",
        titleExamples,
      ].join("\n");
...
${fixed ? `2. **타이틀은 위에 고정돼 있습니다 — 새로 짓지 않습니다.** 그대로 옮겨 적고, 군집 수만 위 항목 수(${items.length || "타이틀이 약속한 수"})에 맞춥니다.` : `2. **타이틀은 아래 「저장된 타이틀 형식」과 「실제 예시」를 그대로 따릅니다.** 제가 형식을 새로 만들지 않습니다.
   길이는 42자 이하 (예시들이 30~40자입니다). 예시의 어미(까지 / 총정리 / 정리 / 받는 법 / 얼마 받나요 / 계산법)를 골라 씁니다.
   기계가 타이틀에서 항목 수를 세어 대제목 수와 비교합니다. 세는 법:
