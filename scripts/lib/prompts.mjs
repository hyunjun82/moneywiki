...
export function planPrompt({ slug, topic, category, categories, keywords, registry, related, today, rewrite, oldTitle, retryNote, titleRule, fixedTitle, fixedHeadings = [], ctaScreens }) {
  ...
  const titleBlock = fixed
    ? [
        "## ★ 타이틀은 이미 정해졌습니다 — 새로 짓지 않습니다",
        `"${fixed}"`,
        '설계도의 "title" 에 이 문장을 글자 하나 바꾸지 말고 그대로 적습니다. 다듬거나 줄이면 기계가 되돌립니다.',
      ].join("\n")
    : [
...
7. ctas: 독자가 누를 버튼 2~4개. 신청·조회·계산·발급을 실제로 하는 화면. label 은 행동형("임의계속가입 신청하기"). "보기" 로 끝나는 열람형 금지. 어느 대제목 아래에 둘지 forSection 에 h2 문장을 적습니다. "hero": true(첫 화면 대형 버튼)는 그 화면이 **이 글의 주제 자체**를 처리할 때만 붙입니다(상병급여 글 → 상병급여 청구). 모든 실업급여 글에 걸리는 수급자격 신청서 화면처럼 주제와 한 단계 떨어진 화면뿐이면 hero 를 두지 않습니다 — 기계가 떼어 냅니다. 주소는 Playwright 가 실제로 열어 신청·조회 요소가 있는지 봅니다 — 안내·개편·점검 페이지나 열리지 않는 주소는 버려집니다. 고용24·노동포털 버튼은 「행동 화면 등록부」의 주소만 쓰고 버튼 이름은 등록부의 **버튼 문구 그대로** 씁니다(예: "상병급여 청구하기"). 기관 이름·화면 이름을 덧붙여 늘이지 않습니다 — 기계가 등록부 문구로 바꿉니다. 등록부에 그 일을 하는 화면이 없으면 그 버튼은 넣지 않습니다 — 기관 첫 화면·제도 안내 화면·다른 일을 하는 화면으로 보내는 버튼은 기계가 거부합니다. 실손24(insu24.or.kr)처럼 첫 화면이 곧 그 일을 하는 화면이면 도메인만 있는 주소도 됩니다.
...
버튼·링크
- CTA(heroCta · cta · stepbar action) 의 url 은 허용 CTA 목록 안에서만. label 은 짧은 행동형("수급자격 신청하기", "이직확인서 확인하기") — "고용24에서 …", 화면 이름 늘여 쓰기 금지. "보기" 로 끝나는 열람형 금지. 기관 홈 금지.
- heroCta 는 허용 CTA 중 hero 표시가 붙은 것만. hero 표시가 없으면 heroCta 를 넣지 않습니다.
...
- "원문에 없는 인용": quote.text 를 아래 「페이지 원문」·「숫자가 든 문장」에 있는 문장으로 바꿉니다. **복사해서 붙여넣고 앞뒤만 자릅니다** — 줄임표(…)로 끝내거나, 요약·의역·띄어쓰기 손질을 하면 또 떨어집니다.
- "숫자가 있는데 근거 조문(quote)이 없음": 그 대제목 섹션이나 그 소제목에 quote 를 하나 넣습니다. 넣을 quote 는 위와 같은 방법으로 원문에서 그대로 복사합니다.
  그 섹션의 숫자를 뒷받침하는 문장이 증거에 없으면 quote 를 지어내지 말고, **그 숫자가 든 문장을 섹션에서 빼세요.** 이 두 실패를 번갈아 내면 (quote 없음 → 지어낸 quote → 다시 quote 없음) 세 번 다 떨어져 글이 통째로 버려집니다.
